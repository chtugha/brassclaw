...
- [ ] `spec.md` written (v5.5: class codes 00-21+50, trust removal, PlanA-memory
  ...
  **v5.5: `brassclaw_forensic_packets` table §4 (V044 ALTER migration —
  table already exists as V026, altered to add `component_refs` +
  `volatile_tail`)**,
...
- [ ] `plan.md` written (v5.5: Phase 1 Step 1.6 rewritten for default.py Action
  ...
  per-class Rust formatters); Phase 5.5 interceptor activation (6 steps: V044
  ALTER migration (table already exists as V026), InterceptorResult trait change, PgInterceptorStore wiring,
...
- **Verify:** glossary terms match user clarifications; validation table matches
  ...
  **v5.5: interceptor infrastructure confirmed present** (`ForensicPacket` +
  ...
  ALTER in our plan (table already exists as V026, needs ALTER to add
  `component_refs` + `volatile_tail`); Gap 1: `interceptor_store: None` in composition `runtime.rs:1974`;
...
- Add migration `V027__reborn_skills.sql`: `reborn_skills` table per spec §4
...
- Add migration `V028__reborn_intent_inputs.sql`: `reborn_intent_inputs` table
...
- Add migration `V029__reborn_actions.sql`: `reborn_actions` table per spec
...
- `V030__reborn_tools.sql`: `reborn_tools` table per spec §4 (scope tuple,
...
- **`reborn_validation_config` table** (new migration
  `V031__reborn_validation_config.sql`): `(scope, class_code, name_min_len,
...
- `V032__reborn_extensions_unified.sql`: table per spec §4. `class` enum check
...
- **`V033__reborn_recipes.sql`** (new — spec §7 Q15 resolved): `reborn_recipes`
...
- **Migrate `DocType::Recipe` MemoryDocs into `reborn_recipes` (class 21)**
  ...
  dedicated table (spec §4, Phase 5 migration `V037`). ToolSkills do NOT go into
...
- `V034__reborn_monty_vm_settings.sql`: `reborn_monty_vm_settings` table per
...
- `V035__reborn_user_preferences.sql`: `reborn_user_preferences` table per
...
- Add migrations for the 9 former-doctype tables per spec §4 (scope tuple,
  ...
  - `V036__reborn_specs.sql` (class 12, `consumer_tags[]` = `{02:orchestrator,03:llm}`)
  - `V037__reborn_tool_skills.sql` (class 13, `consumer_tags[]` = `{00:rusty,01:monty,02:orchestrator}`)
  - `V038__reborn_plans.sql` (class 14, `consumer_tags[]` = `{01:monty,02:orchestrator}`)
  - `V039__reborn_summaries.sql` (class 15, `consumer_tags[]` = `{02:orchestrator,03:llm}`)
  - `V040__reborn_docus.sql` (class 17, `consumer_tags[]` = `{02:orchestrator,03:llm}`)
  - `V041__reborn_lessons.sql` (class 18, `consumer_tags[]` = `{02:orchestrator,03:llm}`)
  - `V042__reborn_issues.sql` (class 19, `consumer_tags[]` = `{02:orchestrator,03:llm}`)
  - `V043__reborn_notes.sql` (class 20, `consumer_tags[]` = `{02:orchestrator}`)
...
### Step 5.5.0 — `brassclaw_forensic_packets` ALTER migration (V044)
- **The table already exists as V026** (`crates/brassclaw_pg/migrations/V026__forensic_packets.sql`)
  with `prompt JSONB`, `kohai_response`, `sempai_review JSONB`, `chat_record_id`,
  `status`, token columns, and indexes. The `pg_store.rs` module doc reference to
  `V026` is **correct** — do NOT change it.
- `V044__brassclaw_forensic_packets_alter.sql`: **ALTER** the existing table
  to add the interceptor's new columns:
  - `ALTER TABLE brassclaw_forensic_packets ADD COLUMN component_refs JSONB` —
    array of `{class_code, prompt_uid, component_id, schema_version}` from
    `PriorKnowledgeResult.matched_component_ids` (replaces the role of
    `prompt JSONB` for the interceptor's forensic record).
  - `ALTER TABLE brassclaw_forensic_packets ADD COLUMN volatile_tail TEXT` —
    thread history only (the volatile part of the prompt that was not in Part A).
  - The existing `prompt JSONB` column is **kept for backward compatibility**
    (existing rows have it populated; the interceptor stops writing to it but
    old packets remain readable). New interceptor code writes to
    `component_refs` + `volatile_tail` instead.
  - The existing `kohai_response`/token columns, `sempai_review JSONB`,
    `chat_record_id`, `status`, `captured_at`/`completed_at`/`updated_at`
    columns and indexes are unchanged.
- **Do NOT fix `pg_store.rs` module doc** — the `V026` reference is correct
  (V026 created the table; V044 alters it). The original interceptor2.md plan
  said to fix `V026` → `V051` because it assumed the table didn't exist yet;
  our plan corrects this: the table exists, V044 is an ALTER.
- **Verify:** migration applies cleanly after V043; existing rows in
  `brassclaw_forensic_packets` are preserved (ALTER is non-destructive);
  `cargo test -p brassclaw_interceptor`; integration test (embedded Postgres
  applies V000–V044 in order).
...
- **`reassemble_base_prompt()` — uses direct SQL to individual component
  ...
  - **Missing-table guard:** before querying each component table, check
    ...
    (reborn_tools V030, reborn_skills V027) will exist; tables from later
...
| File | Change |
|---|---|
| `migrations/V044__brassclaw_forensic_packets_alter.sql` *(new)* | ALTER existing `brassclaw_forensic_packets` table (V026): add `component_refs JSONB` + `volatile_tail TEXT`; keep `prompt JSONB` for backward compat |
| `crates/brassclaw_interceptor/src/pg_store.rs` | No doc change needed (`V026` reference is correct — V026 created the table, V044 alters it) |
...
```
Step 5.5.0 (V044 ALTER migration — prerequisite, no Rust changes)
  │
  ▼
...
## Phase 8 — PKC Formatting Split (raw PKC vs. LLM-formatted PKC)

**Goal:** Separate raw `prior_knowledge_content` (internal Rust artifact for
KV-cache fingerprinting) from LLM-formatted PKC (structured JSON for model
consumption). Improves small-model parseability without breaking KV-cache
stability.

**Depends on Phase 5** (which creates `__assemble_prior_knowledge__` and
`PriorKnowledgeResult`). Backport this step after Phase 5 lands.

### Step 8.1 — Two-surface PKC + formatted JSON output + volatile always injected

- **Problem:** the raw `prior_knowledge_content` stored in DB rows is
  machine-authored for Rust binary dispatch (ULID refs, step arrays, internal
  format). Small models parse freeform prose poorly; structured JSON is far
  more reliable. But changing the PKC format would break KV-cache
  fingerprinting (stable prefix must be bit-identical turn-to-turn).
  Additionally, the original plan's step-0 pseudocode (Phase 5 Step 5.2a) omits
  the volatile context injection because Step 5.2b was "blocked until Phase 1.5"
  — this phase corrects that omission and clarifies the KV-cache contract.

- **Two-surface design:**
  - `content: String` — raw PKC, used by Rust for dispatch, KV-cache
    fingerprinting, action execution. **Never sent to the LLM.**
  - `formatted_content: String` — LLM-readable JSON generated deterministically
    from the same DB data in the same `(class_code asc, prompt_uid asc)` order.
    Sent into `working_messages`. Token-identical across turns with the same
    matched component set = KV prefix still reusable.
...
- **`format_prior_knowledge_for_llm()` helper (Rust):**
  Takes the assembled components (ordered `(class_code asc, prompt_uid asc)`)
  and serialises a deterministic JSON structure using `serde_json::json!`:
  ```json
  {
    "prior_knowledge": [
      {
  ...
  token sequence → KV-cache prefix identical across turns that match the same
  components. `class_code_label()` maps codes to string names (`14` → `"plan"`,
  `16` → `"action"`, etc., same static table as the Normal Assembly headers in
  Phase 5 Step 5.2a). Fields that are `NULL` in the DB are omitted from the
  JSON (not included as `null`) to keep the token sequence stable.
...
- **Python step-0 block (replaces the Phase 5 Step 5.2a pseudocode):**
  ...
  **Why volatile is always appended (clarification of Phase 5 intent):**
  The original Phase 5 pseudocode omitted `insert_volatile_context_at_n_minus_1`
  because Step 5.2b was blocked pending Phase 1.5. The design intent was always
  that volatile thread context follows the PKC. Under Override the PKC is the
  stable base; volatile sits after it. Under Normal Assembly the injected
  prior-knowledge and volatile both sit at N-1, in that order. KV-cache
  discipline is maintained because the PKC (`formatted_content`) is
  deterministic — adding volatile after it does not disturb the stable prefix.

- **Actions clarification:**
  Actions (class 16) can be used in three ways that interact differently with
  this split:
  1. **Standalone intent match** (class_code == 16 short-circuit at step 0):
     `execute_action_procedure` is called directly; `__assemble_prior_knowledge__`
     is not called; no prompt is built; volatile context is irrelevant.
  2. **Embedded in a Plan's PKC**: the Plan's PKC text references the Action;
     both `content` (raw) and `formatted_content` (JSON entry for the plan) are
     built normally. The Action entry in `formatted_content` includes its name,
     description, and step types.
  3. **Normal Assembly override_prompt_creation=true**: the Action's
     `formatted_content` replaces `working_messages`, then volatile is appended.
     The LLM sees the JSON-formatted action description + volatile tail.
  In all three cases `content` (raw) is never sent to the LLM.

- **No changes to DB schema:** `prior_knowledge_content` and `content` columns
  are unchanged. The `formatted_content` field is generated at assembly time by
  the Rust host function, not persisted.

- **Verify:**
  - Unit test: `format_prior_knowledge_for_llm()` produces identical JSON for
    identical component input (deterministic order + stable schema).
  - Unit test: `__assemble_prior_knowledge__` returns both `content` (raw) and
    `formatted_content` (valid JSON) for a mix of component classes.
  - Integration test: `working_messages` receives `formatted_content`, not
    `content`, under both Override and Normal Assembly.
  - KV-cache stability test: two consecutive turns with the same matched
    component set produce byte-identical `formatted_content`.
  - Volatile injection test: `insert_volatile_context_at_n_minus_1` is called
    after the PKC block regardless of Override or Normal path.
  - Regression test: raw `content` is never present in the messages sent to
    `__llm_complete__`.
