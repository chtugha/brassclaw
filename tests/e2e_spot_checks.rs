...
mod spot_tests {
    ...
    async fn spot_chain_write_read() {
        let _cleanup = CleanupGuard::new().file("/tmp/brassclaw_spot_test.txt");
        let _ = std::fs::remove_file("/tmp/brassclaw_spot_test.txt");
        ...
        rig.send_message(
            "Write the text 'brassclaw spot check' to /tmp/brassclaw_spot_test.txt \
        ...
        let content =
            std::fs::read_to_string("/tmp/brassclaw_spot_test.txt").expect("file should exist");
        assert_eq!(content, "brassclaw spot check");
