...
use brassclaw::db::DatabaseHandles;
use brassclaw::secrets::{CreateSecretParams, SecretsCrypto, SecretsStore};
...
#[cfg(feature = "libsql")]
fn libsql_config(path: &std::path::Path) -> brassclaw::config::DatabaseConfig {
    brassclaw::config::DatabaseConfig {
        backend: brassclaw::config::DatabaseBackend::LibSql,
        url: secrecy::SecretString::from(String::new()),
        pool_size: 1,
        ssl_mode: brassclaw::config::SslMode::Prefer,
...
fn test_crypto() -> Arc<SecretsCrypto> {
    let key = secrecy::SecretString::from(brassclaw::secrets::keychain::generate_master_key_hex());
...
async fn connect_with_handles_returns_db_and_libsql_handle() {
    ...
    let (db, handles) = brassclaw::db::connect_with_handles(&config)
        .await
...
async fn connect_from_config_produces_working_db() {
    ...
    let db = brassclaw::db::connect_from_config(&config)
        .await
...
async fn secrets_store_from_handles_round_trips() {
    ...
    let (_db, handles) = brassclaw::db::connect_with_handles(&config)
        .await
        .expect("connect");

    let crypto = test_crypto();
    let store = brassclaw::secrets::create_secrets_store(crypto, &handles)
...
async fn db_create_secrets_store_standalone_round_trips() {
    ...
    let store = brassclaw::db::create_secrets_store(&config, crypto)
        .await
...
async fn both_secrets_factories_produce_compatible_stores() {
    ...
    let (_db, handles) = brassclaw::db::connect_with_handles(&config)
        .await
        .expect("connect");
    let store_a = brassclaw::secrets::create_secrets_store(Arc::clone(&crypto), &handles)
        .expect("store from handles");

    // Factory 2: db::create_secrets_store (standalone)
    let store_b = brassclaw::db::create_secrets_store(&config, crypto)
        .await
...
async fn extension_manager_with_process_manager_constructs() {
    ...
    // which reads ~/.brassclaw/mcp-servers.json. Asserting empty would leak
    // the developer's local MCP-server configuration into the test, and
    // overriding BRASSCLAW_BASE_DIR from an integration test is unsafe (it
