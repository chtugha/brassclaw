...
//!
//! See: <https://github.com/chtugha/brassclaw/issues/352> (QA plan, item 1.1)

use brassclaw::tools::validate_tool_schema;
use brassclaw::tools::{Tool, ToolRegistry};
...
fn json_tool_freeform_data_field_is_valid() {
    ...
    let tool = brassclaw::tools::builtin::JsonTool;
    let schema = tool.parameters_schema();
...
fn http_tool_headers_array_is_valid() {
    ...
    let tool = brassclaw::tools::builtin::HttpTool::new();
    let schema = tool.parameters_schema();
...
fn time_tool_schema_is_valid() {
    let tool = brassclaw::tools::builtin::TimeTool;
    let schema = tool.parameters_schema();
...
fn shell_tool_schema_is_valid() {
    let tool = brassclaw::tools::builtin::ShellTool::new();
    let schema = tool.parameters_schema();
