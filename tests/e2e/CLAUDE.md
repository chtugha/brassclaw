...
|---------|-------------|
| `brassclaw_binary` | Checks `target/debug/brassclaw`; if absent, runs `cargo build --no-default-features --features libsql` (timeout 600s). |
...
| `brassclaw_server` | Starts the brassclaw binary with a minimal env (see below), waits for `/api/health` (timeout 60s). Yields the base URL. On teardown sends **SIGINT** (not SIGTERM) so the tokio ctrl_c handler triggers a graceful shutdown and LLVM coverage data is flushed. |
| `hosted_oauth_refresh_server` | Starts a second brassclaw instance with a dedicated libSQL DB and `GOOGLE_OAUTH_CLIENT_ID=hosted-google-client-id`, while still pointing `BRASSCLAW_OAUTH_EXCHANGE_URL` at `mock_llm.py`. Yields a dict with `base_url`, `db_path`, `gateway_user_id`, and `mock_llm_url` for the hosted refresh regression scenario. |
| `extension_cleanup_server` | Starts an isolated brassclaw instance with its own temp DB/home/WASM dirs, `SECRETS_MASTER_KEY`, and hosted-style OAuth env so uninstall-cleanup scenarios can inspect the `secrets` table without interfering with the shared E2E server state. |
...
The function-scoped `page` fixture means **each test gets a clean browser context** (cookies, storage, etc.) but reuses the same brassclaw server and browser process. Tests that need the server URL directly (e.g., `test_auth_rejection`) accept `brassclaw_server` as an additional parameter.

### Environment passed to brassclaw in tests

The `brassclaw_server` fixture injects a minimal, deterministic environment:
...
`conftest.py` handles all server startup automatically — you do not need to start brassclaw manually before running `pytest`. The conftest builds the binary (libsql feature), starts the mock LLM, and starts brassclaw with a fresh temp database on every `pytest` invocation.

If you need to test against a manually started brassclaw, you can skip conftest by running pytest with `--co` (collect-only) to understand what would run, or by calling the httpx/REST helpers directly without the `page` fixture.
...
3. Use the `page` fixture for browser tests (function-scoped, fresh context each test). Use `brassclaw_server` directly for pure HTTP tests.
...
async def test_my_endpoint(brassclaw_server):
    headers = {"Authorization": f"Bearer {AUTH_TOKEN}"}
    async with httpx.AsyncClient() as client:
        r = await client.get(f"{brassclaw_server}/api/health", headers=headers)
...
E2E tests run in CI with `cargo-llvm-cov` for coverage collection. The CI workflow (`fix(ci): persist all cargo-llvm-cov env vars for E2E coverage` — PR #559) sets `LLVM_PROFILE_FILE` and related vars before spawning the brassclaw binary so coverage from E2E runs is captured.
