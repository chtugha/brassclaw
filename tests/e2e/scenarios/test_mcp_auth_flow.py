...
async def test_mcp_install(brassclaw_server, mock_llm_server):
    """Install a mock MCP server pointing at mock_llm.py's /mcp endpoint."""
    await _ensure_removed(brassclaw_server, "mock_mcp")
    ...
    ext = await _get_extension(brassclaw_server, "mock_mcp")
...
async def test_mcp_activate_triggers_auth(brassclaw_server):
...
async def test_mcp_oauth_callback(brassclaw_server):
...
async def test_mcp_authenticated_after_oauth(brassclaw_server):
...
async def test_mcp_tools_registered(brassclaw_server):
...
async def test_mcp_auth_mode_cleared_llm_turn_fires(brassclaw_server, page):
...
async def test_mcp_400_activate_triggers_auth(brassclaw_server, mock_llm_server):
    ...
    """
    await _ensure_removed(brassclaw_server, "mock_mcp_400")
...
async def test_mcp_400_oauth_discovery_returns_auth_url(brassclaw_server):
    ...
    """
    ext = await _get_extension(brassclaw_server, "mock_mcp_400")
    if ext is None:
...
async def test_mcp_400_full_oauth_roundtrip(brassclaw_server):
...
async def test_mcp_400_cleanup(brassclaw_server):
...
async def test_mcp_cleanup(brassclaw_server):
