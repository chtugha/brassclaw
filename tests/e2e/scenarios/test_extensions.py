...
_WASM_CHANNEL = {
    ...
    "onboarding": {
        ...
        "credential_next_step": "Next: send the channel any message to receive a pairing code, then paste it into BrassClaw.",
        "setup_url": None,
        "pairing_title": "Claim ownership for Test Channel",
        "pairing_instructions": "Send the channel any message to receive a pairing code, then paste it into BrassClaw.",
...
async def open_channels_with_mock_role(
    ...
    *,
    ...
    await page.goto(
        f"{brassclaw_server}/?token={AUTH_TOKEN}",
...
async def test_wasm_channel_pairing_state_admin_shows_pending_requests(browser, brassclaw_server):
...
async def test_wasm_channel_pairing_state_member_shows_claim_ui(browser, brassclaw_server):
...
async def test_member_pairing_claim_submission_shows_success(browser, brassclaw_server):
...
async def test_member_pairing_claim_failure_shows_error(browser, brassclaw_server):
...
async def test_admin_pairing_manual_code_submit(browser, brassclaw_server):
