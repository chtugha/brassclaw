...
async def test_full_job_routine_completes_with_tools(page, brassclaw_server):
    ...
    routine = await _wait_for_routine(brassclaw_server, name)

    assert routine["id"]
    assert routine["action_type"] == "full_job"

    # Step 2: Trigger the routine
    resp = await api_post(brassclaw_server, f"/api/routines/{routine['id']}/trigger")
    resp.raise_for_status()
    ...
    completed_run = await _wait_for_completed_run(
        brassclaw_server, routine["id"], timeout=60
    )
    ...
    if completed_run.get("job_id"):
        job = await _wait_for_job_terminal(
            brassclaw_server, completed_run["job_id"], timeout=30
        )
...
async def test_cron_routine_appears_and_can_be_manually_triggered(page, brassclaw_server):
    ...
    completed_run = await _wait_for_completed_run(brassclaw_server, routine["id"])
    assert completed_run["trigger_type"] == "manual"
    assert completed_run["status"].lower() == "attention"


async def test_failed_routine_is_visible_in_ui(page, brassclaw_server):
    ...
    failed_run = await _wait_for_completed_run(brassclaw_server, routine["id"], timeout=60)
