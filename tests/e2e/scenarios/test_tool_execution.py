...
async def test_builtin_time_tool(brassclaw_server):
    """A chat request can execute the built-in time tool and persist its result."""
    thread_id = await _create_thread(brassclaw_server)
    await _send_chat_message(brassclaw_server, thread_id, "what time is it")
...
async def test_non_tool_message_still_works(brassclaw_server):
    """Messages that do not trigger tools still get a normal assistant response."""
    thread_id = await _create_thread(brassclaw_server)
    await _send_chat_message(brassclaw_server, thread_id, "What is 2+2?")
