...
async def test_chat_reply_approve_resumes_pending_tool(brassclaw_server):
    """A plain chat reply of 'approve' should resume the pending tool call."""
    await _require_http_approval(brassclaw_server)
    thread_id = await _create_thread(brassclaw_server)

    await _send_chat_message(brassclaw_server, thread_id, "make approval post approval-chat")
    await _wait_for_history(brassclaw_server, thread_id, expect_pending=True)

    await _send_chat_message(brassclaw_server, thread_id, "approve")
...
async def test_chat_reply_deny_rejects_pending_tool(brassclaw_server):
    """A plain chat reply of 'deny' should reject the pending tool call."""
    await _require_http_approval(brassclaw_server)
    thread_id = await _create_thread(brassclaw_server)

    await _send_chat_message(brassclaw_server, thread_id, "make approval post approval-denied")
    await _wait_for_history(brassclaw_server, thread_id, expect_pending=True)

    await _send_chat_message(brassclaw_server, thread_id, "deny")
    history = await _wait_for_history(
        brassclaw_server,
        thread_id,
        expect_pending=False,
...
async def test_slash_approve_is_thread_scoped_api(brassclaw_server):
    """Sending '/approve' in thread A must not resolve a pending gate in thread B."""
    await _require_http_approval(brassclaw_server)
    thread_a = await _create_thread(brassclaw_server)
    thread_b = await _create_thread(brassclaw_server)
    ...
    await _wait_for_history(brassclaw_server, thread_b, expect_pending=True)

    await _send_chat_message(brassclaw_server, thread_a, "/approve")
...
async def test_chat_reply_always_auto_approves_next_same_tool(brassclaw_server):
    """A plain chat reply of 'always' should auto-approve the same tool next time."""
    await _require_http_approval(brassclaw_server)
    thread_id = await _create_thread(brassclaw_server)

    await _send_chat_message(brassclaw_server, thread_id, "make approval post approval-always-a")
    await _wait_for_history(brassclaw_server, thread_id, expect_pending=True)

    await _send_chat_message(brassclaw_server, thread_id, "always")
    ...
    await _send_chat_message(brassclaw_server, thread_id, "make approval post approval-always-b")
    history = await _wait_for_history(
        brassclaw_server,
        thread_id,
        expect_pending=False,
...
async def test_bare_yes_treated_as_chat_when_no_approval_pending_api(brassclaw_server):
    ...
    await _send_chat_message(brassclaw_server, thread_id, "yes")
...
async def test_bare_no_treated_as_chat_when_no_approval_pending_api(brassclaw_server):
    """Sending 'no' via API when no approval is pending should reach the LLM."""
    thread_id = await _create_thread(brassclaw_server)

    await _send_chat_message(brassclaw_server, thread_id, "no")
