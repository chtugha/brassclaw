...
async def _reload_and_switch_to_thread(page, brassclaw_server, thread_id):
    """Reload the page and navigate back to the given thread."""
    await page.goto(f"{brassclaw_server}/?token={AUTH_TOKEN}", timeout=15000)
...
async def test_message_persists_across_page_reload(page, brassclaw_server):
    ...
    resp = await api_post(brassclaw_server, "/api/chat/thread/new")
    assert resp.status_code == 200, resp.text
    ...
    await _wait_for_completed_turn(brassclaw_server, thread_id)
    await _reload_and_switch_to_thread(page, brassclaw_server, thread_id)
    ...
    resp = await api_get(brassclaw_server, f"/api/chat/history?thread_id={thread_id}")
    assert resp.status_code == 200, resp.text
    turns = resp.json()["turns"]
...
async def test_tool_calls_rendered_as_activity_cards_after_reload(page, brassclaw_server):
    ...
    await _wait_for_tool_in_history(brassclaw_server, thread_id, "echo")
    await _reload_and_switch_to_thread(page, brassclaw_server, thread_id)
...
async def test_tool_calls_expandable_after_reload(page, brassclaw_server):
...
async def test_background_thread_shows_processing_indicator(page, brassclaw_server):
    ...
    await _wait_for_completed_turn(brassclaw_server, thread_a, timeout=30)
...
async def test_no_stale_processing_indicator_for_completed_thread(page, brassclaw_server):
...
async def test_processing_indicator_shows_for_incomplete_turn(page, brassclaw_server):
    ...
    while asyncio.get_running_loop().time() < deadline:
        resp = await api_get(
            brassclaw_server, f"/api/chat/history?thread_id={thread_a}"
        )
    ...
    if not found_processing:
        ...
        await _wait_for_completed_turn(brassclaw_server, thread_a)
        return
...
async def test_refresh_preserves_in_progress_turn(page, brassclaw_server):
    ...
    await _wait_for_completed_turn(brassclaw_server, thread_id, timeout=30)
    await page.locator(SEL["message_assistant"]).filter(
        has_text="4"
    ).wait_for(state="visible", timeout=15000)
...
async def test_switching_back_preserves_in_progress_turn(page, brassclaw_server):
...
async def test_sidebar_refresh_keeps_active_thread_outside_summary_window(
    ...
    turns = await _wait_for_completed_turn(brassclaw_server, thread_a)
    assert any(
        "Summary refresh should keep this thread" in (turn.get("user_input") or "")
        for turn in turns
    ), turns

    resp = await api_get(brassclaw_server, f"/api/chat/history?thread_id={latest_thread}")
    assert resp.status_code == 200, resp.text
    assert not any(
        "Summary refresh should keep this thread" in (turn.get("user_input") or "")
