...
async def test_tool_permission_toggle_persists(page, brassclaw_server):
    ...
    async with httpx.AsyncClient() as client:
        r = await client.put(
            f"{brassclaw_server}/api/settings/tools/echo",
    ...
    async with httpx.AsyncClient() as client:
        r2 = await client.get(
            f"{brassclaw_server}/api/settings/tools",
            headers=headers,
            timeout=10,
        )
...
async def test_always_approve_persists_across_sessions(browser, brassclaw_server):
...
async def test_disabled_tool_state_is_persisted(brassclaw_server):
...
async def test_locked_tool_put_returns_400(brassclaw_server):
    """PUT always_allow for tool_remove (locked) → 400."""
    headers = _headers()

    async with httpx.AsyncClient() as client:
        r = await client.put(
            f"{brassclaw_server}/api/settings/tools/tool_remove",
            json={"state": "always_allow"},
            headers=headers,
            timeout=10,
        )
