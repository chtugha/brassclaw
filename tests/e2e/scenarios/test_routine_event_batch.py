...
@pytest.mark.asyncio
async def test_create_event_trigger_routine(page, brassclaw_server):
...
async def test_event_trigger_skips_non_matching_message(
    ...
    assert await _get_routine_runs(brassclaw_server, routine["id"]) == []
...
async def test_channel_filter_applied_correctly(
    ...
    http_run = await _wait_for_completed_run(brassclaw_server, http_routine["id"])
    await asyncio.sleep(2)
    telegram_runs = await _get_routine_runs(brassclaw_server, telegram_routine["id"])
...
async def test_event_trigger_respects_cooldown(
    ...
    first_run = await _wait_for_completed_run(brassclaw_server, routine["id"])
    ...
    runs = await _get_routine_runs(brassclaw_server, routine["id"])
...
async def test_event_routine_can_be_paused_and_resumed(
    ...
    disabled = await _toggle_routine(brassclaw_server, routine["id"], False)
    ...
    enabled = await _toggle_routine(brassclaw_server, routine["id"], True)
