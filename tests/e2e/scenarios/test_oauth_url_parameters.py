...
async def _get_extension(brassclaw_server, name):
...
@pytest.fixture
async def installed_gmail(brassclaw_server):
    ...
    if ext:
        r = await api_post(brassclaw_server, "/api/extensions/gmail/remove", timeout=30)
...
@pytest.fixture
async def auth_url(brassclaw_server, installed_gmail):
...
async def test_oauth_url_state_is_unique(brassclaw_server, installed_gmail, oauth_params, auth_url):
