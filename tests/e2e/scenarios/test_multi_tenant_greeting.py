...
async def test_multi_tenant_initial_greeting_is_persisted_once(browser, brassclaw_server):
    ...
    alice_context, alice_page = await _open_user_page(browser, brassclaw_server, alice_token)
    try:
        await _assert_single_greeting(alice_page)
        alice_thread_id, alice_turns = await _assistant_history(brassclaw_server, alice_token)
        assert len(alice_turns) == 1
        assert GREETING_MARKER in (alice_turns[0].get("response") or "").lower()
    finally:
        await alice_context.close()

    bob_context, bob_page = await _open_user_page(browser, brassclaw_server, bob_token)
    try:
        await _assert_single_greeting(bob_page)
        bob_thread_id, bob_turns = await _assistant_history(brassclaw_server, bob_token)
        assert len(bob_turns) == 1
        assert GREETING_MARKER in (bob_turns[0].get("response") or "").lower()
    finally:
        await bob_context.close()

    charlie_context, charlie_page = await _open_user_page(browser, brassclaw_server, charlie_token)
    try:
        await _assert_single_greeting(charlie_page)
        charlie_thread_id, charlie_turns = await _assistant_history(
            brassclaw_server, charlie_token
        )
    ...
    alice_reload_context, alice_reload_page = await _open_user_page(
        browser, brassclaw_server, alice_token
    )
    try:
        await _assert_single_greeting(alice_reload_page)
        _, alice_turns_after = await _assistant_history(brassclaw_server, alice_token)
        assert len(alice_turns_after) == 1
    finally:
        await alice_reload_context.close()

    bob_reload_context, bob_reload_page = await _open_user_page(
        browser, brassclaw_server, bob_token
    )
    try:
        await _assert_single_greeting(bob_reload_page)
        _, bob_turns_after = await _assistant_history(brassclaw_server, bob_token)
