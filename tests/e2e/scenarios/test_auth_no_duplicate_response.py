...
This test:
1. Starts an brassclaw instance with a GitHub skill + mock API (returns 401 without auth)
...
_DB_TMPDIR = tempfile.TemporaryDirectory(prefix="brassclaw-auth-sse-e2e-")
_HOME_TMPDIR = tempfile.TemporaryDirectory(prefix="brassclaw-auth-sse-e2e-home-")
...
@pytest.fixture(scope="module")
async def auth_sse_server(brassclaw_binary, mock_llm_server, mock_api):
    ...
    env = {
        ...
        "BRASSCLAW_REBORN_WEBUI_USER_ID": "e2e-auth-sse-tester",
        "HTTP_HOST": "127.0.0.1",
