...
async def test_plan_list_via_api(brassclaw_server):
    ...
    async with __import__("httpx").AsyncClient() as client:
        thread_r = await client.post(
            f"{brassclaw_server}/api/chat/thread/new",
            headers=headers,
            timeout=15,
        )
        ...
        send_r = await client.post(
            f"{brassclaw_server}/api/chat/send",
            headers=headers,
        ...
        for _ in range(60):
            r = await client.get(
                f"{brassclaw_server}/api/chat/history",
                headers=headers,
