...
mod tests {
    ...
    async fn wasm_github_list_issues_coerces_string_limit() {
        let expected_url =
            "https://api.github.com/repos/chtugha/brassclaw/issues?state=open&per_page=50";

        let trace = LlmTrace {
            model_name: "test-wasm-coercion-list-issues".to_string(),
            turns: vec![crate::support::trace_llm::TraceTurn {
                user_input: "List issues in chtugha/brassclaw with limit 50".to_string(),
                steps: vec![
                    TraceStep {
                        request_hint: None,
        ...
        run_trace(trace, "List issues in chtugha/brassclaw with limit 50").await;
    }
    ...
    async fn wasm_github_get_issue_coerces_string_issue_number() {
        let expected_url = "https://api.github.com/repos/chtugha/brassclaw/issues/42";

        let trace = LlmTrace {
            model_name: "test-wasm-coercion-get-issue".to_string(),
            turns: vec![crate::support::trace_llm::TraceTurn {
                user_input: "Get issue 42 from chtugha/brassclaw".to_string(),
                steps: vec![
                    TraceStep {
                        request_hint: None,
                        response: TraceResponse::ToolCalls {
        ...
        run_trace(trace, "Get issue 42 from chtugha/brassclaw").await;
    }
    ...
    async fn wasm_github_list_prs_coerces_string_limit() {
        let expected_url =
            "https://api.github.com/repos/chtugha/brassclaw/pulls?state=open&per_page=25";

        let trace = LlmTrace {
            model_name: "test-wasm-coercion-list-prs".to_string(),
            turns: vec![crate::support::trace_llm::TraceTurn {
                user_input: "List PRs in chtugha/brassclaw".to_string(),
                steps: vec![
                    TraceStep {
                        request_hint: None,
                        response: TraceResponse::ToolCalls {
        ...
        run_trace(trace, "List PRs in chtugha/brassclaw").await;
    }
    ...
    async fn wasm_github_search_code_coerces_string_pagination() {
        let expected_url = "https://api.github.com/search/code?q=repo%3Anearai%2Fbrassclaw%20path%3Asrc%20Tool&per_page=10&page=2&sort=indexed&order=desc";

        let trace = LlmTrace {
            model_name: "test-wasm-coercion-search-code".to_string(),
            turns: vec![crate::support::trace_llm::TraceTurn {
                user_input: "Search code in chtugha/brassclaw".to_string(),
                steps: vec![
                    TraceStep {
                        request_hint: None,
                        response: TraceResponse::ToolCalls {
                            tool_calls: vec![TraceToolCall {
                                ...
                                arguments: json!({
                                    "action": "search_code",
                                    "query": "repo:chtugha/brassclaw path:src Tool",
                                    "limit": "10",
                                    "page": "2",
                                    "sort": "indexed",
                                    "order": "desc"
        ...
        run_trace(trace, "Search code in chtugha/brassclaw").await;
    }
    ...
    async fn wasm_github_create_branch_replays_two_step_ref_flow() {
        let source_ref_url = "https://api.github.com/repos/chtugha/brassclaw/git/ref/heads/main";
        let create_ref_url = "https://api.github.com/repos/chtugha/brassclaw/git/refs";
    ...
    async fn wasm_github_create_or_update_file_puts_contents_payload() {
        let expected_url = "https://api.github.com/repos/chtugha/brassclaw/contents/docs/replay.md";
        let expected_body = json!({
    ...
    async fn wasm_github_create_release_posts_expected_payload() {
        let expected_url = "https://api.github.com/repos/chtugha/brassclaw/releases";
        let expected_body = json!({
            "tag_name": "v1.2.3",
            "draft": false,
            "prerelease": true,
