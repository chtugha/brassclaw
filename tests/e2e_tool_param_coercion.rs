...
mod tests {
    ...
    async fn e2e_coerces_oneof_discriminated_union_params() {
        let trace = LlmTrace {
            ...
            turns: vec![crate::support::trace_llm::TraceTurn {
                user_input: "List issues in chtugha/brassclaw with limit 100".to_string(),
                steps: vec![
                    TraceStep {
                        request_hint: None,
                        response: TraceResponse::ToolCalls {
                    ...
                    TraceStep {
                        request_hint: None,
                        response: TraceResponse::Text {
                            content: "Found issues in chtugha/brassclaw with limit 100.".to_string(),
                            input_tokens: 150,
                            output_tokens: 20,
                        },
                        expected_tool_results: Vec::new(),
        ...
        rig.send_message("List issues in chtugha/brassclaw with limit 100")
            .await;
        let responses = rig.wait_for_responses(1, Duration::from_secs(15)).await;
    ...
    async fn e2e_coerces_oneof_get_issue_variant() {
        ...
        rig.send_message("Get issue 42 from chtugha/brassclaw").await;
        let responses = rig.wait_for_responses(1, Duration::from_secs(15)).await;
