...
use brassclaw::{agent::HeartbeatRunner, config::Config, history::Store, workspace::Workspace};
use brassclaw_llm::{create_llm_provider, create_session_manager};
...
async fn test_heartbeat_end_to_end() {
    ...
    let _ = tracing_subscriber::fmt()
        .with_env_filter("brassclaw=debug")
        .try_init();
    ...
    let hb_config = brassclaw::agent::HeartbeatConfig::default();
    let hygiene_config = brassclaw::workspace::hygiene::HygieneConfig::default();
    ...
    println!("=== Result ===\n");
    match &result {
        brassclaw::agent::HeartbeatResult::Ok => {
        ...
        brassclaw::agent::HeartbeatResult::Skipped => {
        ...
        brassclaw::agent::HeartbeatResult::Failed(err) => {
