...
name: web-ui-test
version: 0.1.0
description: Test the BrassClaw web UI using the Claude for Chrome browser extension.
activation:
  keywords:
    - test web ui
    - test the ui
...
Use this skill when manually testing the BrassClaw web gateway UI via the Claude for Chrome browser extension.

## Prerequisites

- BrassClaw must be running with `GATEWAY_ENABLED=true`
...
Wait for "Agent brassclaw ready and listening" in the logs before proceeding.
...
```bash
rm -rf ~/.brassclaw/installed_skills/<skill-name>
```
