...
name: local-test
version: 0.1.0
description: Build, run, and test BrassClaw locally using Docker containers and Chrome MCP browser automation.
activation:
  keywords:
    - test locally
    - local test
...
Use this skill to build, run, and test BrassClaw web gateway changes locally using `Dockerfile.test` and Chrome MCP browser automation tools.
...
| `RUST_LOG` | Logging verbosity | `brassclaw=debug` |
...
```bash
docker run --rm -d --name ic-test-a -p 3003:3003 -e ONBOARD_COMPLETED=true -e CLI_ENABLED=false -e NEARAI_API_KEY=dummy brassclaw-test
docker run --rm -d --name ic-test-b -p 3004:3003 -e ONBOARD_COMPLETED=true -e CLI_ENABLED=false -e NEARAI_API_KEY=dummy brassclaw-test
```
...
docker ps --filter ancestor=brassclaw-test -q | xargs -r docker stop

# Remove the test image
docker rmi brassclaw-test
```
