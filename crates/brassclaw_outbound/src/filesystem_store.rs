...
//! [`RootFilesystem`](brassclaw_filesystem::RootFilesystem) surface accessed
//! through a [`ScopedFilesystem`]. The [`MountView`](brassclaw_host_api::MountView)
//! wired by composition resolves `/outbound` to a tenant/user-scoped
//! [`VirtualPath`](brassclaw_host_api::VirtualPath) (e.g.
...
use brassclaw_event_projections::{ProjectionCursor, ProjectionScope};
...
use brassclaw_host_api::{ResourceScope, ScopedPath, TenantId, ThreadId, UserId};
use brassclaw_turns::{TurnActor, TurnScope};
use serde::Serialize;
...
/// enforced by the [`MountView`](brassclaw_host_api::MountView) the
    ...
    F: RootFilesystem,
{
    ...
    /// [`brassclaw_processes::filesystem_store::put_with_byte_fallback`] so
...
/// is a control character; `validate_scope_id` in `brassclaw_host_api` rejects
...
mod tests {
    ...
    fn sentinel_is_control_character_rejected_by_scope_id_validators() {
        ...
        // ever contain. `validate_scope_id` (`brassclaw_host_api::ids`)
