...
async fn filesystem_store_satisfies_outbound_contract_on_in_memory_backend() {
    ...
    // brassclaw_filesystem stands in as the underlying mount; in production
    // this would be a libSQL- or Postgres-backed RootFilesystem, or an
    // HSM-decorated mount, with no consumer-side code change.
    let backend = std::sync::Arc::new(brassclaw_filesystem::InMemoryBackend::new());
...
fn now() -> brassclaw_host_api::Timestamp {
    chrono::Utc::now()
}
...
async fn filesystem_outbound_store_writes_tenant_id_indexed_projection() {
    ...
    let deliveries_prefix =
        brassclaw_host_api::ScopedPath::new("/outbound/deliveries".to_string()).unwrap();
    let virtual_prefix = scoped
        .resolve(&scope.to_resource_scope(), &deliveries_prefix)
        .unwrap();
    let tenant_key = brassclaw_filesystem::IndexKey::new("tenant_id").unwrap();
    ...
    let hit = backend
        .query(
            ...
            &Filter::Eq {
                ...
                value: brassclaw_filesystem::IndexValue::Text(scope.tenant_id.as_str().to_string()),
            },
    ...
    let miss = backend
        .query(
            ...
            &Filter::Eq {
                key: tenant_key,
                value: brassclaw_filesystem::IndexValue::Text("tenant-b".to_string()),
            },
            Page::new(0, Page::MAX_LIMIT),
        )
        .await
