...
- Fast local check: `cargo test -p brassclaw_outbound`
- Backend parity check without live Postgres: `BRASSCLAW_SKIP_POSTGRES_TESTS=1 cargo test -p brassclaw_outbound --all-features`
- Lint check: `cargo clippy -p brassclaw_outbound --all-targets --all-features -- -D warnings`
