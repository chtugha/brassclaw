...
//! contract harness from `brassclaw_hooks::predicate_state::contract`.
//!
...
//! `BRASSCLAW_HOOKS_POSTGRES_URL` (or `DATABASE_URL`) to a libpq URL. With
//! no URL the tests print a skip notice and pass, matching the
//! env-gated-skip pattern used by `brassclaw_reborn_event_store` and
//! `brassclaw_filesystem` (no testcontainers dependency).
//!
//! # Isolation
//!
...
macro_rules! pg_contract {
    ($name:ident) => {
        ...
        async fn $name() {
            ...
            brassclaw_hooks::predicate_state::contract::$name(move || {
...
impl brassclaw_hooks::predicate_state::PredicateStateBackend for PgBackendHandle {
    async fn record_invocation(
        &self,
        key: &brassclaw_hooks::predicate_state::InvocationKey,
        event_id: &brassclaw_hooks::predicate_state::PredicateEventId,
        now: chrono::DateTime<chrono::Utc>,
        window: std::time::Duration,
    ) -> Result<u32, brassclaw_hooks::predicate_state::PredicateBackendError> {
        self.0.record_invocation(key, event_id, now, window).await
    }

    async fn record_value(
        &self,
        key: &brassclaw_hooks::predicate_state::ValueKey,
        event_id: &brassclaw_hooks::predicate_state::PredicateEventId,
        now: chrono::DateTime<chrono::Utc>,
    ...
    ) -> Result<u64, brassclaw_hooks::predicate_state::PredicateBackendError> {
