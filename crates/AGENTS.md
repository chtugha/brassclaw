...
- UI presentation: `brassclaw_tui` or `brassclaw_webui_v2`. The v1 root `src/` tree was removed in Phase 6; root `src/main.rs` is now only a thin compat shim delegating to `brassclaw_reborn_cli` (it carries legacy `--no-onboard`/`--cli-only`/`--no-db`/`--auto-approve` flags for E2E fixtures). All real backend/channel code lives under `crates/`.
