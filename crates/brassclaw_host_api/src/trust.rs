//! Requested-trust vocabulary for BrassClaw Reborn.
//!
...
//! and [`RequestedTrustClass`]; the host policy engine in `brassclaw_trust`
...
//!   eventual built-in tools (see `crates/brassclaw_trust/CONTRACT.md` §9 for
//!   the migration path).
...
//!   from `brassclaw_trust::TrustPolicy::evaluate` and is attached to
//!   [`crate::ExecutionContext::trust`] at dispatch time.
//!
//! See `brassclaw_trust::TrustPolicy` for the engine that bridges request to
//! effective trust, `crates/brassclaw_trust/CONTRACT.md` for the full
...
/// their own and must be matched against host policy in `brassclaw_trust`
...
/// privileged request is decided by `brassclaw_trust::TrustPolicy::evaluate`,
/// not by parsing.
...
/// | Built-in tools (eventual) | `Bundled` | See `crates/brassclaw_trust/CONTRACT.md` §9 for migration |
///
...
/// treated as the same package — the `brassclaw_trust` policy binds trust to
