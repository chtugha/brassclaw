//! Host runtime glue for BrassClaw Reborn WASM v2 product adapters.
//!
//! # Trust-model warning
//!
...
//!   [`brassclaw_product_adapters::ProtocolAuthEvidence::Verified`] before any
//!   adapter parse step.
//! * `WebhookAuth` — bridge that returns a `Verified` evidence constructed via
//!   the public `mark_*_verified` helpers in `brassclaw_product_adapters::auth`.
