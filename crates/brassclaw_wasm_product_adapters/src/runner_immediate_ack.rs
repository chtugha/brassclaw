    ...
    pub async fn process_verified_webhook_immediate_ack_with_observer(
        ...
        while let Some(result) = tasks.try_join_next() {
            if let Err(error) = result {
                tracing::debug!(
                    target = "brassclaw::product_adapter::runner",
                    error = %error,
...
mod tests {
    ...
    use brassclaw_product_adapters::identity::{
    ...
    #[async_trait]
    impl brassclaw_product_adapters::ProductWorkflow for AckWorkflow {
        ...
        async fn resolve_projection_subscription(
            ...
            Err(ProductAdapterError::Internal {
                detail: brassclaw_product_adapters::redaction::RedactedString::new(
    ...
    #[async_trait]
    impl brassclaw_product_adapters::ProductWorkflow for BlockingWorkflow {
        async fn submit_inbound(
            &self,
            _envelope: ProductInboundEnvelope,
        ) -> Result<ProductInboundAck, ProductAdapterError> {
    ...
    fn runner_with_workflow(
        parse_count: Arc<AtomicUsize>,
        workflow: Arc<dyn brassclaw_product_adapters::ProductWorkflow>,
        max_in_flight: usize,
