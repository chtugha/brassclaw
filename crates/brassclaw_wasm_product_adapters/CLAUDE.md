# brassclaw_wasm_product_adapters guardrails

Owns the thin WASM ProductAdapter host-glue boundary for BrassClaw Reborn.
...
- Do not perform production HTTP, DNS/private-IP checks, redirect handling, response limits, secret injection, leak scanning, or redaction here. Actual network egress must delegate through host-runtime services backed by `brassclaw_network` and secret leases.
...
- Runtime-specific WIT/request shapes may live here, but shared product-surface DTOs stay in `brassclaw_product_adapters`.
...
- `brassclaw_architecture` boundary tests pin crate dependencies, local guardrails, and WIT trust-boundary invariants.
