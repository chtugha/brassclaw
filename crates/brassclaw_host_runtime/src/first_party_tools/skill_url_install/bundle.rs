...
use brassclaw_skills::normalize_safe_relative_path;
...
impl BundleCollector {
    ...
    pub(super) fn push_file(
        ...
        if relative == Path::new("SKILL.md") {
            if bytes.len() as u64 > brassclaw_skills::MAX_PROMPT_FILE_SIZE {
                return Err(FirstPartyCapabilityError::new(
                    RuntimeDispatchErrorKind::OutputTooLarge,
                ));
            }
            ...
            if self.files.len() >= brassclaw_skills::MAX_INSTALL_BUNDLE_FILES {
                return Err(FirstPartyCapabilityError::new(
                    RuntimeDispatchErrorKind::OutputTooLarge,
                ));
            }
