//! Host runtime facade for BrassClaw Reborn.
//!
//! `brassclaw_host_runtime` is the narrow boundary upper Reborn services build
...
//!   [`brassclaw_capabilities::CapabilityHost`] (which itself coordinates
...
pub use first_party_tools::{
    ...
    READ_FILE_CAPABILITY_ID, SHELL_CAPABILITY_ID, SPAWN_SUBAGENT_CAPABILITY_ID, TIME_CAPABILITY_ID,
...
pub enum RuntimeWorkId {
    Invocation(brassclaw_host_api::InvocationId),
    Process(ProcessId),
    Gate(RuntimeGateId),
}
