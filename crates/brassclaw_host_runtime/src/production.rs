...
//! [`brassclaw_capabilities::CapabilityHost`] with neutral kernel services —
...
//! [`TrustDecision`](brassclaw_trust::TrustDecision) instead of caller-supplied
...
use brassclaw_authorization::{CapabilityLeaseStore, TrustAwareCapabilityDispatchAuthorizer};
...
use brassclaw_extensions::{ExtensionPackage, ExtensionRegistry, SharedExtensionRegistry};
...
use brassclaw_trust::{HostTrustPolicy, TrustDecision, TrustError, TrustPolicy, TrustProvenance};
use brassclaw_turns::run_profile::LoopSafeSummary;
...
impl DefaultHostRuntime {
    ...
    async fn fail_matching_blocked_resume_on_preflight_error(
        &self,
        context: &brassclaw_host_api::ExecutionContext,
        capability_id: &CapabilityId,
...
) -> Result<brassclaw_trust::TrustPolicyInput, TrustEvaluationError> {
    package
        .trust_policy_input(
...
fn auth_required_outcome(
    ...
    credential_requirements: Vec<brassclaw_host_api::RuntimeCredentialAuthRequirement>,
) -> RuntimeCapabilityOutcome {
...
pub(crate) fn failure_kind_from(error: &CapabilityInvocationError) -> RuntimeFailureKind {
    match error {
        ...
        CapabilityInvocationError::ObligationFailed { kind, .. } => match kind {
            brassclaw_capabilities::CapabilityObligationFailureKind::Audit => {
                RuntimeFailureKind::Backend
            }
            brassclaw_capabilities::CapabilityObligationFailureKind::Mount => {
                RuntimeFailureKind::Authorization
            }
            brassclaw_capabilities::CapabilityObligationFailureKind::Network => {
                RuntimeFailureKind::Network
            }
            brassclaw_capabilities::CapabilityObligationFailureKind::Output => {
                RuntimeFailureKind::OutputTooLarge
            }
            brassclaw_capabilities::CapabilityObligationFailureKind::Resource => {
                RuntimeFailureKind::Resource
            }
            brassclaw_capabilities::CapabilityObligationFailureKind::Secret => {
                RuntimeFailureKind::Authorization
            }
        },
...
mod tests {
    ...
    //! [`brassclaw_host_api::DispatchFailureKind`] values. Their display
    ...
    use super::*;
    use brassclaw_capabilities::CapabilityInvocationError;
