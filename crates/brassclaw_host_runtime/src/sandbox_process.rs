...
const DEFAULT_IMAGE: &str = "brassclaw-worker:latest";
...
impl RebornSandboxConfig {
    pub fn new(workspace_root: impl Into<PathBuf>) -> Self {
        Self {
            ...
            image: std::env::var("BRASSCLAW_REBORN_SANDBOX_IMAGE")
                .or_else(|_| std::env::var("BRASSCLAW_SANDBOX_IMAGE"))
    ...
    pub fn with_local_mount_source(
        mut self,
        virtual_root: brassclaw_host_api::VirtualPath,
...
mod tests {
    ...
    fn default_sandbox_disables_ambient_network_and_secret_affordance() {
        ...
        assert!(env.contains(&"BRASSCLAW_REBORN_NETWORK_MODE=disabled".to_string()));
        assert!(env.contains(&"BRASSCLAW_REBORN_SECRET_MODE=disabled".to_string()));
    }
    ...
    fn network_broker_exposes_proxy_env_without_none_network_mode() {
        ...
        assert!(env.contains(&"BRASSCLAW_REBORN_NETWORK_MODE=brokered".to_string()));
        assert!(
            env.contains(&"BRASSCLAW_REBORN_HTTP_PROXY=http://broker.internal:8181".to_string())
        );
    ...
    fn network_broker_port_uses_docker_host_gateway_proxy_url() {
        ...
        let config = RebornSandboxConfig::new("/tmp/reborn-sandbox")
            .with_network_broker_port(TEST_BROKER_PORT);
        let env = config.command_env(HashMap::new()).unwrap();
        let proxy_url = format!(
            "http://{}:{TEST_BROKER_PORT}",
            broker::docker_host_gateway()
        );
    ...
    fn unix_socket_network_broker_preserves_none_network_mode_and_mounts_socket() {
        ...
        assert!(env.contains(&"BRASSCLAW_REBORN_NETWORK_MODE=brokered".to_string()));
        assert!(env.contains(
            &"BRASSCLAW_REBORN_HTTP_BROKER_SOCKET=/tmp/brassclaw-http-broker.sock".to_string()
        ));
        assert!(
            env.contains(&"BRASSCLAW_REBORN_HTTP_BROKER_URL=http://brassclaw-broker".to_string())
        );
        assert_eq!(
            binds,
            vec!["/tmp/reborn-http-broker.sock:/tmp/brassclaw-http-broker.sock:rw".to_string()]
        );
    }
    ...
    fn secret_broker_exposes_endpoint_without_secret_material() {
        ...
        assert!(env.contains(&"BRASSCLAW_REBORN_SECRET_MODE=brokered".to_string()));
        assert!(env.contains(
            &"BRASSCLAW_REBORN_SECRET_BROKER_URL=https://broker.internal/secrets".to_string()
        ));
        assert!(
            env.iter()
    ...
    fn unix_socket_secret_broker_exposes_socket_without_secret_material() {
        ...
        assert!(env.contains(&"BRASSCLAW_REBORN_SECRET_MODE=brokered".to_string()));
        assert!(env.contains(
            &"BRASSCLAW_REBORN_SECRET_BROKER_SOCKET=/tmp/brassclaw-secret-broker.sock".to_string()
        ));
        assert!(
            env.iter()
                .all(|entry| !entry.contains("sk-") && !entry.contains("token="))
        ...
        assert_eq!(
            binds,
            vec!["/tmp/reborn-secret-broker.sock:/tmp/brassclaw-secret-broker.sock:rw".to_string()]
        );
    }
    ...
    async fn container_launch_config_applies_unix_socket_broker_env_binds_and_none_network() {
        ...
        assert!(binds.contains(&format!(
            "{}:/tmp/brassclaw-http-broker.sock:rw",
            network_socket.display()
        )));
        assert!(binds.contains(&format!(
            "{}:/tmp/brassclaw-secret-broker.sock:rw",
    ...
    async fn container_launch_config_applies_http_proxy_broker_env_and_drops_none_network() {
        ...
        assert!(
            binds
                .iter()
                .all(|bind| !bind.contains("brassclaw-http-broker.sock"))
        );
    }
