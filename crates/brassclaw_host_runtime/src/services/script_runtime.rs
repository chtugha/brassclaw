//! Script runtime contracts owned by the host-runtime composition layer.
//!
//! The script lane lives here as a host-runtime-private seam, hosting the
//! Docker-backed executor that backs extension manifests declaring
//! `runtime = "script"`. Types are owned by `brassclaw_host_runtime` so the
//! upper Reborn product layer must never reach into a separate substrate
//! crate. All dispatch wiring for the script lane lives alongside the
//! other runtime adapters in `crate::services`.
...
pub struct ScriptHostHttpRequest {
    pub scope: ResourceScope,
    pub capability_id: CapabilityId,
    pub method: brassclaw_host_api::NetworkMethod,
    pub url: String,
    pub headers: Vec<(String, String)>,
    pub body: Vec<u8>,
    pub network_policy: brassclaw_host_api::NetworkPolicy,
    pub credential_injections: Vec<brassclaw_host_api::RuntimeCredentialInjection>,
...
mod tests {
    ...
    use brassclaw_host_api::{CapabilityId, InvocationId, ResourceScope, TenantId, UserId};
    ...
    fn docker_backend_disables_ambient_network_before_image_and_manifest_args() {
        ...
        let old_args_file = std::env::var_os("BRASSCLAW_DOCKER_ARGS_FILE");
        let _restore = EnvRestore(vec![
            ("PATH", old_path.clone()),
            ("BRASSCLAW_DOCKER_ARGS_FILE", old_args_file),
        ]);
        ...
        unsafe {
            ...
            std::env::set_var("BRASSCLAW_DOCKER_ARGS_FILE", &args_file);
        }
    ...
    fn sample_docker_request() -> ScriptBackendRequest {
        ScriptBackendRequest {
            provider: brassclaw_host_api::ExtensionId::new("script").unwrap(),
