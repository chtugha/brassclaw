...
async fn host_runtime_http_egress_port_executes_with_host_staged_credentials() {
    ...
    assert!(matches!(
        error,
        brassclaw_host_api::RuntimeHttpEgressError::Credential { .. }
    ));
...
async fn host_http_egress_treats_expired_staged_secret_as_missing() {
    ...
    assert!(matches!(
        error,
        brassclaw_host_api::RuntimeHttpEgressError::Credential { ref reason }
...
async fn host_runtime_services_with_security_audit_sink_records_leak_block() {
    use brassclaw_events::{InMemorySecurityAuditSink, SecurityBoundary, SecurityDecision};
...
fn test_descriptor(runtime: RuntimeKind, effects: Vec<EffectKind>) -> CapabilityDescriptor {
    CapabilityDescriptor {
        ...
        provider: brassclaw_host_api::ExtensionId::new("test").unwrap(),
        runtime,
...
fn stage_secret_error_maps_auth_and_backend_variants() {
    ...
    use brassclaw_host_api::CredentialStageError::{AuthRequired, Backend};
