...
async fn host_runtime_services_routes_structured_github_wasm_search_through_runtime_http_egress() {
    ...
    let expected_url =
        "https://api.github.com/search/issues?q=repo%3Anearai%2Fbrassclaw%20is%3Aissue&per_page=1";
    ...
    let outcome = services
        ...
        .invoke_capability(wasm_runtime_request_for_scope(
            capability_id.clone(),
            scope,
            json!({"repo": "chtugha/brassclaw", "type": "issue", "limit": 1}),
        ))
        .await
        .unwrap();
...
async fn host_runtime_services_restages_github_product_auth_for_multi_request_wasm_capability() {
    ...
    assert_eq!(
        requests[0].url,
        "https://api.github.com/repos/chtugha/brassclaw/git/ref/heads/main"
    );
...
async fn host_runtime_services_missing_github_runtime_secret_blocks_on_auth() {
    ...
    let outcome = services
        ...
        .invoke_capability(wasm_runtime_request_for_scope(
            capability_id.clone(),
            scope,
            json!({"query": "repo:chtugha/brassclaw is:issue", "limit": 1}),
        ))
        .await
        .unwrap();
...
async fn bundled_github_wasm_executes_search_get_and_comment_operations() {
    ...
    let get_issue_http = Arc::new(RecordingWasmHostHttp::ok(WasmHttpResponse {
        status: 200,
        headers_json: "{}".to_string(),
        body: br#"{"number":2,"title":"Reborn GitHub issue","state":"open","html_url":"https://github.com/chtugha/brassclaw/issues/2"}"#.to_vec(),
    }));
    let get_issue = execute_bundled_github_wasm(
        "github.get_issue",
        json!({"owner": "nearai", "repo": "brassclaw", "issue_number": 2}),
    ...
    assert_single_wasm_request(
        &get_issue_http,
        "GET",
        "https://api.github.com/repos/chtugha/brassclaw/issues/2",
        None,
    );
    ...
    let comment_http = Arc::new(RecordingWasmHostHttp::ok(WasmHttpResponse {
        status: 201,
        headers_json: "{}".to_string(),
        body: br##"{"id":44,"html_url":"https://github.com/chtugha/brassclaw/issues/2#issuecomment-44","body":"Reborn WASM comment"}"##.to_vec(),
    }));
    ...
    assert_single_wasm_request(
        &comment_http,
        "POST",
        "https://api.github.com/repos/chtugha/brassclaw/issues/2/comments",
...
async fn bundled_github_wasm_builds_query_from_structured_search_fields() {
    ...
    assert_single_wasm_request(
        &http,
        "GET",
        "https://api.github.com/search/issues?q=repo%3Anearai%2Fbrassclaw%20author%3Aserrrfirat%20state%3Aopen%20is%3Aissue&per_page=1",
        None,
    );
}
...
async fn bundled_github_wasm_create_branch_propagates_missing_source_ref() {
    ...
    assert_eq!(
        requests[0].url,
        "https://api.github.com/repos/chtugha/brassclaw/git/ref/heads/missing-branch"
    );
}
...
async fn bundled_github_wasm_builds_create_repo_fork_and_release_requests() {
    ...
    let fork_http = Arc::new(RecordingWasmHostHttp::ok(WasmHttpResponse {
        status: 202,
        headers_json: "{}".to_string(),
        body: br#"{"name":"brassclaw-fork"}"#.to_vec(),
    }));
...
fn github_asset_root() -> std::path::PathBuf {
    std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../..")
        .join("crates/brassclaw_first_party_extensions/assets/github")
}
...
fn google_asset_root(package_id: &str) -> std::path::PathBuf {
    std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../..")
        .join("crates/brassclaw_first_party_extensions/assets")
        .join(package_id)
}
...
fn assert_single_wasm_request(
    ...
    assert_eq!(headers["User-Agent"], "BrassClaw-GitHub-Reborn-WASM");
