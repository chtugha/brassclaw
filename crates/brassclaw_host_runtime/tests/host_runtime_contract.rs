...
async fn default_runtime_returns_completed_outcome_for_authorized_dispatch() {
    ...
    match outcome {
        brassclaw_host_runtime::RuntimeCapabilityOutcome::Completed(completed) => {
...
async fn default_runtime_surfaces_approval_required_with_persisted_request_id() {
    ...
    let run_state = Arc::new(InMemoryRunStateStore::new());
    let approval_requests = Arc::new(InMemoryApprovalRequestStore::new());
    let leases: Arc<dyn brassclaw_authorization::CapabilityLeaseStore> =
    ...
    match outcome {
        brassclaw_host_runtime::RuntimeCapabilityOutcome::ApprovalRequired(gate) => {
...
async fn default_runtime_propagates_unavailable_when_run_state_lookup_fails_during_approval() {
    ...
    match error {
        brassclaw_host_runtime::HostRuntimeError::Unavailable { reason } => {
            assert!(
                !reason.contains("/"),
...
async fn default_runtime_returns_failed_for_unknown_capability() {
    ...
    match outcome {
        brassclaw_host_runtime::RuntimeCapabilityOutcome::Failed(failure) => {
            assert_eq!(failure.capability_id, capability_id());
            assert_eq!(
                failure.kind,
                brassclaw_host_runtime::RuntimeFailureKind::MissingRuntime
            );
        }
...
async fn default_runtime_surfaces_authorization_failure_when_authorizer_denies() {
    ...
    match outcome {
        brassclaw_host_runtime::RuntimeCapabilityOutcome::Failed(failure) => {
            ...
            assert_eq!(
                failure.kind,
                brassclaw_host_runtime::RuntimeFailureKind::Authorization
            );
        }
...
async fn default_runtime_status_filters_to_running_records_only() {
    ...
    for invocation_id in [running_id, completed_id, failed_id] {
        run_state
            .start(brassclaw_run_state::RunStart {
                invocation_id,
...
impl RunStateStore for FailingRecordsRunStateStore {
    ...
    async fn block_approval(
        ...
        approval: brassclaw_host_api::ApprovalRequest,
