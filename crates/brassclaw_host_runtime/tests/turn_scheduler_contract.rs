...
impl TurnStateStore for DurableLikeTurnStore {
    async fn submit_turn(
        ...
        admission_policy: &dyn brassclaw_turns::TurnAdmissionPolicy,
        run_profile_resolver: &dyn brassclaw_turns::RunProfileResolver,
    ...
    for _ in 0..8 {
        handle
            ...
            .notify_queued_run(TurnRunWake {
                ...
                run_id: brassclaw_turns::TurnRunId::new(),
                status: TurnStatus::Queued,
                event_cursor: brassclaw_turns::EventCursor::default(),
            })
            .unwrap();
    }
...
async fn wake_notifier_drains_submitted_run() {
    ...
    assert_eq!(
        store
            .get_run_state(brassclaw_turns::GetRunStateRequest { scope, run_id })
            .await
            .unwrap()
            .status,
...
async fn wait_for_status<S>(
    store: &S,
    scope: TurnScope,
    run_id: brassclaw_turns::TurnRunId,
    expected: TurnStatus,
) where
...
fn accepted_run_id(response: SubmitTurnResponse) -> brassclaw_turns::TurnRunId {
