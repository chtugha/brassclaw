...
use brassclaw_filesystem::{InMemoryBackend, LocalFilesystem, RootFilesystem, ScopedFilesystem};
...
async fn host_http_egress_removes_staged_secret_before_network_errors() {
    ...
    assert!(matches!(
        error,
        brassclaw_host_api::RuntimeHttpEgressError::Network { .. }
    ));
...
async fn host_http_egress_fails_closed_when_real_scoped_filesystem_write_fails() {
    ...
    root.mount_local(
        VirtualPath::new("/projects/workspace").unwrap(),
        brassclaw_host_api::HostPath::from_path_buf(temp.path().to_path_buf()),
    )
    .unwrap();
    let scoped_filesystem = Arc::new(ScopedFilesystem::with_fixed_view(
        Arc::new(root),
...
async fn host_http_egress_blocks_credential_shaped_response_body() {
    ...
    assert!(matches!(
        error,
        brassclaw_host_api::RuntimeHttpEgressError::Response { ref reason, .. }
...
async fn host_http_egress_blocks_percent_encoded_credential_path_before_network() {
    ...
    assert!(matches!(
        error,
        brassclaw_host_api::RuntimeHttpEgressError::Request { ref reason, .. }
...
async fn host_http_egress_blocks_runtime_supplied_sensitive_headers_before_network() {
    ...
    assert!(matches!(
        error,
        brassclaw_host_api::RuntimeHttpEgressError::Request { .. }
    ));
