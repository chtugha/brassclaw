...
mod tests {
    ...
    fn envelope(payload: ProductOutboundPayload) -> ProductOutboundEnvelope {
        ProductOutboundEnvelope {
            ...
            projection_cursor: brassclaw_product_adapters::ProjectionCursor::new("cursor:slack")
                .expect("valid"),
    ...
    async fn auth_prompts_render_to_slack_http_egress() {
        ...
        let run_id = TurnRunId::new();
        let payload =
            ProductOutboundPayload::AuthPrompt(brassclaw_product_adapters::AuthPromptView {
                turn_run_id: run_id,
    ...
    async fn render_outbound_egress_transport_errors_classified() {
        use brassclaw_product_adapters::ProtocolHttpEgressError;
        ...
        let cases: &[EgressCase] = &[
            ...
            (
                ProtocolHttpEgressError::PolicyDenied {
                    reason: brassclaw_product_adapters::RedactedString::new("blocked"),
                },
                |e| matches!(e, ProductAdapterError::EgressDenied { .. }),
    ...
    async fn render_outbound_progress_and_keepalive_are_deferred() {
        use brassclaw_product_adapters::{ProgressKind, ProgressUpdateView};
