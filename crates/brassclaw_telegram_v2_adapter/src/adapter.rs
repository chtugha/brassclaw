        ...
        .map_err(|err| match err {
            ...
            crate::payload::PayloadParseError::InvalidJson { reason } => {
                ProductAdapterError::MalformedInboundPayload {
                    reason: brassclaw_product_adapters::redaction::RedactedString::new(reason),
                }
            }
            crate::payload::PayloadParseError::MissingUpdateId => {
                ProductAdapterError::MalformedInboundPayload {
                    reason: brassclaw_product_adapters::redaction::RedactedString::new(
...
fn map_egress_error(err: ProtocolHttpEgressError) -> ProductAdapterError {
    let reason = brassclaw_product_adapters::redaction::RedactedString::new(err.to_string());
    match err {
        ProtocolHttpEgressError::Timeout
        | ProtocolHttpEgressError::Network(_)
...
mod tests {
    ...
    fn test_outbound_target() -> brassclaw_product_adapters::ProductOutboundTarget {
        ...
        .expect("valid");
        brassclaw_product_adapters::ProductOutboundTarget::new(reply, conv, None)
    }
    ...
    fn test_projection_cursor() -> brassclaw_product_adapters::ProjectionCursor {
        brassclaw_product_adapters::ProjectionCursor::new("test-cursor").expect("valid")
    }
    ...
    fn parse_inbound_refuses_unverified_evidence() {
        ...
        let unverified = ProtocolAuthEvidence::failed(
            brassclaw_product_adapters::ProtocolAuthFailure::SharedSecretMismatch,
        );
        let err = adapter
    ...
    async fn render_outbound_final_reply_uses_constrained_egress() {
        ...
        let sink = brassclaw_product_adapters::FakeOutboundDeliverySink::new();
        let envelope = ProductOutboundEnvelope {
            ...
            payload: ProductOutboundPayload::FinalReply(
                brassclaw_product_adapters::FinalReplyView {
                    turn_run_id: brassclaw_turns::TurnRunId::new(),
                    text: "hi".into(),
    ...
    async fn render_outbound_progress_skipped_when_capability_off() {
        ...
        let envelope = ProductOutboundEnvelope {
            ...
            payload: ProductOutboundPayload::Progress(
                brassclaw_product_adapters::ProgressUpdateView {
                    turn_run_id: brassclaw_turns::TurnRunId::new(),
                    kind: brassclaw_product_adapters::ProgressKind::Typing,
                    generated_at: chrono::Utc::now(),
                },
            ),
            delivery_attempt_id: uuid::Uuid::new_v4(),
    ...
    async fn render_outbound_records_retryable_on_telegram_5xx() {
        ...
        egress.program_response(
            "api.telegram.org",
            Ok(brassclaw_product_adapters::EgressResponse::new(
                502,
                Vec::new(),
            )),
        );
