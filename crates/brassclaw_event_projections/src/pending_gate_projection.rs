...
use brassclaw_host_api::{AgentId, ProjectId, TenantId, ThreadId, Timestamp, UserId};
use brassclaw_turns::{
...
impl TurnEventSink for PendingGateProjection {
    async fn publish(&self, event: TurnLifecycleEvent) -> Result<(), brassclaw_turns::TurnError> {
        self.project_event(event)
            .await
            .map_err(|error| match error {
                ProjectionError::InvalidRequest { reason } => {
                    brassclaw_turns::TurnError::InvalidRequest {
                ...
                ProjectionError::TurnEventRebaseRequired { .. } => {
                    brassclaw_turns::TurnError::Unavailable {
                        reason: "pending gate projection failed: turn event rebase required"
                            .to_string(),
                    }
                }
                ProjectionError::RebaseRequired { .. } => brassclaw_turns::TurnError::Unavailable {
                    reason: "pending gate projection failed: runtime rebase required".to_string(),
                },
                ProjectionError::Source { operation } => brassclaw_turns::TurnError::Unavailable {
