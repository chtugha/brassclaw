...
use brassclaw_turns::{GateRef, TurnBlockedGateKind, TurnEventKind, TurnRunId, TurnStatus};
...
async fn publish_maps_projection_failure_to_turn_unavailable() {
    ...
    match err {
        brassclaw_turns::TurnError::Unavailable { reason } => {
...
async fn publish_maps_invalid_projection_failure_to_turn_invalid_request() {
    ...
    match err {
        brassclaw_turns::TurnError::InvalidRequest { reason } => {
