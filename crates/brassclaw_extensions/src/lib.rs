//! Extension manifest and registry contracts for BrassClaw Reborn.
//!
//! `brassclaw_extensions` discovers and validates extension packages, extracts
...
use brassclaw_trust::TrustPolicyInput;
...
impl ExtensionPackage {
    ...
    /// pass it to `brassclaw_trust::TrustPolicy::evaluate` to get an effective
...
impl ExtensionDiscovery {
    ...
    fn extension_dir_id(entry: &brassclaw_filesystem::DirEntry) -> Option<ExtensionId> {
    ...
    async fn load_package_entry<F>(
        fs: &F,
        root: &VirtualPath,
        entry: &brassclaw_filesystem::DirEntry,
        expected: ExtensionId,
        source: ManifestSource,
        host_port_catalog: &HostPortCatalog,
        host_api_contracts: &HostApiContractRegistry,
