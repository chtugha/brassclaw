...
//! - extension IDs starting with `brassclaw.` are reserved for HostBundled;
...
pub const RESERVED_HOST_BUNDLED_ID_PREFIX: &str = "brassclaw.";
...
///
/// This is a *structural* bound enforced by `brassclaw_extensions` at parse
...
/// registrar (`brassclaw_hooks::registrar::MAX_HOOKS_PER_EXTENSION`); the
...
/// TOML payload (see [`HookSectionEntryV2`]) so `brassclaw_extensions` never
...
/// `brassclaw_hooks::HookManifestEntry` (which applies its own field-level
...
///
/// **Clean-boundary contract:** `brassclaw_extensions` is substrate and must
/// not depend on `brassclaw_hooks`. So this DTO does *not* embed the typed
/// hook entry (`brassclaw_hooks::manifest::HookManifestEntry`). Instead it
...
/// loader (`brassclaw_reborn_composition`, which depends on *both* crates)
...
pub struct HookSectionEntryV2 {
    ...
    /// `brassclaw_hooks::HookManifestEntry` by the composition loader (via
...
/// `brassclaw.product_adapter/v1` or `brassclaw.capability_provider/v1`. It is
...
///
/// `brassclaw_extensions` owns the generic envelope and section dispatch. Domain
...
pub struct ExtensionManifestV2 {
    ...
    /// only ever comes from `brassclaw_trust::TrustPolicy::evaluate` on a
    ...
    /// loader projects them into typed `brassclaw_hooks::HookManifestEntry`
...
pub enum ManifestV2Error {
    ...
    #[error(
        "manifest source {manifest_source:?} is not allowed to use legacy top-level capabilities; declare brassclaw.capability_provider/v1 host_api instead"
    )]
