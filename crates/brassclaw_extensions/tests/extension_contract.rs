...
async fn discovery_validates_host_api_manifest_with_supplied_contracts() {
    ...
    contracts
        .register(std::sync::Arc::new(TestProductAdapterContract {
            id: HostApiId::new("brassclaw.product_adapter/v1").unwrap(),
        }))
        .unwrap();
...
//
// `brassclaw_extensions` carries hook declarations as structurally-typed
...
// semantic projection into typed `brassclaw_hooks::HookManifestEntry` values
