...
[[capabilities]]
id = "brassclaw.memory.native.context.retrieve"
...
fn host_bundled_accepts_non_reserved_id() {
    // Spec: the `brassclaw.` prefix is reserved *for* HostBundled. It is not
...
fn rejects_manifest_exceeding_max_size() {
    use brassclaw_extensions::{MAX_MANIFEST_BYTES, ManifestV2Error};
...
fn rejects_unknown_host_api_fail_closed() {
    ...
    let toml = telegram_multi_host_api_manifest()
        .replace("brassclaw.product_adapter/v1", "brassclaw.unknown/v1");
