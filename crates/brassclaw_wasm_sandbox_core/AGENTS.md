# Agent Map — brassclaw_wasm_sandbox_core
...
- Neighboring runtime crates:
  - `crates/brassclaw_wasm/AGENTS.md`
  - `crates/brassclaw_wasm_product_adapters/AGENTS.md`
...
- Fast local check: `cargo test -p brassclaw_wasm_sandbox_core`
- Boundary check after dependency/API changes: `cargo test -p brassclaw_architecture`
- Run downstream runtime tests when changing exported sandbox primitives: `cargo test -p brassclaw_wasm` and `cargo test -p brassclaw_wasm_product_adapters`.
