...
fn auth_guidance(provider: &str) -> String {
    ...
    let (env_hint, extra) = match normalized.as_str() {
        "nearai" | "near_ai" | "near" => (
            "Set NEARAI_API_KEY (from https://cloud.near.ai) or run `brassclaw onboard` to log in",
            "",
        ),
        "openai" => (
        ...
        "openai_codex" | "codex" => ("Run `brassclaw login --openai-codex` to authenticate", ""),
        "github_copilot" => (
            "Set GITHUB_COPILOT_TOKEN or run `brassclaw onboard --step provider` to log in via device code",
            "",
        ),
        _ => (
...
mod tests {
    ...
    fn auth_failed_error_includes_guidance() {
        ...
        assert!(
            msg.contains("brassclaw onboard"),
    ...
    fn snapshot_auth_failed_nearai() {
        insta::assert_snapshot!(
            ...
            @"Authentication failed for provider 'nearai'. Set NEARAI_API_KEY (from https://cloud.near.ai) or run `brassclaw onboard` to log in. Or run `brassclaw onboard --step provider` to configure interactively."
        );
    }
    ...
    fn snapshot_auth_failed_openai() {
        insta::assert_snapshot!(
            ...
            @"Authentication failed for provider 'openai'. Set OPENAI_API_KEY (from https://platform.openai.com/api-keys). Or run `brassclaw onboard --step provider` to configure interactively."
        );
    }
    ...
    fn snapshot_auth_failed_anthropic() {
        insta::assert_snapshot!(
            ...
            @"Authentication failed for provider 'anthropic'. Set ANTHROPIC_API_KEY (from https://console.anthropic.com/settings/keys). Or run `brassclaw onboard --step provider` to configure interactively."
        );
    }
    ...
    fn snapshot_auth_failed_ollama() {
        insta::assert_snapshot!(
            ...
            @"Authentication failed for provider 'ollama'. Ensure Ollama is running locally (no API key needed). Set OLLAMA_BASE_URL if not at default http://localhost:11434. Or run `brassclaw onboard --step provider` to configure interactively."
        );
    }
    ...
    fn snapshot_auth_failed_openai_compatible() {
        insta::assert_snapshot!(
            ...
            @"Authentication failed for provider 'openai_compatible'. Set LLM_API_KEY and LLM_BASE_URL for your OpenAI-compatible endpoint. Or run `brassclaw onboard --step provider` to configure interactively."
        );
    }
    ...
    fn snapshot_auth_failed_tinfoil() {
        insta::assert_snapshot!(
            ...
            @"Authentication failed for provider 'tinfoil'. Set TINFOIL_API_KEY. Or run `brassclaw onboard --step provider` to configure interactively."
        );
    }
    ...
    fn snapshot_auth_failed_bedrock() {
        insta::assert_snapshot!(
            ...
            @"Authentication failed for provider 'bedrock'. Configure AWS credentials (AWS_ACCESS_KEY_ID/AWS_SECRET_ACCESS_KEY or AWS_PROFILE). Or run `brassclaw onboard --step provider` to configure interactively."
        );
    }
    ...
    fn snapshot_auth_failed_unknown_provider() {
        ...
        insta::assert_snapshot!(
            ...
            @"Authentication failed for provider 'some_future_provider'. Check that the required API key environment variable is set for this provider. Or run `brassclaw onboard --step provider` to configure interactively."
        );
    }
}
