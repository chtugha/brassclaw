...
const GOOG_API_CLIENT: &str = concat!("gl-rust/1.0.0 brassclaw/", env!("CARGO_PKG_VERSION"));
...
fn generate_pkce_params() -> PKCEParams {
    ...
    let code_verifier = brassclaw_common::pkce::generate_code_verifier();
    let code_challenge = brassclaw_common::pkce::s256_challenge(code_verifier.as_bytes());
...
impl GeminiOauthProvider {
    ...
    fn to_gemini_request(
        ...
        // sets it because it displays thoughts to the user. BrassClaw's reasoning
