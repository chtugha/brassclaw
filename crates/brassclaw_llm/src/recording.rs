...
//!
//! Enable by setting `BRASSCLAW_RECORD_TRACE=1` at runtime.
...
/// and API keys.
///
/// `brassclaw_safety::LeakDetector` runs on response bodies before they
...
/// `brassclaw_safety::LeakDetector` runs on response bodies upstream in
...
fn unwrap_tool_output(content: &str) -> std::borrow::Cow<'_, str> {
    if let Some(body) = brassclaw_safety::SafetyLayer::unwrap_tool_output(content) {
...
impl RecordingLlm {
    ...
    ///
    /// - `BRASSCLAW_RECORD_TRACE` — any non-empty value enables recording
    /// - `BRASSCLAW_TRACE_OUTPUT` — file path (default: `./trace_{timestamp}.json`)
    /// - `BRASSCLAW_TRACE_MODEL_NAME` — model_name field (default: `recorded-{inner.model_name()}`)
    pub fn from_env(inner: Arc<dyn LlmProvider>) -> Option<Arc<Self>> {
        let enabled = std::env::var("BRASSCLAW_RECORD_TRACE")
            .ok()
            .filter(|v| !v.is_empty());
        enabled?;

        let output_path = std::env::var("BRASSCLAW_TRACE_OUTPUT")
            .ok()
            .filter(|v| !v.is_empty())
            .map(PathBuf::from)
            .unwrap_or_else(|| {
        ...
        let model_name = std::env::var("BRASSCLAW_TRACE_MODEL_NAME")
            .ok()
            .filter(|v| !v.is_empty())
    ...
    /// workspace type out of `brassclaw_llm` avoids a heavy reverse dep.
...
mod tests {
    ...
    fn from_env_returns_none_when_unset() {
        let _env_lock = brassclaw_common::env_helpers::lock_env();
        let prior = std::env::var_os("BRASSCLAW_RECORD_TRACE");
        // SAFETY: Tests serialize env access with lock_env().
        unsafe { std::env::remove_var("BRASSCLAW_RECORD_TRACE") };
        let stub = Arc::new(StubLlm::new("response"));
        ...
        match prior {
            Some(value) => {
                // SAFETY: Tests serialize env access with lock_env().
                unsafe { std::env::set_var("BRASSCLAW_RECORD_TRACE", value) };
            }
            None => {
                // SAFETY: Tests serialize env access with lock_env().
                unsafe { std::env::remove_var("BRASSCLAW_RECORD_TRACE") };
