...
//! OAuth flow. Tokens are stored in `~/.brassclaw/session.json` and refreshed
...
pub struct SessionConfig {
    ...
    /// Path to session file (e.g., ~/.brassclaw/session.json).
...
impl SessionManager {
    ...
    pub async fn near_wallet_login(
        ...
        let response = self
            .client
            .post(&url)
            .header(reqwest::header::USER_AGENT, "brassclaw")
        ...
        if !response.status().is_success() {
            ...
            let preview = brassclaw_common::truncate_for_preview(&body, 200);
