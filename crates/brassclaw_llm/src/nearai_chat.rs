        ...
        if !status.is_success() {
            ...
            if matches!(status_code, 500..=599) {
                tracing::debug!(
                    ...
                    status = status_code,
                    body_preview =
                        brassclaw_common::truncate_for_preview(&response_text, 512).as_str(),
    ...
    async fn list_models_inner(&self) -> Result<Vec<ModelInfo>, LlmError> {
        ...
        Err(LlmError::InvalidResponse {
            ...
            reason: format!(
                ...
                brassclaw_common::truncate_preview(&response_text, 300)
            ),
        })
    }
}
...
/// `brassclaw_llm::reasoning`. Emitted at `TRACE` level so default loggers
...
fn emit_reasoning_trace(reasoning: Option<&str>) {
    if let Some(rc) = reasoning.filter(|s| !s.is_empty()) {
        tracing::trace!(target: "brassclaw_llm::reasoning", "{rc}");
    }
}
...
mod tests {
    ...
    /// dedicated `brassclaw_llm::reasoning` target.
    #[test]
    ...
    fn empty_reasoning_emits_no_event() {
        ...
        assert!(
            !logs_contain("brassclaw_llm::reasoning"),
    ...
    fn reasoning_emission_uses_dedicated_target() {
        ...
        assert!(
            logs_contain("brassclaw_llm::reasoning"),
            "emission should use brassclaw_llm::reasoning target"
        );
    ...
    async fn test_resolve_bearer_token_session_beats_env_var() {
        ...
        impl EnvLockGuard {
            fn new() -> Self {
                Self {
                    _guard: brassclaw_common::env_helpers::lock_env(),
                }
            }
        }
        struct EnvVarGuard {
