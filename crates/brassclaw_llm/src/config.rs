...
use brassclaw_common::paths::brassclaw_base_dir;
...
pub struct OpenAiCodexConfig {
    ...
    /// Path to session file (default: ~/.brassclaw/openai_codex_session.json).
...
impl Default for OpenAiCodexConfig {
    fn default() -> Self {
        Self {
            ...
            session_path: brassclaw_base_dir().join("openai_codex_session.json"),
...
impl NearAiConfig {
    ...
    pub(crate) fn for_model_discovery() -> Self {
        let api_key = brassclaw_common::env_helpers::env_or_override("NEARAI_API_KEY")
            .filter(|k| !k.is_empty())
        ...
        let base_url = brassclaw_common::env_helpers::env_or_override("NEARAI_BASE_URL")
