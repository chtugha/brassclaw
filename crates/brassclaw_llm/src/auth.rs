//! Public auth facade for `brassclaw_llm`.
//!
//! External callers (the setup wizard, the `brassclaw login` subcommand, and
...
/// `brassclaw_llm` can read directly.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
///
/// Keeps display/browser-launch concerns out of `brassclaw_llm`: the wizard,
...
pub enum LoginRequest {
    ...
    /// persisted to disk by `brassclaw_llm`; the caller does not see them.
...
impl OpenAiCodexLoginOptions {
    ...
    ///
    /// Use this from `brassclaw login --openai-codex` so config-file
...
/// It is `None` when `brassclaw_llm` already persisted credentials on disk
