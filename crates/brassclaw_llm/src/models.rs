...
pub(crate) async fn fetch_anthropic_models(cached_key: Option<&str>) -> Vec<(String, String)> {
    ...
    let oauth_token = if api_key.is_none() {
        brassclaw_common::env_helpers::env_or_override("ANTHROPIC_OAUTH_TOKEN")
            .filter(|t| !t.is_empty())
    } else {
        None
    };
...
pub fn build_nearai_model_fetch_config() -> crate::config::LlmConfig {
    let auth_base_url = brassclaw_common::env_helpers::env_or_override("NEARAI_AUTH_URL")
    ...
    crate::config::LlmConfig {
        ...
        session: crate::session::SessionConfig {
            auth_base_url,
            session_path: brassclaw_common::paths::brassclaw_base_dir().join("session.json"),
        },
