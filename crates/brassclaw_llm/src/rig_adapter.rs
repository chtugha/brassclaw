//! Generic adapter that bridges rig-core's `CompletionModel` trait to BrassClaw's `LlmProvider`.
//!
...
/// Convert BrassClaw messages to rig-core format.
///
...
/// Convert BrassClaw tool definitions to rig-core format.
///
...
/// Convert BrassClaw tool_choice string to rig-core ToolChoice.
...
fn extract_response(
    ...
    for content in choice.iter() {
        match content {
            ...
            // Image variants are not mapped to BrassClaw types
            _ => {}
        }
    }
...
mod tests {
    ...
    /// reasoning round-trip *inside* rig-core. BrassClaw's RigAdapter sits
    ...
    /// BrassClaw conversion. On the next request it rebuilt rig messages
    ...
    ///   2. round-trip those onto an BrassClaw `ChatMessage`
    ...
    fn reasoning_and_signature_round_trip_through_chat_message() {
        ...
        // --- BrassClaw stores the assistant message + tool result ---
        ...
        // --- Turn 2: BrassClaw rebuilds the rig request from stored messages ---
        let messages = vec![
