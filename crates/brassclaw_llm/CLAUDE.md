...
| `recording.rs` | `RecordingLlm` — trace capture for E2E replay testing (`BRASSCLAW_RECORD_TRACE`) |
...
- If Codex is logged in with API-key mode, BrassClaw uses the standard OpenAI endpoint.
- If Codex is logged in with ChatGPT OAuth mode, BrassClaw routes to the private `chatgpt.com/backend-api/codex` Responses API via `codex_chatgpt.rs`.
...
**Dual auth modes:**
- **Session token** (default): `NEARAI_SESSION_TOKEN=sess_...`, base URL = `https://private.near.ai`. Tokens are persisted to `~/.brassclaw/session.json` (mode 0600) and optionally to the DB `settings` table (`nearai.session_token`). On 401 responses where the body contains "session" + "expired"/"invalid", `NearAiChatProvider` calls `session.handle_auth_failure()` which triggers the interactive OAuth login flow and retries once. Plain `AuthFailed` 401s are not retried.
...
**Tool message flattening:** NEAR AI's API doesn't support `role: "tool"` messages in the standard format. `nearai_chat.rs` defaults `flatten_tool_messages = true` and delegates the rewrite to `nearai_tool_message_flattening.rs`. Assistant messages that only carry provider tool-call protocol are dropped instead of converted into assistant-authored prose; assistant prose is preserved when present. Tool results become user-side observations using the shared `brassclaw_common::provider_transcript` grammar (`Tool result from <name>: <content>`). Use `NearAiChatProvider::new_with_flatten(..., false)` to disable for compliant endpoints.
...
1. Create `crates/brassclaw_llm/src/myprovider.rs` implementing `LlmProvider`
...
`host.rs` defines four traits that decouple `brassclaw_llm` from the binary:
...
**Auth flow:** Device code OAuth via `auth.openai.com/api/accounts/deviceauth/*` endpoints. On first run, displays a code for the user to enter at a URL. Tokens are persisted to `~/.brassclaw/openai_codex_session.json` (mode 0600) and auto-refreshed before expiry.
...
Raw provider
  ...
  → RecordingLlm            (trace capture; only when BRASSCLAW_RECORD_TRACE is set)
```
...
Set `BRASSCLAW_RECORD_TRACE=1` to enable live trace recording via `RecordingLlm`. Traces are JSON files containing: memory snapshot, HTTP exchanges from tools, and LLM steps (user inputs, text responses, tool call responses). Replay these in E2E tests via `TraceLlm`. Configure output path with `BRASSCLAW_TRACE_OUTPUT` (default: `trace_{timestamp}.json`).
