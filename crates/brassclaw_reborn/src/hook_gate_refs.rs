...
//!
//! `brassclaw_hooks` owns the hook middleware contract, but it deliberately
...
/// Per-invocation metadata captured outside `brassclaw_hooks` and consumed by
...
///
/// Existing approval resolution in `brassclaw_approvals::ApprovalResolver`
...
/// for router-backed hook gate factories without changing `brassclaw_hooks`.
