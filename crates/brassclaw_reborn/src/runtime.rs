    ...
    G: HostManagedModelGateway + ?Sized + Send + Sync + 'static,
{
    ...
    pub coordinator: Arc<dyn brassclaw_turns::TurnCoordinator>,
    ...
    G: HostManagedModelGateway + ?Sized + Send + Sync + 'static,
{
    ...
    let spawn_decorator = Arc::new(SubagentSpawnCapabilityDecorator::new(
        SubagentSpawnDeps {
            coordinator: Arc::clone(&coordinator) as Arc<dyn brassclaw_turns::TurnCoordinator>,
            child_runs,
            ...
            gate_store: Arc::clone(&parts.subagent_gate_store)
                as Arc<dyn brassclaw_loop_support::SubagentGateResolutionStore>,
...
impl SubagentSpawnCapabilityDecorator {
    fn new(
        ...
        let spawn_id =
            CapabilityId::new(brassclaw_loop_support::DEFAULT_SPAWN_SUBAGENT_CAPABILITY_ID)
                .map_err(|error| DefaultPlannedRuntimeBuildError::RunProfile(error.to_string()))?;
        Ok(Self {
