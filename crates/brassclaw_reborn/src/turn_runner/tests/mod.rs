...
fn test_run_state(scope: TurnScope, status: TurnStatus) -> TurnRunState {
    TurnRunState {
        ...
        resolved_run_profile_id: brassclaw_turns::RunProfileId::default_profile(),
...
impl TurnRunTransitionPort for MockTransitionPort {
    ...
    async fn record_model_route_snapshot(
        ...
        self.calls
            .lock()
            .expect("lock")
            .push(TransitionCall::RecordModelRouteSnapshot);
        request
            .snapshot
            .validate()
            .map_err(|reason| TurnError::InvalidRequest { reason })?;
        self.route_snapshot_requests
            .lock()
            .expect("lock")
            .push(request.clone());
        let mut state = test_run_state(test_scope(), TurnStatus::Running);
        state.resolved_model_route = Some(request.snapshot);
        Ok(state)
    }
...
impl brassclaw_turns::run_profile::LoopRunInfoPort for StubHost {
    fn run_context(&self) -> &brassclaw_turns::run_profile::LoopRunContext {
        &self.context
    }
}

#[async_trait]
impl brassclaw_turns::run_profile::LoopContextPort for StubHost {
...
#[async_trait]
impl brassclaw_turns::run_profile::LoopPromptPort for StubHost {
...
#[async_trait]
impl brassclaw_turns::run_profile::LoopInputPort for StubHost {
...
#[async_trait]
impl brassclaw_turns::run_profile::LoopModelPort for StubHost {
...
#[async_trait]
impl brassclaw_turns::run_profile::LoopCompactionPort for StubHost {
    async fn compact_loop_context(
        &self,
        _request: brassclaw_turns::run_profile::LoopCompactionRequest,
    ) -> Result<
...
#[async_trait]
impl brassclaw_turns::run_profile::LoopCapabilityPort for StubHost {
...
#[async_trait]
impl brassclaw_turns::run_profile::LoopTranscriptPort for StubHost {
...
#[async_trait]
impl brassclaw_turns::run_profile::LoopCheckpointPort for StubHost {
    async fn checkpoint(
        &self,
...
#[async_trait]
impl brassclaw_turns::run_profile::LoopProgressPort for StubHost {
...
#[async_trait]
impl brassclaw_turns::run_profile::LoopCancellationPort for StubHost {
    fn observe_cancellation(&self) -> Option<brassclaw_turns::run_profile::LoopCancellationSignal> {
        None
    }

    async fn cancellation_requested(&self) -> brassclaw_turns::run_profile::LoopCancellationSignal {
...
impl brassclaw_turns::run_profile::LoopRetrievalPort for StubHost {
...
impl MockHostFactory {
    fn host_for_claim(claimed: &ClaimedTurnRun) -> StubHost {
        let mut context = brassclaw_turns::run_profile::LoopRunContext::new(
        ...
        if let Some(snapshot) = claimed.state.resolved_model_route.clone() {
            context = context.with_resolved_model_route(snapshot);
        }
        StubHost { context }
    }
}
...
impl HostFactory for MockHostFactory {
    async fn create_host(
        ...
        Ok(Box::new(Self::host_for_claim(claimed)))
    }
}
