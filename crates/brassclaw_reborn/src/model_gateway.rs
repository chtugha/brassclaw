...
//! that contract to the shared `brassclaw_llm` provider abstraction.
...
use brassclaw_host_api::sha256_digest_token;
use brassclaw_llm::{
...
use brassclaw_threads::{ProviderToolCallReferenceEnvelope, SessionThreadService, ThreadScope};
use brassclaw_turns::run_profile::LoopModelUsage;
use brassclaw_turns::{
    TurnId, TurnRunId,
    run_profile::{
...
impl LlmModelProfilePolicy {
    ...
    /// to its per-token price via [`brassclaw_llm::costs::model_cost`].
    /// Profiles whose `model_override` is unknown to the LLM cost table
    /// fall back to [`brassclaw_llm::costs::default_cost`] (roughly GPT-4o
...
/// [`HostManagedLoopModelPort`](brassclaw_turns::run_profile::HostManagedLoopModelPort):
...
/// Host-managed model gateway backed by the shared `brassclaw_llm::LlmProvider` abstraction.
#[derive(Clone)]
...
async fn complete_model_request<P>(
    ...
    capabilities: Option<Arc<dyn brassclaw_turns::run_profile::LoopCapabilityPort>>,
...
fn validate_provider_replay_identity(
    ...
    provider_call.validate().map_err(|error| {
        brassclaw_loop_support::raw_host_managed_model_error(
...
mod tests {
    ...
    fn tool_result_replay_prefers_model_observation_over_safe_summary() {
        ...
        let envelope = brassclaw_threads::ToolResultReferenceEnvelope::with_model_observation(
            "result:tool-error",
            brassclaw_threads::ToolResultSafeSummary::new("tool failed").expect("safe summary"),
            observation.clone(),
        )
