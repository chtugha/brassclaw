...
//! downstream consumer is `brassclaw_reborn_composition`, which composes the
...
//! `brassclaw_architecture` enforce that nothing else takes a normal cargo
...
//! (`brassclaw_reborn::driver_registry::DriverRegistry`,
//! `brassclaw_reborn::model_gateway::LlmProviderModelGateway`, …) so that a
