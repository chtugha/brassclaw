...
impl HookPromptMaterializationSink for InstructionStoreBackedHookSink {
    fn put(
        &self,
        role: &str,
        content_ref: &brassclaw_turns::LoopMessageRef,
        safe_content: String,
...
// `LoopCapabilityInputResolver` is now defined in `brassclaw_loop_support`
...
/// [`crate::brassclaw_hooks::points::SanitizedArguments`] already caps per-string
...
/// `brassclaw_hooks` middleware as a
/// [`brassclaw_hooks::middleware::CapabilityInputResolver`].
///
...
///   [`brassclaw_hooks::points::SanitizedArguments`]) apply to predicate
...
impl HookCapabilityInputResolver for HookCapabilityInputResolverAdapter {
    async fn resolve(
        &self,
        invocation: &brassclaw_turns::run_profile::CapabilityInvocation,
...
// `LoopCapabilityResultWriter` is now defined in `brassclaw_loop_support`
// (workspace-10 refactor); imported above.
...
impl EventTriggeredHookSubscription {
    ...
    fn effective_read_scope(
        &self,
        run_scope: &brassclaw_turns::TurnScope,
        thread_scope: &brassclaw_threads::ThreadScope,
    ...
    /// caller, and widening its visibility would let other `brassclaw_reborn`
    ...
    async fn run(
        ...
        loop {
                ...
                .await
            {
                ...
                Err(brassclaw_events::EventError::ReplayGap {
                    requested,
                    earliest,
                }) => {
                    tracing::error!(
...
async fn emit_subscription_terminated_note(
    ...
    let summary = match brassclaw_turns::run_profile::LoopSafeSummary::new(safe_summary) {
        Ok(s) => s,
        Err(_) => {
    ...
    let milestone = brassclaw_turns::run_profile::LoopHostMilestone {
        ...
        kind: brassclaw_turns::run_profile::LoopHostMilestoneKind::DriverNote {
            kind: brassclaw_turns::run_profile::LoopDriverNoteKind::EventSubscriptionTerminated,
    ...
    G: HostManagedModelGateway + ?Sized,
{
    ...
    /// `brassclaw_reborn_composition::hooks` ("Predicate counter scoping").
    ...
    hook_gate_ref_factory: Option<Arc<dyn brassclaw_hooks::middleware::HookGateRefFactory>>,
...
pub type HookGateRefFactoryBuilder = Arc<
    dyn Fn(&LoopRunContext) -> Arc<dyn brassclaw_hooks::middleware::HookGateRefFactory>
        + Send
        + Sync,
>;
    ...
    G: HostManagedModelGateway + ?Sized + Send + Sync + 'static,
{
    ...
    /// [`brassclaw_turns::run_profile::HookMilestoneSink`] (typically a
    /// [`brassclaw_turns::run_profile::RunScopedHookMilestoneSink`] wrapping
    ...
    pub fn with_hook_gate_ref_factory(
        mut self,
        factory: Arc<dyn brassclaw_hooks::middleware::HookGateRefFactory>,
    ) -> Self {
    ...
    where
        F: Fn(&LoopRunContext) -> Arc<dyn brassclaw_hooks::middleware::HookGateRefFactory>
            + Send
            + Sync
            + 'static,
    {
...
impl LoopTranscriptPort for RebornLoopDriverHost {
    ...
    ) -> Result<brassclaw_turns::LoopMessageRef, AgentLoopHostError> {
    ...
    G: HostManagedModelGateway + ?Sized + Send + Sync + 'static,
{
    ...
    > {
        ...
        host_result
            .map(|host| {
                Box::new(host)
                    as Box<dyn brassclaw_turns::run_profile::AgentLoopDriverHost + Send + Sync>
            })
...
fn validate_thread_scope(
    ...
    // Reborn text-only hosts currently wrap `brassclaw_threads::ThreadScope`,
...
fn turn_error_to_host_error(error: TurnError) -> AgentLoopHostError {
    match &error {
        TurnError::Unauthorized => brassclaw_loop_support::raw_agent_loop_host_error(
            "checkpoint_state",
            "access",
        ...
        TurnError::InvalidRequest { .. } => brassclaw_loop_support::raw_agent_loop_host_error(
            "checkpoint_state",
            "request",
        ...
        TurnError::Unavailable { .. } => brassclaw_loop_support::raw_agent_loop_host_error(
            "checkpoint_state",
            "store",
            AgentLoopHostErrorKind::Unavailable,
        ...
        TurnError::ScopeNotFound => brassclaw_loop_support::raw_agent_loop_host_error(
            "checkpoint_state",
            "scope_lookup",
        ...
        TurnError::Conflict { .. } => brassclaw_loop_support::raw_agent_loop_host_error(
            "checkpoint_state",
            "write",
            AgentLoopHostErrorKind::CheckpointRejected,
        ...
        TurnError::CapacityExceeded { .. } => brassclaw_loop_support::raw_agent_loop_host_error(
            "checkpoint_state",
            "write",
            AgentLoopHostErrorKind::Unavailable,
        ...
        TurnError::InvalidTransition { .. } => brassclaw_loop_support::raw_agent_loop_host_error(
            "checkpoint_state",
            "write",
            AgentLoopHostErrorKind::CheckpointRejected,
        ...
        TurnError::LeaseMismatch => brassclaw_loop_support::raw_agent_loop_host_error(
            "checkpoint_state",
            "write",
            AgentLoopHostErrorKind::CheckpointRejected,
        ...
        TurnError::ThreadBusy(_) | TurnError::AdmissionRejected(_) => {
            brassclaw_loop_support::raw_agent_loop_host_error(
                "checkpoint_state",
                "admission",
                AgentLoopHostErrorKind::Unavailable,
...
mod hook_resolver_adapter_tests {
    ...
    use super::*;
    use brassclaw_host_api::{AgentId, CapabilityId, ProjectId, TenantId, ThreadId};
...
mod event_subscription_scope_tests {
    use super::*;
    use brassclaw_events::{InMemoryDurableEventLog, RuntimeEvent};
    ...
    impl DurableEventLog for HeadCursorFailingLog {
        async fn append(
            &self,
            _event: RuntimeEvent,
        ) -> Result<brassclaw_events::EventLogEntry<RuntimeEvent>, brassclaw_events::EventError>
        {
        ...
        async fn read_after_cursor(
            ...
            _limit: usize,
        ) -> Result<brassclaw_events::EventReplay<RuntimeEvent>, brassclaw_events::EventError>
        {
        ...
        async fn head_cursor(
            ...
            _stream: &EventStreamKey,
            _after: EventCursor,
        ) -> Result<EventCursor, brassclaw_events::EventError> {
            Err(brassclaw_events::EventError::DurableLog {
    ...
    async fn event_triggered_head_cursor_failure_emits_terminated_milestone() {
        ...
        let dispatcher = brassclaw_hooks::dispatch::HookDispatcherBuilder::new(
            brassclaw_hooks::HookRegistry::new(),
        )
        .build_arc();
...
mod turn_error_to_host_error_tests {
    use super::*;
    use brassclaw_turns::{TurnCapacityResource, TurnError};
