...
mod tests {
    ...
    use brassclaw_host_api::{AgentId, CapabilityId, TenantId, ThreadId};
    use brassclaw_loop_support::{
    ...
    async fn material_source_fails_loud_on_goal_miss() {
        ...
        let context = brassclaw_agent_loop::test_support::test_run_context("missing-goal");
    ...
    async fn material_source_combines_static_direction_goal_and_allowlist() {
        let store = Arc::new(InMemoryBoundedSubagentGoalStore::new());
        let context = brassclaw_agent_loop::test_support::test_run_context("goal");
        store
            .put_goal(
                &context.scope,
                context.run_id,
        ...
        assert!(
            material
                .allowed_capabilities
                .iter()
                .any(|cap| cap.as_str() == brassclaw_host_runtime::HTTP_CAPABILITY_ID)
        );
    }
    ...
    async fn gate_backed_material_source_uses_gate_flavor_and_goal_store() {
        ...
        let context = brassclaw_agent_loop::test_support::test_run_context("gate-backed-goal");
        store
            .put_goal(
                &context.scope,
                context.run_id,
    ...
    async fn gate_backed_material_source_falls_back_to_thread_metadata() {
        ...
        let mut context = brassclaw_agent_loop::test_support::test_run_context("thread-flavor");
    ...
    async fn gate_backed_material_source_errors_when_no_flavor_is_recorded() {
        ...
        let gate_store = Arc::new(BoundedSubagentGateResolutionStore::new());
        let thread_service = Arc::new(InMemorySessionThreadService::default());
        let mut context = brassclaw_agent_loop::test_support::test_run_context("missing-flavor");
    ...
    async fn gate_backed_material_source_errors_when_flavor_is_unknown() {
        ...
        let gate_store = Arc::new(BoundedSubagentGateResolutionStore::new());
        let thread_service = Arc::new(InMemorySessionThreadService::default());
        let mut context = brassclaw_agent_loop::test_support::test_run_context("unknown-flavor");
