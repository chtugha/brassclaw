...
use brassclaw_loop_support::{SubagentDefinition, SubagentDefinitionResolver, SubagentKindId};
use brassclaw_turns::{RunProfileRequest, TurnRunId, run_profile::AgentLoopHostError};
            ...
            requested_run_profile: RunProfileRequest::new(SUBAGENT_PLANNED_PROFILE_ID).map_err(
                |reason| {
                    AgentLoopHostError::new(
                        brassclaw_turns::run_profile::AgentLoopHostErrorKind::Internal,
                        reason,
                    )
                },
            )?,
...
mod tests {
    ...
    use brassclaw_loop_support::DEFAULT_SPAWN_SUBAGENT_CAPABILITY_ID;
    ...
    mod caller_level {
        ...
        use async_trait::async_trait;
        use brassclaw_agent_loop::test_support::test_run_context;
        use brassclaw_host_api::{CapabilityId, RuntimeKind};
        ...
        use brassclaw_turns::{LoopResultRef, RunProfileId, RunProfileVersion};
        ...
        async fn filter_for_flavor(
            ...
            brassclaw_loop_support::CapabilitySurfaceProfileFilter,
            Arc<HostSurfaceSpy>,
        ) {
        ...
        async fn visible_ids(
            filter: &brassclaw_loop_support::CapabilitySurfaceProfileFilter,
        ) -> Vec<String> {
            let mut ids = filter
