...
//!
//! `RebornLibSqlLocalTriggerAccessStore` (libSQL) is available behind the
//! `webui-user-store` feature.  `PgRebornLocalTriggerAccessStore` is the
//! unconditional PostgreSQL replacement; it targets the `brassclaw_local_access`
//! table created by V021.
//!
...
#[cfg(all(test, feature = "webui-user-store"))]
mod tests {
    use super::*;
...
impl PgRebornLocalTriggerAccessStore {
    ...
    /// Seed the local-dev trigger access row used by Reborn-owned fire-time
    /// trigger authorization. Existing rows are left untouched.
    pub async fn seed_local_access(
        &self,
        seed: LocalTriggerAccessSeed<'_>,
    ) -> Result<(), RebornLocalTriggerAccessStoreError> {
    ...
    pub async fn reconcile_local_access(
        ...
        // Load current active rows for this source/scope.
        let rows = client
            .query(
                "SELECT user_id \
