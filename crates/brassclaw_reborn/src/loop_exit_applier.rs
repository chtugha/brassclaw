...
//!
//! `brassclaw_turns` owns the trusted applier and the private validation policy.
//! This module provides Reborn-specific evidence adapters.

use std::sync::Arc;

use async_trait::async_trait;
use brassclaw_loop_support::RunCancellationFactory;
...
pub use brassclaw_turns::loop_exit::{
    ...
    S: SessionThreadService + ?Sized,
{
    ...
    loop_checkpoint_store: Arc<dyn brassclaw_turns::LoopCheckpointStore>,
    ...
    S: SessionThreadService + ?Sized + Send + Sync,
{
    ...
    async fn verify_final_checkpoint(
        ...
        let checkpoint = self
            .loop_checkpoint_store
            .get_loop_checkpoint(brassclaw_turns::GetLoopCheckpointRequest {
                scope: request.scope.clone(),
    ...
    async fn verify_failure_evidence(
        ...
        let state = match brassclaw_agent_loop::state::LoopExecutionState::from_checkpoint_payload(
            checkpoint_state.payload.as_bytes(),
            brassclaw_agent_loop::state::CheckpointKind::Final,
        ) {
            Ok(state) => state,
...
fn thread_scope_from_turn_scope(scope: &TurnScope) -> Result<ThreadScope, TurnError> {
    // `brassclaw_threads::ThreadScope` is currently agent-scoped. Reject
