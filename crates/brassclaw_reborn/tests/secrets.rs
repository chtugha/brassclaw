...
use brassclaw_host_api::{InvocationId, ResourceScope, SecretHandle, TenantId, UserId};
use brassclaw_reborn::secrets::{
...
use brassclaw_secrets::{SecretMaterial, SecretStoreError};
...
async fn reborn_secret_store_fails_closed_on_consume_when_existing_rows_use_another_master_key() {
    ...
    let handle = brassclaw_host_api::SecretHandle::new("openai_key").unwrap();
    store
        .put(
            scope.clone(),
            handle.clone(),
