...
async fn planned_driver_live_default_smoke() {
    ...
    let resolved = resolver
        .resolve_run_profile(brassclaw_turns::RunProfileResolutionRequest::interactive_default())
        .await
...
async fn planned_driver_resume_rejects_mismatched_ids_before_checkpoint_load() {
    ...
    let other_context = brassclaw_agent_loop::test_support::test_run_context("foreign-run");
...
async fn planned_driver_resume_rejects_mismatched_profile_before_checkpoint_load() {
    ...
    let host = ForbiddenResumeHost::new(context.clone());
    let mut request = resume_request(&context, TurnCheckpointId::new());
    let other_context = brassclaw_agent_loop::test_support::test_run_context("foreign-profile");
...
impl brassclaw_turns::run_profile::LoopRecipePort for ForbiddenResumeHost {
    fn recipe_lookup(&self) -> Option<&dyn brassclaw_turns::run_profile::RecipeLookup> {
        None
    }
}

impl brassclaw_turns::run_profile::LoopRetrievalPort for ForbiddenResumeHost {
