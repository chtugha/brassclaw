...
use brassclaw_hooks::HookRegistrar;
use brassclaw_hooks::dispatch::{HookDispatcher, HookDispatcherBuilder};
use brassclaw_hooks::error::HookError;
use brassclaw_hooks::evaluator::PredicateEvaluator;
use brassclaw_hooks::failure_policy::{FailureCategory, FailureDisposition};
use brassclaw_hooks::identity::{ExtensionId, HookId, HookLocalId, HookVersion};
use brassclaw_hooks::installed_hook::PredicateBackedBeforeCapabilityHook;
use brassclaw_hooks::kinds::observer::NoteCategory;
use brassclaw_hooks::manifest::{
...
use brassclaw_hooks::registry::{HookBindingScope, HookPointSpec, HookRegistry};
use brassclaw_hooks::sink::{
...
use brassclaw_reborn::hook_gate_refs::{
...
fn descriptor_with_provider(
    capability_id: &str,
    provider: Option<brassclaw_host_api::ExtensionId>,
) -> CapabilityDescriptorView {
    CapabilityDescriptorView {
        capability_id: CapabilityId::new(capability_id).expect("capability id literal is valid"),
        provider,
        runtime: brassclaw_host_api::RuntimeKind::Wasm,
...
fn event_triggered_dispatcher_with_scope(
    ...
    HookDispatcherBuilder::new(HookRegistry::new())
        ...
        .install_installed_event_triggered(
            ...
            HookPhase::Telemetry,
            event_kind,
            brassclaw_host_api::ExtensionId::new(owning_extension).expect("valid ext id"),
            scope,
...
fn event_triggered_dispatcher_with_subjects(
    ...
    scope: HookBindingScope,
    owning_extension: &str,
    subjects: &[(HookId, brassclaw_host_api::ExtensionId)],
) -> Arc<HookDispatcher> {
...
fn pause_approval_dispatcher() -> Arc<HookDispatcher> {
    ...
    HookDispatcherBuilder::new(HookRegistry::new())
        .install_installed_before_capability(
            hook_id,
            HookPhase::Policy,
            brassclaw_host_api::ExtensionId::new("integration-tests").expect("valid ext id"),
            HookBindingScope::Global,
...
impl Fixture {
    async fn new() -> Self {
        ...
        let state = brassclaw_turns::TurnRunState {
...
async fn event_triggered_hook_respects_own_capabilities_scope_filter() {
    ...
    let stream = EventStreamKey::from_scope(&scope);
    let log = Arc::new(InMemoryDurableEventLog::new());
    let ext_a = brassclaw_host_api::ExtensionId::new("ext-a").expect("valid ext");
    let ext_b = brassclaw_host_api::ExtensionId::new("ext-b").expect("valid ext");
...
async fn event_triggered_subscription_with_foreign_tenant_stream_fails_host_build() {
    let fixture = Fixture::new().await;
    let log = Arc::new(InMemoryDurableEventLog::new());
    let foreign_tenant = brassclaw_host_api::TenantId::new("tenant-foreign").expect("valid tenant");
...
async fn event_triggered_subscription_with_foreign_user_stream_fails_host_build() {
    let fixture = Fixture::new().await;
    let log = Arc::new(InMemoryDurableEventLog::new());
    let foreign_user = brassclaw_host_api::UserId::new("user-foreign").expect("valid user");
...
async fn event_triggered_self_lifecycle_event_does_not_redispatch() {
    ...
    let stream = EventStreamKey::from_scope(&scope);
    let log = Arc::new(InMemoryDurableEventLog::new());
    let ext = brassclaw_host_api::ExtensionId::new("ext-self").expect("valid ext");
...
async fn event_triggered_replay_gap_emits_subscription_terminated_milestone() {
    use brassclaw_turns::run_profile::{LoopDriverNoteKind, LoopHostMilestoneKind};
    let fixture = Fixture::new().await;
    let scope = runtime_scope(&fixture);
    let stream = EventStreamKey::from_scope(&scope);
    let log = Arc::new(InMemoryDurableEventLog::new());
...
/// dispatcher-level unit test in `brassclaw_hooks` proves a manually wired
...
async fn factory_hook_security_audit_sink_records_deny_through_build() {
    ...
    assert_eq!(
        event
            .capability_id
            .as_ref()
            .map(brassclaw_host_api::CapabilityId::as_str),
        Some("cap.blocked"),
...
async fn pause_approval_hook_surfaces_as_approval_required_with_real_gate_ref() {
    ...
    let host = fixture
        ...
        .with_hook_gate_ref_factory_builder({
            let f: Arc<dyn brassclaw_hooks::middleware::HookGateRefFactory> = Arc::new(factory);
            move |_| Arc::clone(&f)
        })
        .build_text_only_host_with_capabilities(fixture.request(), inner.clone())
        .await
...
async fn router_backed_gate_ref_factory_sources_context_per_mint_when_reused() {
    ...
    let host_factory = fixture
        ...
        .with_hook_dispatcher(pause_approval_dispatcher())
        .with_hook_gate_ref_factory_builder({
            let f: Arc<dyn brassclaw_hooks::middleware::HookGateRefFactory> =
    ...
    let turn_id_b = brassclaw_turns::TurnId::new();
...
// `brassclaw_hooks::middleware::{model_port, transcript_port, checkpoint_port}`
...
async fn after_model_fires_exactly_once_at_durable_boundary() {
    ...
    let bundle = host
        .build_prompt_bundle(brassclaw_turns::run_profile::LoopPromptBundleRequest {
            mode: brassclaw_turns::run_profile::PromptMode::TextOnly,
            context_cursor: None,
    ...
    host.finalize_assistant_message(brassclaw_turns::run_profile::FinalizeAssistantMessage {
...
async fn observer_panic_does_not_fail_model_call() {
    ...
    let response = host
        .finalize_assistant_message(brassclaw_turns::run_profile::FinalizeAssistantMessage {
            reply: brassclaw_turns::run_profile::AssistantReply {
...
async fn installed_hook_with_own_scope_does_not_fire_on_other_provider_capabilities() {
    ...
    let dispatcher = HookDispatcherBuilder::new(HookRegistry::new())
        .install_installed_before_capability(
            hook_id,
            HookPhase::Policy,
            brassclaw_host_api::ExtensionId::new("ext-a").expect("valid ext id"),
            HookBindingScope::OwnCapabilities,
            Box::new(AlwaysDeny),
        )
...
fn own_capabilities_dispatcher(owning_ext: &str, local_id: &str) -> Arc<HookDispatcher> {
    ...
    HookDispatcherBuilder::new(HookRegistry::new())
        .install_installed_before_capability(
            hook_id,
            HookPhase::Policy,
            brassclaw_host_api::ExtensionId::new(owning_ext).expect("valid ext id"),
            HookBindingScope::OwnCapabilities,
            Box::new(AlwaysDeny),
        )
        .expect("install installed hook with own-scope")
...
async fn own_capabilities_hook_fires_when_provider_matches() {
    let fixture = Fixture::new().await;
    let ext_a = brassclaw_host_api::ExtensionId::new("ext-a").expect("valid ext id");
...
async fn own_capabilities_hook_does_not_fire_when_provider_differs() {
    let fixture = Fixture::new().await;
    let ext_b = brassclaw_host_api::ExtensionId::new("ext-b").expect("valid ext id");
    let inner = Arc::new(ProviderAwareCapabilityPort::new(vec![
...
async fn hook_telemetry_attribution_is_per_run_not_captured() {
    ...
    let factory_with_hook = fixture.factory().with_hook_dispatcher_builder_factory(|| {
        use brassclaw_hooks::dispatch::HookDispatcherBuilder as HDBuilder;
        use brassclaw_hooks::registry::HookRegistry as HReg;
        let hook_id = HookId::derive(
        ...
        Ok(HDBuilder::new(HReg::new())
            .install_installed_before_capability(
                hook_id,
                HookPhase::Policy,
                brassclaw_host_api::ExtensionId::new("ext-tele").expect("valid ext id"),
                HookBindingScope::Global,
                Box::new(AlwaysDeny),
            )
    ...
    let new_turn_id = brassclaw_turns::TurnId::new();
...
async fn before_prompt_hook_message_is_resolvable_via_factory_wiring() {
    use brassclaw_hooks::dispatch::HookDispatcherBuilder as HDBuilder;
    use brassclaw_hooks::registry::HookRegistry as HReg;
    use brassclaw_hooks::sink::{RestrictedBeforePromptHook, RestrictedMutatorSink};
    ...
    impl RestrictedBeforePromptHook for InjectingHook {
        async fn evaluate(
            &self,
            _ctx: &brassclaw_hooks::points::BeforePromptHookContext,
            sink: &mut dyn RestrictedMutatorSink,
        ) {
            let _ = sink.add_envelope_snippet(
                "injected hook context".to_string(),
                brassclaw_hooks::kinds::mutator::PatchOrdinalHint::Last,
            );
        }
    }
    ...
    let dispatcher = HDBuilder::new(HReg::new())
        .install_installed_before_prompt(
            hook_id,
            HookPhase::Policy,
            brassclaw_host_api::ExtensionId::new("ext-prompt").expect("valid ext id"),
            HookBindingScope::Global,
