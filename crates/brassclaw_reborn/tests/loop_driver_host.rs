...
async fn progress_event_serde_roundtrip_all_variants() {
    ...
    let bundle_ref = brassclaw_turns::run_profile::LoopPromptBundleRef::for_run(&context, "bundle")
        .expect("bundle ref");
...
async fn turn_runner_worker_completes_after_libsql_turn_and_thread_services_reopen() {
    ...
    let run_id = {
        ...
        let submit = turn_store
            .submit_turn(
                ...
                &brassclaw_turns::AllowAllTurnAdmissionPolicy,
                &resolver,
            )
            .await
            .unwrap();
    ...
    let transition_port: Arc<dyn brassclaw_turns::runner::TurnRunTransitionPort> =
        turn_store.clone();
...
/// [`MountView`](brassclaw_host_api::MountView) so tenant isolation is
...
async fn build_libsql_thread_service(
    ...
    brassclaw_threads::FilesystemSessionThreadService::new(scoped)
}
...
async fn libsql_filesystem_turn_store(
    db: Arc<libsql::Database>,
) -> brassclaw_turns::FilesystemTurnStateStore<brassclaw_filesystem::LibSqlRootFilesystem> {
    use brassclaw_filesystem::{LibSqlRootFilesystem, ScopedFilesystem};
    use brassclaw_host_api::{MountAlias, MountGrant, MountPermissions, MountView, VirtualPath};
    ...
    brassclaw_turns::FilesystemTurnStateStore::new(scoped)
}
...
async fn turn_runner_blocks_on_approval_then_coordinator_resume_completes_same_run() {
    ...
    assert_eq!(
        turn_store
            ...
            .kind,
        brassclaw_turns::TurnEventKind::Resumed,
...
// factory shaped exactly like the one `brassclaw_reborn_composition::hooks`
...
/// `brassclaw_reborn_composition::hooks::NoOpObserverHook`. The only surviving
/// `NoOpObserverHook` is the composition crate's test-only one at
/// `brassclaw_reborn_composition::hooks::tests::NoOpObserverHook`. This constant
...
const E2E_NOOP_OBSERVER_PATH: &str = "brassclaw_reborn::tests::loop_driver_host::E2eNoOpObserver";
...
async fn build_runtime_host_with_optional_hooks(
    ...
    hook_factory: Option<brassclaw_reborn::loop_driver_host::HookDispatcherBuilderFactory>,
) -> (
...
// doubles that exercise the `brassclaw_reborn` host-factory PLUMBING — i.e. that
...
// `brassclaw_reborn_composition::hooks` and
// `brassclaw_reborn_composition::runtime` tests. Keep this split: host plumbing
...
fn extension_deny_hook_factory(
    ...
    Arc::new(move || {
        ...
        let ext_id =
            brassclaw_host_api::ExtensionId::new(extension.clone()).expect("valid extension id");
...
impl HostInputQueue for SingleMessageQueue {
    async fn next_after(
        &self,
        _run_id: TurnRunId,
        after: brassclaw_turns::run_profile::LoopInputCursorToken,
        _limit: usize,
        ...
        match pending {
            Some(input) => {
                let cursor =
                    brassclaw_turns::run_profile::LoopInputCursorToken::new("input-cursor:1")
                        .unwrap();
                let ack_token =
                    brassclaw_turns::run_profile::LoopInputAckToken::new("input-ack:1").unwrap();
                Ok(HostInputBatch {
...
async fn text_only_host_maps_explicit_unknown_runtime_outcome_to_failure() {
    ...
    assert_eq!(
        ...
        CapabilityFailureKind::Unknown(
            brassclaw_turns::run_profile::CapabilityFailureKindValue::new("streaming")
...
async fn text_only_host_rejects_mismatched_capability_authority_context() {
    ...
    assert!(matches!(
        error,
        brassclaw_reborn::loop_driver_host::RebornLoopDriverHostError::InvalidRequest { .. }
    ));
    assert!(runtime.invocations().is_empty());
}
...
struct RecordingHostRuntime {
    surface: Mutex<brassclaw_host_runtime::VisibleCapabilitySurface>,
...
impl RecordingHostRuntime {
    fn with_surface(surface: brassclaw_host_runtime::VisibleCapabilitySurface) -> Self {
        Self {
    ...
    fn set_surface(&self, surface: brassclaw_host_runtime::VisibleCapabilitySurface) {
...
) -> brassclaw_host_runtime::VisibleCapabilitySurface {
    host_runtime_surface_with_version(
...
fn host_runtime_surface_with_version(
    ...
    brassclaw_host_runtime::VisibleCapabilitySurface {
        version: brassclaw_host_runtime::CapabilitySurfaceVersion::new(version).unwrap(),
...
fn host_runtime_visible_request(
    ...
    brassclaw_host_runtime::VisibleCapabilityRequest::new(
        context,
...
fn driver_host_error(
    error: brassclaw_turns::run_profile::AgentLoopHostError,
...
impl TurnStateStore for StaticTurnStateStore {
    ...
    ) -> Result<brassclaw_turns::ResumeTurnResponse, TurnError> {
...
impl HostFixture {
    ...
    async fn stage_checkpoint_state(
        ...
        payload: &[u8],
    ) -> brassclaw_turns::CheckpointStateRecord {
    ...
    fn respond_with_capability_calls(&self) {
        *self.response.lock().unwrap() = Ok(HostManagedModelResponse {
            ...
            output: ParentLoopOutput::CapabilityCalls(vec![
                brassclaw_turns::run_profile::CapabilityCallCandidate {
