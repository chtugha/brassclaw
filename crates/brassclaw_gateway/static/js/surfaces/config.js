...
//
// Widget authors call BrassClaw.registerWidget({ id, name, slot, init, ... })
// from their module script. The init() function receives a container DOM element
// and the BrassClaw.api object for authenticated fetch, event subscription, etc.

// Define `window.BrassClaw` as a non-writable, non-configurable property
// rather than `window.BrassClaw = window.BrassClaw || {}`. The `|| {}` form
// would honor any pre-existing value on `window.BrassClaw`, which in
...
// has no inline scripts that touch `window.BrassClaw`, so this is
...
// `BrassClaw` object after the fact — its only path is to mutate properties
// on the fixed object, which is the same authority every other widget has.
Object.defineProperty(window, 'BrassClaw', {
  value: {},
  writable: false,
  configurable: false,
  enumerable: true,
});
BrassClaw.widgets = new Map();
BrassClaw._widgetInitQueue = [];
BrassClaw._chatRenderers = [];
 ...
 */
BrassClaw.registerWidget = function(def) {
  if (!def.id || !def.init) {
    console.error('[BrassClaw] Widget registration requires id and init:', def);
    return;
  }
  BrassClaw.widgets.set(def.id, def);
 ...
 */
BrassClaw.registerChatRenderer = function(def) {
  if (!def.id || !def.match || !def.render) {
    console.error('[BrassClaw] Chat renderer requires id, match, and render:', def);
    return;
  }
  BrassClaw._chatRenderers.push(def);
  // Sort by priority (higher first)
  BrassClaw._chatRenderers.sort(function(a, b) {
...
BrassClaw.api = {
  ...
  fetch: function(path, opts) {
    ...
    try {
      ...
      return Promise.reject(
        new TypeError('BrassClaw.api.fetch: invalid URL ' + JSON.stringify(path))
      );
    }
    if (resolved.origin !== window.location.origin) {
      return Promise.reject(
        new TypeError(
          'BrassClaw.api.fetch: cross-origin requests are not allowed (got ' +
  ...
  _dispatch: function(eventType, data) {
    ...
    for (var i = 0; i < handlers.length; i++) {
      try { handlers[i](data); } catch (e) {
        console.error('[BrassClaw] Widget event handler error (' + eventType + '):', e);
      }
    }
  },
  ...
  share: function(opts) {
    ...
    var text = opts.text || '';
    var hashtags = opts.hashtags || 'DeFi,BrassClaw';
    ...
    dlBtn.onclick = function() {
      ...
      a.download = 'brassclaw-portfolio-gains.png';
      a.click();
