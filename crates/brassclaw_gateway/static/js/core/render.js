...
// extensible chat renderers via BrassClaw.registerChatRenderer().
...
function upgradeStructuredData(contentEl) {
  ...
  var renderers = (window.BrassClaw && BrassClaw._chatRenderers) || [];
  for (var i = 0; i < renderers.length; i++) {
    try {
      ...
      console.error('[BrassClaw] Chat renderer "' + renderers[i].id + '" failed:', e);
    }
  }
  ...
  var layoutCfg = window.__BRASSCLAW_LAYOUT__;
