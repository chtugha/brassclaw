function _addWidgetTab(def) {
  ...
  if (!tabBar || !tabContent) {
    ...
    BrassClaw._widgetInitQueue.push(def);
    return;
  }
  ...
  try {
    def.init(panel, BrassClaw.api);
  } catch (e) {
    console.error('[BrassClaw] Widget "' + def.id + '" init failed:', e);
...
if (window.__BRASSCLAW_LAYOUT__) {
  (function() {
    var layout = window.__BRASSCLAW_LAYOUT__;
...
if (BrassClaw._widgetInitQueue && BrassClaw._widgetInitQueue.length > 0) {
  BrassClaw._widgetInitQueue.forEach(function(def) {
    _addWidgetTab(def);
  });
...
if (window.__BRASSCLAW_LAYOUT__
    && window.__BRASSCLAW_LAYOUT__.tabs
    && window.__BRASSCLAW_LAYOUT__.tabs.default_tab
    && !window.location.hash) {
  switchTab(window.__BRASSCLAW_LAYOUT__.tabs.default_tab);
}
