...
function authenticate() {
  ...
  apiFetch('/api/chat/threads')
    .then(() => {
      sessionStorage.setItem('brassclaw_token', token);
      initApp();
    })
    .catch(() => {
      sessionStorage.removeItem('brassclaw_token');
...
async function authenticateWithNear() {
  ...
  try {
    ...
    var signed = await wallet.signMessage({
      ...
      recipient: challenge.recipient || 'brassclaw',
      nonce: nonceBytes,
    });
...
(function autoAuth() {
  ...
  if (sessionStorage.getItem('brassclaw_oidc') === '1') {
    oidcProxyAuth = true;
  }
  const saved = sessionStorage.getItem('brassclaw_token');
  if (saved) {
  ...
  fetch('/api/gateway/status', { credentials: 'include' }).then(function(r) {
    if (r.ok) {
      oidcProxyAuth = true;
      sessionStorage.setItem('brassclaw_oidc', '1');
      document.getElementById('auth-screen').style.display = 'none';
      document.getElementById('app').style.display = 'flex';
      initApp();
    }
