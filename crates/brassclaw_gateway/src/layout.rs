...
pub struct BrandingConfig {
    /// Page title (replaces default "BrassClaw").
    ...
    /// side (`window.__BRASSCLAW_LAYOUT__` and
...
/// (`window.__BRASSCLAW_LAYOUT__`) and the response body of
...
impl BrandingConfig {
    ...
    /// `window.__BRASSCLAW_LAYOUT__` JSON island. The first consumer that
...
mod tests {
    ...
    fn test_branding_serialize_drops_hostile_urls() {
        ...
        // `window.__BRASSCLAW_LAYOUT__` or `GET /api/frontend/layout`.
        let hostile = BrandingConfig {
