...
pub const NONCE_PLACEHOLDER: &str = "__BRASSCLAW_CSP_NONCE__";
...
/// emits `window.__BRASSCLAW_LAYOUT__` so the browser-side IIFE can read it
...
///
/// **Before `</body>`:**
/// - Layout config as `window.__BRASSCLAW_LAYOUT__`
    ...
    match serde_json::to_string(&bundle.layout) {
        Ok(layout_json) => {
            ...
            body_injections.push(format!(
                "<script nonce=\"{NONCE_PLACEHOLDER}\">window.__BRASSCLAW_LAYOUT__ = {safe_layout};</script>"
            ));
        }
        Err(e) => {
            ...
            // ships without `window.__BRASSCLAW_LAYOUT__`, and the IIFE in
            ...
            tracing::warn!(
                error = %e,
                "failed to serialize LayoutConfig for window.__BRASSCLAW_LAYOUT__ injection — \
...
mod tests {
    ...
    const MINIMAL_HTML: &str =
        "<!DOCTYPE html><html><head><title>BrassClaw</title></head><body></body></html>";
    ...
    fn test_assemble_index_no_customizations() {
        ...
        assert!(result.contains("window.__BRASSCLAW_LAYOUT__"));
    ...
    fn test_assemble_index_branding_title() {
        ...
        assert!(!result.contains("<title>BrassClaw</title>"));
    }
    ...
    fn test_assemble_index_layout_json_script_breakout_escaped() {
        ...
        // of the `window.__BRASSCLAW_LAYOUT__` script injection.
        let bundle = FrontendBundle {
            layout: LayoutConfig {
                branding: BrandingConfig {
        ...
        // but it's also present inside the JSON string in window.__BRASSCLAW_LAYOUT__.
        let layout_start = result.find("window.__BRASSCLAW_LAYOUT__").unwrap();
