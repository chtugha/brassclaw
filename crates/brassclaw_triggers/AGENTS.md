...
- Fast local check: `cargo test -p brassclaw_triggers`
- Backend check: `cargo test -p brassclaw_triggers --features libsql`
- Lint check: `cargo clippy -p brassclaw_triggers --all-targets --all-features -- -D warnings`
- Boundary check after dependency changes: `cargo test -p brassclaw_architecture reborn_crate_dependency_boundaries_hold`
