...
/// [`RebornServicesApi::stream_events`]: brassclaw_product_workflow::RebornServicesApi::stream_events
...
///
/// List all tools with current permission state for the authenticated caller.
///
/// TODO: This currently returns an empty list. Full implementation requires
/// bridging to the v1 tool registry or adding tool permission methods to
/// RebornServicesApi. For now, the frontend should continue using the v1
/// endpoint at `/api/settings/tools`.
pub async fn get_tools(
    State(_state): State<WebUiV2State>,
    Extension(_caller): Extension<WebUiAuthenticatedCaller>,
) -> Result<Json<ToolPermissionsResponse>, WebUiV2HttpError> {
    // Stub implementation - returns empty list
    // Frontend should use /api/settings/tools until this is fully implemented
    Ok(Json(ToolPermissionsResponse { tools: vec![] }))
}

/// `PUT /api/webchat/v2/tools/{tool_name}`
///
/// Update permission state for a single tool.
///
/// TODO: This currently returns not implemented. Full implementation requires
/// bridging to the v1 settings store or adding tool permission methods to
/// RebornServicesApi. For now, the frontend should continue using the v1
/// endpoint at `/api/settings/tools/:name`.
pub async fn update_tool_permission(
    State(_state): State<WebUiV2State>,
    Extension(_caller): Extension<WebUiAuthenticatedCaller>,
    Path(_tool_name): Path<String>,
    Json(_body): Json<UpdateToolPermissionRequest>,
) -> Result<Json<ToolPermissionEntry>, WebUiV2HttpError> {
    // Stub implementation - returns unavailable error
    // Frontend should use /api/settings/tools/:name until this is fully implemented
...
/// Response for `GET /api/webchat/v2/tools`.
#[derive(Debug, Serialize)]
pub struct ToolPermissionsResponse {
    pub tools: Vec<ToolPermissionEntry>,
}

/// Single tool entry in the tools list.
#[derive(Debug, Serialize)]
...
/// Body for `PUT /api/webchat/v2/tools/{tool_name}`.
#[derive(Debug, Deserialize)]
