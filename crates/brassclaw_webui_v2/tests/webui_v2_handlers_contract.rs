...
use brassclaw_product_workflow::{
    ...
    MontyVmStatusResponse, RebornAutomationInfo, RebornAutomationRunHistoryResponse,
...
impl RebornServicesApi for StubServices {
    async fn create_thread(
        ...
        Ok(RebornCreateThreadResponse {
            thread: SessionThreadRecord {
                thread_id: brassclaw_host_api::ThreadId::new("thread:fake").expect("thread id"),
                scope: brassclaw_threads::ThreadScope {
    ...
    async fn submit_turn(
        ...
        Ok(RebornSubmitTurnResponse::Submitted {
            thread_id: brassclaw_host_api::ThreadId::new(
    ...
    async fn get_timeline(
        ...
        Ok(RebornTimelineResponse {
            thread: SessionThreadRecord {
                thread_id: brassclaw_host_api::ThreadId::new(request.thread_id.clone())
                    .expect("thread id"),
                scope: brassclaw_threads::ThreadScope {
                    tenant_id: TenantId::new("tenant-alpha").expect("tenant"),
                    agent_id: AgentId::new("agent-alpha").expect("agent"),
...
async fn stream_events_last_event_id_header_takes_precedence_over_query() {
    ...
    let header_cursor =
        brassclaw_product_workflow::ProjectionCursor::new("cursor-from-header").expect("cursor");
    let query_cursor =
        brassclaw_product_workflow::ProjectionCursor::new("cursor-from-query").expect("cursor");
...
/// contract under test (it lives in `brassclaw_product_adapters`); these
