...
impl TurnStateStore for FailingTurnStateStore {
    ...
    async fn request_cancel(
        &self,
        _request: brassclaw_turns::CancelRunRequest,
    ) -> Result<brassclaw_turns::CancelRunResponse, TurnError> {
        unreachable!("active_run_ref_state only calls get_run_state")
    }

    async fn get_run_state(
        &self,
        _request: brassclaw_turns::GetRunStateRequest,
    ) -> Result<brassclaw_turns::TurnRunState, TurnError> {
...
fn submit_request_for(
    scope: TurnScope,
    idempotency_key: &str,
) -> brassclaw_turns::SubmitTurnRequest {
    brassclaw_turns::SubmitTurnRequest {
        scope,
        actor: turn_actor(),
...
async fn active_run_ref_state_classifies_missing_nonterminal_and_terminal() {
    let store = brassclaw_turns::InMemoryTurnStateStore::default();
    let resolver = InMemoryRunProfileResolver::default();
    let scope = turn_scope("active-run-ref-state");

    assert_eq!(
        brassclaw_turns::active_run_ref_state(&store, scope.clone(), None)
            .await
            .unwrap(),
    ...
    let accepted = store
        ...
        .await
        .unwrap();
    let brassclaw_turns::SubmitTurnResponse::Accepted { run_id, status, .. } = accepted;
    assert_eq!(status, TurnStatus::Queued);

    assert_eq!(
        brassclaw_turns::active_run_ref_state(&store, scope.clone(), Some(run_id))
            .await
            .unwrap(),
    ...
    let cancel = store
        .request_cancel(brassclaw_turns::CancelRunRequest {
            scope: scope.clone(),
            actor: turn_actor(),
            run_id,
            reason: brassclaw_turns::SanitizedCancelReason::UserRequested,
...
async fn active_run_ref_state_treats_missing_lookup_as_missing() {
    ...
    assert_eq!(
        brassclaw_turns::active_run_ref_state(&store, scope, Some(TurnRunId::new()))
            .await
            .unwrap(),
        TurnActiveRunRefState::Missing
    );
...
async fn active_run_ref_state_propagates_non_scope_not_found_errors() {
    let result = brassclaw_turns::active_run_ref_state(
