...
impl TurnScope {
    ...
    /// Convert into a [`brassclaw_host_api::ResourceScope`] for filesystem
    ...
    pub fn to_resource_scope(&self) -> brassclaw_host_api::ResourceScope {
        let mut scope = brassclaw_host_api::ResourceScope::system();
        scope.tenant_id = self.tenant_id.clone();
        scope.user_id = self.explicit_owner_user_id().cloned().unwrap_or_else(|| {
            UserId::from_trusted(brassclaw_host_api::SYSTEM_RESERVED_ID.to_string())
        });
