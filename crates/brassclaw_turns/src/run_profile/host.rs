...
pub struct ProviderToolCallReplay {
    ...
    /// Opaque provider thought-signature metadata, not an BrassClaw auth signature.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// Provider-facing tool definition derived from a visible BrassClaw capability.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ProviderToolDefinition {
    /// Canonical BrassClaw capability id backing this provider tool.
    pub capability_id: CapabilityId,
...
pub trait LoopCheckpointPort: Send + Sync {
    ...
    /// `brassclaw_loop_support` and wrap the host's `CheckpointStateStore`.
    ///
