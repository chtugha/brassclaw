...
use brassclaw_host_api::{CapabilityId, ExtensionId, RuntimeKind};
...
pub enum LoopHostMilestoneKind {
    ...
    /// `brassclaw_hooks::HookId::to_hex`). The hook crate cannot be imported
    /// here without breaking the architecture-enforced dependency direction
    /// (`brassclaw_turns -> brassclaw_hooks` is forbidden), so the hook id is
    ...
    HookDispatched {
        ...
        /// Carried into [`brassclaw_events::RuntimeEvent::provider`] so
...
/// `brassclaw_hooks` is a process-wide shared object (`Arc<HookDispatcher>`)
...
/// `brassclaw_reborn` wraps the real [`LoopHostMilestoneSink`] in an adapter
