...
Product-facing workflow facade for BrassClaw Reborn (issue #3280).
...
| `ProductConversationBindingService` | Adapter from product workflow bindings to `brassclaw_conversations` with trusted installation→tenant mapping |
...
- `brassclaw_product_adapters` — trait definitions, envelope/ack types, `ProjectionStream` for SSE
- `brassclaw_approvals` / `brassclaw_authorization` — canonical approval resolution and scoped lease issue ports used by approval interactions
- `brassclaw_auth` — typed product-auth continuation events consumed by the workflow auth bridge
- `brassclaw_conversations` — canonical actor/conversation binding and thread route ownership
- `brassclaw_run_state` — approval request store contract surfaced through approval resolution/read-model ports
- `brassclaw_turns` — turn coordinator, scope, IDs
- `brassclaw_threads` — session thread service contract
- `brassclaw_host_api` — canonical identifiers (TenantId, UserId, etc.)
...
Outbound delivery orchestration starts only after `brassclaw_outbound` resolves
