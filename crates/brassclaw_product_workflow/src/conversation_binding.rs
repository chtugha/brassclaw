//! Adapter from product workflow binding requests to `brassclaw_conversations`.
...
pub enum ProductActorBindingPolicy {
    ...
    ResolveActor {
        ...
        actor_pairings: Arc<dyn brassclaw_conversations::ConversationActorPairingService>,
    },
}
...
pub struct ProductConversationBindingService {
    conversations: Arc<dyn brassclaw_conversations::ConversationBindingService>,
...
fn ensure_resolved_actor_matches_expected_user(
    ...
    resolution: &brassclaw_conversations::ConversationBindingResolution,
) -> Result<(), ProductWorkflowError> {
    ...
    async fn resolve_binding(
                ...
                .is_some()
        {
                ...
                .await
            {
                ...
                Ok(_) | Err(brassclaw_conversations::InboundTurnError::BindingRequired { .. }) => {}
...
fn conversation_request(
    request: &ResolveBindingRequest,
    tenant_id: TenantId,
) -> Result<brassclaw_conversations::ResolveConversationRequest, ProductWorkflowError> {
    Ok(brassclaw_conversations::ResolveConversationRequest {
        tenant_id,
...
fn conversation_adapter_kind(
    adapter_id: &ProductAdapterId,
) -> Result<brassclaw_conversations::AdapterKind, ProductWorkflowError> {
    brassclaw_conversations::AdapterKind::new(adapter_id.as_str()).map_err(map_conversation_error)
}

fn conversation_installation_id(
    installation_id: &AdapterInstallationId,
) -> Result<brassclaw_conversations::AdapterInstallationId, ProductWorkflowError> {
    brassclaw_conversations::AdapterInstallationId::new(installation_id.as_str())
        .map_err(map_conversation_error)
}

fn conversation_event_id(
    event_id: &brassclaw_product_adapters::ExternalEventId,
) -> Result<brassclaw_conversations::ExternalEventId, ProductWorkflowError> {
    brassclaw_conversations::ExternalEventId::new(event_id.as_str()).map_err(map_conversation_error)
}

fn conversation_actor_ref(
    actor_ref: &brassclaw_product_adapters::ExternalActorRef,
) -> Result<brassclaw_conversations::ExternalActorRef, ProductWorkflowError> {
    brassclaw_conversations::ExternalActorRef::new(actor_ref.kind(), actor_ref.id())
        .map_err(map_conversation_error)
}

fn conversation_conversation_ref(
    conversation_ref: &brassclaw_product_adapters::ExternalConversationRef,
) -> Result<brassclaw_conversations::ExternalConversationRef, ProductWorkflowError> {
    brassclaw_conversations::ExternalConversationRef::new(
...
fn conversation_route_kind(
    route_kind: ProductConversationRouteKind,
) -> brassclaw_conversations::ConversationRouteKind {
    match route_kind {
        ProductConversationRouteKind::Direct => {
            brassclaw_conversations::ConversationRouteKind::Direct
        }
        ProductConversationRouteKind::Shared => {
            brassclaw_conversations::ConversationRouteKind::Shared
        }
    }
}
...
fn map_conversation_error(error: brassclaw_conversations::InboundTurnError) -> ProductWorkflowError {
    match error {
        ...
        brassclaw_conversations::InboundTurnError::BindingRequired { .. } => {
            ProductWorkflowError::BindingRequired {
                reason: "external actor is not paired with a canonical user".into(),
            }
        }
        brassclaw_conversations::InboundTurnError::AccessDenied { .. }
        | brassclaw_conversations::InboundTurnError::BindingConflict { .. }
        | brassclaw_conversations::InboundTurnError::ThreadNotFound { .. } => {
            ProductWorkflowError::BindingAccessDenied
        }
        brassclaw_conversations::InboundTurnError::StatePoisoned
        | brassclaw_conversations::InboundTurnError::DurableState { .. } => {
            ProductWorkflowError::Transient {
                reason: "conversation binding store unavailable".into(),
            }
        }
        brassclaw_conversations::InboundTurnError::TurnSubmissionFailed { error } => {
