...
use brassclaw_approvals::{ApprovalResolutionError, ApprovalResolver, DenyApproval, LeaseApproval};
use brassclaw_authorization::{CapabilityLeaseStatus, CapabilityLeaseStore};
use brassclaw_events::AuditSink;
use brassclaw_host_api::{Action, ApprovalRequestId, CapabilityId, ResourceScope};
use brassclaw_run_state::{ApprovalRequestStore, ApprovalStatus, RunStateError};
...
mod tests {
    ...
    use brassclaw_run_state::{ApprovalRequestStore, InMemoryApprovalRequestStore};
    ...
    fn approval_request(
        invocation_fingerprint: Option<brassclaw_host_api::InvocationFingerprint>,
    ) -> ApprovalRequest {
        ApprovalRequest {
