...
use brassclaw_product_workflow::{
    ...
    ProductWorkflowError, RebornAutomationInfo, RebornAutomationRunHistoryResponse,
    ...
    RebornCreateAutomationResponse, RebornDeleteAutomationResponse, RebornDeleteThreadRequest,
    RebornExtensionOnboardingState, RebornFireAutomationNowResponse, RebornGetAutomationResponse,
    ...
    RebornTimelineRequest, RebornUpdateAutomationResponse, ResolveApprovalInteractionRequest,
...
impl FakeTurnCoordinator {
    ...
    fn last_submission_scope(&self) -> Option<brassclaw_turns::TurnScope> {
        self.submissions
            .lock()
            .expect("lock")
            .last()
...
impl ApprovalInteractionService for RecordingApprovalInteractionService {
    ...
    ) -> Result<ResolveApprovalInteractionResponse, brassclaw_product_workflow::ProductWorkflowError>
    {
...
impl AuthInteractionService for RecordingAuthInteractionService {
    ...
    ) -> Result<ListPendingAuthInteractionsResponse, brassclaw_product_workflow::ProductWorkflowError>
    {
    ...
    ) -> Result<ResolveAuthInteractionResponse, brassclaw_product_workflow::ProductWorkflowError>
    {
        let run_id = request.run_id_hint.expect("webui passes run_id");
...
impl LifecycleProductFacade for RecordingLifecycleFacade {
    async fn execute(
        ...
        _action: brassclaw_product_workflow::LifecycleProductAction,
    ) -> Result<LifecycleProductResponse, brassclaw_product_workflow::ProductWorkflowError> {
...
async fn busy_submit_clears_skill_activation_message() {
    ...
    let coordinator = Arc::new(FakeTurnCoordinator::with_submit_error(
        TurnError::ThreadBusy(brassclaw_turns::ThreadBusy {
            active_run_id,
...
async fn submit_turn_returns_internal_when_skill_activation_recorder_fails() {
    ...
    let services = RebornServices::new(threads, coordinator.clone())
        .with_skill_activation_recorder(|_, _, _| {
            Err(brassclaw_product_workflow::RebornServicesError {
...
async fn list_extensions_projects_onboarding_payload_through_reborn_services() {
    ...
    assert_eq!(
        ...
        Some("Paste the GitHub token BrassClaw should use.")
    );
    assert_eq!(
...
fn onboarding_fixture() -> LifecycleExtensionOnboarding {
    LifecycleExtensionOnboarding {
        ...
        credential_instructions: Some("Paste the GitHub token BrassClaw should use.".to_string()),
...
async fn seed_thread_messages(
    ...
    let parsed_thread_id = brassclaw_host_api::ThreadId::new(thread_id).expect("thread id");
