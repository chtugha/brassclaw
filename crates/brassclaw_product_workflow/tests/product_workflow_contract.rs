        ...
        Ok(ListPendingApprovalsResponse {
            approvals: self
                ...
                .map(|(gate_ref, run_id)| PendingApprovalInteractionView {
                    ...
                    action: brassclaw_product_workflow::ApprovalInteractionActionView::Other,
                })
                .collect(),
        })
    }
    ...
    async fn list_pending(
        ...
        Ok(ListPendingAuthInteractionsResponse {
            auth_interactions: vec![PendingAuthInteractionView {
                ...
                flow_id: brassclaw_auth::AuthFlowId::new(),
                status: AuthInteractionStatus::AwaitingUser,
                provider: brassclaw_auth::AuthProviderId::new("gmail").expect("provider"),
...
async fn concrete_approval_resolution_rejects_unknown_installation_via_product_binding_service() {
    ...
    let conversation_port: Arc<dyn brassclaw_conversations::ConversationBindingService> =
        conversations.clone();
...
async fn auth_deny_from_threaded_direct_prompt_uses_base_direct_binding() {
    ...
    conversations
        .pair_external_actor(
            ...
            brassclaw_conversations::AdapterKind::new("test_adapter").expect("adapter"),
            brassclaw_conversations::AdapterInstallationId::new("install_alpha")
                .expect("installation"),
            brassclaw_conversations::ExternalActorRef::new("test", "user1").expect("actor"),
...
async fn approval_resolution_always_allow_is_rejected_without_approval_interaction() {
    ...
    assert!(matches!(
        err,
        ProductAdapterError::WorkflowRejected {
            kind: brassclaw_product_adapters::ProductWorkflowRejectionKind::InvalidRequest,
            status_code: 400,
            retryable: false,
            ..
        }
...
async fn projection_subscription_requires_existing_conversation_binding() {
    ...
    conversations
        .pair_external_actor(
            TenantId::new("tenant:alpha").expect("tenant"),
            brassclaw_conversations::AdapterKind::new("test_adapter").expect("adapter"),
            brassclaw_conversations::AdapterInstallationId::new("install_alpha").expect("install"),
            brassclaw_conversations::ExternalActorRef::new("test", "user1").expect("actor"),
            UserId::new("user:alice").expect("user"),
        )
        .await;
...
async fn concrete_product_workflow_accepts_shared_route_participant_on_existing_thread() {
    ...
    let adapter_kind = brassclaw_conversations::AdapterKind::new("test_adapter").expect("adapter");
    let installation_id =
        brassclaw_conversations::AdapterInstallationId::new("install_alpha").expect("install");
    let conversations = Arc::new(InMemoryConversationServices::default());
    conversations
    ...
    conversations
        .pair_external_actor(
            ...
            adapter_kind,
            installation_id,
            brassclaw_conversations::ExternalActorRef::new("test", "user2").expect("actor"),
    ...
    for (install, tenant, user) in [
        ...
        conversations
            .pair_external_actor(
                ...
                brassclaw_conversations::AdapterKind::new("test_adapter").expect("adapter"),
                brassclaw_conversations::AdapterInstallationId::new(install).expect("install"),
                brassclaw_conversations::ExternalActorRef::new("test", "user1").expect("actor"),
...
async fn shared_lookup_binding_rejects_existing_binding_when_resolved_actor_differs() {
    ...
    ConversationBindingPort::resolve_or_create_binding_with_trusted_scope(
        ...
        brassclaw_conversations::ResolveConversationRequest {
            tenant_id: tenant_id.clone(),
            adapter_kind,
            adapter_installation_id: installation_id,
            external_actor_ref: brassclaw_conversations::ExternalActorRef::new("test", "user1")
                .expect("actor"),
            external_conversation_ref: brassclaw_conversations::ExternalConversationRef::new(
                Some("T-team"),
                "C-eng",
                Some("thread-1"),
                Some("msg-1"),
            ...
            .expect("conversation"),
            external_event_id: brassclaw_conversations::ExternalEventId::new(
                "evt:seed-shared-lookup",
            )
            .expect("event"),
            route_kind: brassclaw_conversations::ConversationRouteKind::Shared,
...
async fn lookup_binding_does_not_backfill_legacy_ownerless_shared_route() {
    ...
    ConversationBindingPort::resolve_or_create_binding(
        ...
        brassclaw_conversations::ResolveConversationRequest {
            ...
            .expect("conversation"),
            external_event_id: brassclaw_conversations::ExternalEventId::new("evt:legacy-shared")
                .expect("event"),
            route_kind: brassclaw_conversations::ConversationRouteKind::Shared,
...
async fn concrete_product_workflow_bot_mention_uses_shared_route() {
    ...
    assert_eq!(
        binding.route_kinds(),
        vec![brassclaw_product_workflow::ProductConversationRouteKind::Shared]
    );
}
...
async fn concrete_product_workflow_reuses_prepared_binding_for_content_only_policy_rewrite() {
    ...
    assert_eq!(
        binding.route_kinds(),
        vec![brassclaw_product_workflow::ProductConversationRouteKind::Direct]
    );
}
...
async fn concrete_product_workflow_recomputes_route_after_policy_rewrites_trigger() {
    ...
    assert_eq!(
        binding.route_kinds(),
        vec![
            brassclaw_product_workflow::ProductConversationRouteKind::Direct,
            brassclaw_product_workflow::ProductConversationRouteKind::Shared,
        ]
    );
}
...
#[async_trait]
impl brassclaw_conversations::ConversationBindingService for CountingConversationBindingService {
    async fn resolve_or_create_binding(
        &self,
        request: brassclaw_conversations::ResolveConversationRequest,
    ) -> Result<
        brassclaw_conversations::ConversationBindingResolution,
    ...
    async fn link_conversation_to_thread(
        &self,
        request: brassclaw_conversations::LinkConversationRequest,
    ) -> Result<
        brassclaw_conversations::LinkedConversationBinding,
        brassclaw_conversations::InboundTurnError,
    > {
