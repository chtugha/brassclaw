...
use brassclaw_product_workflow::InboundTurnOutcome;
use brassclaw_reborn::planned_driver_factory::PLANNED_DEFAULT_PROFILE_ID;
use brassclaw_threads::MessageStatus;
...
async fn product_live_harness_invokes_builtin_echo_through_host_runtime() {
    let harness = ProductLiveAgentLoopHarness::new(ProductLiveAgentLoopHarnessConfig {
        ...
        host_runtime_capability: Some(HostRuntimeCapabilityConfig {
            capability_id: brassclaw_host_runtime::ECHO_CAPABILITY_ID.to_string(),
    ...
    assert_eq!(
        ...
        brassclaw_host_runtime::ECHO_CAPABILITY_ID
    );
    assert_eq!(
