...
use brassclaw_telegram_v2_adapter::{
...
impl ReplyTargetBindingValidator for FakeReplyTargetBindingValidator {
    async fn validate_reply_target(
        &self,
        request: brassclaw_outbound::ReplyTargetValidationRequest,
...
impl ProductOutboundTargetResolver for FakeProductOutboundTargetResolver {
    async fn resolve_product_outbound_target_metadata(
        &self,
        target: &brassclaw_outbound::ValidatedReplyTargetBinding,
...
impl OutboundStateStore for StatusFailingOutboundStore {
    ...
    ) -> Result<Option<brassclaw_event_projections::ProjectionCursor>, OutboundError> {
        self.inner.load_subscription_cursor(request).await
    }

    async fn advance_subscription_cursor(
        &self,
        request: brassclaw_outbound::AdvanceSubscriptionCursorRequest,
    ) -> Result<(), OutboundError> {
...
impl ProductAdapter for SynchronousResponseAdapter {
    ...
    fn parse_inbound(
        &self,
        _raw_payload: &[u8],
        _auth_evidence: &brassclaw_product_adapters::ProtocolAuthEvidence,
    ) -> Result<brassclaw_product_adapters::ParsedProductInbound, ProductAdapterError> {
        Err(ProductAdapterError::Internal {
            detail: brassclaw_product_adapters::RedactedString::new("not used"),
        })
    }
...
fn delivery_request(scope: TurnScope) -> brassclaw_outbound::PrepareCommunicationDeliveryRequest {
    brassclaw_outbound::PrepareCommunicationDeliveryRequest {
        ...
        projection_ref: brassclaw_outbound::ProjectionUpdateRef::new("projection:update:1")
...
fn requested_outbound_delivery_request(
    ...
    brassclaw_outbound::PrepareCommunicationDeliveryRequest {
        ...
        turn_run_id: Some(TurnRunId::new()),
        projection_ref: brassclaw_outbound::ProjectionUpdateRef::new("projection:update:requested")
            .expect("valid projection ref"),
        attempted_at: Utc::now(),
    }
}
...
fn system_event_delivery_request(
    ...
    brassclaw_outbound::PrepareCommunicationDeliveryRequest {
        ...
        turn_run_id: Some(TurnRunId::new()),
        projection_ref: brassclaw_outbound::ProjectionUpdateRef::new("projection:update:system")
            .expect("valid projection ref"),
        attempted_at: Utc::now(),
    }
}

fn trigger_context() -> brassclaw_outbound::TriggerCommunicationContext {
    brassclaw_outbound::TriggerCommunicationContext {
...
async fn authorized_final_reply_renders_through_telegram_egress_after_validation() {
    ...
    assert_eq!(
        attempts[0].status,
        brassclaw_outbound::OutboundDeliveryStatus::Delivered
    );
}
...
async fn deferred_render_keeps_attempt_pending_and_skips_delivery_status_side_effects() {
    ...
    assert_eq!(
        attempts[0].status,
        brassclaw_outbound::OutboundDeliveryStatus::Pending
    );
}
...
async fn mismatched_payload_kind_marks_authorized_attempt_failed_without_render() {
    ...
    assert!(matches!(
        err,
        brassclaw_product_workflow::ProductOutboundDeliveryError::PayloadKindMismatch {
    ...
    assert_eq!(
        attempts[0].status,
        brassclaw_outbound::OutboundDeliveryStatus::Failed
    );
    assert_eq!(
        attempts[0].failure_kind,
        Some(brassclaw_outbound::DeliveryFailureKind::Rejected)
    );
}
...
async fn target_metadata_failure_with_status_update_failure_preserves_workflow_error() {
    ...
    assert!(matches!(
        err,
        brassclaw_product_workflow::ProductOutboundDeliveryError::Workflow {
            status_update_error: Some(ProductOutboundStatusUpdateFailure::Backend),
            ..
        }
    ));
    ...
    assert_eq!(
        status_update_requests[0].failure_kind,
        Some(brassclaw_outbound::DeliveryFailureKind::TransportUnavailable)
    );
}
...
async fn adapter_render_failure_is_returned_and_marks_attempt_failed() {
    ...
    assert!(matches!(
        err,
        brassclaw_product_workflow::ProductOutboundDeliveryError::Adapter {
            status_update_error: None,
            ..
        }
    ));
...
async fn revoked_or_rejected_target_does_not_call_render_or_egress() {
    ...
    assert_eq!(
        ...
        Some(brassclaw_outbound::DeliveryFailureKind::AuthorizationRevoked)
    );
    assert_eq!(validator.calls(), 1);
    assert_eq!(preferences.load_calls(), 1);
    assert_eq!(resolver.calls(), 0);
