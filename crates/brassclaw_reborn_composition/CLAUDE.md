# brassclaw_reborn_composition guardrails
...
- Do not depend on the root `brassclaw` crate or `src/` modules.
...
- Product auth composition must use `brassclaw_auth` trait-shaped ports. Do not
...
- The first WebUI-mounted OAuth route keeps raw PKCE verifiers in a bounded,
  expiring process-local cache because `brassclaw_auth` durable records may
...
5. **Descriptor-driven per-route body limit**
   ...
   `BodyLimitPolicy` from `brassclaw_webui_v2::webui_v2_routes()` and,
...
8. **Descriptor-driven per-route rate limit**
   ...
   `brassclaw_webui_v2::webui_v2_routes()` plus mounted product-auth
...
9. `webui_v2_router(WebUiV2State::new(bundle.api))` — the v2
   handlers from `brassclaw_webui_v2` (create-thread, list-threads, delete-thread,
...
shipped by
`brassclaw_reborn_webui_ingress::webui_v2_auth_router`, which
...
- **Logout actually revokes.** `POST /auth/logout` calls
  ...
  `crates/brassclaw_reborn_webui_ingress/tests/session_round_trip.rs`
...
`Set-Cookie: brassclaw_session=...; HttpOnly` on its OAuth
...
| SSO login (Google) | `GET /auth/providers`, `GET /auth/login/{p}`, `GET /auth/callback/{p}`, `POST /auth/logout` | Same paths on the v2 listener via `brassclaw_reborn_webui_ingress::webui_v2_auth_router`, merged into `webui_v2_app` through [`WebuiServeConfig::with_public_route_mount`] (typed `{ router, descriptors }` so the per-route body-limit / rate-limit middleware applies) | Mapped (Google); GitHub + NEAR follow under #4116 |
...
- **Body limit** — descriptor-driven per-route via
  ...
  `brassclaw_webui_v2::webui_v2_routes()`: `create_thread` 16 KiB,
...
- **Static security headers** — `nosniff`, `DENY`, CSP applied via
  ...
  base-uri 'self'`.
- **Connection limit (SSE)** — bounded by `brassclaw_webui_v2`'s own
...
- **Caller construction** — `WebUiAuthenticatedCaller` is built from
  ...
  `crates/brassclaw_product_workflow/CLAUDE.md`.
...
- `src/webui_rate_limit.rs::tests` — unit tests for the sliding-window
  ...
  `brassclaw_webui_v2::webui_v2_routes()`, and
