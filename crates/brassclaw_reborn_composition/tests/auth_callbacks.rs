...
async fn oauth_callback_handler_reports_completed_continuation_dispatch_failure_as_retryable() {
    ...
    assert_eq!(retry.status, brassclaw_auth::AuthFlowStatus::Completed);
...
async fn oauth_callback_handler_routes_exchange_failures_through_provider_boundary() {
    ...
    let parsed: brassclaw_reborn_composition::RebornOAuthCallbackError =
    ...
    let flow = services
        ...
        .expect("flow lookup")
        .expect("flow record");
    assert_eq!(flow.status, brassclaw_auth::AuthFlowStatus::Failed);
