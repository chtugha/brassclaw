...
use brassclaw_auth::{AuthProviderId, CredentialAccountId, CredentialAccountService};
use brassclaw_host_api::{AgentId, InvocationId, ProjectId, ResourceScope, TenantId, UserId};
...
fn caller_scope_with_invocation_and_thread(
    ...
    thread_id: brassclaw_host_api::ThreadId,
) -> AuthProductScope {
    AuthProductScope::new(
        ResourceScope {
            tenant_id: TenantId::new(TENANT).expect("tenant"),
...
) -> brassclaw_auth::CredentialAccountId {
    let account = shared
...
fn auth_prompt_view_serialises_optional_fields_when_present() {
    use brassclaw_product_adapters::{AuthPromptChallengeKind, AuthPromptView};
...
fn auth_prompt_view_omits_optional_fields_when_absent() {
    use brassclaw_product_adapters::AuthPromptView;
    use brassclaw_turns::TurnRunId;
