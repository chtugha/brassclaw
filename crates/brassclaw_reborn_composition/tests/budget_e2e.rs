...
async fn f1_happy_path_records_actual_usd_in_ledger() {
    ...
    let user_account =
        ResourceAccount::user(tenant, brassclaw_host_api::UserId::new("f1-owner").unwrap());
...
async fn f2_crossing_warn_threshold_emits_warned_event() {
    ...
    let user_account = ResourceAccount::user(
        tenant.clone(),
        brassclaw_host_api::UserId::new("f2-owner").unwrap(),
    );
    governor
        .set_limit(
            user_account.clone(),
...
async fn f6_hard_cap_denied_before_provider_call() {
    ...
    let user_account = ResourceAccount::user(
        ...
        brassclaw_host_api::UserId::new("f6-owner").unwrap(),
    );
    governor
        .set_limit(
            user_account.clone(),
...
async fn c1_provider_tokens_reconcile_to_actual_usd() {
    ...
    let user_account = ResourceAccount::user(
        ...
        brassclaw_host_api::UserId::new("c1-owner").unwrap(),
    );
    governor
        .set_limit(
            user_account,
...
async fn c2_unknown_model_in_cost_table_uses_default_cost_fallback() {
    ...
    let user_account = ResourceAccount::user(
        ...
        brassclaw_host_api::UserId::new("c2-owner").unwrap(),
    );
    let usage = governor
...
async fn c3_zero_cost_model_records_zero_spend() {
    ...
    let user_account = ResourceAccount::user(
        ...
        brassclaw_host_api::UserId::new("c3-owner").unwrap(),
    );
    let usage = governor
        .usage_for(&user_account)
        .expect("usage_for read succeeds");
...
async fn d3_seeding_policy_installs_default_cap_on_first_touch() {
    ...
    let user_account_seed_check = ResourceAccount::user(
        ...
        brassclaw_host_api::UserId::new("d3-owner").unwrap(),
    );
...
async fn d1_agent_deny_preserves_user_warn_event() {
    ...
    let user_id = brassclaw_host_api::UserId::new("d1-owner").unwrap();
    let agent_id = brassclaw_host_api::AgentId::new("d1-agent").unwrap();
...
async fn broadcast_sink_publishes_events_to_subscribers() {
    ...
    assert!(
        received
            .iter()
            .any(|e| matches!(e, brassclaw_resources::BudgetEvent::Reserved { .. })),
        "broadcast must surface Reserved — got {received:?}"
    );
    assert!(
        received
            .iter()
            .any(|e| matches!(e, brassclaw_resources::BudgetEvent::Reconciled { .. })),
...
async fn budget_test_gateway_scripted_replies_drive_per_turn_costs() {
    ...
    let user_account = ResourceAccount::user(
        ...
        brassclaw_host_api::UserId::new("scripted-owner").unwrap(),
    );
    governor
        .set_limit(
            user_account,
