...
mod slack_personal_binding_pairing_mount_tests {
    ...
    use brassclaw_reborn_composition::slack_serve::SlackUserId;
    use brassclaw_reborn_composition::{
...
async fn ws_upgrade_uses_canonical_host_over_client_host_when_configured() {
    ...
    use brassclaw_reborn_composition::WebuiServeConfig;
    ...
    let app = brassclaw_reborn_composition::webui_v2_app(bundle, config).expect("app");
    let (addr, handle) = spawn_serve(app).await;
...
async fn every_webui_v2_descriptor_is_mounted_on_composed_app() {
    ...
    for descriptor in brassclaw_webui_v2::webui_v2_routes() {
...
async fn static_root_serves_index_with_substituted_csp_nonce() {
    ...
    assert!(
        !body.contains("__BRASSCLAW_CSP_NONCE__"),
...
async fn static_chat_hook_listens_for_oauth_callback_completion() {
    ...
    assert!(
        body.contains("brassclaw:product-auth:oauth-complete"),
...
/// `brassclaw_reborn_webui_ingress::webui_v2_auth_router`) must
