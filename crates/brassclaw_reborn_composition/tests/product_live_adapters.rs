...
use brassclaw_threads::{InMemorySessionThreadService, SessionThreadService, ThreadScope};
...
async fn capability_io_rejects_cross_run_input_and_result_refs() {
    ...
    assert_eq!(
        input_error.kind,
        brassclaw_turns::run_profile::AgentLoopHostErrorKind::ScopeMismatch
    );
...
async fn adapter_bundle_wires_required_product_live_components() {
    ...
    assert_eq!(
        readiness,
        brassclaw_loop_support::ProductLiveCancellationReadiness::ExternallyControllable
    );
