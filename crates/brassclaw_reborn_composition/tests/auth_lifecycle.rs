...
use brassclaw_host_api::{ExtensionId, InvocationId, ResourceScope, SecretHandle, UserId};
use brassclaw_reborn_composition::{RebornAuthContinuationDispatcher, RebornProductAuthServices};
...
#[async_trait]
impl brassclaw_auth::AuthProviderClient for AccessOnlyRefreshProvider {
...
#[async_trait]
impl brassclaw_auth::AuthProviderClient for CountingRefreshProvider {
    async fn exchange_callback(
        &self,
        _context: OAuthProviderExchangeContext,
        _request: OAuthProviderCallbackRequest,
...
async fn refresh_credential_account_uses_product_auth_facade_and_redacts_response() {
    ...
    let account = auth
        .create_account(NewCredentialAccount {
            scope: owner.clone(),
            provider: provider(),
            label: brassclaw_auth::CredentialAccountLabel::new("work").unwrap(),
...
async fn refresh_credential_account_maps_facade_errors_to_stable_codes() {
    ...
    let account = auth
        .create_account(NewCredentialAccount {
            scope: owner.clone(),
            provider: provider(),
            label: brassclaw_auth::CredentialAccountLabel::new("shared").unwrap(),
            status: CredentialAccountStatus::Expired,
    ...
    assert_eq!(error.code, brassclaw_auth::AuthErrorCode::CrossScopeDenied);
...
async fn refresh_credential_account_rejects_system_owned_accounts_before_provider_call() {
    ...
    let account = auth
        .create_account(NewCredentialAccount {
            scope: owner.clone(),
            provider: provider(),
            label: brassclaw_auth::CredentialAccountLabel::new("system").unwrap(),
...
async fn cleanup_credentials_for_lifecycle_uses_facade_and_quarantine_report() {
    ...
    let owned = auth
        .create_account(NewCredentialAccount {
            scope: owner.clone(),
            provider: provider(),
            label: brassclaw_auth::CredentialAccountLabel::new("owned").unwrap(),
            status: CredentialAccountStatus::Configured,
    ...
    let quarantined = auth
        .create_account(NewCredentialAccount {
            scope: owner.clone(),
            provider: provider(),
            label: brassclaw_auth::CredentialAccountLabel::new("quarantine").unwrap(),
            status: CredentialAccountStatus::Configured,
            ownership: CredentialOwnership::ExtensionOwned,
            owner_extension: Some(extension.clone()),
            granted_extensions: Vec::new(),
