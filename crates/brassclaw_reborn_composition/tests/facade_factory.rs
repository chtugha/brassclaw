...
fn trigger_management_execution_context() -> ExecutionContext {
    let grants = CapabilitySet {
        grants: vec![
            local_dev_grant(
                brassclaw_host_runtime::TRIGGER_CREATE_CAPABILITY_ID,
                vec![EffectKind::DispatchCapability, EffectKind::ExternalWrite],
            ),
            local_dev_grant(
                brassclaw_host_runtime::TRIGGER_LIST_CAPABILITY_ID,
                vec![EffectKind::DispatchCapability],
            ),
            local_dev_grant(
                brassclaw_host_runtime::TRIGGER_REMOVE_CAPABILITY_ID,
                vec![EffectKind::DispatchCapability, EffectKind::ExternalWrite],
            ),
        ],
    };
...
async fn production_google_oauth_config_uses_factory_built_product_auth_ports() {
    ...
    let result = build_reborn_services(
        ...
        .with_google_oauth_backend(brassclaw_reborn_composition::OAuthClientConfig {
...
async fn production_factory_built_product_auth_manual_token_round_trips() {
    ...
    let scope = auth_scope("alice");
    let provider = brassclaw_auth::AuthProviderId::new("manual-provider").unwrap();
    let label = brassclaw_auth::CredentialAccountLabel::new("manual production").unwrap();
    let challenge = product_auth
...
async fn production_rejects_local_only_runtime_policy() {
    ...
    assert!(
        report.contains(
            brassclaw_host_runtime::ProductionWiringComponent::RuntimePolicy,
            brassclaw_host_runtime::ProductionWiringIssueKind::LocalOnlyImplementation,
        ),
...
async fn production_libsql_resolved_secret_master_key_rejects_invalid_env_key() {
    ...
    let _env = EnvVarGuard::set(
        brassclaw_secrets::keychain::SECRETS_MASTER_KEY_ENV,
    ...
    assert!(matches!(
        ...
        Err(RebornBuildError::Secret(
            brassclaw_secrets::SecretError::InvalidMasterKey
        ))
    ));
}
