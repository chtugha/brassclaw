...
async fn pump_until_pending_gate(
...
) -> brassclaw_resources::BudgetGateId {
    ...
    let pending = store
        .list_pending(&brassclaw_host_api::ResourceScope::system())
        .expect("list pending");
    assert_eq!(
        pending.len(),
        1,
...
async fn f3_approval_with_increased_limit_unblocks_retry() {
    ...
    // succeeds.
    let store = runtime.budget_gate_store().expect("gate store");
    let approver = brassclaw_host_api::UserId::new("f3-approver").unwrap();
    ...
    let resolved = store
        .resolve(
            &brassclaw_host_api::ResourceScope::system(),
            gate_id,
    ...
    // that surface.
    runtime
        .apply_resolved_budget_gate(&brassclaw_host_api::ResourceScope::system(), gate_id)
    ...
    assert_eq!(reply.status, brassclaw_turns::TurnStatus::Completed);
    assert_eq!(
        gateway.call_count(),
        1,
...
async fn f4_cancel_keeps_budget_blocked_on_retry() {
    ...
    let store = runtime.budget_gate_store().expect("gate store");
    let canceller = brassclaw_host_api::UserId::new("f4-canceller").unwrap();
    let resolved = store
        .resolve(
            &brassclaw_host_api::ResourceScope::system(),
            gate_id,
...
async fn f5_expiry_marks_gate_terminal_and_keeps_budget_blocked() {
    ...
    let expired = store
        .expire_pending_older_than(&brassclaw_host_api::ResourceScope::system(), cutoff)
...
async fn gate_opened_event_carries_id_that_matches_persisted_gate() {
    ...
    {
        if let brassclaw_resources::BudgetEvent::GateOpened { gate_id, .. } = event {
