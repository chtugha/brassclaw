...
use brassclaw_hooks::identity::{ExtensionId as HookExtensionId, HookId, HookLocalId, HookVersion};
use brassclaw_hooks::registry::HookPointSpec;
use brassclaw_host_api::{TenantId, VirtualPath};
...
async fn third_party_hook_is_projected_while_capability_stays_out_of_capability_surface() {
    ...
    // catalog or surface. (See the `brassclaw_architecture` source assertion for
...
async fn matrix_before_capability_installed_deny_is_allowed_and_fires() {
    use brassclaw_hooks::points::{BeforeCapabilityHookContext, SanitizedArguments};
    ...
    let ctx = BeforeCapabilityHookContext::new(
        ...
        Some(brassclaw_host_api::ExtensionId::new("gate-ext").expect("ext")),
    );
...
async fn matrix_owning_extension_is_derived_not_spoofable() {
    ...
    assert_eq!(
        ...
        Some(&brassclaw_host_api::ExtensionId::new("claimant").expect("ext")),
