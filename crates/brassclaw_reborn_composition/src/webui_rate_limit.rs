...
//! native surface.
//!
//! `brassclaw_webui_v2::webui_v2_routes()` returns an
...
use brassclaw_host_api::ingress::{IngressRouteDescriptor, RateLimitPolicy, RateLimitScope};
    ...
    let bucket_key = match scope {
        RateLimitScope::PerCaller => {
            let Some(caller) = request.extensions().get::<WebUiAuthenticatedCaller>() else {
                tracing::debug!(
                    target = "brassclaw::reborn::webui_rate_limit",
...
mod tests {
    ...
    fn build_rate_limit_state_accepts_webui_v2_descriptors() {
        let descriptors = brassclaw_webui_v2::webui_v2_routes();
