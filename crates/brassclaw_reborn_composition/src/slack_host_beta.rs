...
use brassclaw_host_api::{AgentId, ProjectId, ResourceScope, TenantId, UserId};
use brassclaw_outbound::{InMemoryOutboundStateStore, OutboundStateStore};
...
use brassclaw_slack_v2_adapter::{
...
fn build_slack_events_route_mount_with_resolvers(
    ...
    let preferences: Arc<dyn brassclaw_outbound::CommunicationPreferenceRepository> = outbound;
...
mod tests {
    ...
    use brassclaw_threads::{ListThreadsForScopeRequest, ThreadHistoryRequest, ThreadScope};
    use brassclaw_turns::{
    ...
    async fn wait_for_slack_thread_history(
        runtime: &RebornRuntime,
    ) -> brassclaw_threads::ThreadHistory {
