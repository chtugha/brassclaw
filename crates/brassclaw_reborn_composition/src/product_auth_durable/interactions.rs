    ...
    F: RootFilesystem + 'static,
{
    ...
    async fn find_reusable_manual_token_account(
        ...
        let owner = brassclaw_auth::CredentialAccountOwnerScope::from_scope(&pending.scope);
    ...
    async fn cleanup_manual_secret(
        ...
        access_secret: &Option<brassclaw_host_api::SecretHandle>,
    ) {
        ...
        if let Some(access_secret) = access_secret
            && let Err(e) = self.secret_store.delete(scope, access_secret).await
        {
            tracing::debug!(error = %e, "product_auth interactions: best-effort access-secret purge failed; orphan is unreachable");
        }
    }
}
...
fn validate_reusable_manual_token_account(
    ...
    if account.provider != pending.provider
        ...
        || account.status == brassclaw_auth::CredentialAccountStatus::Revoked
    {
...
struct StoredManualTokenInteraction {
    ...
    scope: brassclaw_auth::AuthProductScope,
    provider: brassclaw_auth::AuthProviderId,
    label: brassclaw_auth::CredentialAccountLabel,
