...
use brassclaw_filesystem::{FilesystemError, RootFilesystem};
use brassclaw_host_api::{ExtensionId, VirtualPath};
...
impl WireManifestRecord {
    fn into_manifest_record(self) -> Result<ExtensionManifestRecord, ExtensionInstallationError> {
        let host_ports = brassclaw_host_runtime::default_host_port_catalog()
            .map_err(invalid_installation_error)?;
