...
impl RebornProjectionServices {
    ...
    pub(crate) fn with_model_failure_explainer_factory(
        ...
        system_inference: Arc<
            dyn Fn() -> Arc<dyn brassclaw_turns::run_profile::SystemInferencePort> + Send + Sync,
        >,
    ) -> Self {
    ...
    match item {
        ...
        ProjectionStreamItem::Update(envelope) => {
            ...
            match envelope.as_ref() {
                brassclaw_event_streams::ProductProjectionEnvelope::ThreadUpdates(replay) => {
                    replay_payloads(
                        scope,
                        display_previews,
                        replay,
                ...
                brassclaw_event_streams::ProductProjectionEnvelope::ThreadLiveUpdate(update) => {
...
async fn runtime_payloads_for_item(
    ...
    let snapshot = ProjectionSnapshot {
        timeline: brassclaw_event_projections::ThreadTimeline {
            entries: Vec::new(),
        },
        runs,
        capability_activities,
