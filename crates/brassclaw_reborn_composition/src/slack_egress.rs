...
use brassclaw_wasm_product_adapters::{EgressPolicy, EgressPolicyError, EgressPolicyTarget};
...
fn slack_extension_id() -> Result<ExtensionId, ProtocolHttpEgressError> {
    ExtensionId::new("brassclaw_slack").map_err(|error| ProtocolHttpEgressError::PolicyDenied {
...
fn map_runtime_http_error(error: RuntimeHttpEgressError) -> ProtocolHttpEgressError {
    match error.reason_code() {
        brassclaw_host_api::RuntimeHttpEgressReasonCode::PolicyDenied
        | brassclaw_host_api::RuntimeHttpEgressReasonCode::RequestDenied => {
            ProtocolHttpEgressError::PolicyDenied {
                reason: RedactedString::new(error.stable_runtime_reason()),
            }
        }
        brassclaw_host_api::RuntimeHttpEgressReasonCode::ResponseBodyLimitExceeded => {
            ProtocolHttpEgressError::LeakDetected
        }
        brassclaw_host_api::RuntimeHttpEgressReasonCode::CredentialUnavailable
        | brassclaw_host_api::RuntimeHttpEgressReasonCode::NetworkError
        | brassclaw_host_api::RuntimeHttpEgressReasonCode::ResponseError => {
...
mod tests {
    ...
    use brassclaw_processes::{InMemoryProcessResultStore, InMemoryProcessStore, ProcessServices};
    use brassclaw_product_adapters::{
