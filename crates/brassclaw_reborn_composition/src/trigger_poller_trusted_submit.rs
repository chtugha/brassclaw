...
use brassclaw_turns::{AdmissionRejectionReason, TurnError};
    ...
    B: ConversationBindingService,
{
    async fn materialize_prompt(
        ...
        .await
        .map_err(classify_materializer_inbound_error)?;
        let content_ref = brassclaw_triggers::TriggerInboundContentRef::new(format!(
            "thread-message:{}",
            accepted.message_id
        ))?;
...
) -> Result<brassclaw_threads::AcceptedInboundMessage, InboundTurnError> {
...
mod tests {
    ...
    use brassclaw_host_api::{ProjectId, TenantId, ThreadId, UserId};
    use brassclaw_safety::{InjectionWarning, Severity};
    use brassclaw_threads::{
    ...
    impl ConversationBindingService for PanicBindingService {
        ...
        async fn link_conversation_to_thread(
            &self,
            _request: brassclaw_conversations::LinkConversationRequest,
        ) -> Result<brassclaw_conversations::LinkedConversationBinding, InboundTurnError> {
            panic!("foreign-tenant materialization must reject before conversation linking")
        }

        async fn validate_reply_target(
            &self,
            _request: brassclaw_conversations::ValidateReplyTargetRequest,
        ) -> Result<brassclaw_conversations::ReplyTargetBinding, InboundTurnError> {
    ...
    impl CanonicalSessionThreadService for InterceptingPromptThreadService {
        ...
        ) -> Result<brassclaw_threads::ThreadHistory, SessionThreadError> {
    ...
    fn thread_busy_inbound_errors_are_retryable_backend_failures() {
        let error = classify_materializer_inbound_error(InboundTurnError::TurnSubmissionFailed {
            error: TurnError::ThreadBusy(brassclaw_turns::ThreadBusy {
    ...
    fn retryable_turn_errors_are_backend_failures() {
        for error in [
            ...
            TurnError::CapacityExceeded {
                resource: brassclaw_turns::TurnCapacityResource::SubmitTurn,
                cap: 1,
            },
            TurnError::Conflict {
    ...
    async fn unsafe_trigger_prompt_is_rejected_before_turn_submission() {
        let conversations = brassclaw_conversations::InMemoryConversationServices::default();
