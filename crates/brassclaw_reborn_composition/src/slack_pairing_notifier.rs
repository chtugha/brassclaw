...
use brassclaw_slack_v2_adapter::SLACK_API_HOST;
...
impl SlackPersonalBindingPairingNotifier for SlackPairingChallengeHttpNotifier {
    async fn send_pairing_challenge(
        ...
        let body = serde_json::to_vec(&SlackPairingPostMessage {
            channel,
            text: format!(
                "Connect this Slack account to Brassclaw by entering code {} in WebChat.",
...
impl SlackPairingChallengeHttpNotifier {
    ...
    async fn send_slack_request(
        ...
        path: &'static str,
        body: Vec<u8>,
    ) -> Result<brassclaw_product_adapters::EgressResponse, SlackPersonalBindingPairingError> {
