...
async fn webui_event_stream_enriches_activity_with_display_preview_from_store() {
    ...
    let timeline_message_id = brassclaw_threads::ThreadMessageId::new();
...
async fn capability_display_preview_store_redacts_unsafe_paths_and_secrets() {
    ...
    let preview = store
        .preview(&CapabilityActivityProjection {
            ...
            status: brassclaw_event_projections::CapabilityActivityStatus::Completed,
            provider: None,
            runtime: None,
            process_id: None,
            output_bytes: Some(42),
            error_kind: None,
            last_cursor: brassclaw_events::EventCursor::new(1),
...
async fn webui_projection_snapshot_bounds_activity_fanout_before_payload_mapping() {
    ...
    let cursor =
        EventProjectionCursor::for_scope(projection_scope, brassclaw_events::EventCursor::new(1));
    let snapshot = ProjectionSnapshot {
        ...
        capability_activities: (0..(WEBUI_PROJECTION_PAGE_LIMIT + 10))
            .map(|index| CapabilityActivityProjection {
                ...
                capability_id: capability.clone(),
                thread_id: Some(thread_id.clone()),
                status: brassclaw_event_projections::CapabilityActivityStatus::Running,
                provider: None,
                runtime: None,
                process_id: None,
                output_bytes: None,
                error_kind: None,
                last_cursor: brassclaw_events::EventCursor::new(index as u64 + 1),
                updated_at: chrono::Utc::now(),
            })
            .collect(),
