use super::*;
use brassclaw_product_adapters::ProductOutboundPayload;
...
impl PreviewProjectionFixture {
    fn completed(
        ...
        let cursor = EventProjectionCursor::for_scope(
            projection_scope,
            brassclaw_events::EventCursor::new(1),
        );
    ...
    fn add_completed_activity(
        ...
        capability_name: &str,
        updated_at: chrono::DateTime<chrono::Utc>,
        cursor: brassclaw_events::EventCursor,
    ) -> PreviewActivityRefs {
        let capability = CapabilityId::new(format!("builtin.{capability_name}")).unwrap();
        let invocation_id = InvocationId::new();
        let run_id = TurnRunId::new();
...
async fn webui_projection_with_pending_preview_on_first_activity_defers_second_activity() {
    ...
    let second = fixture.add_completed_activity(
        ...
        "list_files",
        now,
        brassclaw_events::EventCursor::new(2),
    );
    fixture.record_input("read_file");
