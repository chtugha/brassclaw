...
use brassclaw_auth::{AuthProviderId, OAuthAuthorizationUrl};
...
impl AuthChallengeProvider for FakeAuthChallengeProvider {
    async fn challenge_for_gate(
        ...
        run_id: TurnRunId,
        gate_ref: &str,
        _credential_requirements: &[brassclaw_host_api::RuntimeCredentialAuthRequirement],
    ) -> Result<Option<AuthChallengeView>, brassclaw_auth::AuthProductError> {
...
impl AuthChallengeProvider for FailingAuthChallengeProvider {
    async fn challenge_for_gate(
        ...
        Err(brassclaw_auth::AuthProductError::BackendUnavailable)
    }
}
