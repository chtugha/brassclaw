...
use brassclaw_threads::{FinalizedAssistantMessageByRunRequest, SessionThreadService, ThreadScope};
use brassclaw_turns::{
...
const SLACK_WORKING_MESSAGE: &str = "Brassclaw is thinking...";
...
impl SlackFinalReplyDeliveryObserver {
    ...
    async fn deliver_run_notification(
        ...
        let delivery = brassclaw_outbound::PrepareCommunicationDeliveryRequest {
        ...
        let _outcome = prepare_and_render_product_outbound(
            ...
            ProductOutboundDeliveryRequest {
                delivery,
                payload,
                projection_cursor: brassclaw_product_adapters::ProjectionCursor::new(projection_id)
    ...
    async fn post_slack_working_message(
        ...
        .await
        {
            ...
            Err(error) => {
                tracing::warn!(
                    target = "brassclaw::reborn::slack_delivery",
                    error = %error,
...
fn slack_auth_prompt_view(
    envelope: &ProductInboundEnvelope,
    mut view: brassclaw_product_adapters::AuthPromptView,
) -> brassclaw_product_adapters::AuthPromptView {
...
enum SlackFinalReplyDeliveryError {
    ...
    Thread(#[from] brassclaw_threads::SessionThreadError),
    #[error("outbound delivery failed: {0}")]
    Outbound(#[from] brassclaw_product_workflow::ProductOutboundDeliveryError),
...
#[async_trait]
impl brassclaw_outbound::ThreadProjectionAccessPolicy for AllowNoProjectionAccess {
    async fn authorize_projection_access(
        &self,
        _request: brassclaw_outbound::ThreadProjectionAccessRequest,
    ) -> Result<brassclaw_outbound::ThreadProjectionAccessClaim, OutboundError> {
...
fn should_deliver_after_ack(envelope: &ProductInboundEnvelope, ack: &ProductInboundAck) -> bool {
    ...
    !matches!(
        ...
        ProductInboundPayload::AuthResolution(payload)
            if matches!(
                &payload.result,
                brassclaw_product_adapters::AuthResolutionResult::Denied
            )
    ) && !matches!(
        envelope.payload(),
        ProductInboundPayload::ApprovalResolution(payload)
            if payload.decision == brassclaw_product_adapters::ApprovalDecision::Deny
    ) && !matches!(
        envelope.payload(),
...
mod tests {
    ...
    use brassclaw_turns::AcceptedMessageRef;
    ...
    fn envelope(payload: ProductInboundPayload) -> ProductInboundEnvelope {
        let adapter_id =
            brassclaw_product_adapters::ProductAdapterId::new("slack_v2").expect("adapter");
    ...
    fn auth_denial_ack_does_not_enter_slack_delivery_loop() {
        let payload = ProductInboundPayload::AuthResolution(
            brassclaw_product_adapters::AuthResolutionPayload::new(
                "gate:auth-test",
                brassclaw_product_adapters::AuthResolutionResult::Denied,
            )
    ...
    fn auth_completion_ack_still_enters_slack_delivery_loop() {
        let payload = ProductInboundPayload::AuthResolution(
            brassclaw_product_adapters::AuthResolutionPayload::new(
                "gate:auth-test",
                brassclaw_product_adapters::AuthResolutionResult::CallbackCompleted {
                    callback_ref: brassclaw_auth::AuthFlowId::new().to_string(),
                },
            )
            .expect("auth resolution"),
        );
