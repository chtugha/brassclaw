...
//!   point used by the standalone `brassclaw-reborn` binary and any
...
/// types are re-exported so host wiring (`brassclaw-reborn serve`, the CLI
...
pub fn open_reborn_identity_resolver(
    ...
    let root = std::sync::Arc::new(brassclaw_filesystem::InMemoryBackend::default());
    ...
    .expect("mount view");
    let filesystem = std::sync::Arc::new(brassclaw_filesystem::ScopedFilesystem::with_fixed_view(
        root, view,
    ));
    std::sync::Arc::new(
        brassclaw_reborn_identity::FilesystemRebornIdentityStore::new(
            filesystem,
            tenant_id.clone(),
...
/// the CLI mirror `brassclaw_reborn::model_routes::ModelSlot`.
pub fn reborn_model_slot_names() -> Vec<&'static str> {
    brassclaw_reborn::model_routes::ModelSlot::all()
        .iter()
...
pub fn reborn_runtime_readiness_snapshot() -> RebornRuntimeReadinessSnapshot {
    let mut registry = brassclaw_reborn::driver_registry::DriverRegistry::new();
    let text_only_driver = RebornRuntimeComponentStatus::from_result(
        brassclaw_reborn::planned_driver_factory::register_default_text_only_driver(
            &mut registry,
            brassclaw_reborn::text_loop_driver::TextOnlyModelReplyDriverConfig::default(),
        ),
    );
    let family_registry = brassclaw_reborn::app_loop_family::build_loop_family_registry();
    let planned_driver = match &family_registry {
        Ok(family_registry) => RebornRuntimeComponentStatus::from_result(
            brassclaw_reborn::planned_driver_factory::register_default_planned_driver(
                &mut registry,
    ...
    let subagent_planned_driver = match family_registry {
        Ok(family_registry) => RebornRuntimeComponentStatus::from_result(
            brassclaw_reborn::planned_driver_factory::register_subagent_planned_driver(
                &mut registry,
                family_registry,
            ),
        ),
...
/// [`brassclaw_host_api::ResourceScope::system`]) routes records under
...
pub enum RebornCompositionError {
    ...
    Filesystem(#[from] brassclaw_filesystem::FilesystemError),
    ...
    RunProfile(#[from] brassclaw_turns::run_profile::RunProfileRegistryError),
...
mod two_tenant_isolation_tests {
    ...
    use brassclaw_host_api::{AgentId, InvocationId, ProjectId, SecretHandle, TenantId, UserId};
    use brassclaw_secrets::{FilesystemSecretStore, SecretMaterial, SecretStore, SecretsCrypto};
