...
use brassclaw_auth::{CredentialAccountRecordSource, CredentialAccountService};
        ...
        let egress = request
            ...
            .ok_or_else(|| {
                FirstPartyCapabilityError::new(
                    brassclaw_host_api::RuntimeDispatchErrorKind::NetworkDenied,
                )
            })?
            .clone();
        let result = self
...
fn runtime_credentials(
    ...
    Ok(vec![RuntimeCredentialRequirement {
        ...
        source: RuntimeCredentialRequirementSource::ProductAuthAccount {
            provider: RuntimeCredentialAccountProviderId::new(brassclaw_auth::GOOGLE_PROVIDER_ID)?,
...
fn gsuite_credential_requirements(
    ...
    let provider = RuntimeCredentialAccountProviderId::new(brassclaw_auth::GOOGLE_PROVIDER_ID)
...
impl GsuiteCredentialStager for ProductAuthRuntimeGsuiteCredentialStager {
    async fn stage(
        ...
        // type aliases for brassclaw_host_api::CredentialStageError — no conversion needed.
        self.runtime_ports
...
mod tests {
    ...
    fn gmail_auth_failure_maps_to_google_oauth_requirement() {
        ...
        assert_eq!(
            ...
            vec![brassclaw_auth::GOOGLE_GMAIL_READONLY_SCOPE.to_string()]
        );
    }
}
