...
) -> HashMap<(brassclaw_host_api::TenantId, brassclaw_turns::TurnRunId), TriggerActiveRunState> {
    snapshot
        .runs
        .iter()
        .map(|run| {
...
fn active_run_state_from_index(
    run_index: &HashMap<
        (brassclaw_host_api::TenantId, brassclaw_turns::TurnRunId),
...
mod tests {
    ...
    fn turn_run_record(
        ...
        let scope = TurnScope::new(
            ...
            None,
            None,
            brassclaw_host_api::ThreadId::new(format!("thread-{run_id}")).expect("thread id"),
        );
        TurnRunRecord {
            run_id,
            turn_id: TurnId::new(),
            ...
            reply_target_binding_ref: brassclaw_turns::ReplyTargetBindingRef::new(format!(
                "reply:{run_id}"
            ))
