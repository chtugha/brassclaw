...
//! `brassclaw_reborn` or `brassclaw_llm` directly.
...
pub struct ResolvedRebornLlm {
    provider_id: String,
    model: String,
    pub(crate) config: brassclaw_llm::LlmConfig,
}
...
impl ResolvedRebornLlm {
    ...
    pub fn from_llm_config(config: brassclaw_llm::LlmConfig) -> Self {
        Self {
...
pub struct RebornRuntimeInput {
    ...
    /// override skips).
    #[cfg(any(test, feature = "test-support"))]
    pub(crate) model_cost_table_override: Option<Arc<dyn brassclaw_loop_support::ModelCostTable>>,
}
...
impl RebornRuntimeInput {
    ...
    pub fn with_model_cost_table_override(
        mut self,
        cost_table: Arc<dyn brassclaw_loop_support::ModelCostTable>,
    ) -> Self {
