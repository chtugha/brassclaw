...
pub(super) fn protected_mutation_policy() -> IngressPolicy {
    IngressPolicy::new(IngressPolicyParts {
        ...
        scope_source: brassclaw_host_api::IngressScopeSource::AuthenticatedCaller,
...
pub(super) fn callback_policy() -> IngressPolicy {
    IngressPolicy::new(IngressPolicyParts {
        ...
        scope_source: brassclaw_host_api::IngressScopeSource::HostResolved,
...
pub(super) async fn scoped_update_binding_for_requester(
    ...
    match account {
        ...
        Err(AuthProductError::BackendUnavailable) => {
            tracing::warn!(
                target: "brassclaw_reborn_composition::product_auth::oauth",
...
pub(super) fn sha256_hex(value: &str) -> String {
    brassclaw_common::hashing::sha256_hex(value.as_bytes())
}
...
mod tests {
    ...
    use brassclaw_capabilities::{CapabilityObligationHandler, CapabilityObligationRequest};
    use brassclaw_host_api::{
    ...
    impl RebornAuthContinuationDispatcher for NoopDispatcher {
        async fn dispatch_auth_continuation(
            &self,
            _event: brassclaw_auth::AuthContinuationEvent,
    ...
    struct RecordingDispatcher {
        events: Mutex<Vec<brassclaw_auth::AuthContinuationEvent>>,
    }

    impl RecordingDispatcher {
        fn events(&self) -> Vec<brassclaw_auth::AuthContinuationEvent> {
            self.events
                .lock()
    ...
    impl RebornAuthContinuationDispatcher for RecordingDispatcher {
        async fn dispatch_auth_continuation(
            &self,
            event: brassclaw_auth::AuthContinuationEvent,
        ) -> Result<(), AuthProductError> {
            self.events
                .lock()
                .expect("recording dispatcher lock")
    ...
    async fn extension_oauth_start_handler_does_not_bind_reconnect_accounts_outside_exact_scope() {
        ...
        let shared = Arc::new(brassclaw_auth::InMemoryAuthProductServices::new());
        let product_auth =
    ...
    impl RuntimeHttpEgress for PanickingDcrEgress {
        ...
        ) -> Result<RuntimeHttpEgressResponse, brassclaw_host_api::RuntimeHttpEgressError> {
