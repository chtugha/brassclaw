...
//!
//! The `brassclaw_webui_v2` crate ships handlers that dispatch through
...
//! `brassclaw-reborn serve` subcommand (on a follow-up PR) consumes the
//! same `Router` and owns the listener lifecycle on the host side.
//!
//! ### Why no serve-and-bind helper here
//!
//! `brassclaw_reborn_composition` sits in the Reborn product/API
...
use brassclaw_auth::GoogleOAuthRouteConfig;
...
pub struct WebuiServeConfig {
    ...
    /// per-route [`BodyLimitPolicy`](brassclaw_host_api::ingress::BodyLimitPolicy)
    /// declared in `brassclaw_webui_v2::webui_v2_routes()`; that
    ...
    /// by `brassclaw_reborn_webui_ingress::webui_v2_auth_router`
...
impl WebuiServeConfig {
    ...
    /// `brassclaw_reborn_webui_ingress::webui_v2_auth_router` plugs
...
/// - WebChat v2 route set from `brassclaw_webui_v2::webui_v2_router`
///
/// The returned [`Router`] is the seam between this composition crate
/// and host-owned ingress code: tests drive it via
/// `tower::ServiceExt::oneshot`, and the standalone `brassclaw-reborn
...
pub fn webui_v2_app_with_lifecycle(
    ...
    let app = app
        ...
        // `brassclaw_webui_v2_static` bundle. Routed AFTER the
...
fn panic_handler(
    ...
    tracing::error!(
        target = "brassclaw::reborn::webui_serve",
