...
use brassclaw_hooks::manifest::HookManifestEntry;
use brassclaw_hooks::predicate_state::{InMemoryPredicateStateBackend, PredicateStateBackend};
...
fn reborn_replay_error(
    reason: String,
) -> brassclaw_reborn::loop_driver_host::RebornLoopDriverHostError {
    brassclaw_reborn::loop_driver_host::RebornLoopDriverHostError::InvalidRequest { reason }
}
...
/// vocabulary (clean-boundary contract: `brassclaw_extensions` stays free of
...
/// bypass the registrar's ceiling + attribution. An `brassclaw_architecture`
...
pub(super) fn project_extension_hook_sets(
    ...
    tenant_root: Option<&brassclaw_host_api::VirtualPath>,
...
pub(super) fn project_hook_entries(
    extension_id: &str,
    hooks: &[brassclaw_extensions::HookSectionEntryV2],
        ...
        + 'static,
{
    ...
    let fallback_tenant =
        brassclaw_host_api::TenantId::new("reborn-hook-projection").map_err(|error| {
    ...
    let (audit_tenant, audit_root): (
        &brassclaw_host_api::TenantId,
        Option<&brassclaw_host_api::VirtualPath>,
