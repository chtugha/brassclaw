...
//!    [`brassclaw_hooks::manifest::HookManifestEntry`], and installs them
//!    through [`brassclaw_hooks::registrar::HookRegistrar::install`] at the
...
//! [`brassclaw_hooks::evaluator::PredicateEvaluator`] + its state backend, the
...
//! ([`brassclaw_hooks::evaluator::PredicateEvaluator::with_state_backend`]) so
...
///
/// Re-exported from `brassclaw_reborn` so the type is identical to the one
...
pub use brassclaw_reborn::loop_driver_host::HookDispatcherBuilderFactory;
...
impl HooksActivationConfig {
    ...
    pub fn from_env() -> Self {
        Self::from_env_values(
            brassclaw_common::env_helpers::env_or_override(HOOKS_ENABLED_ENV),
            brassclaw_common::env_helpers::env_or_override(HOOKS_THIRD_PARTY_ENABLED_ENV),
        )
    }
