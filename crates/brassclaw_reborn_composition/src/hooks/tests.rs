...
use brassclaw_extensions::v2::ManifestSource;
use brassclaw_extensions::{ExtensionManifest, ExtensionPackage};
use brassclaw_hooks::HookPhase;
use brassclaw_hooks::identity::{HookId, HookVersion};
use brassclaw_hooks::points::ObserverHookContext;
use brassclaw_hooks::registry::HookPointSpec;
use brassclaw_hooks::sink::{ObserverHook, ObserverSink};
use brassclaw_host_api::{HostPortCatalog, VirtualPath};
...
const TEST_NOOP_OBSERVER_CANONICAL_PATH: &str =
    "brassclaw_reborn_composition::hooks::tests::NoOpObserverHook";
...
use brassclaw_hooks::identity::{ExtensionId as HookExtensionId, HookLocalId};
...
body = { mode = "nonsense" }
"#;
    // HostBundled ids are reserved to the `brassclaw.` prefix.
    let registry = projection(registry_with_manifest_source(
        "brassclaw.broken",
        &manifest_toml("brassclaw.broken", hooks_block),
...
fn tenant_root_is_the_fixed_system_extensions_root() {
    let a = tenant_extension_root(&brassclaw_host_api::TenantId::new("alpha").expect("a"))
        .expect("root a");
    let b = tenant_extension_root(&brassclaw_host_api::TenantId::new("beta").expect("b"))
        .expect("root b");
...
async fn third_party_subflag_off_yields_builtin_only_projection() {
    ...
    let tenant = brassclaw_host_api::TenantId::new("alpha").expect("tenant");
...
/// contracts require the `brassclaw.capability_provider/v1` host_api form for
...
async fn write_manifest<F: brassclaw_filesystem::RootFilesystem>(fs: &F, id: &str, body: &str) {
    fs.write_file(
        &brassclaw_host_api::VirtualPath::new(format!("/system/extensions/{id}/manifest.toml"))
...
async fn failed_merge_does_not_consume_hook_budget() {
    ...
    let contracts = brassclaw_host_runtime::default_host_api_contract_registry()
...
"#;
    ...
    let real_tenant = brassclaw_host_api::TenantId::new("acme-real-tenant").expect("tenant");
