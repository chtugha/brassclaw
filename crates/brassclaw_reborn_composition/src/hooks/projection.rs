...
pub(super) struct HookProjection {
    pub(super) extension_id: brassclaw_host_api::ExtensionId,
    ...
    pub(super) source: brassclaw_extensions::ManifestSource,
    /// Package root, for the projection-layer containment check only.
    pub(super) root: brassclaw_host_api::VirtualPath,
    /// The declared `[[hooks]]` payloads — the ONLY package content carried.
    pub(super) hooks: Vec<brassclaw_extensions::HookSectionEntryV2>,
}
...
impl HookProjection {
    ...
    pub(super) fn from_package(package: &brassclaw_extensions::ExtensionPackage) -> Option<Self> {
...
/// [`brassclaw_host_runtime::HostRuntimeServices::new`] (it becomes the
...
///
/// The discovery layer ([`brassclaw_extensions::ExtensionDiscovery`] /
/// [`brassclaw_extensions::ExtensionPackage::from_manifest`]) hardcodes package
...
/// per-tenant [`brassclaw_filesystem::ScopedFilesystem`] / per-identity backend,
...
) -> Result<brassclaw_host_api::VirtualPath, RebornBuildError> {
    brassclaw_host_api::VirtualPath::new("/system/extensions").map_err(|error| {
...
pub(super) fn enforce_root_containment(
    tenant_root: &brassclaw_host_api::VirtualPath,
    package_root: &brassclaw_host_api::VirtualPath,
) -> Result<(), String> {
...
pub struct ThirdPartyDiscoveryInput<'a, F: brassclaw_filesystem::RootFilesystem> {
    pub filesystem: &'a F,
    pub tenant_id: &'a brassclaw_host_api::TenantId,
}
...
where
    F: brassclaw_filesystem::RootFilesystem,
{
        ...
        && let Some(input) = third_party_input
    {
        ...
        let discovered = match brassclaw_host_runtime::discover_extensions_tolerant_bounded(
            input.filesystem,
            &root,
