...
pub enum RebornBuildError {
    ...
    HostRuntime(#[from] brassclaw_host_runtime::HostRuntimeError),
    #[error("reborn event store build failed")]
    EventStore(#[from] brassclaw_reborn_event_store::RebornEventStoreError),
    #[error("reborn secret store build failed")]
    Secret(#[from] brassclaw_secrets::SecretError),
    ...
    Resource(#[from] brassclaw_resources::ResourceError),
    #[error("reborn run state build failed")]
    RunState(#[from] brassclaw_run_state::RunStateError),
    #[error("reborn capability lease store build failed")]
    CapabilityLease(#[from] brassclaw_authorization::CapabilityLeaseError),
...
impl From<brassclaw_host_runtime::ProductionWiringReport> for crate::RebornCompositionError {
    fn from(report: brassclaw_host_runtime::ProductionWiringReport) -> Self {
        Self::ProductionWiring { report }
    }
}

impl From<brassclaw_host_runtime::ProductionWiringReport> for RebornBuildError {
    fn from(report: brassclaw_host_runtime::ProductionWiringReport) -> Self {
        Self::ProductionWiring { report }
    }
}
...
mod tests {
    ...
    fn composition_unexpected_tenant_sandbox_process_port_becomes_invalid_config() {
        let error = RebornBuildError::from(
            crate::RebornCompositionError::UnexpectedTenantSandboxProcessPort {
                process_backend: brassclaw_host_api::ProcessBackendKind::LocalHost,
            },
        );
    ...
    fn composition_run_profile_becomes_planned_run_profile_resolver() {
        let error = RebornBuildError::from(crate::RebornCompositionError::RunProfile(
            brassclaw_turns::run_profile::RunProfileRegistryError::InvalidProfile {
