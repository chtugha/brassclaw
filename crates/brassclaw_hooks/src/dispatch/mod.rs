...
//! `brassclaw_reborn::loop_driver_host` and lands in a follow-up slice.
...
impl HookDispatcher {
    ...
    /// [`brassclaw_turns::run_profile::RunScopedHookMilestoneSink`] (or
    ...
    pub(crate) fn count_bindings_for_extension(
        &self,
        extension: &brassclaw_host_api::ExtensionId,
    ) -> usize {
    ...
    ///
    /// `owning_extension` is the [`brassclaw_host_api::ExtensionId`] of the
    ...
    pub(crate) fn install_observer(
        ...
        owning_extension: Option<brassclaw_host_api::ExtensionId>,
        scope: HookBindingScope,
    ...
    pub async fn dispatch_observer_at(
        &self,
        point: HookPointSpec,
        tenant: brassclaw_host_api::TenantId,
    ...
    /// hooks are fed by durable event-log replay in `brassclaw_reborn`, not by
    ...
    /// [`brassclaw_host_api::CapabilityId`] on a best-effort basis. If the
    ...
    fn record_capability_block_audit(&self, capability_name: &str) {
        ...
        if let Ok(cap_id) = brassclaw_host_api::CapabilityId::new(capability_name.to_string()) {
...
/// use std::sync::Arc;
/// use std::time::Duration;
/// use brassclaw_hooks::dispatch::HookDispatcherBuilder;
/// use brassclaw_hooks::registry::HookRegistry;
///
...
mod tests {
    ...
    fn tenant() -> brassclaw_host_api::TenantId {
        brassclaw_host_api::TenantId::new("alpha").expect("tenant ok")
    }

    fn host_ext() -> brassclaw_host_api::ExtensionId {
        brassclaw_host_api::ExtensionId::new("ext").expect("ext id ok")
    }
    ...
    async fn hook_deny_with_unparseable_capability_name_records_event_without_capability_id() {
        ...
        // else-arm.
        assert!(
            brassclaw_host_api::CapabilityId::new(String::new()).is_err(),
    ...
    async fn multiple_phase_denies_record_exactly_one_hook_deny_event() {
        use brassclaw_events::{InMemorySecurityAuditSink, SecurityAuditSink, SecurityBoundary};
    ...
    async fn pause_approval_does_not_record_hook_deny_event() {
        use brassclaw_events::{InMemorySecurityAuditSink, SecurityAuditSink};
    ...
    async fn own_capabilities_observer_filters_foreign_providers() {
        let owner = brassclaw_host_api::ExtensionId::new("ext.owner").expect("ok");
        let other = brassclaw_host_api::ExtensionId::new("ext.other").expect("ok");
    ...
    fn compile_time_seal_test() {
        ...
        //
        // Reachable only from inside `brassclaw_hooks`. External callers must
    ...
    /// `brassclaw_reborn` reads a registry-sourced extension hook and
    ...
    /// "Loader responsibility" section in `crates/brassclaw_hooks/CLAUDE.md`.
    ///
    ...
    async fn tier_specific_installers_are_documented_as_loader_contract() {
        ...
        dispatcher
            .install_installed_before_capability(
                installed_cap,
                HookPhase::Policy,
                brassclaw_host_api::ExtensionId::new("loader-installed").expect("valid ext"),
    ...
    use brassclaw_turns::run_profile::{InMemoryHookMilestoneSink, LoopHostMilestoneKind};
    ...
    fn host_ext_named(name: &str) -> brassclaw_host_api::ExtensionId {
        brassclaw_host_api::ExtensionId::new(name).expect("valid ext id")
    }
    ...
    fn event_resource_scope() -> brassclaw_host_api::ResourceScope {
        let user = brassclaw_host_api::UserId::new("user-ev").expect("valid user");
        let invocation = brassclaw_host_api::InvocationId::new();
        brassclaw_host_api::ResourceScope::local_default(user, invocation).expect("valid scope")
    }

    fn event_capability() -> brassclaw_host_api::CapabilityId {
        brassclaw_host_api::CapabilityId::new("event.fixture").expect("valid capability")
    }
    ...
    async fn providerless_hook_meta_event_resolves_provider_via_registry() {
        let owner = brassclaw_host_api::ExtensionId::new("ext").expect("valid extension id");
    ...
    async fn hook_failed_with_spoofed_provider_does_not_fire_target_extension_hooks() {
        let ext_a = brassclaw_host_api::ExtensionId::new("ext-a").expect("valid extension id");
        let ext_b = brassclaw_host_api::ExtensionId::new("ext-b").expect("valid extension id");
    ...
    async fn unknown_lifecycle_hook_id_with_carried_provider_stays_inert() {
        let target = brassclaw_host_api::ExtensionId::new("target").expect("valid extension id");
