...
pub trait PrivilegedMutatorSink: Send {
    ...
    /// raw body; `brassclaw_prompt_envelope::wrap_untrusted` performs the
...
mod tests {
    ...
    async fn event_triggered_sink_is_observer_only_at_type_level() {
        ...
        let tenant = brassclaw_host_api::TenantId::new("t".to_string()).expect("valid tenant");
        let user = brassclaw_host_api::UserId::new("u".to_string()).expect("valid user");
        let invocation_id = brassclaw_host_api::InvocationId::new();
        let scope = brassclaw_host_api::ResourceScope::local_default(user, invocation_id)
            .expect("valid scope");
        let event = brassclaw_events::RuntimeEvent::hook_failed(
            scope,
            brassclaw_host_api::CapabilityId::new("hooks.failed").expect("valid capability"),
        ...
        let ctx = EventTriggeredHookContext {
            tenant_id: tenant,
            event: &event,
            event_cursor: brassclaw_events::EventCursor::new(1),
            is_replay: false,
        };
