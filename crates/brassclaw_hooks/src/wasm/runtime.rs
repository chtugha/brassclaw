...
// Shared resource limiter extracted into `brassclaw_wasm_limiter` so both
// `brassclaw_hooks` and `brassclaw_wasm` depend on it through Cargo rather
...
// `brassclaw_hooks -> brassclaw_wasm` is preserved.
use brassclaw_wasm_limiter::WasmResourceLimiter;
...
pub struct WasmHookModuleRequest<'a> {
    pub extension_id: &'a brassclaw_host_api::ExtensionId,
