...
//! [`brassclaw_prompt_envelope`] helper — the same primitive memory-context
...
use brassclaw_prompt_envelope::{EnvelopeSource, EnvelopeTrust, wrap_untrusted};
...
/// adapter-wrap [`brassclaw_turns::run_profile::InstructionMaterializationStore`]
...
/// so this seam stays narrow and keeps `brassclaw_hooks` decoupled from
...
pub struct HookedLoopPromptPort {
    ...
    /// [`brassclaw_turns::run_profile::InstructionMaterializationStore`]);
...
mod tests {
    ...
    use async_trait::async_trait;
    use brassclaw_turns::run_profile::{LoopPromptBundle, LoopPromptBundleRef, PromptMode};
    ...
    impl PrivilegedBeforePromptHook for TrustedHijackHook {
        async fn evaluate(
            ...
            // `brassclaw_prompt_envelope`. The trusted-snippet constructor
