...
use brassclaw_turns::LoopResultRef;
...
impl crate::middleware::gate_ref::HookGateRefFactory for FailingGateRefFactory {
    async fn mint_approval_ref(
        &self,
        _reason: &str,
    ) -> Result<brassclaw_turns::LoopGateRef, AgentLoopHostError> {
...
fn snapshot_fixture_invocation() -> CapabilityInvocation {
    CapabilityInvocation {
        surface_version: brassclaw_turns::run_profile::CapabilitySurfaceVersion::new("snapshot:v1")
        ...
        input_ref: brassclaw_turns::run_profile::CapabilityInputRef::new(
...
fn invocation_arguments_digest_differs_for_different_input_refs() {
    ...
    let surface = brassclaw_turns::run_profile::CapabilitySurfaceVersion::new("v").expect("ok");
...
fn invocation_arguments_digest_differs_for_different_capability_ids() {
    let surface = brassclaw_turns::run_profile::CapabilitySurfaceVersion::new("v").expect("ok");
    let input_ref =
        brassclaw_turns::run_profile::CapabilityInputRef::new("input:shared").expect("ok");
    let a = CapabilityInvocation {
        surface_version: surface.clone(),
...
use brassclaw_host_api::ExtensionId as HostExtensionId;
...
fn install_predicate_hook(
    ...
    dispatcher
        .install_installed_before_capability(
            hook_id,
            HookPhase::Policy,
            brassclaw_host_api::ExtensionId::new("ext-test").expect("valid"),
