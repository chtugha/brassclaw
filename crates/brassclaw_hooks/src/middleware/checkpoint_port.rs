...
use brassclaw_turns::TurnCheckpointId;
use brassclaw_turns::run_profile::{AgentLoopHostError, LoopCheckpointPort, LoopCheckpointRequest};
...
mod tests {
    ...
    use async_trait::async_trait;
    use brassclaw_turns::run_profile::{LoopCheckpointKind, LoopCheckpointStateRef};
    use std::sync::Mutex;
            ...
            *self.calls.lock().expect("not poisoned") += 1;
            if self.fail {
                return Err(AgentLoopHostError::new(
                    brassclaw_turns::run_profile::AgentLoopHostErrorKind::CheckpointRejected,
