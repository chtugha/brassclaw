...
//! `brassclaw_turns`, and what downstream SSE / audit / replay consumers parse
...
/// `validate_name_segment` limit in `brassclaw_host_api::ids` so the two
...
/// - `brassclaw_hooks::identity::ExtensionId` (this type) is a transparent
...
///
/// ```ignore
/// let host_ext: brassclaw_host_api::ExtensionId = /* ... */;
/// let identity_ext: brassclaw_hooks::identity::ExtensionId = (&host_ext).into();
...
impl From<&brassclaw_host_api::ExtensionId> for ExtensionId {
    fn from(host: &brassclaw_host_api::ExtensionId) -> Self {
        ...
        // (both mirror `brassclaw_host_api::ids::validate_name_segment`).
...
mod tests {
    ...
    /// `LoopHostMilestoneKind::Hook*` variants in `brassclaw_turns`, and what
    ...
    fn extension_id_from_host_api_round_trips() {
        let host = brassclaw_host_api::ExtensionId::new("github-mcp.v1").expect("valid host id");
    ...
    /// `From<&brassclaw_host_api::ExtensionId> for ExtensionId`: if the two
    ...
    fn extension_id_from_host_api_round_trips_grammar_corners() {
        ...
        for raw in corners {
            let host = brassclaw_host_api::ExtensionId::new(raw)
