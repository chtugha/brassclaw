...
//! summaries defined in `brassclaw_turns`.
//!
//! The `brassclaw_turns` crate cannot depend on `brassclaw_hooks` (that boundary
//! is enforced by `brassclaw_architecture`). To let the dispatcher emit
...
use brassclaw_turns::run_profile::HookDecisionSummary;
...
/// in this crate and cannot cross into `brassclaw_turns`.
...
mod tests {
    ...
    /// same output as `HookId::to_hex()`. Downstream `brassclaw_turns`
