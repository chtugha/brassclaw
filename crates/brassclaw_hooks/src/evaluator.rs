    ...
    pub fn warn_in_memory_backend_active_in_production(&self) {
        tracing::warn!(
            "predicate evaluator is using the in-memory backend: replay dedup is \
             ...
             durable backend (see crates/brassclaw_hooks/docs/successors/03-persistent-counter.md)."
        );
    }
...
mod tests {
    ...
    fn tenant() -> brassclaw_host_api::TenantId {
        brassclaw_host_api::TenantId::new("alpha").expect("ok")
    }
    ...
    async fn invocation_counter_partitions_by_tenant() {
        ...
        let now = base();
        let alpha = brassclaw_host_api::TenantId::new("alpha").expect("ok");
        let beta = brassclaw_host_api::TenantId::new("beta").expect("ok");
