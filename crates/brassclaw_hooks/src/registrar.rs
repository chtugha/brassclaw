...
impl HookRegistrar {
    ...
    pub fn install(
        ...
        // `brassclaw_host_api::ExtensionId` is the authority-bearing identifier
        ...
        // (`brassclaw_reborn_composition`) replays the same validated install
...
mod tests {
    ...
    use crate::manifest::{HookManifestBody, HookManifestKind, HookManifestScope};
    ...
    fn extension() -> brassclaw_host_api::ExtensionId {
        brassclaw_host_api::ExtensionId::new("polymarket-trader").expect("valid extension id")
    }
    ...
    /// Phase 6 of the v1-removal plan removed the WASM hook backend
    /// (`HookManifestBody::Wasm`, `WasmHookRuntime`, `WasmResourceLimiter`).
    /// Ensure that the now-unsupported body variant is rejected at install
    /// time so a future schema migration cannot silently downgrade to
    /// predicate logic.
    #[test]
    fn install_rejects_wasm_body_variant_in_schema_validation() {
        let toml = r#"
id = "wasm-only"
...
export = "evaluate"
"#;
        let parsed: Result<crate::manifest::HookManifestEntry, _> = toml::from_str(toml);
        assert!(
            parsed.is_err(),
            "WASM hook body must not be accepted by the manifest schema after Phase 6"
        );
    }
    ...
    async fn installer_propagates_owning_extension_and_scope_from_manifest() {
        ...
        let host_extension =
            brassclaw_host_api::ExtensionId::new("polymarket-trader").expect("valid ext id");
