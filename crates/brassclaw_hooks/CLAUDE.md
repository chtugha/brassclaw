# brassclaw_hooks — Reborn loop hook framework
...
- The runner-facing `AgentLoopDriver` trait — that stays in `brassclaw_turns`.
- The concrete `LoopCapabilityPort` / `LoopPromptPort` / `LoopModelPort` impls —
  those stay in `brassclaw_loop_support` and `brassclaw_reborn`.
- The Reborn-side middleware composition that wraps host ports — that lives in
  `brassclaw_reborn::loop_driver_host` and consumes types from this crate.
...
```
brassclaw_turns       -> no dependency on brassclaw_hooks
brassclaw_hooks       -> depends on brassclaw_turns + brassclaw_host_api
brassclaw_reborn      -> depends on brassclaw_hooks for host composition (follow-up)
brassclaw_engine      -> no hook ownership; optional future driver consumer
```

Architecture test in `brassclaw_architecture::tests::reborn_dependency_boundaries`
proves the `brassclaw_turns -> brassclaw_hooks` edge stays absent.
...
- **Builtin** — compiled into BrassClaw, identity = crate path + symbol. May
  produce any decision kind via `BuiltinHookSink`.
- **Trusted** — user-placed in `~/.brassclaw/hooks/` or workspace `hooks/`. Cannot
...
`brassclaw_reborn` (or any other internal crate) reads a hook from the
...
- All model-visible hook output is bounded, typed, redacted/trust-labeled, and
  ...
  `brassclaw_host_runtime::memory_context` once that helper is extracted).
