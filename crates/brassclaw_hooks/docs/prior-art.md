...
> Purpose: validate the BrassClaw hooks design against well-established
> hook/policy/extension systems. For each axis where BrassClaw diverges,
...
| **ICLAW** | BrassClaw `brassclaw_hooks` | LLM agent loop |
...
## Where BrassClaw stands out (vs prior art)
...
## Where BrassClaw is conventional (and should be)
...
## Where BrassClaw is conventional but **shouldn't** be (open questions)
...
## Where BrassClaw diverges but the *why* is weak (review needed)
...
- Axes were chosen *after* drafting BrassClaw's design, so the table is unavoidably colored by ICLAW's vocabulary. A second pass with axes chosen from one of the survey systems (e.g., K8S's admission-controller checklist) would be a useful adversarial check.
