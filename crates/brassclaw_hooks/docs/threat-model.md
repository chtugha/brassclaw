...
> the `brassclaw_hooks` framework. For each vector: the mitigation, the
...
In scope:
- The `brassclaw_hooks` crate and its public API
...
- The cross-crate seam to `brassclaw_turns` (milestone sink) and
  `brassclaw_events` (RuntimeEvent projection)
- The `brassclaw_prompt_envelope` leaf crate
...
| T6 | Tamper with milestone-event projection to drop `HookFailed` events | A5 | Projector is in `brassclaw_reborn`; sits behind the same audit substrate as the rest of the loop; events are append-only | `milestone_events::tests::all_hook_milestones_projected` | Med (depends on `brassclaw_events` substrate integrity, which is its own threat model) |
| T7 | Tamper with `brassclaw_prompt_envelope` byte cap to exfiltrate larger payload | A1 | 4 KiB cap enforced in `wrap_untrusted`; cap is a const, not configurable per call | `prompt_envelope::tests::body_cap_enforced` | Low |
...
| I6 | Audit log itself leaks user data to operators who shouldn't see it | A5 | Audit substrate access is governed by `brassclaw_events` policies (separate threat model) | N/A here | Defer |
