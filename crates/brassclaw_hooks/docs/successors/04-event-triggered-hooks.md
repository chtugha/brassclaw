...
```rust
// in brassclaw_hooks::points::event_triggered
//
...
- **Subscription side**: a new dispatcher path
  ...
  reborn factory wires it to `brassclaw_events`'s event-stream
...
- Hook dispatch receives the durable `RuntimeEvent` directly. `brassclaw_hooks`
  depends on `brassclaw_events` as of PR #3640 — the milestone projection in
  PR #3573 covered `brassclaw_reborn`'s milestone wiring, but the hook crate
  ...
  network, WASM, or Reborn internals; the `brassclaw_architecture` boundary
  test enforces this.

## Cross-cutting constraints

- **Cross-crate boundary**: `brassclaw_hooks` already depends on
  `brassclaw_events` for the milestone projection (PR #3573), and it
...
6. **Cross-crate boundary**: `brassclaw_architecture` test confirms
   `brassclaw_hooks` doesn't gain a forbidden dep on
   `brassclaw_events` or `brassclaw_host_runtime`.
...
- Cross-crate dep direction: `brassclaw_events` is already a
  dependency of `brassclaw_hooks` (via the milestone projection in
  PR #3573), so the dep direction is established. The new traffic is
  the *consumer-side* of the event stream, which means
  `brassclaw_hooks` learns about `EventStreamSubscriber` or whatever
  trait `brassclaw_events` exposes.
