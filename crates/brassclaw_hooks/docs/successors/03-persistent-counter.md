...
the rest of the project (`src/db/mod.rs`, `brassclaw_events`). The
...
driven by `event_id` (which is `RuntimeEventId` from `brassclaw_events`,
...
- **`InMemoryPredicateStateBackend`**: keeps current behavior; used
  for tests + the standalone `brassclaw_hooks` integration tests that
...
`brassclaw_hooks::predicate_state::PredicateStateBackend` is `pub`,
