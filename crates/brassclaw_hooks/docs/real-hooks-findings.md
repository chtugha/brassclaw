...
> Companion to [`crates/brassclaw_hooks/tests/real_hooks.rs`](../tests/real_hooks.rs).
>
...
Both seams live in `brassclaw_reborn`. From a standalone
`brassclaw_hooks` test, the best a hook author can do is:
...
an `brassclaw_reborn` integration test
...
either (a) depending on `brassclaw_reborn` as a dev-dep (heavy + the
...
2. Ship a `MockCapabilityInputResolver` as part of `brassclaw_hooks`'s
...
3. Accept the friction and document that NumericSum hooks must be
   integration-tested via `brassclaw_reborn`. Cheapest, but it's a
...
### F5 — `HookId::derive` requires the crate's `identity::ExtensionId`, not `brassclaw_host_api::ExtensionId`

**What happened:** A hook author already holds an
`brassclaw_host_api::ExtensionId` (the authoritative identifier). To
mint a `HookId` via `HookId::derive`, they need
`brassclaw_hooks::identity::ExtensionId` — a different newtype.
...
1. Add a `From<&brassclaw_host_api::ExtensionId>` impl for
   `brassclaw_hooks::identity::ExtensionId`. One-liner, makes the
...
| F5 — Two `ExtensionId` types are confusing | Low | **Fixed** — `From<&brassclaw_host_api::ExtensionId>` impl + cross-link rustdoc |
