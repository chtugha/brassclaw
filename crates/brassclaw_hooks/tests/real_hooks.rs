//! Three "real" hooks built against the public `brassclaw_hooks` API.
//!
...
//!    arguments (the resolver wiring lives in `brassclaw_reborn`, so the
...
//!    `crates/brassclaw_reborn/tests/hooks_integration.rs`).
//!
...
use brassclaw_hooks::HookTrustClass;
use brassclaw_hooks::dispatch::HookDispatcherBuilder;
use brassclaw_hooks::evaluator::PredicateEvaluator;
use brassclaw_hooks::identity::HookLocalId;
use brassclaw_hooks::kinds::gate::GateDecisionView;
use brassclaw_hooks::kinds::mutator::{HookPatchView, PatchOrdinalHint, SnippetBodyView};
use brassclaw_hooks::manifest::{HookManifestBody, HookManifestEntry, HookManifestKind};
use brassclaw_hooks::ordering::HookPhase;
use brassclaw_hooks::points::{BeforeCapabilityHookContext, BeforePromptHookContext};
use brassclaw_hooks::predicate::{
    CapabilityPredicate, HookPredicateSpec, OnExceededAction, ValueOrRateBound,
};
use brassclaw_hooks::registrar::HookRegistrar;
use brassclaw_hooks::registry::HookRegistry;
use brassclaw_hooks::sink::{PrivilegedBeforePromptHook, PrivilegedMutatorSink};
use brassclaw_host_api::{ExtensionId, TenantId};
...
async fn polymarket_daily_cap_denies_after_ten_invocations() {
    ...
    let ctx = |digest: [u8; 32]| {
        BeforeCapabilityHookContext::new(
            ...
            digest,
            brassclaw_hooks::points::SanitizedArguments::unresolved(),
...
/// the resolver path lives in `brassclaw_reborn`, so this test only
/// exercises that the manifest is well-formed and that the registrar
/// accepts it. The dispatch-time fire-or-not test is in
/// `crates/brassclaw_reborn/tests/hooks_integration.rs::numeric_sum_predicate_caps_total_value_against_real_inputs`.
...
async fn large_stake_approval_with_unresolved_args_fails_closed() {
    ...
    // not wired in (the default in `brassclaw_hooks` standalone), a
    // NumericSum predicate dispatched against unresolved arguments
    // must NOT permit the call. Production wiring lives in
    // `brassclaw_reborn`; here we confirm the fail-closed posture.
    let extension = ExtensionId::new("polymarket-trader").expect("valid ext id");
    let registrar = HookRegistrar::new(Arc::new(PredicateEvaluator::new()));
    let builder = HookDispatcherBuilder::new(HookRegistry::new());
...
async fn pii_redaction_warning_injects_trusted_snippet() {
    use brassclaw_hooks::identity::{ExtensionId as IdentExtensionId, HookId, HookVersion};

    let hook_id = HookId::derive(
        &IdentExtensionId::new("brassclaw-builtin").expect("valid IdentExtensionId in test"),
        "1.0.0",
