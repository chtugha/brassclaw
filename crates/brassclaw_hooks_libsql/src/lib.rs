...
//! `brassclaw_hooks`. The framework crate (`brassclaw_hooks`) owns the public
...
//! which lives in its own `brassclaw_hooks_postgres` crate for the same reason.
//!
...
pub mod test_support {
    use brassclaw_hooks::predicate_hash::{self, Digest};
    use brassclaw_hooks::predicate_state::InvocationKey;

    /// `scope_hash` (tenant digest) bytes — see
    /// [`brassclaw_hooks::predicate_hash::scope_hash`]. Returns the canonical
    ...
    /// [`brassclaw_hooks::predicate_hash::invocation_key_hash`]. Mirrors the
