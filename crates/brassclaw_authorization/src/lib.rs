//! Capability authorization contracts for BrassClaw Reborn.
//!
//! `brassclaw_authorization` evaluates authority-bearing host API contracts. It
...
/// [`CompositeRootFilesystem`](brassclaw_filesystem::CompositeRootFilesystem)
/// or the in-memory backend for tests). The [`ScopedFilesystem`] resolves
/// the `/authorization` alias to a tenant/user-scoped
/// [`VirtualPath`](brassclaw_host_api::VirtualPath) per its
/// [`MountView`](brassclaw_host_api::MountView) and enforces per-op ACL
    ...
    F: RootFilesystem,
{
    ...
    /// [`VirtualPath`](brassclaw_host_api::VirtualPath) results because
...
/// `brassclaw_trust::InvalidationBus` runs listeners synchronously, while this
...
// mount alias. The [`MountView`](brassclaw_host_api::MountView) granted by
// composition resolves the alias to a tenant/user-scoped
// [`VirtualPath`](brassclaw_host_api::VirtualPath) before any backend op —
...
/// [`VirtualPath`](brassclaw_host_api::VirtualPath)s) so the per-op ACL
