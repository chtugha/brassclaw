...
//!
//! Usage:
//!   cargo run -p brassclaw_tui --example dev
//!
//! Hot-reload loop (recompiles + restarts on any source change):
//!   cargo watch -x 'run -p brassclaw_tui --example dev' -w crates/brassclaw_tui/src
//!
//! This compiles in ~5s instead of minutes because it skips the entire
//! brassclaw binary (database, LLM, WASM, Docker, etc.).
...
fn main() {
    let config = TuiAppConfig {
        ...
        workspace_path: std::env::current_dir()
            ...
            .unwrap_or_else(|_| "~/projects/brassclaw".into()),
        memory_count: 42,
            ...
            while let Some(user_msg) = msg_rx.recv().await {
                ...
                let response = format!(
                    "You said: **{msg}**\n\nThis is a mock response from the dev harness. \
                     Edit `crates/brassclaw_tui/src/` and watch it reload.",
                );
