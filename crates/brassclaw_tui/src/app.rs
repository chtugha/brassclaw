...
/// Write the current log ring buffer to a timestamped file under `~/.brassclaw/logs/`
/// (falls back to `$BRASSCLAW_HOME/logs/` or the current directory). Returns the
...
/// without racing `BRASSCLAW_HOME` env mutation across threads.
...
fn log_output_dir() -> Option<std::path::PathBuf> {
    if let Ok(custom) = std::env::var("BRASSCLAW_HOME") {
