# brassclaw_process_sandbox guardrails
...
- Do not reach into `brassclaw_host_runtime`'s dispatcher/services composition; this crate is for dynamic sandbox process execution behind `ProcessExecutor` and shares no types with the extension runtime lanes (`Mcp`/`FirstParty`/`System` in `brassclaw_extensions`). (The v1 `script_runtime` lane this guardrail once separated from was removed in Phase 4.)
