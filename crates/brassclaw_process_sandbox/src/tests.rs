...
use brassclaw_processes::{ProcessCancellationToken, ProcessExecutionRequest, ProcessExecutor};
...
fn sample_config(root: &Path) -> DockerProcessSandboxConfig {
    DockerProcessSandboxConfig {
        ...
        broker: Some(DockerBrokerConfig {
            ...
            ca_cert_container_path: "/brassclaw/broker/ca.pem".to_string(),
        }),
    }
}
...
fn plan_validation_rejects_mounts_over_system_paths() {
    let mut plan = sample_plan();
    plan.mounts.workspace.container_path = "/etc/brassclaw".to_string();
...
fn docker_args_never_include_secret_material() {
    ...
    assert!(joined.contains("BRASSCLAW_EGRESS_LOCKDOWN=broker-only"));
    ...
    assert!(
        invocation
            .container_name
            .starts_with("brassclaw-sandbox-run-")
    );
}
...
fn install_and_run_phases_have_different_mount_and_network_policies() {
    ...
    assert!(!install_args.contains("BRASSCLAW_EGRESS_LOCKDOWN=broker-only"));
    assert!(install_args.contains("dst=/brassclaw/state/tools,readonly"));
    assert!(install_args.contains("dst=/brassclaw/state/cache,readonly"));
    assert!(run_args.contains("BRASSCLAW_EGRESS_LOCKDOWN=broker-only"));
    assert!(run_args.contains("dst=/brassclaw/state/tools,readonly"));
    assert!(run_args.contains("dst=/brassclaw/state/cache,readonly"));
}
