...
use brassclaw_llm::recording::{TraceFile, TraceResponse};

pub const TRACE_CONTRIBUTION_SCHEMA_VERSION: &str = "brassclaw.trace_contribution.v1";
pub const TRACE_CONTRIBUTION_POLICY_VERSION: &str = "2026-04-24";
pub const DETERMINISTIC_REDACTION_PIPELINE_VERSION: &str = "brassclaw-deterministic-secret-path-v1";
...
pub const TRACE_REMOTE_REQUEST_TIMEOUT_ENV: &str = "BRASSCLAW_TRACE_REMOTE_REQUEST_TIMEOUT_MS";
...
pub struct TraceContributionEnvelope {
    ...
    pub brassclaw: BrassclawTraceMetadata,
...
pub struct BrassclawTraceMetadata {
    pub version: String,
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl Default for StandingTraceContributionPolicy {
    fn default() -> Self {
        Self {
            ...
            bearer_token_env: "BRASSCLAW_TRACE_SUBMIT_TOKEN".to_string(),
...
impl RawTraceContribution {
    pub fn from_recorded_trace(
        ...
        Self {
            ...
            created_at,
            brassclaw: BrassclawTraceMetadata {
...
pub fn privacy_filter_adapter_from_env() -> Option<Arc<dyn PrivacyFilterAdapter>> {
    let command = std::env::var("BRASSCLAW_TRACE_PRIVACY_FILTER_COMMAND").ok()?;
    let command = command.trim();
    if command.is_empty() {
        return None;
    }

    let args = std::env::var("BRASSCLAW_TRACE_PRIVACY_FILTER_ARGS")
        .ok()
        .map(|raw| {
    ...
    if let Some(timeout_ms) = parse_usize_env("BRASSCLAW_TRACE_PRIVACY_FILTER_TIMEOUT_MS") {
        adapter = adapter.with_timeout(Duration::from_millis(timeout_ms as u64));
    }
    if let Some(max_input_bytes) = parse_usize_env("BRASSCLAW_TRACE_PRIVACY_FILTER_MAX_INPUT_BYTES")
    {
        adapter = adapter.with_input_limit(max_input_bytes);
    }
    let max_stdout = parse_usize_env("BRASSCLAW_TRACE_PRIVACY_FILTER_MAX_STDOUT_BYTES");
    let max_stderr = parse_usize_env("BRASSCLAW_TRACE_PRIVACY_FILTER_MAX_STDERR_BYTES");
...
impl DeterministicTraceRedactor {
    pub fn new(known_path_prefixes: Vec<String>) -> Self {
        ...
        Self {
            leak_detector: brassclaw_safety::LeakDetector::new(),
            known_path_prefixes,
    ...
    fn redact_text_with_state(
        ...
        let ranges = scan
            ...
            .map(|m| {
                ...
                if matches!(
                    m.severity,
                    brassclaw_safety::LeakSeverity::High | brassclaw_safety::LeakSeverity::Critical
                ) {
                    report.blocked_secret_detected = true;
                }
                m.location.clone()
...
impl TraceRedactor for DeterministicTraceRedactor {
    async fn redact_trace(
        ...
        let trace_card_channel = trace.brassclaw.channel;
...
pub fn derived_artifact_invalidation_marker(
    ...
    DerivedArtifactInvalidationMarker {
        schema_version: "brassclaw.trace_derived_artifact_invalidation.v1".to_string(),
...
pub fn trace_contribution_dir_for_scope(scope: Option<&str>) -> PathBuf {
    let base = brassclaw_common::paths::brassclaw_base_dir().join("trace_contributions");
    match scope {
        ...
        match self {
            Self::InviteCodeMissing => {
                "the workload token did not carry an invite_code claim. \
                 Re-run `brassclaw traces opt-in --upload-token-invite-code <CODE> ...` with the operator-issued code, \
...
async fn fetch_trace_upload_claim_from_issuer(
    ...
    let client = reqwest::Client::builder()
        ...
        .user_agent("brassclaw-trace-commons-upload-claim/0.1")
    ...
    let request_body = TraceUploadClaimIssuerRequest {
        schema_version: "brassclaw.trace_upload_claim_request.v1",
...
fn trace_remote_http_client() -> Result<reqwest::Client, TraceRemoteRequestFailure> {
    ...
    reqwest::Client::builder()
        ...
        .redirect(reqwest::redirect::Policy::none())
        .user_agent("brassclaw-trace-commons-client")
        .build()
        .map_err(|error| {
...
fn trace_queue_telemetry_failure_kind(error: &anyhow::Error) -> TraceQueueTelemetryFailureKind {
    for cause in error.chain() {
        ...
        if let Some(llm_error) = cause.downcast_ref::<brassclaw_llm::error::LlmError>()
...
fn trace_queue_telemetry_failure_kind_for_llm_error(
    error: &brassclaw_llm::error::LlmError,
) -> Option<TraceQueueTelemetryFailureKind> {
    match error {
        brassclaw_llm::error::LlmError::AuthFailed { .. }
        | brassclaw_llm::error::LlmError::SessionExpired { .. }
        | brassclaw_llm::error::LlmError::SessionRenewalFailed { .. } => {
            Some(TraceQueueTelemetryFailureKind::Credential)
        }
        brassclaw_llm::error::LlmError::RateLimited { .. } => {
            Some(TraceQueueTelemetryFailureKind::HttpRejection)
        }
        brassclaw_llm::error::LlmError::RequestFailed { .. } => {
            Some(TraceQueueTelemetryFailureKind::Network)
        }
        _ => None,
    }
...
mod tests {
    use super::*;
    use base64::Engine;
    use brassclaw_llm::recording::{TraceStep, TraceToolCall};
    ...
    fn deterministic_text_redactor_redacts_generic_local_paths() {
        ...
        let (redacted, report) =
            redactor.redact_text("read /tmp/brassclaw/private/token.txt before upload");
    ...
    fn tool_specific_payload_redaction_removes_email_content_fields() {
        ...
        let payload = serde_json::json!({
            ...
            "body": "Please review /tmp/brassclaw/private.txt",
            "public_id": "message-1"
        });
        ...
        assert!(!json.contains("/tmp/brassclaw/private.txt"));
    ...
    async fn policy_aware_submit_uses_bounded_request_timeout() {
        ...
        let _token_guard = EnvVarRestore::set(token_env, "super-secret-token");
        let _timeout_guard = EnvVarRestore::set("BRASSCLAW_TRACE_REMOTE_REQUEST_TIMEOUT_MS", "50");
        let app = axum::Router::new().route(
            "/v1/traces",
            axum::routing::post(|| async {
    ...
    async fn policy_aware_direct_submit_uses_default_credential_provider() {
        let _token_env = EnvVarRestore::set(
            "BRASSCLAW_TRACE_COMMONS_DIRECT_SUBMIT_TEST_TOKEN",
            "direct-submit-token",
        );
        let seen = Arc::new(std::sync::Mutex::new(Vec::<String>::new()));
        let seen_for_submit = seen.clone();
        ...
        apply_credit_estimate_to_envelope(&mut envelope);
        let policy = StandingTraceContributionPolicy {
            bearer_token_env: "BRASSCLAW_TRACE_COMMONS_DIRECT_SUBMIT_TEST_TOKEN".to_string(),
            ..Default::default()
        };
    ...
    fn queue_telemetry_classifies_typed_llm_auth_and_session_failures_without_raw_details() {
        ...
        let cases = [
            (
                anyhow::Error::from(brassclaw_llm::error::LlmError::AuthFailed {
                    provider: "trace-secret-provider".to_string(),
                }),
                TraceQueueTelemetryFailureKind::Credential,
            ),
            (
                anyhow::Error::from(brassclaw_llm::error::LlmError::SessionExpired {
                    provider: "trace-secret-provider".to_string(),
                }),
                TraceQueueTelemetryFailureKind::Credential,
            ),
    ...
    async fn server_rescrub_redacts_late_leaks_before_storage() {
        ...
        envelope.events[0].redacted_content =
            Some("late leak at /tmp/brassclaw/private/token.txt".to_string());
        envelope.events[1].structured_payload = serde_json::json!({
            "Authorization": "Bearer abcdefghijklmnopqrstuvwxyz123456",
            "path": "/tmp/brassclaw/private/token.txt"
        });
        ...
        assert!(!json.contains("/tmp/brassclaw/private/token.txt"));
        assert!(!json.contains("abcdefghijklmnopqrstuvwxyz"));
        assert!(
            envelope
                .privacy
    ...
    fn standing_policy_serde_back_compat_when_invite_code_missing() {
        ...
        let legacy_json = r#"{
            ...
            "bearer_token_env": "BRASSCLAW_TRACE_SUBMIT_TOKEN",
            ...
            "upload_token_workload_token_env": "BRASSCLAW_TRACE_WORKLOAD_TOKEN",
