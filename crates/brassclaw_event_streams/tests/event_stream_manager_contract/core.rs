...
async fn fetch_snapshot_maps_projection_snapshot_errors() {
    let scope = projection_scope("thread-a");
    let rebase_requested =
        ProjectionCursor::for_scope(scope.clone(), EventCursor::new(super::STALE_CURSOR));
    ...
    for (projection_error, expected) in [
        ...
        (
            ProjectionError::TurnEventRebaseRequired {
                requested: brassclaw_turns::EventCursor(0),
                earliest: brassclaw_turns::EventCursor(5),
            },
            ProjectionStreamError::InvalidRequest {
        ...
        (
            ProjectionError::MissingProjectionMetadata {
                field: brassclaw_event_projections::MissingMetadataField::OwnerUserId,
            },
            ProjectionStreamError::InvalidRequest {
