...
fn validate_product_thread_payload(
    ...
    let all_thread_entries_match = |entries: &[brassclaw_event_projections::TimelineEntry]| {
        entries
            .iter()
            .all(|entry| entry.thread_id.as_ref() == Some(thread_id))
    };
    let all_run_statuses_match = |runs: &[brassclaw_event_projections::RunStatusProjection]| {
        runs.iter()
            .all(|run| run.thread_id.as_ref() == Some(thread_id))
    };
    let all_capability_activities_match =
        |activities: &[brassclaw_event_projections::CapabilityActivityProjection]| {
            activities
                .iter()
