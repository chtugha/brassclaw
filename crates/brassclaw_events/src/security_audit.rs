...
//! decisions* — the moments when a defense-in-depth boundary in BrassClaw
...
use brassclaw_host_api::{CapabilityId, ResourceScope};
...
impl SecurityAuditSink for TracingSecurityAuditSink {
    fn record(&self, event: SecurityAuditEvent) {
        ...
        // payload-free invariant.
        tracing::debug!(
            target: "brassclaw::security_audit",
...
mod tests {
    ...
    fn event_can_be_enriched_with_scope_and_capability() {
        use brassclaw_host_api::{InvocationId, UserId};
        ...
        .expect("valid scope");
        let cap = CapabilityId::new("brassclaw.echo".to_string()).expect("valid capability id");
