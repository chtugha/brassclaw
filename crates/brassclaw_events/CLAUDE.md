...
append-log substrate for BrassClaw Reborn.
...
`crates/brassclaw_reborn_event_store/`. Keep storage drivers out of this
substrate crate: downstream store crates should depend on `brassclaw_events`,
...
Forbidden dependencies (enforced by `brassclaw_architecture`): authorization,
approvals, capabilities, dispatcher, extensions, host_runtime, secrets,
network, mcp, processes, resources, run_state, scripts, wasm.

`brassclaw_filesystem` and `brassclaw_memory` are **deliberately not forbidden**
...
`crates/brassclaw_architecture/tests/reborn_dependency_boundaries.rs`; if the
