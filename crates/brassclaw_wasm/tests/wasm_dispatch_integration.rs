...
fn preserved_wasm_error_usage(error: &brassclaw_wasm::WasmError) -> Option<ResourceUsage> {
    if let brassclaw_wasm::WasmError::ExecutionFailed { usage, .. } = error
...
fn registry_with_package(manifest: &str) -> brassclaw_extensions::ExtensionRegistry {
...
fn wasm_error_kind(error: &brassclaw_wasm::WasmError) -> RuntimeDispatchErrorKind {
    match error {
        brassclaw_wasm::WasmError::EngineCreationFailed(_) => RuntimeDispatchErrorKind::Executor,
