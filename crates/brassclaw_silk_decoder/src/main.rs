//! `brassclaw-silk-decoder` — reads raw SILK v3 bytes on stdin, writes a WAV-wrapped
//! 16-bit little-endian mono PCM stream on stdout.
//!
//! Isolated here so the main BrassClaw build does not need libclang. The host
...
//!
//! Usage:
//!   brassclaw-silk-decoder [--sample-rate 24000]
...
fn main() -> ExitCode {
    ...
    let sample_rate = match parse_sample_rate(&args) {
        Ok(rate) => rate,
        Err(message) => {
            eprintln!("brassclaw-silk-decoder: {message}");
    ...
    if input.is_empty() {
        eprintln!("brassclaw-silk-decoder: empty input on stdin");
        return ExitCode::from(1);
    }
    ...
    let pcm = match decode_silk(&input, sample_rate) {
        Ok(pcm) => pcm,
        Err(error) => {
            eprintln!("brassclaw-silk-decoder: SILK decode failed: {error}");
            return ExitCode::from(3);
        }
    };
    if pcm.is_empty() {
        eprintln!("brassclaw-silk-decoder: SILK decoder returned empty PCM");
        return ExitCode::from(3);
    }
    if pcm.len() > MAX_PCM_BYTES {
        eprintln!(
            "brassclaw-silk-decoder: decoded PCM exceeds {MAX_PCM_BYTES} bytes ({} bytes)",
            pcm.len()
        );
        return ExitCode::from(3);
    }
    ...
    let wav = match pcm_s16le_to_wav(&pcm, sample_rate as u32) {
        Ok(bytes) => bytes,
        Err(error) => {
            eprintln!("brassclaw-silk-decoder: {error}");
            return ExitCode::from(3);
        }
    };
    ...
    if let Err(error) = stdout.write_all(&wav) {
        eprintln!("brassclaw-silk-decoder: failed to write WAV to stdout: {error}");
        return ExitCode::from(1);
    }
    if let Err(error) = stdout.flush() {
        eprintln!("brassclaw-silk-decoder: failed to flush stdout: {error}");
        return ExitCode::from(1);
    }
    ExitCode::SUCCESS
}
