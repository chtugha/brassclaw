...
BrassClaw build does **not** need `libclang`.
...
and produces `target/release/brassclaw-silk-decoder` (relative to the
crate). It is excluded from the top-level workspace so it does not
participate in `cargo build --workspace`.

## Install

The BrassClaw host looks for the binary in this order:

1. `BRASSCLAW_SILK_DECODER` environment variable (a path to the binary)
2. A sibling of the running `brassclaw` executable named
   `brassclaw-silk-decoder` (with `.exe` on Windows)
3. `brassclaw-silk-decoder` on `$PATH`
