# Agent Map — brassclaw_silk_decoder
...
- This crate is excluded from the workspace so the main BrassClaw build does not require libclang.

## What This Crate Owns

- Standalone helper binary `brassclaw-silk-decoder`.
- WeChat raw SILK v3 voice-note decoding to WAV.
- CLI/protocol behavior for the helper process.

## Do Not Move In Here

- Main workspace dependencies or root BrassClaw runtime behavior.
- Channel/product workflow logic beyond the decoder protocol.
- Secrets, network access, database access, or agent state.

## Validation

- Build/check from repo root: `cargo build --manifest-path crates/brassclaw_silk_decoder/Cargo.toml`
