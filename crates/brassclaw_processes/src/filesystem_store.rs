...
//! [`ScopedFilesystem`](brassclaw_filesystem::ScopedFilesystem). The paths in
...
use brassclaw_host_api::{ProcessId, ResourceScope, ScopedPath, VirtualPath};
...
/// [`MountView`](brassclaw_host_api::MountView) per-operation ACL and
/// resolves the `/processes` alias to a tenant-scoped
/// [`VirtualPath`](brassclaw_host_api::VirtualPath) before any backend
            ...
            let record: ProcessRecord =
                serde_json::from_slice(&entry.entry.body).map_err(|error| {
                    FilesystemError::Backend {
                        ...
                        operation: brassclaw_filesystem::FilesystemOperation::Query,
