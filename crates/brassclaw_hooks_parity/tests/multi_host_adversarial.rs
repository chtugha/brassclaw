...
//!   reachable server via `BRASSCLAW_HOOKS_POSTGRES_URL` / `DATABASE_URL`;
...
mod libsql_cluster {
    ...
    impl Cluster {
        ...
        pub async fn distinct_invocation_scopes(&self, tenant: &str) -> usize {
            let scope = brassclaw_hooks_libsql::test_support::scope_hash_bytes(tenant);
...
mod postgres_cluster {
    ...
    /// `BRASSCLAW_REQUIRE_POSTGRES=1` turns a missing/unreachable Postgres into a
        ...
        ($url:ident) => {
            match prepare().await {
                ...
                None => {
                    if require_postgres() {
                        panic!(
                            "BRASSCLAW_REQUIRE_POSTGRES=1 but Postgres multi-host parity \
