...
/// When `BRASSCLAW_REQUIRE_POSTGRES=1`, a missing/unreachable Postgres backend is
...
pub(crate) fn require_postgres_or_skip(script_name: &str) {
    ...
    if required {
        panic!(
            "[{script_name}] BRASSCLAW_REQUIRE_POSTGRES=1 but the Postgres parity leg \
             ...
             BRASSCLAW_HOOKS_POSTGRES_URL / DATABASE_URL unset/unreachable). \
    ...
    eprintln!(
        "[{script_name}] Postgres leg SKIPPED (no --features postgres or no reachable \
         ...
         BRASSCLAW_REQUIRE_POSTGRES=1 to make this a hard failure in CI."
    );
}
