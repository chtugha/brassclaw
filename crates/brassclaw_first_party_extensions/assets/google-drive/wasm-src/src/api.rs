...
fn multipart_boundary(metadata: &str, content: &str) -> String {
    ...
    loop {
        let boundary = format!("brassclaw_upload_boundary_{hash:016x}_{attempt:016x}");
...
mod tests {
    ...
    fn multipart_boundary_does_not_appear_in_metadata_or_content() {
        let metadata = r#"{"name":"brassclaw_upload_boundary_marker"}"#;
        let content = "body with brassclaw_upload_boundary_marker";

        let boundary = multipart_boundary(metadata, content);

        assert!(!metadata.contains(&boundary));
        assert!(!content.contains(&boundary));
        assert!(boundary.starts_with("brassclaw_upload_boundary_"));
    }
}
