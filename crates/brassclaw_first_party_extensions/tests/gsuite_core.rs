...
async fn calendar_handler_integration_tests() {
    ...
    let auth = auth_with_google_account(
        ...
        vec![
            ...
            provider_scope(brassclaw_auth::GOOGLE_CALENDAR_EVENTS_SCOPE),
        ],
    )
    .await;
...
async fn calendar_create_event_does_not_forward_list_query_fields() {
    ...
    let auth = auth_with_google_account(
        &scope,
        vec![provider_scope(brassclaw_auth::GOOGLE_CALENDAR_EVENTS_SCOPE)],
    )
    .await;
    let egress = Arc::new(RecordingEgress::permissive_success());
...
async fn gsuite_handler_refreshes_expired_google_token_once_and_retries() {
    ...
    brassclaw_auth::CredentialAccountService::create_account(
        auth.as_ref(),
    ...
    let refreshed = brassclaw_auth::CredentialAccountService::select_unique_configured_account(
        auth.as_ref(),
        brassclaw_auth::CredentialAccountSelectionRequest::new(
            auth_scope(&scope),
...
async fn gsuite_handler_refresh_retries_with_the_same_account_after_account_selection_changes() {
    ...
    let initial_account = brassclaw_auth::CredentialAccountService::create_account(
        &seed_auth,
        NewCredentialAccount {
            scope: auth_scope(&scope),
            provider: google_provider_id().unwrap(),
    ...
    .await
    .unwrap();
    let alternate_account = brassclaw_auth::CredentialAccountService::create_account(
        &seed_auth,
        NewCredentialAccount {
            scope: auth_scope(&scope),
            provider: google_provider_id().unwrap(),
...
async fn gsuite_handler_does_not_refresh_on_non_401_unauthenticated_response() {
    ...
    let account = brassclaw_auth::CredentialAccountService::select_unique_configured_account(
        auth.as_ref(),
        brassclaw_auth::CredentialAccountSelectionRequest::new(
            auth_scope(&scope),
            google_provider_id().unwrap(),
...
#[async_trait]
impl brassclaw_auth::CredentialAccountRecordSource for AccountSwitchingAuthService {
...
#[async_trait]
impl brassclaw_auth::CredentialAccountService for AccountSwitchingAuthService {
    ...
    async fn update_status(
        ...
        account_id: brassclaw_auth::CredentialAccountId,
...
async fn gsuite_handler_fails_before_egress_when_scope_is_missing() {
    ...
    assert_eq!(
        error.kind(),
        brassclaw_host_api::RuntimeDispatchErrorKind::Client
    );
    assert!(egress.requests().is_empty());
}
...
async fn gsuite_handler_rejects_missing_required_input() {
    ...
    assert_eq!(
        error.kind(),
        brassclaw_host_api::RuntimeDispatchErrorKind::InputEncode
    );
}
...
async fn add_attendees_refreshes_expired_get_and_retries_patch() {
    ...
    brassclaw_auth::CredentialAccountService::create_account(
        ...
        NewCredentialAccount {
            ...
            access_secret: Some(SecretHandle::new("google-old-access").unwrap()),
            refresh_secret: Some(SecretHandle::new("google-old-refresh").unwrap()),
            scopes: vec![provider_scope(brassclaw_auth::GOOGLE_CALENDAR_EVENTS_SCOPE)],
        },
    )
    .await
    .unwrap();
