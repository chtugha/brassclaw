...
pub enum GsuiteCredentialDispatchReason {
    ...
    MissingAccessSecret,
    AuthRequired {
        required_secrets: Vec<brassclaw_host_api::SecretHandle>,
    },
    BackendAuth,
    HostApi,
}
...
impl GsuiteDispatchError {
    ...
    pub fn auth_requirement(&self) -> Option<Vec<brassclaw_host_api::SecretHandle>> {
...
pub struct GsuiteCredentialStageRequest<'a> {
    ...
    pub capability_id: &'a CapabilityId,
    pub access_secret: &'a brassclaw_host_api::SecretHandle,
}

/// Alias for [`brassclaw_host_api::CredentialStageError`].
///
/// The shared type lives in `brassclaw_host_api` so that both the GSuite staging
/// trait and the host-runtime staging layer use the same type without a
/// cross-crate conversion step.
pub type GsuiteCredentialStageError = brassclaw_host_api::CredentialStageError;
...
enum CapabilityExecutionOutcome {
    Response {
        response: brassclaw_host_api::RuntimeHttpEgressResponse,
...
) -> Result<brassclaw_host_api::RuntimeHttpEgressResponse, GsuiteDispatchError> {
...
fn runtime_request(
    request: &GsuiteDispatchRequest<'_>,
    access_secret: brassclaw_host_api::SecretHandle,
    method: NetworkMethod,
    url: String,
    body: Vec<u8>,
...
fn map_stage_error(
    ...
    required_secret: brassclaw_host_api::SecretHandle,
) -> GsuiteDispatchError {
    match error {
...
#[cfg(test)]
mod tests {
    use brassclaw_host_api::{HostApiError, RuntimeHttpEgressResponse};
    ...
    fn map_credential_error_tests() {
        assert_eq!(
            map_credential_error(GoogleCredentialError::Recovery(
                brassclaw_auth::CredentialRecoveryProjection::setup_required(
                    brassclaw_auth::AuthProviderId::new("google").unwrap(),
                    brassclaw_auth::CredentialRecoveryReason::NoAccount,
                    Vec::new(),
                ),
            ))
            .kind(),
        ...
        assert_eq!(
            map_credential_error(GoogleCredentialError::Recovery(
                brassclaw_auth::CredentialRecoveryProjection::account_selection_required(
                    brassclaw_auth::AuthProviderId::new("google").unwrap(),
                    Vec::new(),
                ),
            ))
        ...
        let backend_error = map_credential_error(GoogleCredentialError::Auth(
            brassclaw_auth::AuthProductError::BackendUnavailable,
        ));
        ...
        let configured_recovery = map_credential_error(GoogleCredentialError::Recovery(
            brassclaw_auth::CredentialRecoveryProjection::configured(
                brassclaw_auth::AuthProviderId::new("google").unwrap(),
                brassclaw_auth::CredentialAccountProjection {
                    id: brassclaw_auth::CredentialAccountId::new(),
                    provider: brassclaw_auth::AuthProviderId::new("google").unwrap(),
                    label: brassclaw_auth::CredentialAccountLabel::new("Google").unwrap(),
                    status: brassclaw_auth::CredentialAccountStatus::Configured,
                    ownership: brassclaw_auth::CredentialOwnership::UserReusable,
    ...
    fn auth_requirement_classifies_each_credential_dispatch_reason() {
        ...
        let configured_proj = brassclaw_auth::CredentialRecoveryProjection::configured(
            brassclaw_auth::AuthProviderId::new("google").unwrap(),
            brassclaw_auth::CredentialAccountProjection {
                id: brassclaw_auth::CredentialAccountId::new(),
                provider: brassclaw_auth::AuthProviderId::new("google").unwrap(),
        ...
        let handle = brassclaw_host_api::SecretHandle::new("google-access-token").unwrap();
