...
//! `crates/brassclaw_run_state/tests/run_state_contract.rs` and
//! `crates/brassclaw_processes/tests/process_store_contract.rs`.
...
async fn filesystem_session_thread_service_isolates_two_tenants_with_same_user_project_ids() {
    ...
    let replay = service_b
        .replay_accepted_inbound_message(brassclaw_threads::ReplayAcceptedInboundMessageRequest {
            scope: scope_b,
            actor_id: "actor-a".into(),
...
async fn filesystem_list_threads_for_scope_is_scope_filtered_and_paginated() {
    use brassclaw_threads::ListThreadsForScopeRequest;
