...
//! this module are alias-relative [`ScopedPath`](brassclaw_host_api::ScopedPath)
//! strings — at every op the [`ScopedFilesystem`] resolves the alias against
//! its caller-supplied [`MountView`](brassclaw_host_api::MountView) and enforces
//! per-grant ACL before backend dispatch. The composition layer wires the
//! alias to a tenant/user-scoped
//! [`VirtualPath`](brassclaw_host_api::VirtualPath), so tenant isolation is
