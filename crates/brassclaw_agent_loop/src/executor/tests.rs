...
async fn reply_admission_rejects_candidate_before_finalizing_and_continues() {
    ...
    let host = MockHost::new(vec![reply_response(), calls_response(), reply_response()])
        .with_batch_outcomes(vec![brassclaw_turns::run_profile::CapabilityBatchOutcome {
...
async fn capability_stage_returns_after_batch_summary() {
    ...
    let step = CapabilityStage
        .process(
            ...
            CapabilityInput {
                state,
                surface: brassclaw_turns::run_profile::LoopCapabilityPort::visible_capabilities(
                    &host,
...
async fn terminate_hint_after_batch_completes_without_extra_model_call() {
    ...
    let completed = host
        ...
        .find_map(|event| match event {
            brassclaw_turns::run_profile::LoopProgressEvent::CapabilityBatchCompleted {
                result_count,
                denied_count,
                gated_count,
                failed_count,
    ...
    let host = MockHost::new(vec![calls_response()])
        .with_batch_outcomes(vec![brassclaw_turns::run_profile::CapabilityBatchOutcome {
            outcomes: vec![CapabilityOutcome::Denied(
                brassclaw_turns::run_profile::CapabilityDenied {
                    reason_kind:
                        brassclaw_turns::run_profile::CapabilityDeniedReasonKind::EmptySurface,
    ...
    let host = MockHost::new(vec![provider_two_calls_response(), reply_response()])
        .with_batch_outcomes(vec![brassclaw_turns::run_profile::CapabilityBatchOutcome {
            outcomes: vec![
                ...
                CapabilityOutcome::Denied(brassclaw_turns::run_profile::CapabilityDenied {
                    reason_kind:
                        brassclaw_turns::run_profile::CapabilityDeniedReasonKind::EmptySurface,
                    safe_summary: "provider call denied".to_string(),
                }),
