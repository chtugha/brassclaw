...
pub(super) enum ModelStep {
    Response(
        ...
        brassclaw_turns::run_profile::LoopModelResponse,
    ),
            ...
            match ctx.host.stream_model(request.clone()).await {
                Ok(response) => {
                    match &response.output {
                        brassclaw_turns::run_profile::ParentLoopOutput::AssistantReply(reply) => {
                            debug!(
                                iteration = state.iteration,
                        ...
                        brassclaw_turns::run_profile::ParentLoopOutput::CapabilityCalls(calls) => {
                            debug!(
                                iteration = state.iteration,
