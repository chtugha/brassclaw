...
mod support {
    ...
    use brassclaw_host_api::{CapabilityId, PackageIdentity};
...
fn t3_host_assignment_via_bundled_registry_grants_effective_trust() {
    ...
    let identity = bundled_identity("brassclaw_core", Some("digest_v1"));
    let decision = policy
        .evaluate(&input(identity, RequestedTrustClass::SystemRequested))
        .unwrap();
...
fn t4_host_assignment_alone_grants_no_capability() {
    ...
    let identity = bundled_identity("brassclaw_core", None);
    let decision = policy
        .evaluate(&input(
            identity.clone(),
            RequestedTrustClass::SystemRequested,
    ...
    let authorizer = FakeAuthorizer::new();
    let capability = cap("brassclaw_core.shutdown");
    assert!(!authorizer.invoke_privileged(&identity, &capability, decision.effective_trust));
}
...
fn t5_effective_system_without_grant_denies_invocation() {
    let identity = bundled_identity("brassclaw_core", None);
    let capability = cap("brassclaw_core.purge_workspace");
    let authorizer = FakeAuthorizer::new();
...
fn t11_digest_drift_forces_grant_reissue() {
    let prev = bundled_identity("brassclaw_core", Some("digest_v1"));
    let curr = bundled_identity("brassclaw_core", Some("digest_v2"));
