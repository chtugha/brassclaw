//! Host-controlled trust-class policy engine for BrassClaw Reborn.
//!
//! `brassclaw_trust` is the bridge between the *requested* trust an untrusted
...
//!
//! See `crates/brassclaw_trust/CONTRACT.md` for the full cross-crate
...
#[cfg(test)]
mod tests {
    //! Lib-level smoke tests that run on bare `cargo test -p brassclaw_trust`.
    ...
    fn fail_closed_policy_returns_default_for_local_manifest() {
        use brassclaw_host_api::{PackageId, PackageIdentity, PackageSource, RequestedTrustClass};
        ...
        assert_eq!(
            ...
            brassclaw_host_api::TrustClass::Sandbox
        );
