# `brassclaw_trust` — cross-crate contract

**Status:** V1 contract
**Crate:** `crates/brassclaw_trust`
**Depends on:** `brassclaw_host_api` (vocabulary)
**Consumed by:** `brassclaw_authorization` (and any future grant-store /
...
`brassclaw_trust` evaluates host trust policy for any manifest-bearing
...
`TrustClass` (in `brassclaw_host_api`) names the four ceilings; the
...
|---|---|---|---|
| `RequestedTrustClass` | `brassclaw_host_api::trust` | manifest / registry entry | freely deserializable; not authority |
| `TrustClass` | `brassclaw_host_api::runtime` | wire vocabulary | `FirstParty`, `System` reject deserialization (`#[serde(skip_deserializing)]`) |
| `EffectiveTrustClass` | `brassclaw_trust::decision` | `HostTrustPolicy::evaluate` | privileged constructors are crate-private; serializable for audit but **not** deserializable |
| `HostTrustAssignment` | `brassclaw_trust::decision` | host-controlled bundle/admin loaders | production seeding token for policy entries; **not** an authorization input and not deserializable |
...
`brassclaw_host_api::capability`) carries the manifest's *declarative
...
Trust class on its own grants nothing. `brassclaw_authorization`
...
| Grant / lease | per-tool `PermissionMode` decision; no formal `CapabilityGrant` | `CapabilityGrant` / `CapabilityLease` (`brassclaw_authorization`, see `capability-access.md`) | Unified — grants for both |
| Approval | per-tool `approval` setting + UnlessAutoApproved gate | gate via `brassclaw_authorization` + obligation handlers | Unified — see `approvals.md` |
...
| Network | per-tool allowlist (HTTP tool's `allowed_hosts`) | network obligations (`network.md`) | Unified through `brassclaw_network` |
...
Those should be added as follow-on slices once `brassclaw_authorization`
