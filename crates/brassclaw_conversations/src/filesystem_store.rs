...
//! [`MountView`](brassclaw_host_api::MountView) and enforces per-grant ACL
//! before backend dispatch. The composition layer wires the alias to a
//! tenant/user-scoped [`VirtualPath`](brassclaw_host_api::VirtualPath), so
...
use brassclaw_host_api::{HostApiError, ResourceScope, ScopedPath};
use brassclaw_turns::{AcceptedMessageRef, IdempotencyKey, SubmitTurnResponse};
...
/// [`MountView`](brassclaw_host_api::MountView), not in this store.
...
impl RebornFilesystemConversationServices {
    ...
    pub async fn pair_external_actor(
        ...
        user_id: brassclaw_host_api::UserId,
...
impl ConversationBindingService for RebornFilesystemConversationServices {
    ...
    async fn resolve_or_create_binding_with_trusted_scope(
        &self,
        request: ResolveConversationRequest,
        trusted_agent_id: Option<brassclaw_host_api::AgentId>,
        trusted_project_id: Option<brassclaw_host_api::ProjectId>,
        trusted_owner_user_id: Option<brassclaw_host_api::UserId>,
    ) -> Result<ConversationBindingResolution, InboundTurnError> {
        self.inner
...
impl SessionThreadService for RebornFilesystemConversationServices {
    ...
    async fn inbound_message_turn_submission(
        &self,
        message_ref: &brassclaw_turns::AcceptedMessageRef,
    ) -> Result<Option<brassclaw_turns::SubmitTurnResponse>, InboundTurnError> {
        self.inner
    ...
    async fn inbound_message_turn_submission_key(
        &self,
        message_ref: &brassclaw_turns::AcceptedMessageRef,
    ) -> Result<brassclaw_turns::IdempotencyKey, InboundTurnError> {
        self.inner
    ...
    async fn mark_inbound_message_turn_submitted(
        &self,
        message_ref: &brassclaw_turns::AcceptedMessageRef,
        response: brassclaw_turns::SubmitTurnResponse,
    ) -> Result<(), InboundTurnError> {
        self.inner
