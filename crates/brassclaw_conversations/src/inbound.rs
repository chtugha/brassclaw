...
use brassclaw_turns::{AdmissionRejectionReason, SubmitTurnRequest, TurnCoordinator, TurnError};
...
fn submit_trusted_trigger_outcome(
    ...
    let run_id = match &response.turn_submission {
        Some(brassclaw_turns::SubmitTurnResponse::Accepted { run_id, .. }) => *run_id,
        None => {
...
mod tests {
    ...
    fn submit_trusted_trigger_outcome_rejects_missing_turn_submission() {
        ...
        assert!(matches!(
            error,
            brassclaw_triggers::TriggerError::Backend { reason }
    ...
    fn classify_trusted_trigger_inbound_error_maps_retryable_backend_cases_to_opaque_backend() {
        ...
        for error in [
            ...
            assert!(matches!(
                classified,
                brassclaw_triggers::TriggerError::InvalidMaterialization { reason }
    ...
    fn trusted_trigger_response(
        ...
        let actor = brassclaw_turns::TurnActor::new(actor_user_id);
