...
async fn first_bind_does_not_trust_unvalidated_requested_scope_hints() {
    ...
    let resolution = services
        .resolve_or_create_binding(brassclaw_conversations::ResolveConversationRequest {
            tenant_id: tenant(),
            adapter_kind: web(),
            adapter_installation_id: default_installation(),
...
struct UntrustedOnlyBindingService {
    ...
    resolve_requests: Arc<Mutex<Vec<brassclaw_conversations::ResolveConversationRequest>>>,
...
impl UntrustedOnlyBindingService {
    ...
    fn resolve_requests(&self) -> Vec<brassclaw_conversations::ResolveConversationRequest> {
...
impl TurnCoordinator for CapacityFailureTurnCoordinator {
    ...
    async fn submit_turn(
        ...
        Err(TurnError::capacity_exceeded(
            brassclaw_turns::TurnCapacityResource::SubmitTurn,
            1,
        ))
    }
    ...
    async fn submit_turn(
        ...
        if submissions.len() == 1 {
            return Err(TurnError::ThreadBusy(ThreadBusy {
                ...
                event_cursor: brassclaw_turns::events::EventCursor(1),
            }));
        }
