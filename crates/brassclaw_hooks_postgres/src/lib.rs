...
//! backends live *out of crate* and depend on `brassclaw_hooks` with the
//! `contract-tests` feature — exactly the way
//! [`brassclaw_reborn_event_store`] is a separate per-domain durable crate
//! rather than living in `brassclaw_events`. Keeping the Postgres
//! dependency surface out of `brassclaw_hooks` keeps the hook framework
//! itself DB-free.
//!
//! [`PredicateStateBackend`]:
//!     brassclaw_hooks::predicate_state::PredicateStateBackend
//! [`brassclaw_reborn_event_store`]: https://docs.rs/brassclaw_reborn_event_store
