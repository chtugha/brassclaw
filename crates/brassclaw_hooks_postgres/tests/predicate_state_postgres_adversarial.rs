...
//!
//! Gated on a reachable Postgres via `BRASSCLAW_HOOKS_POSTGRES_URL` /
...
async fn per_key_sample_cap_fails_closed_under_flood() {
    use brassclaw_hooks::predicate_state::PredicateBackendError;
...
/// `BRASSCLAW_HOOKS_POSTGRES_URL` / `DATABASE_URL`; skipped (passing) otherwise.
...
async fn scope_lru_eviction_serializes_against_victim_writes_no_deadlock() {
    ...
    let pool = build_pool(&url).expect("pool");
    let client = pool.get().await.expect("client");
    let hot_hash = brassclaw_hooks_postgres::test_support::invocation_key_hash_bytes(&hot);
    let rows: i64 = client
        .query_one(
    ...
    let scope = brassclaw_hooks_postgres::test_support::scope_hash_bytes(tenant);
