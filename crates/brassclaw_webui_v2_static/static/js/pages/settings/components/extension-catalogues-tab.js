    ...
    return html`
      <${Card} padding="md">
        <p className="text-sm text-[var(--v2-danger-text)]">
          ${t("extensionCatalogues.failedLoad", { message: query.error?.message ?? String(query.error) })}
        </p>
      <//>
    `;
  }
    ...
    return html`
      <${Card} padding="lg">
        <h3 className="text-lg font-semibold text-[var(--v2-text-strong)]">
          ${t("extensionCatalogues.none")}
        </h3>
        <p className="mt-2 max-w-md text-sm leading-6 text-[var(--v2-text-muted)]">
          ${t("extensionCatalogues.noneDesc")}
        </p>
      <//>
    `;
  }
