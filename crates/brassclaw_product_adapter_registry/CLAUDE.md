# brassclaw_product_adapter_registry guardrails

Owns ProductAdapter host-api projection contracts for BrassClaw Reborn.

- ProductAdapter declarations live in the single Extension Manifest v2 as
  `brassclaw.product_adapter/v1` host-api sections. This crate owns the typed
  ...
  `brassclaw_extensions`. This crate must not introduce a second ProductAdapter
...
- Do **not** depend on legacy `ChannelsConfig`, `ExtensionManager`,
  ...
  boundary test in `crates/brassclaw_architecture` enforces this.
- Credential bindings store `brassclaw_host_api::SecretHandle` only. Raw
...
- Health updates are generic extension installation state and use
  `brassclaw_extensions::ExtensionHealthMessage` for redacted debug output.
