...
//!
//! ```text
//! brassclaw_extensions::ExtensionInstallationStore
//!   manifests/installations for any extension
//!
//! list_enabled_product_adapter_entries(store)
//!   → filter enabled installations whose manifest carries brassclaw.product_adapter/v1
...
pub use brassclaw_extensions::ManifestHash;

// ---------------------------------------------------------------------------
// Constants
// ---------------------------------------------------------------------------

pub const PRODUCT_ADAPTER_HOST_API_ID: &str = "brassclaw.product_adapter/v1";
...
/// `brassclaw.product_adapter/v1` host-api section, then pairs each with its
...
fn project_product_adapter_sections(
    ...
    // `brassclaw_extensions` validates host-api sections from its internal
    ...
    // API in `brassclaw_extensions` instead of caching private parser state here.
