# Agent Map — brassclaw_first_party_extension_ports

## Start Here

- Read `Cargo.toml` for actual dependencies and feature shape.
- Use these neighboring contracts before changing behavior:
  - `crates/brassclaw_first_party_extensions/AGENTS.md`
  - `crates/brassclaw_loop_support/AGENTS.md`
...
- Concrete tool behavior that can live below host runtime in `brassclaw_first_party_extensions`.
...
- Fast local check: `cargo test -p brassclaw_first_party_extension_ports`
- Boundary check after dependency/API changes: `cargo test -p brassclaw_architecture reborn_crate_dependency_boundaries_hold`
- Composition check when exposed ports change: `cargo test -p brassclaw_reborn_composition local_dev_runtime`
