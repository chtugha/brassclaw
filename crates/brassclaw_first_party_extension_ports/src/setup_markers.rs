...
mod tests {
    ...
    use brassclaw_host_api::{AgentId, MountView, ProjectId, TenantId};
    use brassclaw_turns::{
    ...
    async fn run_context() -> LoopRunContext {
        ...
        LoopRunContext::new(
            TurnScope::new(
                ...
                brassclaw_host_api::ThreadId::new("thread-a").expect("valid thread"),
            ),
            TurnId::new(),
            TurnRunId::new(),
            resolved,
        ...
        .with_actor(TurnActor::new(
            brassclaw_host_api::UserId::new("user-a").expect("valid user"),
        ))
    }
