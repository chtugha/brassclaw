...
//! on top of this; `brassclaw_llm` and other workspace crates use this module
...
///
/// `brassclaw` keeps a separate `INJECTED_VARS` overlay populated from the
/// encrypted secrets store (so API keys can be read without `set_var`).
/// `brassclaw_llm` does not have direct access to that overlay, so the main
...
mod tests {
    ...
    fn runtime_override_round_trip() {
        let _guard = lock_env();
        set_runtime_env("BRASSCLAW_TEST_RUNTIME_OVERRIDE", "1");
        assert_eq!(
            env_or_override("BRASSCLAW_TEST_RUNTIME_OVERRIDE"),
            Some("1".to_string())
        );
    }
    ...
    fn empty_runtime_override_treated_as_unset() {
        let _guard = lock_env();
        set_runtime_env("BRASSCLAW_TEST_EMPTY", "");
        assert_eq!(env_or_override("BRASSCLAW_TEST_EMPTY"), None);
    }

    #[test]
    fn runtime_override_can_be_removed() {
        let _guard = lock_env();
        set_runtime_env("BRASSCLAW_TEST_REMOVE", "1");
        remove_runtime_env("BRASSCLAW_TEST_REMOVE");
        assert_eq!(env_or_override("BRASSCLAW_TEST_REMOVE"), None);
    }
}
