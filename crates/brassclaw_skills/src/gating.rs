...
mod tests {
    ...
    fn test_missing_binary_fails() {
        let req = GatingRequirements {
            bins: vec!["__brassclaw_nonexistent_binary_xyz__".to_string()],
            ..Default::default()
        };
        let result = check_requirements_sync(&req);
        assert!(!result.passed);
    ...
    fn test_missing_env_var_fails() {
        let req = GatingRequirements {
            env: vec!["__BRASSCLAW_TEST_NONEXISTENT_VAR__".to_string()],
            ..Default::default()
        };
        let result = check_requirements_sync(&req);
        assert!(!result.passed);
    ...
    fn test_missing_config_fails() {
        let req = GatingRequirements {
            config: vec!["/nonexistent/path/brassclaw_test.conf".to_string()],
            ..Default::default()
        };
        let result = check_requirements_sync(&req);
        assert!(!result.passed);
