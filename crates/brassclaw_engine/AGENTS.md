...
- Read `Cargo.toml` for dependencies (internally only `brassclaw_common` and `brassclaw_skills`) and note the ReDoS guardrail at the top of `src/lib.rs`: do not add `fancy-regex` without redesigning `executor/orchestrator.rs::__regex_match__`.
