...
use brassclaw_loop_support::HostManagedLoopCompactionPort;
...
impl CompactionFixture {
    ...
    async fn create_replacement_summary(
        ...
        end_sequence: u64,
        content: &str,
    ) -> brassclaw_threads::SummaryArtifact {
        self.threads
            .create_summary_artifact(brassclaw_threads::CreateSummaryArtifactRequest {
                scope: self.scope.clone(),
                thread_id: self.thread_id.clone(),
                start_sequence,
                end_sequence,
