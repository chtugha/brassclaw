...
fn host_capability_port_composition_rejects_external_raw_construction() {
    ...
    assert!(
        offenders.is_empty(),
        "HostRuntimeLoopCapabilityPort must be constructed only inside brassclaw_loop_support; offenders: {offenders:#?}"
    );
}
...
async fn host_capability_port_composition_factory_builds_loop_capability_port() {
    ...
    assert_eq!(
        ...
        "brassclaw.loop.capability_info"
    );
}
...
async fn factory_stages_provider_tool_call_arguments_without_custom_resolver_override() {
    ...
    assert!(
        matches!(
            outcome,
            brassclaw_turns::run_profile::CapabilityOutcome::Completed(_)
        ),
...
impl LoopCapabilityInputResolver for UnusedInputResolver {
    async fn resolve_capability_input(
        ...
        _input_ref: &brassclaw_turns::run_profile::CapabilityInputRef,
...
fn dispatch_trust_decision() -> TrustDecision {
    TrustDecision {
        ...
        authority_ceiling: AuthorityCeiling {
            allowed_effects: vec![brassclaw_host_api::EffectKind::DispatchCapability],
...
fn should_skip(path: &Path) -> bool {
    path.components().any(|component| {
        ...
        name == "brassclaw_loop_support" || name == "tests" || name == "target"
    })
}
