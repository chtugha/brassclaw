...
use brassclaw_process_sandbox::{SandboxProcessPlan, ValidatedSandboxProcessPlan};
use brassclaw_turns::{
...
struct PreparedProviderToolCall {
    surface_version: brassclaw_turns::run_profile::CapabilitySurfaceVersion,
    capability_id: CapabilityId,
...
impl HostRuntimeLoopCapabilityPort {
    ...
    fn snapshot_for(
        &self,
        version: &brassclaw_turns::run_profile::CapabilitySurfaceVersion,
...
fn is_process_sandbox_capability(capability_id: &CapabilityId) -> bool {
    capability_id.as_str() == brassclaw_process_sandbox::PROCESS_SANDBOX_CAPABILITY_ID
}
...
fn invocation_context_from_visible(
    ...
    trust: brassclaw_host_api::TrustClass,
...
fn loop_surface_version(
    version: &str,
) -> Result<brassclaw_turns::run_profile::CapabilitySurfaceVersion, AgentLoopHostError> {
    brassclaw_turns::run_profile::CapabilitySurfaceVersion::new(version).map_err(|_| {
        AgentLoopHostError::new(
            AgentLoopHostErrorKind::Internal,
...
mod tests {
    ...
    fn provider_argument_normalization_coerces_stringified_array_into_oneof_variant() {
        ...
        let normalized = normalize_provider_arguments(
            &serde_json::json!({
                "headers": "[{\"name\":\"User-Agent\",\"value\":\"BrassClaw/1.0\"}]"
            }),
            &schema,
            "provider arguments",
        )
    ...
    fn provider_argument_normalization_coerces_stringified_object_into_oneof_variant() {
        ...
        let normalized = normalize_provider_arguments(
            &serde_json::json!({
                "headers": "{\"User-Agent\":\"BrassClaw/1.0\"}"
            }),
            &schema,
            "provider arguments",
        )
    ...
    fn provider_argument_preparation_validates_required_fields_before_dispatch() {
        ...
        let error = prepare_provider_arguments(
            &serde_json::json!({ "owner": "nearai", "repo": "brassclaw" }),
            &schema,
            "provider arguments",
        )
        ...
        assert!(
            brassclaw_turns::run_profile::LoopSafeSummary::new(error.safe_summary.clone()).is_ok()
        );
    }
    ...
    fn provider_argument_preparation_validates_composed_object_schema_after_normalization() {
        ...
        let normalized = prepare_provider_arguments(
            &serde_json::json!({ "query": "repo:chtugha/brassclaw", "page": "2" }),
            &schema,
            "provider arguments",
        )
    ...
    async fn runtime_capability_invocation_emits_dispatch_lifecycle_milestones() {
        ...
        assert!(matches!(
            &milestones[0].kind,
            brassclaw_turns::run_profile::LoopHostMilestoneKind::CapabilityInvoked {
        ...
        assert!(matches!(
            &milestones[1].kind,
            brassclaw_turns::run_profile::LoopHostMilestoneKind::CapabilityCompleted {
                capability_id: actual,
                provider,
    ...
    async fn runtime_capability_failed_and_unknown_outcomes_emit_failure_milestones() {
        ...
        for (outcome, expected_kind) in cases {
            ...
            assert!(matches!(
                &milestones[1].kind,
                brassclaw_turns::run_profile::LoopHostMilestoneKind::CapabilityFailed {
                    capability_id: actual,
                    provider: Some(provider),
    ...
    async fn runtime_capability_with_reserved_synthetic_id_is_rejected_from_surface() {
        ...
        let provider_id = ExtensionId::new("brassclaw.loop").expect("valid provider id");
    ...
    async fn process_sandbox_capability_invocation_uses_spawn_with_validated_plan() {
        let capability_id =
            CapabilityId::new(brassclaw_process_sandbox::PROCESS_SANDBOX_CAPABILITY_ID)
    ...
    async fn provider_runtime_tool_call_schema_failure_is_model_visible() {
        ...
        let Some(brassclaw_turns::run_profile::CapabilityFailureDetail::InvalidInput { issues }) =
            detail
        else {
        ...
        assert_eq!(
            issues[0].code,
            brassclaw_turns::run_profile::CapabilityInputIssueCode::MissingRequired
        );
    ...
    async fn provider_runtime_tool_call_schema_failure_preserves_type_mismatch_detail() {
        ...
        assert!(
            issues.iter().any(|issue| {
                issue.path == "message"
                    && issue.code
                        == brassclaw_turns::run_profile::CapabilityInputIssueCode::TypeMismatch
        ...
        assert!(
            issues.iter().any(|issue| {
                issue.path == "unexpected"
                    && issue.code
                        == brassclaw_turns::run_profile::CapabilityInputIssueCode::UnexpectedField
            }),
    ...
    impl HostRuntime for RecordingHostRuntime {
        ...
        async fn spawn_capability(
            ...
            Ok(RuntimeCapabilityOutcome::SpawnedProcess(
                brassclaw_host_runtime::RuntimeProcessHandle {
                    process_id: brassclaw_host_api::ProcessId::new(),
                    capability_id: request.capability_id,
                },
            ))
        }
    ...
    struct FailOnceTerminalMilestoneSink {
        ...
        milestones: Mutex<Vec<brassclaw_turns::run_profile::LoopHostMilestone>>,
    }

    impl FailOnceTerminalMilestoneSink {
        fn milestones(&self) -> Vec<brassclaw_turns::run_profile::LoopHostMilestone> {
    ...
    impl LoopHostMilestoneSink for FailOnceTerminalMilestoneSink {
        async fn publish_loop_milestone(
            &self,
            milestone: brassclaw_turns::run_profile::LoopHostMilestone,
        ) -> Result<(), AgentLoopHostError> {
    ...
    fn execution_context(thread: &str) -> ExecutionContext {
        let thread_id = brassclaw_host_api::ThreadId::new(thread).expect("valid thread id");
