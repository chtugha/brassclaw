...
fn system_inference_ref(
    task_id: uuid::Uuid,
    label: &'static str,
) -> Result<brassclaw_turns::LoopMessageRef, SystemInferenceError> {
    brassclaw_turns::LoopMessageRef::new(format!("msg:system-inference.{label}.{task_id}")).map_err(
        |_| SystemInferenceError::Failed {
...
mod tests {
    ...
    impl LoopModelPolicyGuard for DenySystemInferencePolicyGuard {
        async fn check_model_work_policy(
            ...
            assert!(matches!(
                request.kind,
                brassclaw_turns::run_profile::ModelWorkKind::SystemInference { .. }
            ));
