...
use brassclaw_turns::run_profile::LoopModelCapabilityView;
...
///
/// `LoopModelCapabilityView` is defined in `brassclaw_turns::run_profile` as the
/// shared prompt contract, but the narrowing policy and dropped-capability
/// logging live in `brassclaw_loop_support` because this crate owns the prompt
