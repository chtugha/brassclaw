...
//! mount alias. The [`ScopedFilesystem`](brassclaw_filesystem::ScopedFilesystem)
...
async fn put_bytes_compat<F>(
    filesystem: &ScopedFilesystem<F>,
    resource_scope: &brassclaw_host_api::ResourceScope,
    path: &ScopedPath,
    body: Vec<u8>,
    cas: CasExpectation,
) -> Result<(), TurnError>
...
mod tests {
    ...
    use brassclaw_filesystem::{DirEntry, FileStat, FileType, RecordVersion, VersionedEntry};
