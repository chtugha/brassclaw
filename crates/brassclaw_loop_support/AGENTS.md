# Agent Map — brassclaw_loop_support
...
- Use these neighboring contracts before changing behavior:
  ...
  - `crates/brassclaw_capabilities/AGENTS.md`
  - `crates/brassclaw_skills/AGENTS.md`
  - `crates/brassclaw_reborn/CLAUDE.md`

## What This Crate Owns

- Loop host support services for `AgentLoopHost` / `brassclaw_turns` loop ports.
...
- Fast local check: `cargo test -p brassclaw_loop_support`
- Boundary check after dependency/API changes: `cargo test -p brassclaw_architecture`
- Run `cargo test -p brassclaw_turns` and `cargo test -p brassclaw_reborn` when host-port contracts change.
