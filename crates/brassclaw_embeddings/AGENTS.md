...
- `src/app.rs` (and `src/cli/mod.rs`) call `brassclaw_embeddings::create_provider(&cfg, ProviderDeps { session, bedrock_setup })` and attach the result to `Workspace` via `with_embeddings_cached`.
