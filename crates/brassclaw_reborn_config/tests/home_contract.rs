...
use brassclaw_reborn_config::{REBORN_HOME_ENV, RebornConfigError, RebornHome, RebornHomeSource};
...
fn default_reborn_home_is_scoped_under_user_home() {
    let temp = tempfile::tempdir().expect("tempdir");
    let expected = temp.path().join(".brassclaw").join("reborn");
    ...
    assert!(!temp.path().join(".brassclaw").exists());
}
...
fn rejects_reborn_home_equal_to_home_v1_state_root() {
    let temp = tempfile::tempdir().expect("tempdir");
    let path = temp.path().join(".brassclaw");
...
fn rejects_reborn_home_symlink_to_home_v1_state_root() {
    ...
    let temp = tempfile::tempdir().expect("tempdir");
    let v1_root = temp.path().join(".brassclaw");
