...
//!
//! Operator-facing file at `$BRASSCLAW_REBORN_HOME/config.toml`. Read once
//! at process start by `brassclaw-reborn run`. Provides the *selection*
//! layer of the three-layer config model:
//!
//! - **Catalog**: `providers.json` (this crate exposes the path; the
//!   composition root loads the file via `brassclaw_llm::ProviderRegistry`).
...
//!
//! Layering note: this crate must stay free of BrassClaw workspace
...
//! typed counterparts live in `brassclaw_host_api` (TenantId, AgentId,
//! UserId, ProjectId, DeploymentMode, RuntimeProfile, ApprovalPolicy) or
//! `brassclaw_reborn_composition` (RebornDriverChoice, RebornHarnessId).
...
/// `brassclaw_reborn_composition::RebornRuntimeApiVersion::V1`. A future
/// major bump fails parse closed; minor bumps are accepted.
pub const REBORN_CONFIG_API_VERSION: &str = "brassclaw.runtime/v1";
...
pub struct RebornConfigFile {
    /// API version. When set, must be parseable as `brassclaw.runtime/vN.M`
    ...
    /// `brassclaw_reborn_webui_ingress` when the standalone CLI's
    ...
    /// `brassclaw-reborn serve` only when the binary is built with the
...
pub struct PolicySection {
    ...
    /// `brassclaw_host_api::runtime_policy::DeploymentMode`.
...
/// Stringly typed by design — the `brassclaw_reborn_config` crate stays
...
pub struct WebuiSection {
    ...
    /// at composition time (`brassclaw-reborn serve` accepts `0` only
    ...
    /// `BRASSCLAW_REBORN_WEBUI_TOKEN`. The token VALUE never appears in
    ...
    /// `BRASSCLAW_REBORN_WEBUI_USER_ID`. Stringly typed; composition
...
pub struct SlackSection {
    ...
    /// is not mounted by `brassclaw-reborn serve`.
...
/// here. Env vars (`BRASSCLAW_TRIGGER_POLLER_*`) override this section.
#[derive(Debug, Clone, Default, Deserialize)]
#[serde(deny_unknown_fields)]
...
fn validate_api_version(found: &str, path: &Path) -> Result<(), RebornConfigFileError> {
    // Expected shape: `brassclaw.runtime/vMAJOR.MINOR` (minor optional).
    let Some(rest) = found.strip_prefix("brassclaw.runtime/v") else {
        return Err(RebornConfigFileError::InvalidApiVersion {
            path: path.display().to_string(),
            found: found.to_string(),
            reason: "expected prefix `brassclaw.runtime/v`".to_string(),
        });
    };
...
"#;
        ...
        assert_eq!(
            ...
            Some("BRASSCLAW_REBORN_SLACK_SIGNING_SECRET")
        );
    }
...
"#;
    ...
    fn rejects_future_major_api_version_fail_closed() {
        let toml = r#"
api_version = "brassclaw.runtime/v9"
"#;
        let err =
    ...
    fn accepts_v1_minor_bumps_forward_compat() {
        for version in ["brassclaw.runtime/v1.0", "brassclaw.runtime/v1.7"] {
    ...
    fn rejects_malformed_api_version() {
        let toml = r#"
api_version = "brassclaw.runtime/notaversion"
"#;
        let err = RebornConfigFile::parse_text(toml, &attributed())
    ...
    fn rejects_malformed_api_version_minor() {
        let toml = r#"
api_version = "brassclaw.runtime/v1.not-a-number"
"#;
        let err = RebornConfigFile::parse_text(toml, &attributed())
