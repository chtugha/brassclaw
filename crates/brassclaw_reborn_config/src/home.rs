...
pub const REBORN_HOME_ENV: &str = "BRASSCLAW_REBORN_HOME";

const V1_BASE_DIR_ENV: &str = "BRASSCLAW_BASE_DIR";
...
fn default_home_from_candidate(
    ...
    Ok(path.join(".brassclaw").join("reborn"))
}
...
fn default_v1_state_root_from_home(raw_home: &OsString) -> Option<PathBuf> {
    validated_absolute_candidate(raw_home).map(|path| path.join(".brassclaw"))
}
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::V1StateRoot { name, .. } => {
                write!(
                    formatter,
                    "{name} must not point at the v1 BrassClaw state root"
                )
            }
