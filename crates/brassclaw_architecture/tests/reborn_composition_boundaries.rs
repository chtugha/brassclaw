...
const COMPOSITION_CRATE: &str = "brassclaw_reborn_composition";
...
fn composition_public_api_is_facade_shaped() {
    let lib = std::fs::read_to_string(
        workspace_root().join("crates/brassclaw_reborn_composition/src/lib.rs"),
    )
    .expect("composition lib readable");
    let input = std::fs::read_to_string(
        workspace_root().join("crates/brassclaw_reborn_composition/src/input.rs"),
    )
    .expect("composition input readable");
    let factory = std::fs::read_to_string(
        workspace_root().join("crates/brassclaw_reborn_composition/src/factory.rs"),
    )
...
/// `brassclaw_reborn_composition` (the crate that owns the untrusted projection
...
fn composition_crate_installs_installed_tier_only_through_registrar() {
    let crate_src = workspace_root().join("crates/brassclaw_reborn_composition/src");
...
fn package_dependencies(package: &Value) -> Option<(String, Vec<String>)> {
    ...
    let dependencies = package["dependencies"]
        ...
        .filter(|name| name.starts_with("brassclaw_"))
