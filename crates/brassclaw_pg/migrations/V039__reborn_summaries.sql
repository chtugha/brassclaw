-- V039__reborn_summaries.sql
-- Summaries component table for BrassClaw Reborn (Phase 5 Step 5.3, class 15).
--
-- Former DocType::Summary documents migrated to a first-class component table.
-- Summaries capture what a thread accomplished (background context).
-- consumer_tags default: {02:orchestrator, 03:llm} + 05:validator until validated.
--
-- Spec references: §3.3, §3.7 (class 15), §3.9, §3.12, §3.13/§3.14 (SCH-02),
-- Phase 5 Step 5.3.

CREATE SEQUENCE IF NOT EXISTS reborn_summaries_prompt_uid_seq;

CREATE TABLE IF NOT EXISTS reborn_summaries (
    ...
    -- class_code = 15 (Summary)
    class_code              SMALLINT    NOT NULL DEFAULT 15
        CHECK (class_code = 15),
    prompt_uid              BIGINT      NOT NULL DEFAULT nextval('reborn_summaries_prompt_uid_seq'),
    ...
    CONSTRAINT reborn_summaries_pk PRIMARY KEY (id),
    CONSTRAINT reborn_summaries_scope_name_unique
        UNIQUE (tenant_id, user_id, agent_id, project_id, name)
);

CREATE INDEX IF NOT EXISTS reborn_summaries_scope_idx
    ON reborn_summaries (tenant_id, user_id, agent_id, project_id);
CREATE INDEX IF NOT EXISTS reborn_summaries_scope_status_idx
    ON reborn_summaries (tenant_id, user_id, agent_id, project_id, validation_status);
CREATE INDEX IF NOT EXISTS reborn_summaries_scope_uid_idx
    ON reborn_summaries (tenant_id, user_id, agent_id, project_id, prompt_uid);
CREATE INDEX IF NOT EXISTS reborn_summaries_consumer_tags_gin_idx
    ON reborn_summaries USING GIN (consumer_tags);
...
CREATE TRIGGER reborn_summaries_updated_at
    BEFORE UPDATE ON reborn_summaries
