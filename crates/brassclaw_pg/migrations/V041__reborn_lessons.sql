-- V041__reborn_lessons.sql
-- Lessons component table for BrassClaw Reborn (Phase 5 Step 5.3, class 18).
--
-- Former DocType::Lesson documents migrated to a first-class component table.
-- Lessons encode durable learnings from experience (prevent repeating mistakes).
-- consumer_tags default: {02:orchestrator, 03:llm} + 05:validator until validated.
--
-- Spec references: §3.3, §3.7 (class 18), §3.9, §3.12, §3.13/§3.14 (SCH-02),
-- Phase 5 Step 5.3.

CREATE SEQUENCE IF NOT EXISTS reborn_lessons_prompt_uid_seq;

CREATE TABLE IF NOT EXISTS reborn_lessons (
    ...
    -- class_code = 18 (Lesson)
    class_code              SMALLINT    NOT NULL DEFAULT 18
        CHECK (class_code = 18),
    prompt_uid              BIGINT      NOT NULL DEFAULT nextval('reborn_lessons_prompt_uid_seq'),
    ...
    CONSTRAINT reborn_lessons_pk PRIMARY KEY (id),
    CONSTRAINT reborn_lessons_scope_name_unique
        UNIQUE (tenant_id, user_id, agent_id, project_id, name)
);

CREATE INDEX IF NOT EXISTS reborn_lessons_scope_idx
    ON reborn_lessons (tenant_id, user_id, agent_id, project_id);
CREATE INDEX IF NOT EXISTS reborn_lessons_scope_status_idx
    ON reborn_lessons (tenant_id, user_id, agent_id, project_id, validation_status);
CREATE INDEX IF NOT EXISTS reborn_lessons_scope_uid_idx
    ON reborn_lessons (tenant_id, user_id, agent_id, project_id, prompt_uid);
CREATE INDEX IF NOT EXISTS reborn_lessons_consumer_tags_gin_idx
    ON reborn_lessons USING GIN (consumer_tags);
...
CREATE TRIGGER reborn_lessons_updated_at
    BEFORE UPDATE ON reborn_lessons
