...
//! HTTP gateway.
//!
//! `brassclaw_reborn_composition::webui_v2_app` returns a fully composed
...
pub async fn serve_webui_v2(opts: RebornWebuiServeOptions) -> Result<(), RebornWebuiServeError> {
    ...
    tracing::info!(
        target = "brassclaw::reborn::webui_ingress",
        %bound,
...
/// variable. Intended for the standalone `brassclaw-reborn` deployment
