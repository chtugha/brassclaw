# brassclaw_reborn_webui_ingress guardrails

Host-owned counterpart to `brassclaw_reborn_composition::webui_v2_app`.
...
behind the product/API seam in `brassclaw_reborn_composition`.
...
- **Session transport** is one-time login ticket in the callback
  ...
  `brassclaw_reborn_composition/CLAUDE.md` → "Session transport
...
```bash
cargo test -p brassclaw_reborn_webui_ingress --all-features
cargo clippy -p brassclaw_reborn_webui_ingress --all-features --tests -- -D warnings
```
