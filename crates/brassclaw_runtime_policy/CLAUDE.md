# brassclaw_runtime_policy guardrails

- Own the runtime policy resolver: `(DeploymentMode, RuntimeProfile, OrgPolicyConstraints) → EffectiveRuntimePolicy`.
- Depend only on `brassclaw_host_api` for vocabulary and `serde`/`thiserror` for plumbing. Do not pull in runtime crates, host runtime, capability host, secrets, network, or product workflow crates.
...
- Do not re-implement authorization/approvals/grants. The resolver picks backend kinds and policy modes; per-invocation authorization runs on top via `brassclaw_authorization` / `CapabilityHost`.
- No I/O in the resolver. It is a pure function over types from `brassclaw_host_api::runtime_policy`.
