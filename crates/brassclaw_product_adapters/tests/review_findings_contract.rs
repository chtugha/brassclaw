...
use brassclaw_turns::{AcceptedMessageRef, ReplyTargetBindingRef, TurnRunId};
...
fn command_payload_is_bounded_and_serde_validated() {
    ...
    assert!(
        serde_json::from_value::<brassclaw_product_adapters::ApprovalResolutionPayload>(
            oversized_gate
        )
        .is_err()
    );
    ...
    assert!(
        serde_json::from_value::<brassclaw_product_adapters::AuthResolutionPayload>(oversized_auth)
            .is_err()
    );
...
fn ack_and_rejection_types_are_unambiguous_and_typed() {
    ...
    assert_eq!(
        ...
        brassclaw_product_adapters::InboundRetryDisposition::DoNotRetry
    );
    ...
    assert!(brassclaw_product_adapters::fakes::ensure_durable_outcome(
        &ProductInboundAck::Accepted {
            accepted_message_ref: AcceptedMessageRef::new("msg:2").expect("valid"),
            submitted_run_id: TurnRunId::new(),
        }
    ));
    assert!(!brassclaw_product_adapters::fakes::ensure_noop_outcome(
        &ProductInboundAck::Accepted {
...
fn transient_egress_errors_are_retryable_adapter_errors() {
    let timeout: brassclaw_product_adapters::ProductAdapterError =
