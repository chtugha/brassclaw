...
use brassclaw_turns::{AcceptedMessageRef, ReplyTargetBindingRef, TurnActor, TurnRunId, TurnScope};
...
fn cargo_manifest_does_not_pull_in_forbidden_lower_layers() {
    ...
    let package = packages
        .iter()
        .find(|package| package["name"] == "brassclaw_product_adapters")
    ...
    for dep in deps {
        ...
        assert!(
            ...
            "brassclaw_product_adapters must not depend on forbidden lower layer {name}/{package_name}"
        );
    }
}
...
fn source_does_not_import_runner_transition_apis() {
    ...
    for entry in walk_rs_files(&src_root) {
        ...
        if compact.contains("brassclaw_turns::runner")
            || compact.contains("brassclaw_turns::{runner")
            || (compact.contains("brassclaw_turns::{") && compact.contains("runner::"))
        {
...
async fn workflow_fake_records_submit_read_and_subscribe_doors_separately() {
    let workflow = FakeProductWorkflow::new();
    let actor = TurnActor::new(brassclaw_host_api::UserId::new("user:alice").expect("valid"));
    let scope = TurnScope::new(
        brassclaw_host_api::TenantId::new("tenant:alpha").expect("valid"),
        None,
        None,
        brassclaw_host_api::ThreadId::new("thread:alpha").expect("valid"),
    );
...
async fn projection_stream_drains_queued_envelopes() {
    ...
    let envelope = ProductOutboundEnvelope::new(
        ...
        ProductOutboundPayload::FinalReply(brassclaw_product_adapters::FinalReplyView {
...
fn sample_subscription(after_cursor: Option<ProjectionCursor>) -> ProjectionSubscriptionRequest {
    ProjectionSubscriptionRequest {
        actor: brassclaw_turns::TurnActor::new(
            brassclaw_host_api::UserId::new("alice").expect("valid"),
        ),
        scope: brassclaw_turns::TurnScope::new(
            brassclaw_host_api::TenantId::new("tenant-a").expect("valid"),
            None,
            None,
            brassclaw_host_api::ThreadId::new("thread-1").expect("valid"),
        ),
        after_cursor,
    }
}
...
fn payload_alias_matches_reexport() {
    fn _coerce(
        p: brassclaw_product_adapters::inbound::ProductInboundPayload,
