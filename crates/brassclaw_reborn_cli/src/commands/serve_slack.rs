...
#[cfg(feature = "slack-v2-host-beta")]
const DEFAULT_SLACK_SIGNING_SECRET_ENV_VAR: &str = "BRASSCLAW_REBORN_SLACK_SIGNING_SECRET";
#[cfg(feature = "slack-v2-host-beta")]
const DEFAULT_SLACK_BOT_TOKEN_ENV_VAR: &str = "BRASSCLAW_REBORN_SLACK_BOT_TOKEN";

#[cfg(feature = "slack-v2-host-beta")]
pub(crate) fn resolve_slack_config_for_serve(
    section: Option<&brassclaw_reborn_config::SlackSection>,
    tenant_id: &brassclaw_reborn_composition::host_api::TenantId,
    default_agent_id: &brassclaw_reborn_composition::host_api::AgentId,
    default_project_id: Option<&brassclaw_reborn_composition::host_api::ProjectId>,
    default_user_id: &brassclaw_reborn_composition::host_api::UserId,
    config_path: &Path,
...
pub(crate) fn resolve_slack_config_for_serve(
    section: Option<&brassclaw_reborn_config::SlackSection>,
    _tenant_id: &brassclaw_reborn_composition::host_api::TenantId,
    _default_agent_id: &brassclaw_reborn_composition::host_api::AgentId,
    _default_project_id: Option<&brassclaw_reborn_composition::host_api::ProjectId>,
    _default_user_id: &brassclaw_reborn_composition::host_api::UserId,
...
fn optional_slack_user_id_config_value(
    field: &str,
    value: &Option<String>,
) -> anyhow::Result<Option<brassclaw_reborn_composition::host_api::UserId>> {
    optional_slack_config_value(field, value)?
        .map(|raw| {
            brassclaw_reborn_composition::host_api::UserId::new(&raw)
...
fn parse_slack_channel_route_config(
    (index, route): (usize, &brassclaw_reborn_config::SlackChannelRouteSection),
...
pub(crate) fn reject_enabled_slack_without_feature(
    ...
    if section.and_then(|section| section.enabled).unwrap_or(false) {
        anyhow::bail!(
            "[slack].enabled = true requires an brassclaw-reborn binary built with \
...
mod tests {
    ...
    fn slack_host_beta_config_reads_env_secrets_and_defaults_user() {
        ...
        let _signing = EnvGuard::set(
            "BRASSCLAW_TEST_SLACK_SIGNING_SECRET_DEFAULT_USER",
            "signing-secret",
        );
        let _bot = EnvGuard::set("BRASSCLAW_TEST_SLACK_BOT_TOKEN_DEFAULT_USER", "xoxb-token");
        let section = brassclaw_reborn_config::SlackSection {
            enabled: Some(true),
            installation_id: Some("install-alpha".to_string()),
            team_id: Some("T123".to_string()),
    ...
    fn slack_host_beta_config_does_not_require_static_slack_user() {
        ...
        let section = brassclaw_reborn_config::SlackSection {
            ...
            signing_secret_env: Some(
                "BRASSCLAW_TEST_SLACK_SIGNING_SECRET_NO_STATIC_USER".to_string(),
            ),
            bot_token_env: Some("BRASSCLAW_TEST_SLACK_BOT_TOKEN_NO_STATIC_USER".to_string()),
            ..Default::default()
        };
    ...
    fn slack_host_beta_config_rejects_empty_env_secret_value() {
        let _lock = env_lock();
        let _signing = EnvGuard::set("BRASSCLAW_TEST_SLACK_EMPTY_SIGNING_SECRET", "");
        let _bot = EnvGuard::set("BRASSCLAW_TEST_SLACK_EMPTY_BOT_TOKEN", "xoxb-token");
        let section = brassclaw_reborn_config::SlackSection {
            enabled: Some(true),
            installation_id: Some("install-alpha".to_string()),
            team_id: Some("T123".to_string()),
            api_app_id: Some("A123".to_string()),
            slack_user_id: Some("U123".to_string()),
            signing_secret_env: Some("BRASSCLAW_TEST_SLACK_EMPTY_SIGNING_SECRET".to_string()),
            bot_token_env: Some("BRASSCLAW_TEST_SLACK_EMPTY_BOT_TOKEN".to_string()),
            ..Default::default()
        };
        ...
        assert!(
            error
                .to_string()
                .contains("BRASSCLAW_TEST_SLACK_EMPTY_SIGNING_SECRET must not be empty"),
            "message: {error}"
        );
        drop(_signing);
        drop(_bot);
    ...
    fn slack_host_beta_config_accepts_matching_explicit_user_mapping() {
        let _lock = env_lock();
        let _signing = EnvGuard::set(
            "BRASSCLAW_TEST_SLACK_SIGNING_SECRET_MAPPED_USER",
            "signing-secret",
        );
        let _bot = EnvGuard::set("BRASSCLAW_TEST_SLACK_BOT_TOKEN_MAPPED_USER", "xoxb-token");
        let section = brassclaw_reborn_config::SlackSection {
            enabled: Some(true),
            installation_id: Some("install-alpha".to_string()),
            team_id: Some("T123".to_string()),
            ...
            signing_secret_env: Some("BRASSCLAW_TEST_SLACK_SIGNING_SECRET_MAPPED_USER".to_string()),
            bot_token_env: Some("BRASSCLAW_TEST_SLACK_BOT_TOKEN_MAPPED_USER".to_string()),
        };
    ...
    fn slack_host_beta_config_accepts_distinct_shared_subject_user() {
        ...
        let section = brassclaw_reborn_config::SlackSection {
            ...
            channel_routes: Vec::new(),
            signing_secret_env: Some(
                "BRASSCLAW_TEST_SLACK_SIGNING_SECRET_DIVERGENT_USER".to_string(),
            ),
            bot_token_env: Some("BRASSCLAW_TEST_SLACK_BOT_TOKEN_DIVERGENT_USER".to_string()),
        };
    ...
    fn slack_host_beta_config_accepts_channel_routes() {
        ...
        let section = brassclaw_reborn_config::SlackSection {
            ...
            user_id: Some("web-user".to_string()),
            shared_subject_user_id: None,
            channel_routes: vec![brassclaw_reborn_config::SlackChannelRouteSection {
                channel_id: Some("CENG".to_string()),
                subject_user_id: Some("eng-team-agent".to_string()),
            }],
            signing_secret_env: Some(
                "BRASSCLAW_TEST_SLACK_SIGNING_SECRET_CHANNEL_ROUTES".to_string(),
            ),
            bot_token_env: Some("BRASSCLAW_TEST_SLACK_BOT_TOKEN_CHANNEL_ROUTES".to_string()),
        };
    ...
    fn slack_host_beta_config_rejects_invalid_user_id_mapping() {
        let _lock = env_lock();
        let _signing = EnvGuard::set(
            "BRASSCLAW_TEST_SLACK_SIGNING_SECRET_INVALID_USER",
            "signing-secret",
        );
        let _bot = EnvGuard::set("BRASSCLAW_TEST_SLACK_BOT_TOKEN_INVALID_USER", "xoxb-token");
        let section = brassclaw_reborn_config::SlackSection {
            enabled: Some(true),
            installation_id: Some("install-alpha".to_string()),
            team_id: Some("T123".to_string()),
    ...
    fn slack_host_beta_config_reports_unset_signing_secret_env() {
        let _lock = env_lock();
        let _signing = EnvGuard::remove("BRASSCLAW_TEST_SLACK_UNSET_SIGNING_SECRET");
        let _bot = EnvGuard::set("BRASSCLAW_TEST_SLACK_BOT_TOKEN_UNSET_SIGNING", "xoxb-token");
        let section = brassclaw_reborn_config::SlackSection {
            enabled: Some(true),
            installation_id: Some("install-alpha".to_string()),
            team_id: Some("T123".to_string()),
            api_app_id: Some("A123".to_string()),
            slack_user_id: Some("U123".to_string()),
            signing_secret_env: Some("BRASSCLAW_TEST_SLACK_UNSET_SIGNING_SECRET".to_string()),
            bot_token_env: Some("BRASSCLAW_TEST_SLACK_BOT_TOKEN_UNSET_SIGNING".to_string()),
            ..Default::default()
        };
    ...
    #[cfg(feature = "slack-v2-host-beta")]
    fn tenant_id(raw: &str) -> brassclaw_reborn_composition::host_api::TenantId {
        brassclaw_reborn_composition::host_api::TenantId::new(raw).expect("valid tenant id")
    }

    #[cfg(feature = "slack-v2-host-beta")]
    fn agent_id(raw: &str) -> brassclaw_reborn_composition::host_api::AgentId {
        brassclaw_reborn_composition::host_api::AgentId::new(raw).expect("valid agent id")
    }

    #[cfg(feature = "slack-v2-host-beta")]
    fn project_id(raw: &str) -> brassclaw_reborn_composition::host_api::ProjectId {
        brassclaw_reborn_composition::host_api::ProjectId::new(raw).expect("valid project id")
    }

    #[cfg(feature = "slack-v2-host-beta")]
    fn user_id(raw: &str) -> brassclaw_reborn_composition::host_api::UserId {
        brassclaw_reborn_composition::host_api::UserId::new(raw).expect("valid user id")
    }
