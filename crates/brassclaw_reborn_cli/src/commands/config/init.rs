...
///
/// Mirrors v1's `brassclaw config init` ergonomics: refuses to clobber
...
impl ConfigInitCommand {
    pub(crate) fn execute(self, context: RebornCliContext) -> anyhow::Result<()> {
        ...
        println!();
        println!("edit them, then run `brassclaw-reborn run`.");
        Ok(())
    }
}
...
fn config_stub() -> String {
    format!(
        r#"# BrassClaw Reborn boot configuration.
#
# Layout:
...
#
# Regenerate with `brassclaw-reborn config init --force`.
