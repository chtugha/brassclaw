//! WebChat v2 SSO startup config for `brassclaw-reborn serve`.
//!
...
//! `brassclaw_reborn_webui_ingress` (see
//! [`brassclaw_reborn_webui_ingress::build_signed_session_login`]), which
...
pub(crate) fn sso_startup_config_from_env(
    ...
    // OAuth redirect URI.
    let base_url = env::var("BRASSCLAW_REBORN_WEBUI_BASE_URL")
        .ok()
    ...
    let allowed_email_domains = parse_allowed_email_domains(
        non_empty_env("BRASSCLAW_REBORN_WEBUI_ALLOWED_EMAIL_DOMAINS")
            .as_deref()
            .unwrap_or_default(),
    );
    ...
    if allowed_email_domains.is_empty() {
        anyhow::bail!(
            "WebChat v2 SSO providers are configured but \
             BRASSCLAW_REBORN_WEBUI_ALLOWED_EMAIL_DOMAINS is empty. Without an \
    ...
    if is_cleartext_http_scheme(base_url) && !listen_addr.ip().is_loopback() {
        anyhow::bail!(
            "WebChat v2 SSO base URL `{base_url}` uses http:// on a non-loopback interface, \
             ...
             BRASSCLAW_REBORN_WEBUI_BASE_URL to an https:// URL."
        );
    }
    Ok(())
}
...
/// `BRASSCLAW_REBORN_WEBUI_BASE_URL=HTTP://example.com` slip past the
...
/// opted in by setting its `BRASSCLAW_REBORN_WEBUI_*_CLIENT_ID`. The
...
///
/// These WebChat-login vars live in the `BRASSCLAW_REBORN_WEBUI_*`
/// namespace — the same one as `BRASSCLAW_REBORN_WEBUI_TOKEN` /
/// `BRASSCLAW_REBORN_WEBUI_USER_ID` — deliberately separate from the
/// bare `GOOGLE_CLIENT_ID` / `BRASSCLAW_REBORN_GOOGLE_*` vars that the
...
/// and would take down every `brassclaw-reborn` command. The distinct
...
fn oauth_providers_from_env() -> anyhow::Result<Vec<Arc<dyn OAuthProvider>>> {
    ...
    if let Some(client_id) = non_empty_env("BRASSCLAW_REBORN_WEBUI_GOOGLE_CLIENT_ID") {
        let client_secret = non_empty_env("BRASSCLAW_REBORN_WEBUI_GOOGLE_CLIENT_SECRET")
            .ok_or_else(|| {
                anyhow!(
                    "BRASSCLAW_REBORN_WEBUI_GOOGLE_CLIENT_ID is set but \
                     BRASSCLAW_REBORN_WEBUI_GOOGLE_CLIENT_SECRET is missing"
                )
            })?;
        let allowed_hd = non_empty_env("BRASSCLAW_REBORN_WEBUI_GOOGLE_ALLOWED_HD");
        ...
        // `BRASSCLAW_REBORN_WEBUI_ALLOWED_EMAIL_DOMAINS` (admission cannot
        ...
        if allowed_hd.is_none() {
            tracing::warn!(
                ...
                "BRASSCLAW_REBORN_WEBUI_GOOGLE_ALLOWED_HD is unset — Google login is gated \
                 only by BRASSCLAW_REBORN_WEBUI_ALLOWED_EMAIL_DOMAINS, not by a Workspace \
    ...
    if let Some(client_id) = non_empty_env("BRASSCLAW_REBORN_WEBUI_GITHUB_CLIENT_ID") {
        let client_secret = non_empty_env("BRASSCLAW_REBORN_WEBUI_GITHUB_CLIENT_SECRET")
            .ok_or_else(|| {
                anyhow!(
                    "BRASSCLAW_REBORN_WEBUI_GITHUB_CLIENT_ID is set but \
                     BRASSCLAW_REBORN_WEBUI_GITHUB_CLIENT_SECRET is missing"
                )
            })?;
...
fn oauth_http_timeout_from_env() -> Option<Duration> {
    let raw = non_empty_env("BRASSCLAW_REBORN_WEBUI_OAUTH_HTTP_TIMEOUT_SECS")?;
    match raw.trim().parse::<u64>() {
        ...
        _ => {
            tracing::warn!(
                target = "brassclaw::reborn::cli::serve",
                value = %raw,
                "BRASSCLAW_REBORN_WEBUI_OAUTH_HTTP_TIMEOUT_SECS is not a positive integer; \
...
mod tests {
    ...
    const SSO_ENV_VARS: &[&str] = &[
        "BRASSCLAW_REBORN_WEBUI_GOOGLE_CLIENT_ID",
        "BRASSCLAW_REBORN_WEBUI_GOOGLE_CLIENT_SECRET",
        "BRASSCLAW_REBORN_WEBUI_GOOGLE_ALLOWED_HD",
        "BRASSCLAW_REBORN_WEBUI_GITHUB_CLIENT_ID",
        "BRASSCLAW_REBORN_WEBUI_GITHUB_CLIENT_SECRET",
        "BRASSCLAW_REBORN_WEBUI_BASE_URL",
    ...
    fn startup_fails_closed_when_providers_set_but_allowlist_missing() {
        ...
        unsafe {
            std::env::set_var("BRASSCLAW_REBORN_WEBUI_GOOGLE_CLIENT_ID", "client-id");
            std::env::set_var(
                "BRASSCLAW_REBORN_WEBUI_GOOGLE_CLIENT_SECRET",
                "client-secret",
            );
        ...
        unsafe {
            std::env::set_var("BRASSCLAW_REBORN_WEBUI_ALLOWED_EMAIL_DOMAINS", "  , ,");
        }
        assert!(
