...
const DEFAULT_ENV_TOKEN_VAR: &str = "BRASSCLAW_REBORN_WEBUI_TOKEN";
const DEFAULT_ENV_USER_ID_VAR: &str = "BRASSCLAW_REBORN_WEBUI_USER_ID";
...
fn reject_non_loopback_privileged_local_runtime(
    ...
    anyhow::bail!(
        "`brassclaw-reborn serve` refuses non-loopback listener {host} because the selected \
...
fn with_notion_dcr_oauth_backend(
    ...
    services
        .with_notion_dcr_oauth_backend(callback_origin, "Brassclaw")
...
fn print_serve_banner(
    ...
    eprintln!("brassclaw-reborn: WebChat v2 listener");
    eprintln!("  binary    : brassclaw-reborn");
...
mod tests {
    ...
    async fn webui_serve_wires_notion_dcr_into_runtime_services() {
        ...
        let services = brassclaw_reborn_composition::build_reborn_services(services_input)
            .await
