...
//! ([`RebornIdentityResolver`](brassclaw_reborn_composition::RebornIdentityResolver),
...
impl UserDirectory for WebuiUserDirectory {
    async fn resolve(
        ...
        let Some(admitted_email) = self.admitted_email(profile) else {
            ...
            // to BRASSCLAW_REBORN_WEBUI_ALLOWED_EMAIL_DOMAINS. Logs only the
            ...
            let candidate_domains: std::collections::BTreeSet<String> = profile
                ...
                .collect();
            tracing::warn!(
                target: "brassclaw::reborn::webui_ingress::auth",
...
mod tests {
    ...
    fn shared_resolver() -> Arc<dyn RebornIdentityResolver> {
        ...
        brassclaw_reborn_composition::open_reborn_identity_resolver(
