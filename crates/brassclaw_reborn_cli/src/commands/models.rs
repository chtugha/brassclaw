...
impl ModelsListCommand {
    fn execute(self) -> anyhow::Result<()> {
        let slots = brassclaw_reborn_composition::reborn_model_slot_names();
        ...
        println!("BrassClaw Reborn model slots");
        for slot in slots {
            println!("- {}", slot);
        }
...
#[cfg(feature = "root-llm-provider")]
fn print_provider_detail(list: &brassclaw_reborn_composition::RebornProviderList) {
