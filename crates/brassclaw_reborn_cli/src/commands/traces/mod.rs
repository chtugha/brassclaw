...
use brassclaw_reborn_traces::client::{TraceClientHost, TraceClientScope};
...
enum TracesSubcommand {
    ...
    OptIn {
        ...
        #[arg(long, default_value = "BRASSCLAW_TRACE_SUBMIT_TOKEN")]
        ...
        #[arg(long, default_value_t = brassclaw_reborn_traces::contribution::TRACE_UPLOAD_CLAIM_DEFAULT_TIMEOUT_MS)]
    ...
    Preview {
        /// Recorded trace JSON file produced by BRASSCLAW_RECORD_TRACE
...
struct TraceQueueStatusDiagnostics {
    ...
    queue: brassclaw_reborn_traces::contribution::TraceQueueDiagnostics,
...
/// trims `BRASSCLAW_OWNER_ID` and falls back to the legacy "default" scope.
fn resolve_runtime_owner_scope() -> String {
    let env_owner_id = std::env::var("BRASSCLAW_OWNER_ID")
        .ok()
...
async fn preview_recorded_trace(options: PreviewOptions) -> anyhow::Result<()> {
    ...
    let recorded_trace: brassclaw_reborn_traces::recording::TraceFile =
...
fn trace_contribution_dir() -> PathBuf {
    brassclaw_reborn_traces::paths::brassclaw_base_dir().join("trace_contributions")
}
...
fn write_policy(policy: &StandingTraceContributionPolicy) -> anyhow::Result<()> {
    ...
    // user-only access via brassclaw_base_dir.
    #[cfg(unix)]
    {
        use std::io::Write;
