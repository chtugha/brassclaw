...
fn credit_notice_action_flags_parse_through_cli() {
    let ack_cli = parse_cli(["brassclaw-reborn", "traces", "credit", "--notice", "--ack"]);
    let TracesSubcommand::Credit {
        notice: ack_notice,
        ack,
        snooze_hours,
...
fn opt_in_upload_claim_issuer_flags_parse_through_cli() {
    let cli = parse_cli([
        ...
        "BRASSCLAW_TRACE_WORKLOAD_TOKEN",
    ...
    assert_eq!(
        ...
        Some("BRASSCLAW_TRACE_WORKLOAD_TOKEN")
    );
...
fn opt_in_writes_runtime_owner_policy_and_normalizes_selected_tools() {
    ...
    let _runtime_policy_restore = TracePolicyFileRestore::new(
        brassclaw_reborn_traces::contribution::trace_contribution_dir_for_scope(Some(
            &runtime_scope,
        ))
        .join("policy.json"),
    );
    ...
    opt_in(OptInOptions {
        ...
        upload_token_issuer_timeout_ms:
            brassclaw_reborn_traces::contribution::TRACE_UPLOAD_CLAIM_DEFAULT_TIMEOUT_MS,
...
fn queue_status_diagnostics_reports_invite_code_configured() {
    ...
    brassclaw_reborn_traces::contribution::write_trace_policy_for_scope(
        Some(&runtime_scope),
        &configured_policy,
    )
