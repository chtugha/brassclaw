...
use brassclaw_reborn_composition::TriggerPollerSettings;

/// Read a trigger-poller env var with **strict** presence semantics.
///
/// `BRASSCLAW_TRIGGER_POLLER_*` env vars are operator-control knobs: presence
...
/// `BRASSCLAW_TRIGGER_POLLER_INTERVAL_SECS` (env). One hour caps the
...
///    - `BRASSCLAW_TRIGGER_POLLER_ENABLED` — `1`/`true` → enabled, `0`/`false` → disabled
///      (case-insensitive). Overrides any `enabled` value from the config file.
///    - `BRASSCLAW_TRIGGER_POLLER_INTERVAL_SECS` — parse as `u64`; overrides the
...
///    at the `brassclaw_triggers` crate defaults).
///
...
/// (in `brassclaw_triggers`); the CLI layer keeps the guard here so an invalid
...
pub(super) fn trigger_poller_settings(
    ...
    if let Some(raw) = strict_env_var("BRASSCLAW_TRIGGER_POLLER_ENABLED")? {
        match raw.trim().to_ascii_lowercase().as_str() {
            ...
            _ => {
                ...
                anyhow::bail!(
                    "BRASSCLAW_TRIGGER_POLLER_ENABLED must be one of 1, true, 0, false (got {display:?})"
                )
            }
        }
    }

    if let Some(raw) = strict_env_var("BRASSCLAW_TRIGGER_POLLER_INTERVAL_SECS")? {
        let secs: u64 = raw.trim().parse().map_err(|e| {
            let display = truncate_env_value_for_display(&raw);
            anyhow::anyhow!(
                "BRASSCLAW_TRIGGER_POLLER_INTERVAL_SECS must be a positive integer, got {display:?}: {e}"
            )
        })?;
        if secs == 0 || secs > MAX_POLL_INTERVAL_SECS {
            anyhow::bail!(
                "BRASSCLAW_TRIGGER_POLLER_INTERVAL_SECS must be in 1..={MAX_POLL_INTERVAL_SECS}; got {secs}"
            );
        }
...
mod tests {
    ...
    use brassclaw_reborn_config::TriggerPollerConfigSection;
    use std::time::Duration;

    fn make_config_with_trigger_poller(
        section: TriggerPollerConfigSection,
    ) -> brassclaw_reborn_config::RebornConfigFile {
        brassclaw_reborn_config::RebornConfigFile {
    ...
    fn trigger_poller_settings_env_enabled_overrides_config_disabled() {
        ...
        assert!(
            settings.enabled,
            "BRASSCLAW_TRIGGER_POLLER_ENABLED=true must override config enabled=false"
        );
    }
    ...
    fn trigger_poller_settings_env_disabled_overrides_config_enabled() {
        ...
        let _lock = lock_trigger_env();
        let _interval = EnvGuard::clear("BRASSCLAW_TRIGGER_POLLER_INTERVAL_SECS");
        let _enabled = EnvGuard::set("BRASSCLAW_TRIGGER_POLLER_ENABLED", "false");
        ...
        assert!(
            !settings.enabled,
            "BRASSCLAW_TRIGGER_POLLER_ENABLED=false must override config enabled=true"
        );
    }
    ...
    fn trigger_poller_settings_env_interval_zero_is_error() {
        let _lock = lock_trigger_env();
        let _enabled = EnvGuard::clear("BRASSCLAW_TRIGGER_POLLER_ENABLED");
        let _interval = EnvGuard::set("BRASSCLAW_TRIGGER_POLLER_INTERVAL_SECS", "0");

        let err = trigger_poller_settings(None)
            .expect_err("BRASSCLAW_TRIGGER_POLLER_INTERVAL_SECS=0 must be rejected");

        assert!(
            err.to_string()
                .contains("BRASSCLAW_TRIGGER_POLLER_INTERVAL_SECS"),
    ...
    fn trigger_poller_settings_env_interval_non_numeric_preserves_parse_error() {
        let _lock = lock_trigger_env();
        let _enabled = EnvGuard::clear("BRASSCLAW_TRIGGER_POLLER_ENABLED");
        let _interval = EnvGuard::set("BRASSCLAW_TRIGGER_POLLER_INTERVAL_SECS", "10s");
        ...
        assert!(
            msg.contains("BRASSCLAW_TRIGGER_POLLER_INTERVAL_SECS") && msg.contains("10s"),
    ...
    fn trigger_poller_settings_env_enabled_invalid_value_is_error() {
        ...
        let err = trigger_poller_settings(None)
            .expect_err("BRASSCLAW_TRIGGER_POLLER_ENABLED=yes must be rejected");

        let msg = err.to_string();
        assert!(
            msg.contains("BRASSCLAW_TRIGGER_POLLER_ENABLED") && msg.contains("yes"),
            "error must surface env var name and offending value, got: {msg}",
        );
    }
    ...
    fn trigger_poller_settings_env_interval_above_cap_is_error() {
        let _lock = lock_trigger_env();
        let _enabled = EnvGuard::clear("BRASSCLAW_TRIGGER_POLLER_ENABLED");
        let _interval = EnvGuard::set("BRASSCLAW_TRIGGER_POLLER_INTERVAL_SECS", "86400");

        let err = trigger_poller_settings(None)
            .expect_err("BRASSCLAW_TRIGGER_POLLER_INTERVAL_SECS above the cap must be rejected");

        let msg = err.to_string();
        assert!(
            msg.contains("BRASSCLAW_TRIGGER_POLLER_INTERVAL_SECS") && msg.contains("86400"),
    ...
    fn trigger_poller_settings_env_enabled_numeric_one_enables() {
        let _lock = lock_trigger_env();
        let _enabled = EnvGuard::set("BRASSCLAW_TRIGGER_POLLER_ENABLED", "1");
        let _interval = EnvGuard::clear("BRASSCLAW_TRIGGER_POLLER_INTERVAL_SECS");

        let settings = trigger_poller_settings(None).expect("ENABLED=1 must succeed");
        assert!(
            settings.enabled,
            "BRASSCLAW_TRIGGER_POLLER_ENABLED=1 must enable"
        );
    }
    ...
    fn trigger_poller_settings_env_enabled_numeric_zero_disables() {
        let _lock = lock_trigger_env();
        let _enabled = EnvGuard::set("BRASSCLAW_TRIGGER_POLLER_ENABLED", "0");
        let _interval = EnvGuard::clear("BRASSCLAW_TRIGGER_POLLER_INTERVAL_SECS");
        ...
        assert!(
            !settings.enabled,
            "BRASSCLAW_TRIGGER_POLLER_ENABLED=0 must disable"
        );
    }
    ...
    fn trigger_poller_settings_env_enabled_long_value_is_truncated() {
        ...
        let _enabled = EnvGuard::set("BRASSCLAW_TRIGGER_POLLER_ENABLED", &long_value);
        let _interval = EnvGuard::clear("BRASSCLAW_TRIGGER_POLLER_INTERVAL_SECS");
    ...
    fn trigger_poller_settings_env_enabled_empty_is_error() {
        ...
        let _lock = lock_trigger_env();
        let _enabled = EnvGuard::set("BRASSCLAW_TRIGGER_POLLER_ENABLED", "");
        let _interval = EnvGuard::clear("BRASSCLAW_TRIGGER_POLLER_INTERVAL_SECS");
        ...
        let err = trigger_poller_settings(Some(&config)).expect_err(
            "empty BRASSCLAW_TRIGGER_POLLER_ENABLED must be rejected, not silently dropped",
        );
    ...
    fn trigger_poller_settings_env_enabled_whitespace_is_error() {
        ...
        let err = trigger_poller_settings(None)
            .expect_err("whitespace-only BRASSCLAW_TRIGGER_POLLER_ENABLED must be rejected");
    ...
    fn trigger_poller_settings_env_interval_empty_is_error() {
        ...
        let _lock = lock_trigger_env();
        let _enabled = EnvGuard::clear("BRASSCLAW_TRIGGER_POLLER_ENABLED");
        let _interval = EnvGuard::set("BRASSCLAW_TRIGGER_POLLER_INTERVAL_SECS", "");

        let err = trigger_poller_settings(None).expect_err(
            "empty BRASSCLAW_TRIGGER_POLLER_INTERVAL_SECS must be rejected, not silently dropped",
        );
