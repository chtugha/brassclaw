# Agent Map — brassclaw_reborn_event_store
...
- Concrete durable-log backends implementing the `brassclaw_events` `DurableEventLog`/`DurableAuditLog` traits: filesystem (`FilesystemDurableEventLog`, `FilesystemDurableAuditLog`), JSONL (`JsonlDurableEventLog`, `JsonlDurableAuditLog`), and the feature-gated libSQL/Postgres backends (behind the `libsql` / `postgres` features).
...
- Fast local check: `cargo test -p brassclaw_reborn_event_store`
