---
description: Deep audit of the BrassClaw crate for vulnerabilities, bugs, unfinished work, inconsistencies, and oversights
...
BrassClaw convention: `.unwrap()` and `.expect()` are banned in production code. Any occurrence outside `#[cfg(test)]` blocks is a **High severity** finding.
...
BrassClaw has two database backends (PostgreSQL and libSQL). Check both for injection vectors.
...
- Is `thiserror` used consistently for error types (BrassClaw convention)?
...
BrassClaw convention: use `crate::` imports, not `super::`. Flag any `super::` imports in non-test code.
