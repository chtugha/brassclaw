...
description: Generate or update the BrassClaw architecture overview video using Remotion. Use when asked to update, regenerate, or modify the architecture video, add/remove scenes, or reflect codebase changes in the video.
---
...
Read `docs/architecture-video/src/BrassClawArchitecture.tsx` to understand current scene order, durations, and transitions. Then read individual scenes in `docs/architecture-video/src/scenes/` to see what's already covered.
...
│   ├── BrassClawArchitecture.tsx  # Main composition — scene order + transitions
...
3. Import in `BrassClawArchitecture.tsx`
4. Add to the `SCENES` array with duration and transition type
5. `TOTAL_DURATION` auto-computes from the array
6. Verify with: `npx remotion still BrassClawArchitecture --scale=0.25 --frame=<N>`
...
2. **Spot check frames:** `npx remotion still BrassClawArchitecture --scale=0.25 --frame=<N>`
