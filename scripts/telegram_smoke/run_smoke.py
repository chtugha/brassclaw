...
Runs a small set of real Telegram flows against an already-running BrassClaw
...
async def run_attachment_case(client: TelegramClient, cfg: SmokeConfig, bot_id: int) -> None:
    ...
    with tempfile.NamedTemporaryFile("w", suffix=".txt", delete=False) as tmp:
        tmp.write(f"brassclaw telegram smoke attachment {run_id}\n")
...
async def async_main() -> int:
    ...
    if cfg.healthcheck_url:
        print(f"Checking BrassClaw health at {cfg.healthcheck_url} ...")
