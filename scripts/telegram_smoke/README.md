...
By default the script only checks that the bot replies at all. If you run BrassClaw
with a deterministic mock LLM, you can also require a fixed reply substring.

## Requirements

- Python 3.11+
- a local BrassClaw instance already running with Telegram configured
...
2. Start BrassClaw with the Telegram channel enabled.
...
script fails fast when BrassClaw is not up.
...
1. Run the local Rust Telegram regression suite first.
   ...
   ```
2. Start the release-candidate BrassClaw build.
3. Run the real Telegram smoke script.
4. Treat any missing reply or timeout as a release blocker.

## Deterministic Replies

If you want strict assertions, run BrassClaw against a deterministic mock LLM and
set:
...
- `Timed out waiting for bot reply`
  - BrassClaw is down, Telegram is misconfigured, the LLM path is stuck, or the
