...
A standalone WASM component that provides Slack integration for BrassClaw. This serves as both a functional tool and a template for building custom WASM tools.
...
```bash
mkdir -p ~/.brassclaw/tools
cp target/wasm32-wasip2/release/slack_tool.wasm ~/.brassclaw/tools/slack.wasm
cp slack.capabilities.json ~/.brassclaw/tools/
```
...
```bash
brassclaw secret set slack_bot_token "xoxb-your-token-here"
```
...
```bash
brassclaw secret set slack_bot_token "xoxb-..."
```
