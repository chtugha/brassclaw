...
> ✅ **RESOLVED (this session):** `PgRunStateStore`, `PgApprovalRequestStore`, `PgTurnStateStore`, `PgResourceGovernorStore`, `PgCapabilityLeaseStore` — all now wired into `build_pg_backend_production_with_tools` via new `HostRuntimeServices` builder methods (`with_pg_run_state`, `with_pg_turn_state_store`, `with_pg_resource_governor`) added to `crates/brassclaw_host_runtime/src/services/builder.rs`. `build_postgres_production` now calls `build_pg_backend_production_with_tools` instead of the generic `build_backend_production_with_tools`. `PgCapabilityLeaseStore` constructed directly in the pg path (filesystem leases bundle discarded). `PgSubagentGoalStore` is fully implemented and available; per-turn instantiation in `runtime.rs` line 1849 uses `InMemoryBoundedSubagentGoalStore` because this is inside the local-dev runtime path — the production path will pick up `PgSubagentGoalStore` when `build_reborn_runtime` is extended to support postgres profiles.
> ℹ️ `PgSessionThreadService`, `PgProcessStore`/`PgProcessResultStore`, `PgConversationStateStore`, `PgOutboundStateStore`, `PgExtensionInstallationStore` are fully implemented in their respective crates. They are NOT yet wired into the production factory because `build_reborn_runtime` currently enforces `LocalDev`/`LocalDevYolo` profiles only (rejects postgres profile with an explicit error). These stores will be wired when the CLI `serve` command is extended to invoke the postgres production path directly (tracked as Phase-5 factory wiring).
...
#### Step 7.4 — Sempai gateway + rerouting branch + 3-part prompt + KV-cache pre-warm 🔸 `PARTIAL`
...
> ❌ **NOT IMPLEMENTED:** Routing of `proposed_recipe_updates` + `proposed_intent_examples` to Q1 validation queue — fields exist on `SempaiReviewOutcome` but no routing logic found in `loop_driver_host.rs` or elsewhere.
