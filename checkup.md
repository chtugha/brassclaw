...
> ℹ️ **REMAINING GAP:** `turn_state` (turn runs, blocking, approvals), `loop_checkpoint_store`, `checkpoint_state_store`, `run_state`, `capability_leases` in `build_reborn_runtime` still use in-memory stores even on the postgres `serve` path (hybrid path). The full PG wiring of these and the complete pure-PG composition path are tracked in `subplan_pg4_runtime_pg_path.md` Steps 4–9. `ProcessServices` already uses `PgProcessStore`/`PgProcessResultStore` via `ProcessServices::postgres()` in `build_pg_backend_production_with_tools` (dead code). `PgConversationStateStore`, `PgOutboundStateStore`, `PgExtensionInstallationStore` fully implemented in their crates, not yet wired.
