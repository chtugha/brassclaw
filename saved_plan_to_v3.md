...
>
> **O1–O5 complete — H.5 obsolete/skipped.** O1 (mission purge: V064 + 23
> `parent_mission_id` refs), O2.1–O2.4 (`parent_mission_id` removal + mission
> system purge V065 + dead debug-panel/gateway surfaces, incl. the full
> `brassclaw_gateway` crate deletion), O3 (Model A Python `tier_zero`
> early-return branch + its 3 tests removed from `default.py`/
> `orchestrator.rs`), O4 (re-document the reusable H.4 pieces —
> `RecipeTierZero*` events, `TurnRoutingSignals.recipe_id`/`recipe_name`,
> `RecipeOutcomeListener`, `TierZeroOutcome`/`build_tier_zero_outcome` — as
> reused by Model B/C, not Model A; + O3-fallout cleanup of the vestigial
> `code_step_result` step-0 test-helper param) all done + pushed. **H.5 (Model
> A composition integration test) is obsolete/skipped** — Model A is
> dormant/never-built (production turns run on the agent loop, not the engine
> Python runtime), superseded by H.6–H.13 (Model B/C agent-loop Tier-0/Tier-1
> dispatch; H.8 extracts `assemble_prior_knowledge_with_hint` +
...
**Status:** [x] K.1 COMPLETE (incl. K.1.7 SKILL.md export — implemented after the notes below were written; see K.1.7 note). [x] K.2 DELIBERATELY NOT IMPLEMENTED — per user instruction on the Phase K step, external-MCP translation is dropped entirely; a future MCP-server-functionality will let the Orchestrator define LLM-prompt tools that call predefined Orchestrator Skills to perform Rust tool calls. `mcp_translation.rs` is intentionally absent; all external-MCP instructions in this phase are ignored. [x] K.3 COMPLETE — `__retrieve_docs__`/`__get_reduction_rules__` handler+registration removal DONE (Kohai K3, commit `a3ecad97`); `handle_assemble_prior_knowledge` legacy `retrieve_context` fallback block retired (H8.4 — handler replaced by `assemble_prior_knowledge_with_hint`); the DEFERRED deletions (`RamSource` + `retrieval_dbless.rs` + `RetrievalEngine::retrieve_context` + `build_step_context` + legacy `brassclaw_engine::runtime::manager`/`ThreadManager`) all landed with C.7 Model-A retirement (commit `d686424c` — `retrieval_dbless.rs` deleted, `RamSource` absent, `manager.rs`/`ThreadManager` gone); `recipe_library.rs` (legacy MemoryDoc-backed `RecipeLibrary` + `DisabledRecipeLookup`) deleted and `mod recipe_library` removed from `lib.rs` — K.3 fully complete.
