...
description: Deploy BrassClaw to a cloud server
---

Deploy BrassClaw to a remote VPS or cloud server for always-on operation.
...
CREATE USER brassclaw WITH PASSWORD 'your-secure-password';
CREATE DATABASE brassclaw OWNER brassclaw;
EOF
...
# Enable the vector extension in the brassclaw database
...
2. **Custom callback URL**: Set `BRASSCLAW_OAUTH_CALLBACK_URL` to a publicly reachable URL
...
1. **Database**: Select "PostgreSQL"
   - Enter connection string: `postgres://brassclaw:your-secure-password@localhost/brassclaw`
...
<Warning>
**Important:** BrassClaw's orchestrator binds to `0.0.0.0:50051` on Linux for container communication. This port should **not** be exposed externally. The firewall configuration below includes rules to block external access to this port—do not add any UFW allow rules for `50051`.
</Warning>
...
# Configure site
sudo tee /etc/nginx/sites-available/brassclaw <<'EOF'
server {
...
# Enable site
sudo ln -sf /etc/nginx/sites-available/brassclaw /etc/nginx/sites-enabled/
...
Environment="DATABASE_URL=postgres://brassclaw:your-secure-password@localhost/brassclaw"
...
# Create filter
sudo tee /etc/fail2ban/filter.d/brassclaw.conf <<'EOF'
[Definition]
...
# Create jail
sudo tee /etc/fail2ban/jail.d/brassclaw.conf <<EOF
[brassclaw]
enabled = true
port = http,https
filter = brassclaw
logpath = /var/log/brassclaw/web.log
maxretry = 5
bantime = 3600
EOF
