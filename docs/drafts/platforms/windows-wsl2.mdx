...
description: Running BrassClaw on Windows via WSL2 (recommended)
---

WSL2 (Windows Subsystem for Linux 2) is the recommended way to run BrassClaw on Windows. It provides a full Linux environment with near-native performance and seamless port forwarding to Windows.

<Note>
WSL2 is preferred over native Windows for BrassClaw. The Docker sandbox, shell tools, and keyring integrations all work without workarounds under WSL2. For a fully native Windows setup, see [Windows Native](/platforms/windows-native).
</Note>
...
BrassClaw's Web Gateway binds to `127.0.0.1:3000` inside WSL2. Modern Windows 11 and recent Windows 10 versions automatically forward localhost ports from WSL2 to the Windows host.
...
openssl rand -base64 32 > ~/.brassclaw/master.key
chmod 600 ~/.brassclaw/master.key

# Reference it in your .env
BRASSCLAW_MASTER_KEY=$(cat ~/.brassclaw/master.key)
```
...
You should see the Docker hello-world message. BrassClaw can now use Docker for sandbox execution without needing a separate Docker installation inside WSL2.
...
New-NetFirewallRule `
  -DisplayName "BrassClaw Web Gateway" `
  -Direction Inbound `
  -Protocol TCP `
  -LocalPort 3000 `
  -Action Allow `
...
```powershell
New-NetFirewallRule `
  -DisplayName "Block BrassClaw Public" `
  -Direction Inbound `
  -Protocol TCP `
  -LocalPort 3000 `
  -Action Block `
...
- Multi-pane view (BrassClaw logs + interactive shell side by side)
...
## Running BrassClaw as a WSL2 Background Service
...
<CardGroup cols={3}>
  ...
  <Card title="Windows Native" icon="windows" href="/platforms/windows-native">
    Running BrassClaw natively without WSL2 (experimental)
  </Card>
