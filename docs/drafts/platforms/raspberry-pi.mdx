...
description: Running BrassClaw on Raspberry Pi with ARM64 and local inference
---

BrassClaw runs well on Raspberry Pi 4 and Pi 5 with a 64-bit OS. Pair it with Ollama for fully local inference — no cloud dependency, no API keys required.
...
The libSQL backend is strongly recommended for Raspberry Pi — it requires no separate database server and runs embedded in the BrassClaw process.
...
<AccordionGroup>
  <Accordion title="4 GB RAM (Pi 4 / Pi 5 4 GB)" icon="cpu">
    ...
    This model uses ~2 GB RAM and leaves headroom for the OS and BrassClaw.
  </Accordion>
...
### BrassClaw Configuration for Ollama
...
Run BrassClaw as a background service on the Pi.
...
Type=simple
User=pi
EnvironmentFile=/home/pi/.brassclaw/.env
ExecStart=/home/pi/.local/bin/brassclaw run
Restart=on-failure
RestartSec=10s
...
## Accessing BrassClaw from Your Network

By default BrassClaw binds to `127.0.0.1`. To access it from another device on your LAN:
