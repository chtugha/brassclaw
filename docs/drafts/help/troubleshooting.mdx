...
Solutions for common BrassClaw issues.
...
<AccordionGroup>
  ...
  <Accordion title="Permission denied" icon="lock">
    **Solution:**
    ...
    sudo mv ~/.local/bin/brassclaw /usr/local/bin/
    ```
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Database connection failed" icon="database">
    **PostgreSQL:**
    ...
    # Verify connection
    psql postgres://user:pass@localhost/brassclaw
    ```
...
<AccordionGroup>
  <Accordion title="Browser didn't open for OAuth" icon="globe">
    **Solutions:**
    ...
    export BRASSCLAW_OAUTH_CALLBACK_URL=https://your-server:9876
    ```
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Web Gateway token not found" icon="key">
    **Solution:**
    ```bash
    # View logs
    RUST_LOG=brassclaw=info brassclaw run 2>&1 | grep "Gateway auth token"
  ...
  <Accordion title="Owner binding timeout" icon="clock">
    **Solutions:**
    ...
    2. Re-run `brassclaw onboard --channels-only`
...
```bash
# All modules
RUST_LOG=debug brassclaw run

# Just BrassClaw
RUST_LOG=brassclaw=debug brassclaw run

# Specific module
RUST_LOG=brassclaw::agent=debug brassclaw run

# With HTTP requests
RUST_LOG=brassclaw=debug,tower_http=debug brassclaw run
```
...
1. Run `brassclaw doctor --json`
2. Check logs with `RUST_LOG=debug`
3. Search [GitHub Issues](https://github.com/brassclaw-ai/brassclaw/issues)
