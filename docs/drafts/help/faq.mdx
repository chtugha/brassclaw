...
Common questions about BrassClaw.

## General

<AccordionGroup>
  <Accordion title="What is BrassClaw?" icon="help-circle">
    BrassClaw is a secure, self-hosted AI assistant that runs on your own hardware. It provides a personal AI with strong privacy guarantees through multi-layer security.
  </Accordion>

  <Accordion title="Do I need Docker?" icon="container">
    **No for basic use, yes for job sandboxing.**

    BrassClaw runs as a standalone binary. Docker is only required if you want to use:
    - Sandboxed job execution (Claude Code mode)
    - WASM tool building with automatic compilation

    Without Docker, BrassClaw still works fully — just without the Docker sandbox layer.
  </Accordion>

  <Accordion title="Can I run without PostgreSQL?" icon="database">
    **Yes.** BrassClaw includes libSQL (embedded SQLite) as an alternative.
  ...
  <Accordion title="How is BrassClaw different from openclaw?" icon="git-compare">
    | Feature | BrassClaw | openclaw |
    |---------|----------|----------|
    ...
    BrassClaw is the self-hosted version for users who want complete control over their data.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Where is my data stored?" icon="database">
    By default: `~/.brassclaw/`

    ```
    ~/.brassclaw/
    ├── brassclaw.db          # libSQL database
    ...
    Change with `BRASSCLAW_BASE_DIR`.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Why is the wizard asking for a browser?" icon="globe">
    ...
    - Or set `BRASSCLAW_OAUTH_CALLBACK_URL` to a public URL
  </Accordion>

  <Accordion title="How do I run BrassClaw as a service?" icon="server">
    ```bash
    # Install service
    brassclaw service install

    # Start
    sudo systemctl enable --now brassclaw  # Linux
    brew services start brassclaw            # macOS
    ```
  </Accordion>

  <Accordion title="Can I use BrassClaw behind a reverse proxy?" icon="network">
...
<AccordionGroup>
  <Accordion title="How secure is BrassClaw?" icon="shield">
    BrassClaw uses defense in depth:
  ...
  <Accordion title="Can BrassClaw be hacked?" icon="alert-triangle">
    No software is unhackable. BrassClaw reduces risk through:
...
<AccordionGroup>
  <Accordion title="Can I run BrassClaw on a Raspberry Pi?" icon="cpu">
    **Yes**, with caveats:
  ...
  <Accordion title="Can I contribute?" icon="github">
    Yes! BrassClaw is open source:

    - GitHub: https://github.com/brassclaw-ai/brassclaw
...
- [GitHub Issues](https://github.com/brassclaw-ai/brassclaw/issues) — Bug reports
