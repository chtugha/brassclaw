...
BrassClaw uses a zero-exposure credential model: secrets are encrypted at rest and never exposed to untrusted code.
...
```bash
# List secrets
brassclaw secret list

# Get a secret (decrypted)
brassclaw secret get telegram_bot_token

# Set a secret
brassclaw secret set telegram_bot_token "your-token"

# Delete a secret
brassclaw secret delete telegram_bot_token
```
...
Common secret names used by BrassClaw:
...
<AccordionGroup>
  ...
  <Accordion title="Secret not found" icon="search">
    ...
    - Re-run `brassclaw onboard` to reconfigure
  </Accordion>
