...
BrassClaw uses two sandbox layers for tool execution: WASM sandbox for tools, and Docker sandbox for jobs.
...
# Docker image
export SANDBOX_IMAGE=brassclaw-worker:latest
```
...
BrassClaw can run inside Docker and still sandbox jobs:
...
<Warning>
**BrassClaw runs alongside Docker** (for job sandboxing), not inside Docker by default.

- **Default**: BrassClaw binary → spawns containers for jobs
- **Optional**: BrassClaw inside container → still spawns sibling containers
</Warning>

See [Docker Install](/install/docker) for running BrassClaw itself in a container.
