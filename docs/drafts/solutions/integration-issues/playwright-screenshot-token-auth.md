...
description: "Building a UI screenshot capture pipeline that auto-authenticates with BrassClaw web gateway"
...
Building an automated screenshot documentation pipeline for BrassClaw's web gateway UI that:
...
1. **Token URL Construction**: The `getBrassClawUrlWithToken()` function was creating malformed URLs like `/?token=TOKEN/skills` when appending paths
2. **Client-Side Routing**: BrassClaw's web gateway uses client-side routing; only `/` is served by the backend
...
### 4. Auto-Detection of BrassClaw Port
...
| `docs/tests/fixtures/seed.ts` | Fixed `getBrassClawUrlWithToken()` with optional path param |
...
# Run full pipeline
cd /home/opselite/ai_projects/brassclaw-src && bash docs/scripts/capture-screenshots.sh
```

## References

- BrassClaw web gateway auth: `src/channels/web/static/app.js` lines 96-114
