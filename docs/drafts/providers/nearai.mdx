...
description: Default LLM provider for BrassClaw
---

NEAR AI is the default provider for BrassClaw, offering access to multiple models including Claude via a simple OAuth flow.
...
**How it works:**
1. `brassclaw onboard` opens your browser
2. Log in with GitHub or Google
3. Session token saved to `~/.brassclaw/session.json`
4. Automatic renewal
...
<Warning>
**VPS / Remote servers:** Browser OAuth won't work without a browser. Use API key mode or set `BRASSCLAW_OAUTH_CALLBACK_URL` to a publicly reachable URL.
</Warning>
...
```bash
brassclaw config set nearai.session_token sess_xxxxx
```

Or edit `~/.brassclaw/session.json`:
