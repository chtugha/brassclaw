...
description: How BrassClaw's agent runtime processes requests and executes jobs
---

BrassClaw's agent runtime is an always-on loop that accepts messages from any channel, classifies intent, and dispatches work as parallel jobs — each independently tracked through a defined state machine.
...
BrassClaw uses a three-level session hierarchy:
...
As conversations grow, context pressure increases. BrassClaw handles this automatically:
