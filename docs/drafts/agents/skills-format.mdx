...
| `metadata.brassclaw.requires.bins` | string[] | No | Binaries that must exist on `PATH`. Skill is gated out if any are missing. |
| `metadata.brassclaw.requires.env` | string[] | No | Environment variables that must be set. Skill is gated out if any are missing. |
...
~/.brassclaw/skills/my-skill/SKILL.md
