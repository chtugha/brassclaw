...
ClawHub is the official registry for BrassClaw skills. It lets you discover community-built skills, install them with a single command, and keep them up to date — without leaving the agent interface.
...
<Warning>
Skills from ClawHub are installed with **Installed** trust, not **Trusted** trust. They cannot execute shell commands, write files, or make HTTP requests. If you need elevated access, download the SKILL.md manually and place it in `~/.brassclaw/skills/` after reviewing the contents.
</Warning>
...
# Example output:
# kubernetes-deploy  (trusted)    ~/.brassclaw/skills/
# git-helper         (trusted)    <workspace>/skills/
# python-lint        (installed)  ~/.brassclaw/installed_skills/
```
...
Download and install a skill from ClawHub into `~/.brassclaw/installed_skills/`.
...
Remove an installed skill from `~/.brassclaw/installed_skills/`.

```
skill_remove python-lint
```

Trusted skills in `~/.brassclaw/skills/` must be removed manually by deleting the directory.
...
<Steps>
  ...
  <Step title="Install">
    Run `skill_install <slug>`. The agent fetches the SKILL.md from the registry and writes it to `~/.brassclaw/installed_skills/<slug>/SKILL.md`.
  </Step>
...
2. Test it locally by placing it in `~/.brassclaw/skills/`
