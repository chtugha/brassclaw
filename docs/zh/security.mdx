---
title: Security
description: BrassClaw 的纵深防御安全架构
---

BrassClaw 从一开始就把安全作为核心原则。我们采用纵深防御架构，通过多层彼此独立的保护机制，在启用强大智能体能力的同时保障您的数据安全。
...
这张安全架构图展示了 BrassClaw 的**纵深防御**思路：数据在到达 LLM 与外部服务之前，会依次经过四层独立保护。
...
BrassClaw 会扫描所有发往 LLM 的输入，无论是用户输入还是工具执行结果，以识别潜在的敏感信息泄漏。
