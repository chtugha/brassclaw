---
title: Amazon EC2
description: 在 Amazon EC2 实例上托管 BrassClaw
---

Amazon Elastic Compute Cloud（EC2）允许您在 AWS 基础设施上以灵活的定价运行虚拟机，新账户还可享受免费套餐。本指南将带您启动一个 EC2 实例并对其进行安全配置，从而安全地运行 BrassClaw 并将其暴露到互联网。

<Tip>
不想自己搭建基础设施？您可以在 [agent.near.ai](https://agent.near.ai) 上点几下就安装好 BrassClaw。
</Tip>
...
其余所有步骤——更新系统、加固 SSH、安装 Fail2Ban、配置防火墙以及安装 BrassClaw——均完全适用。

---

## 下一步

参考我们的 [快速入门指南](/zh/quickstart) 创建您的第一个 agent，将其连接到 Telegram，并开始探索 BrassClaw 的功能。
