...
BrassClaw 会监控所有正在运行的任务，并自动恢复那些停止推进的任务，无需人工介入。
...
- **日志**：设置 `RUST_LOG=brassclaw::agent::self_repair=debug` 查看详细修复事件
...
<AccordionGroup>
  <Accordion title="任务反复卡住" icon="refresh-cw">
    - 查看 `RUST_LOG=brassclaw::agent::self_repair=debug` 以确认失败原因
