...
WASM 工具运行在 [wasmtime](https://wasmtime.dev/) 沙箱中。除 BrassClaw 暴露的宿主函数外，网络、文件系统、凭据等能力都必须在 `capabilities.json` 中显式声明。

对于需要强隔离的自定义集成，这是 BrassClaw 推荐方案。
