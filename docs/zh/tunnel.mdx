...
隧道将您本地的 BrassClaw 智能体暴露到互联网。当您需要基于 webhook 的频道或希望实现即时消息传递而非轮询时，就需要它。
...
`ngrok` 是一个托管隧道服务，设置简单，非常适合刚开始使用 `brassclaw` 的用户。使用前需要从 [ngrok 控制台](https://dashboard.ngrok.com/get-started/your-authtoken) 获取认证令牌。
...
- `{port}` 表示 BrassClaw 的本地端口
- `{host}` 表示 BrassClaw 的本地主机
...
当隧道在 BrassClaw 外部管理且您已有稳定的公共 URL 时使用此选项。

BrassClaw 将直接使用该 URL，不会启动或管理任何隧道进程。
