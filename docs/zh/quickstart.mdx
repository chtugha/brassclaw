...
本指南将帮助您在 10 分钟内从零开始运行一个 BrassClaw 实例。
...
<Step title="获取 BrassClaw">

<Tabs>
  <Tab title="Agent Hub">
  前往 https://agent.near.ai/ 并使用您偏好的方式登录，然后在私有实例中创建一个 BrassClaw 智能体。
  ...
  使用 BrassClaw 需要提供 SSH 密钥。如果您还没有，可以在终端中使用以下命令生成：
...
<Accordion title="⚠️ 错误：另一个 BrassClaw 实例已在运行">

如果出现错误 `Error: Another BrassClaw instance is already running (PID 38). If this is incorrect, remove the stale PID file: /home/agent/.brassclaw/brassclaw.pid`，只需运行以下命令删除过期的 PID 文件，然后重新启动智能体：
...
<Step title="更新 BrassClaw">

最后，请定期更新 BrassClaw，以获得最新功能和改进。您可以在终端中运行以下命令进行更新：
