...
HTTP Webhook 频道提供 REST API，用于将外部服务与 BrassClaw 集成。
...
配置 GitHub 将事件发送到您的 BrassClaw webhook URL：
...
使用 Zapier 的 Webhook 操作将事件发送到 BrassClaw。
...
<AccordionGroup>
  ...
  <Accordion title="消息未处理" icon="message-circle">
    - 检查日志：`RUST_LOG=brassclaw=debug brassclaw run`
    - 验证 JSON 格式
    - 检查 `user_id` 有效
  </Accordion>
</AccordionGroup>
