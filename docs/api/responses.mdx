---
title: "Responses API"
description: "OpenAI-compatible Responses endpoint backed by the full BrassClaw agent loop"
icon: "code"
---

BrassClaw exposes an OpenAI-compatible Responses API. Unlike a raw LLM passthrough, requests route through the full agent loop, so callers get tools, memory, safety, and server-side conversation state via a standard wire format.
...
| Endpoint | Behavior |
|----------|----------|
| `/api/v1/responses` | Routes through the agent. The model can call BrassClaw tools (file, web, memory, MCP, extensions), retain conversation state via `previous_response_id`, and pause for caller-supplied tools. |
| `/v1/chat/completions` | Raw LLM proxy. No tools, no memory, no agent loop. Use it when you only need a model call. |

If you want BrassClaw's capabilities behind an OpenAI-shaped wire contract, this is the endpoint.
...
<Tabs>
  <Tab title="Gateway token (single-user)">
    ...
    If `GATEWAY_AUTH_TOKEN` is not set, BrassClaw generates 32 random bytes and prints the hex-encoded value (64 characters) to the console on boot.
...
| `instructions` | no | System/developer instructions. Injected ahead of `input` as an `<instructions>` block and stored as part of the persisted user message. See [Per-request instructions](#per-request-instructions) for the BrassClaw-specific replay caveat. |
...
| `x_context` | no | BrassClaw extension. Structured JSON context (≤ 10 KB) prepended as `<user-context>`. The alias `context` is also accepted but may collide with future OpenAI fields. Prefer `x_context`. |
...
BrassClaw replays the thread's history from the conversation store and runs the new turn against it. There is no client-side state to manage. Threads are user-scoped, so a token belonging to a different user cannot resume someone else's thread (the lookup returns `404`).
...
- Only `type: "function"` is accepted. `web_search`, `file_search`, `code_interpreter`, etc. are rejected. BrassClaw routes those through its own registry, not caller-provided definitions.
- Names: `^[A-Za-z0-9_-]{1,64}$`, unique within the request.
- Names must not shadow a registered BrassClaw action (built-in tool, extension tool, or engine v2 capability like `mission_*`, `skill_*`, `memory_*`). Shadowing is rejected to prevent confused-deputy behavior.
...
`x_context` is an BrassClaw extension and is not part of the OpenAI Responses API spec.
...
**BrassClaw caveat — diverges from the OpenAI spec.** The OpenAI Responses API contract is that `instructions` apply only to the current turn and are not carried by `previous_response_id`. BrassClaw currently prepends the `<instructions>` block into the user message and persists it as part of the conversation, so it *is* visible on later turns when history is replayed. Until the handler stores instructions out-of-band, treat them as sticky for the thread and re-send (or override) them explicitly on each turn that needs different behaviour.
...
The OpenAI SDKs are the easiest way to use this endpoint. Point them at BrassClaw and they "just work":
...
const client = new OpenAI({
  ...
  apiKey: process.env.BRASSCLAW_TOKEN,
});
...
- [Sandboxed Tools](/capabilities/sandboxed-tools) — how BrassClaw's built-in tools execute when the agent calls them.
