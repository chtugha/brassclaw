...
This guide will get you from zero to a running BrassClaw instance in under 10 minutes
...
<Step title="Get BrassClaw">

<Tabs>
  <Tab title="Agent Hub">
  Go to https://agent.near.ai/ and login with your preferred method, then create an BrassClaw agent in a private instance.
  ...
  To use BrassClaw, you will need to provide an SSH key. If you don't have one, you can generate it using the following command in your terminal:
...
<Accordion title="⚠️ Error: Another BrassClaw instance is already running">

If you get the error `Error: Another BrassClaw instance is already running (PID 38). If this is incorrect, remove the stale PID file: /home/agent/.brassclaw/brassclaw.pid`, simply run the following command to remove the stale PID file and try starting the agent again:

```
# Remove the stale PID file
rm /home/agent/.brassclaw/brassclaw.pid
...
<Step title="Update BrassClaw">

Finally, make sure to regularly update BrassClaw to get the latest features and improvements. You can update BrassClaw by running the following command in your terminal:
