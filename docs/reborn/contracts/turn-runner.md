...
Product adapters must continue to use `TurnCoordinator`. Runner transition APIs are trusted-worker APIs and remain under `brassclaw_turns::runner`. Driver-facing loop exits remain distinct from trusted runner outcomes; see [`loop-exit.md`](loop-exit.md).
