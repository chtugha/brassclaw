...
- `brassclaw_process_sandbox` defines `ProcessSandboxBackend` as the backend-neutral execution contract.
...
- Physical host paths for `/workspace`, `/brassclaw/state/tools`, and `/brassclaw/state/cache` live in trusted `DockerProcessSandboxConfig`, not in plan JSON.
...
- applies `iptables` broker-only egress lockdown when `BRASSCLAW_EGRESS_LOCKDOWN=broker-only`.
