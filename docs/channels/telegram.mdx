...
You can create a Telegram bot and connect your BrassClaw agent to it. Once configured, you can talk to your agent in direct messages or add it to a group chat so it can participate there.
...
<Steps>
    ...
    After creating your bot, BotFather will give you a token that looks like this: `123456789:ABCdefGhIJKlmNoPQRsTUVwxyZ`. This token is used to authenticate your bot and allow it to access the Telegram API. Keep this token safe and do not share it with anyone. You will need it later to configure the Telegram channel in BrassClaw.
    </Step>
</Steps>

</Step>
<Step title="Configure the Telegram channel in BrassClaw">

    Invoke the BrassClaw CLI onboard wizard using the `--channels-only` flag to configure only the channels without going through the entire onboarding process again:
...
<Step title="Test your Telegram channel">
    Now that you have configured your Telegram channel, it is time to test it out. Start the `brassclaw` agent if it is not already running:
...
`.brassclaw/channels/telegram.capabilities.json` file, which is created
...
BrassClaw can be configured to participate in Telegram group chats, by default the bot will only respond to commands (use `/help` to see the
...
3. **Configure `bot_username`** in BrassClaw:
   ```json
   {
...
The bot responds when a command is used, e.g. `/skills`, or when the bot is mentioned, e.g. `@MyBrassClawBot what's the weather?`
...
When BrassClaw is running in webhook mode, Telegram delivers messages by sending HTTP requests to your public URL. Because that URL is reachable from the internet, any third party could send fake requests to it.

A webhook secret is a shared token you configure in BrassClaw. Telegram includes that token in every request it sends. BrassClaw rejects any request that does not carry the correct token, so only genuine Telegram traffic reaches your agent.

To enable it, add `telegram_webhook_secret` to your `.brassclaw/channels/telegram.capabilities.json`:
...
<AccordionGroup>
  ...
  <Accordion title="Owner binding timeout during setup">
    The wizard waits 120 seconds for the first message. If it times out, send `/start` to your bot in Telegram and re-run `brassclaw onboard --channels-only`.
  </Accordion>
</AccordionGroup>