...
The HTTP Webhook channel provides a REST API for integrating external services with BrassClaw.
...
Configure GitHub to send events to your BrassClaw webhook URL:
...
Use Zapier's Webhook action to send events to BrassClaw.
...
<AccordionGroup>
  ...
  <Accordion title="Messages not processed" icon="message-circle">
    - Check logs: `RUST_LOG=brassclaw=debug brassclaw run`
    - Verify JSON format
