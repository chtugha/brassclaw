...
description: "Build a WASM messaging channel that plugs into BrassClaw"
---
...
version = "0.1.0"
edition = "2021"
description = "My messaging platform channel for BrassClaw"
...
mkdir -p ~/.brassclaw/channels
cp my-channel.wasm ~/.brassclaw/channels/my-channel.wasm
cp my-channel.capabilities.json ~/.brassclaw/channels/
```
...
mkdir -p ~/.brassclaw/channels
cp channels-src/telegram/telegram.wasm channels-src/telegram/telegram.capabilities.json ~/.brassclaw/channels/
```

<Danger>
If you are contributing a channel to the public repository, **do not commit compiled WASM binaries.** They are a supply chain risk — the binary in a PR may not match the source. BrassClaw builds channels from source.
</Danger>
