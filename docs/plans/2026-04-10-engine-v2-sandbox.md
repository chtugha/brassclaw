...
BrassClaw engine v2 (Phase 8 of `docs/plans/2026-03-20-engine-v2-architecture.md`) needs sandboxed execution for tools that touch the filesystem or run shell commands. Today the engine bridge (`src/bridge/effect_adapter.rs:1007`) executes every tool directly on the host with no isolation, even though the underlying engine has a `CapabilityLease` + `EffectType` model that was designed to enable this.
...
- Survives across BrassClaw restarts (started/stopped, never `--rm` except on explicit user action) so the accumulated environment persists

## Cross-reference: Issue chtugha/brassclaw#1894 (Unified Workspace VFS)
...
- When sandbox is off: `MountBackend::Filesystem { root: ~/.brassclaw/projects/<id>/ }` — passthrough to host fs
...
1. **Granularity**: One container per `Project` (`crates/brassclaw_engine/src/types/project.rs`). All threads in the same project — including sub-threads spawned by `rlm_query()` which inherit `project_id` at `crates/brassclaw_engine/src/runtime/manager.rs:505` — share the container.
...
3. **Mount**: `~/.brassclaw/projects/<user_id>/<project_id>/` (host) → `/project/` (container) bind mount, read-write. Project's user files live here.
4. **Daemon model**: Container PID 1 is a minimal init (`tini`). Tool-execution daemon is spawned via `docker exec -i sandbox_daemon` per BrassClaw session. Daemon talks JSON-RPC over stdin/stdout to the host.
...
~/.brassclaw/                               (anonymous writable layer, persists across stop/start)
  projects/                                /
...
The user sees their files in `~/.brassclaw/projects/<user_id>/<project_id>/`. The container sees the same files at `/project/`. Anything the agent installs via `shell_exec` lives in the container's writable layer and persists for the project's lifetime.

### Component layout

```
Host process (BrassClaw)                                Container (per project)
─────────────────────                                  ──────────────────────
...
```rust
// crates/brassclaw_engine/src/workspace/mount.rs (new)
...
pub struct FilesystemBackend {
    root: PathBuf,  // e.g. ~/.brassclaw/projects/<id>/
}
...
- **create**: First sandboxed tool call from any thread in the project, OR explicit `project init`. Builds `~/.brassclaw/projects/<user_id>/<project_id>/` if missing. Runs `docker create --name brassclaw-sandbox-<project_id> --mount type=bind,source=<host>,target=/project brassclaw/sandbox:latest`. Cold path, ~1–2s.
- **start**: Container exists but stopped. `docker start`. Warm path, ~200–500ms. Then `docker exec -i brassclaw-sandbox-<project_id> /usr/local/bin/sandbox_daemon` to attach a daemon process.
...
- **Host restart**: On engine startup, scan for `brassclaw-sandbox-*` containers in stopped state. Leave them. Re-attach lazily on first tool call. Stale containers from deleted projects can be cleaned via a future `brassclaw doctor` subcommand.

## Files and modules

### New code

- `crates/brassclaw_engine/src/workspace/mount.rs` — `MountBackend` trait, `WorkspaceMounts`, `MountError`, `DirEntry`, `ShellOutput`. Minimal subset of #1894's mount-table types.
- `crates/brassclaw_engine/src/types/project.rs` — add `workspace_path: Option<PathBuf>` field to `Project`. Accessor returns `~/.brassclaw/projects/<user_id>/<project_id>/` if unset.
...
- `crates/brassclaw_engine/src/runtime/manager.rs` — no changes; the engine doesn't need to know about sandboxes. All routing lives in the bridge.
...
- **Sub-threads via `rlm_query()`**: Inherit `project_id` (`crates/brassclaw_engine/src/runtime/manager.rs:505`), so they automatically use the parent project's container. No special case needed.
- **Project folder doesn't exist on first call**: `lifecycle::ensure_project_dir(project_id)` creates `~/.brassclaw/projects/<user_id>/<project_id>/` with mode 0700 before `docker create`. Idempotent.
...
- **Image management**: `brassclaw/sandbox:latest` built from `crates/Dockerfile.sandbox`. For local dev, `docker build -f crates/Dockerfile.sandbox -t brassclaw/sandbox:dev .` and use `BRASSCLAW_SANDBOX_IMAGE=brassclaw/sandbox:dev`. v1 documents the manual command; CI publishing is future polish.
- **Stale containers from deleted projects**: Out of scope for v1. Future `brassclaw doctor` subcommand.
- **No project folder configured / sandbox disabled**: When `SANDBOX_ENABLED=false`, the `/project/` mount uses `FilesystemBackend(~/.brassclaw/projects/<id>/)` (or whatever the user configured). Tools still go through the mount table — same code path, different backend.
...
- **Two BrassClaw processes accessing the same project container**: Single-user single-installation assumption for v1. Both would `docker exec -i` separate daemons against the same container — daemons don't share state, but they share the writable filesystem. Out of scope to fully solve; documented as "do not run two BrassClaws against the same project".
...
- Create `crates/brassclaw_engine/src/workspace/mount.rs` with `MountBackend` trait, `WorkspaceMounts`, `MountError`, `DirEntry`, `ShellOutput`.
...
- Add `workspace_path: Option<PathBuf>` to `crates/brassclaw_engine/src/types/project.rs`.
- Add accessor that defaults to `~/.brassclaw/projects/<user_id>/<project_id>/`.
...
- End-to-end test with `ENGINE_V2=true SANDBOX_ENABLED=true`: spawn a thread, agent calls `shell_exec("echo hi > /project/test.txt")`, then `file_read("/project/test.txt")`, verify both go through the container and the host sees the file at `~/.brassclaw/projects/<project_id>/test.txt`.
...
- Documentation in `crates/brassclaw_engine/CLAUDE.md` and `docs/plans/2026-03-20-engine-v2-architecture.md` (mark Phase 8 as in progress).
- Brief note in chtugha/brassclaw#1894 about the partial mount-backend abstraction this lands.
...
docker build -f crates/Dockerfile.sandbox -t brassclaw/sandbox:dev .

# Unit tests for the mount backends and daemon (no Docker needed)
cargo test -p brassclaw_engine workspace::
...
ENGINE_V2=true SANDBOX_ENABLED=true \
  BRASSCLAW_SANDBOX_IMAGE=brassclaw/sandbox:dev \
  RUST_LOG=brassclaw::bridge::sandbox=debug \
  cargo run
...
# Verify on the host:
ls ~/.brassclaw/projects/<project_id>/
cat ~/.brassclaw/projects/<project_id>/foo.txt

# Verify the container persists:
docker ps -a --filter name=brassclaw-sandbox
docker exec -it brassclaw-sandbox-<project_id> ls /root/.cargo/bin/

# Verify cleanup on idle (after 30 min or with shortened timeout)
docker ps --filter name=brassclaw-sandbox  # should show "Exited"
```
...
- `brassclaw doctor` cleanup of stale containers
- WASM channel sandboxing (Phase 8.3 — likely already transparent through the bridge, needs verification)
- chtugha/brassclaw#1894 Phase 2 (route `memory_*` through workspace mounts), Phase 3 (unify tool surface, rename to `read_file`/`write_file`/`list`/`search`), Phase 5 (dynamic mount expansion, future `S3`/`GitRemote` backends)
