...
Already defined in `crates/brassclaw_engine/src/types/mission.rs`.
...
`crates/brassclaw_engine/src/runtime/mission.rs` — already has types, needs execution logic:
...
`crates/brassclaw_engine/src/runtime/mission_prompt.rs`:
...
```python
mission_create(
    goal="Increase BrassClaw test coverage from 60% to 80%",
