# Automated QA Plan for BrassClaw
...
A review of all closed issues and merged bug-fix PRs reveals that most BrassClaw bugs fall into a few recurring categories:
...
Add the same validation for WASM tools (loaded from `~/.brassclaw/tools/`) and MCP tools (mock a simple MCP manifest and validate the schema it produces).
...
docker-build:
  ...
  steps:
    ...
    - name: Build Docker image
      run: docker build -t brassclaw-test:ci .
```
...
**Cost:** High (requires Anthropic computer use API, headless browser, brassclaw running)
...
```
+------------------+     +-----------------+     +------------------+
|  Test Runner     |     |  Headless       |     |  BrassClaw        |
...
1. **Test runner** -- Python or TypeScript script that orchestrates the flow. Starts brassclaw, waits for readiness, launches Playwright browser, runs scenarios.
...
tests/
  e2e/
    conftest.py             # pytest fixtures: start brassclaw, browser
    ...
    Dockerfile.test         # Container for CI: brassclaw + chromium
```

**Fixture: start brassclaw**

```python
@pytest.fixture(scope="session")
async def brassclaw_server():
    """Start brassclaw with gateway enabled, return base URL."""
    env = {
        "CLI_ENABLED": "false",
...
```python
@pytest.fixture
async def browser_agent(brassclaw_server):
    """Playwright browser + Claude computer use agent."""
    async with async_playwright() as p:
        browser = await p.chromium.launch(headless=True)
        page = await browser.new_page(viewport={"width": 1280, "height": 720})
        await page.goto(f"{brassclaw_server}/?token=test-token-e2e")
...
```python
async def test_sse_reconnect_preserves_history(browser_agent, brassclaw_server):
    ...
    await restart_brassclaw(brassclaw_server)
...
```python
async def test_onboarding_wizard_completes(tmp_brassclaw_home):
    """Bugs: #187, #174, #129, #185 (wizard persistence and re-trigger)"""
    # Start brassclaw with a fresh home directory (no prior config)
    ...
    proc = pexpect.spawn(
        "cargo run",
        env={"BRASSCLAW_HOME": str(tmp_brassclaw_home), **base_env},
        timeout=60,
    )

    # Step through wizard
    proc.expect("Welcome to BrassClaw")
    ...
    proc2.expect("Agent brassclaw ready")  # Should skip wizard
    # Must NOT see "Welcome to BrassClaw" again
    assert not proc2.match_any(["Welcome to BrassClaw"], timeout=5)
    proc2.close()
```
