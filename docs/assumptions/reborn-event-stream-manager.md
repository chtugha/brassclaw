...
- `ProductActor` -> `brassclaw_turns::TurnActor`
- explicit projection scope -> `brassclaw_event_projections::ProjectionScope`
- outbound/thread fanout scope -> `brassclaw_turns::TurnScope`, paired with the actor, projection scope, view, and target used for access authorization before fanout candidates are planned
- product target -> `ProjectionTarget` in `brassclaw_event_streams`, carrying existing `ThreadId`, `MissionId`, `InvocationId`, and `ProcessId` types
