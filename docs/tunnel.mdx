...
A tunnel exposes your local BrassClaw agent to the internet. You need it for webhook-based channels and for instant message delivery where polling is not desired.
...
`ngrok` is a managed tunnel service with a minimal setup, ideal if you are just starting with `brassclaw`. To use it, you will need to get an auth token from the [ngrok dashboard](https://dashboard.ngrok.com/get-started/your-authtoken)
...
- `{port}` for BrassClaw's local port
- `{host}` for BrassClaw's local host
...
Use this option when the tunnel is managed outside BrassClaw and you already have a stable public URL.

BrassClaw will use that URL directly and will not start or manage any tunnel process.
