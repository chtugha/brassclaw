...
<Danger>
If you are new to BrassClaw, we recommend you to configure [channels](/channels/telegram), tools, and other settings one at a time instead of 
...
If you are new to BrassClaw, we recommend you to configure channels, tools, and other settings one at a time instead of all at once through the onboard command.
...
BrassClaw supports two database backends: **PostgreSQL** (default, recommended for production) and **libSQL** (embedded SQLite, zero-config). The wizard will guide you through setup — see the [Database Backends](/capabilities/database) page for details.
...
BrassClaw can execute code, run builds, and use tools inside Docker
...
If you are running BrassClaw in an environment without Docker (like a server or a container), you can disable sandboxing
</Info>
