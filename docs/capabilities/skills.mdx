...
<Tip>
BrassClaw can search and install skills from the ClawHub registry, a community-driven repository of pre-built skills covering various domains and use cases.
</Tip>
...
<Steps>
  ...
  <Step title="Score">
    ...
    <Tip>
    If a skill appears in `brassclaw skills list` but the agent doesn't use it, the most common cause is a missing or empty `activation` block.
    </Tip>
...
|---------------|-------------------------------------------------------------|---------------------------------------------------------|
| **Trusted**   | User-placed in `~/.brassclaw/skills/` or workspace `skills/` | All tools available to the agent                        |
| **Installed** | Downloaded from ClawHub registry via `skill_install`        | Read-only tools only — no shell, no file write, no HTTP |

<Warning>
Never place a skill file in the trusted directories unless you have reviewed its contents. A skill in `~/.brassclaw/skills/` has the same tool access as you do.
</Warning>
...
BrassClaw discovers skills from three locations, checked in order:

| Directory                       | Trust     | Description                                                 |
|---------------------------------|-----------|-------------------------------------------------------------|
| `~/.brassclaw/skills/`           | Trusted   | User's global skills, available in all sessions             |
| `<workspace>/skills/`           | Trusted   | Per-workspace skills, activated in that workspace's context |
| `~/.brassclaw/installed_skills/` | Installed | Registry-installed skills from ClawHub                      |
...
When `SKILLS_AUTO_DISCOVER=true` (the default), BrassClaw scans all skill directories at startup and indexes all valid SKILL.md files. New skills added while the agent is running are picked up on the next restart.
