...
BrassClaw monitors all running jobs and automatically recovers those that stop making progress — without requiring human intervention.
...
- **Logs** — `RUST_LOG=brassclaw::agent::self_repair=debug` for detailed repair events
...
<AccordionGroup>
  <Accordion title="Job repeatedly gets stuck" icon="refresh-cw">
    - Check `RUST_LOG=brassclaw::agent::self_repair=debug` for failure reasons
