...
Every unit of work in BrassClaw is a **job**. Jobs run in parallel, each with isolated context, and each progressing through a defined state machine until they complete, fail, or get recovered.
...
BrassClaw runs multiple jobs concurrently. Each job has its own isolated context — memory, tool call history, and conversation state.
