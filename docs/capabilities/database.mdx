...
BrassClaw supports two database backends: **PostgreSQL** and **libSQL** (embedded SQLite). **PostgreSQL is the default** — if you don't set `DATABASE_BACKEND`, BrassClaw will use PostgreSQL and require a `DATABASE_URL`. Choose libSQL for zero-config personal use.

<Warning>
If you start BrassClaw without configuring a database, you'll see: `DATABASE_URL is required`. Run `brassclaw onboard` or set the environment variables manually.
</Warning>
...
<AccordionGroup>
  <Accordion title="Docker Compose (recommended)" icon="container">
    The fastest way to get started. A `docker-compose.yml` is included in the BrassClaw repository:
    ...
    This starts `pgvector/pgvector:pg16` on `127.0.0.1:5432` with database, user, and password all set to `brassclaw`.
...
```bash
# Create database
sudo -u postgres psql -c "CREATE DATABASE brassclaw;"
sudo -u postgres psql -c "CREATE USER brassclaw WITH PASSWORD 'your-password';"
sudo -u postgres psql -c "GRANT ALL PRIVILEGES ON DATABASE brassclaw TO brassclaw;"

# Enable pgvector
sudo -u postgres psql -d brassclaw -c "CREATE EXTENSION IF NOT EXISTS vector;"
```

### BrassClaw Configuration
...
export DATABASE_URL="postgres://brassclaw:***@localhost/brassclaw"
export DATABASE_POOL_SIZE=30  # default; increase for high-throughput workloads
```

<Warning>
`DATABASE_URL` is **required** when using PostgreSQL. If not set, BrassClaw will fail to start with the error: `DATABASE_URL is required — Run 'brassclaw onboard' or set DATABASE_URL environment variable`.
</Warning>
...
- Database is a single file (`~/.brassclaw/brassclaw.db`)
...
export LIBSQL_PATH="/home/user/.brassclaw/brassclaw.db"
```
...
<Warning>
`LIBSQL_AUTH_TOKEN` is **required** when `LIBSQL_URL` is set. BrassClaw will fail to start without it.
</Warning>
...
1. **Using pgloader (recommended):**
   ```bash
   pgloader ~/.brassclaw/brassclaw.db postgresql://brassclaw:***@localhost/brassclaw
   ```

2. **Manual export (may require editing):**
   ```bash
   sqlite3 ~/.brassclaw/brassclaw.db ".dump" > brassclaw.sql
   # Edit brassclaw.sql to remove PRAGMAs and fix type incompatibilities
   psql -d brassclaw -f brassclaw.sql
   ```

3. **Update BrassClaw config:**
   ```bash
   export DATABASE_BACKEND=postgres
   export DATABASE_URL="postgres://user:***@localhost/brassclaw"
   ```
...
1. **Export:**
   ```bash
   pg_dump -h localhost -U brassclaw brassclaw > brassclaw.sql
   ```
...
- Running BrassClaw on a personal laptop/desktop
...
```bash
# Backup
pg_dump -h localhost -U brassclaw brassclaw > backup.sql

# Restore
psql -d brassclaw -f backup.sql
```
...
cp ~/.brassclaw/brassclaw.db ~/.brassclaw/brassclaw.db.backup

# Restore
cp ~/.brassclaw/brassclaw.db.backup ~/.brassclaw/brassclaw.db
