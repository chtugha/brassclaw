...
WASM tools run inside a WebAssembly sandbox powered by [wasmtime](https://wasmtime.dev/). They have access to BrassClaw's host functions (logging, time, workspace), but all other capabilities — network, filesystem, credentials — must be explicitly declared in a `capabilities.json` file.

This is BrassClaw's recommended approach for custom integrations that need stronger isolation than a built-in Rust tool provides.
...
LLM selects tool
    ↓
BrassClaw loads WASM module (from cache or disk)
    ↓
...
WASM modules can call the following host functions provided by BrassClaw:

| Function | Description |
|----------|-------------|
| `log(level, message)` | Write to the BrassClaw log at the given severity level |
...
- `~/.brassclaw/tools/` — User-installed tools
...
# Install from a URL
brassclaw tool install https://example.com/tools/my-tool.wasm
...
The cache is stored at `~/.brassclaw/wasm-cache/`.
