...
description: Common operator configuration reference for BrassClaw
---

BrassClaw is configured through bootstrap environment variables, database-backed settings, and a small set of advanced or provider-specific environment variables. This page documents the primary options used during setup and normal operation.
...
BrassClaw uses a two-layer configuration system:

<AccordionGroup>
  <Accordion title="Layer 1: ~/.brassclaw/.env (Bootstrap)" icon="file">
  ...
  <Accordion title="Layer 2: Database Settings" icon="database">
    ...
    Some security-sensitive and advanced options remain environment-only. Manage database-backed values through the wizard or `brassclaw config` command.
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="Security" icon="shield" href="#security">
    SECRETS_MASTER_KEY, BRASSCLAW_BASE_DIR
  </Card>
</CardGroup>
...
|----------|------|---------|-------------|
| `AGENT_NAME` | string | `brassclaw` | Agent name displayed in responses |
...
| `LIBSQL_PATH` | path | `~/.brassclaw/brassclaw.db` | libSQL database file |
...
export DATABASE_URL="postgres://user:pass@localhost/brassclaw"
...
| `WASM_CHANNELS_DIR` | path | `~/.brassclaw/channels` | Channel modules directory |
...
| `SANDBOX_IMAGE` | string | `brassclaw-worker:latest` | Docker image |
...
| `SKILLS_REGEX_ACTIVATION_ENABLED` | bool | `true` | Enable regex pattern skill auto-activation |
...
| `BRASSCLAW_BASE_DIR` | path | `~/.brassclaw` | Data directory |
| `BRASSCLAW_OAUTH_CALLBACK_URL` | URL | `http://127.0.0.1:9876` | OAuth callback URL |
...
| `BRASSCLAW_INSTANCE_ID` | Relay instance ID. |
...
| Variable | Description |
|----------|-------------|
| `BRASSCLAW_BASE_DIR` | Overrides the BrassClaw data directory. |
| `BRASSCLAW_OWNER_ID` | Stable owner scope for the instance. |
| `BRASSCLAW_PROFILE` | Built-in profile selected during onboarding. |
| `BRASSCLAW_OAUTH_CALLBACK_URL` | OAuth callback URL used by auth flows. |
| `NEARAI_SESSION_TOKEN` | NEAR AI session token; takes precedence over session file. |
| `BRASSCLAW_PID_LOCK_PATH` | Process PID lock path override. |
| `BRASSCLAW_PID_LOCK_HOLD_MS` | PID lock hold duration. |
| `BRASSCLAW_PID_LOCK_CHILD` | Internal child-process PID lock marker. |


## Environment File Example

Create `~/.brassclaw/.env`:
...
# Set value
brassclaw config set llm.backend nearai

# Delete value (reset to default)
brassclaw config delete llm.backend
```
