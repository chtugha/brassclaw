---
title: Amazon EC2
description: Host BrassClaw on an Amazon EC2 Instance
---

Amazon Elastic Compute Cloud (EC2) lets you run virtual machines on AWS infrastructure with flexible pricing and a free tier for new accounts. In this guide we will launch an EC2 instance and configure it securely so you can run BrassClaw and expose it to the internet.
...
Now that we are inside the instance, we need to perform some initial configuration. We want to strengthen security by setting up a dedicated user and preparing the system for running BrassClaw.
...
It is good practice to create a dedicated user with sudo privileges instead of using `ubuntu` for all operations. You can create a new user (for example, `brassclaw`) and add it to the sudo group:
...
```bash
ssh -i <KEY_PATH> brassclaw@<IP_ADDRESS>
```
...
Now that the instance is set up and secured, install BrassClaw:

```bash
curl --proto '=https' --tlsv1.2 -LsSf https://github.com/nearai/brassclaw/releases/latest/download/brassclaw-installer.sh | sh
```

Then start BrassClaw and follow the instructions to complete the setup:
