...
description: Host BrassClaw on a Google Compute Engine VM
---

Google Compute Engine (GCE) lets you run virtual machines on Google's infrastructure with flexible pricing and a generous free tier. In this guide we will create a GCE VM instance and configure it securely so you can run BrassClaw and expose only the endpoints you actually need.

<Tip>
Do not feel like setting up your own infrastructure? You can install BrassClaw with a few clicks on [agent.near.ai](https://agent.near.ai)
</Tip>
...
- **Project name**: choose a descriptive name (e.g. `brassclaw`)
...
- **Name**: choose a descriptive name (e.g. `brassclaw`)
...
BrassClaw's Web Gateway defaults to `127.0.0.1:3000`, which means it is local-only by default and not directly reachable from the internet.
...
<Note>
Restricting SSH to your IP prevents brute-force attacks from the internet. You can update the source range later if your IP changes. Port `8080` is required for webhooks (e.g. Telegram) to deliver events to BrassClaw. On Linux, the orchestrator internal API listens on port `50051` and binds to `0.0.0.0`; unless you add a firewall rule for it, GCE's default deny-all ingress policy will still block external access. Other BrassClaw listeners bind to `127.0.0.1` by default.
</Note>
...
Now that you have access to your VM, harden it before installing BrassClaw.
...
It is good practice to create a dedicated user with sudo privileges instead of relying on the default account. You can create a new user (for example, `brassclaw`) and add it to the sudo group:

```bash
sudo adduser brassclaw
sudo usermod -aG sudo brassclaw
```
...
sudo mkdir -p /home/brassclaw/.ssh

# Copy the authorized_keys from the current user
sudo cp ~/.ssh/authorized_keys /home/brassclaw/.ssh/authorized_keys

# Set the correct permissions (critical — SSH will ignore the file otherwise)
sudo chown -R brassclaw:brassclaw /home/brassclaw/
sudo chmod 700 /home/brassclaw/.ssh
sudo chmod 600 /home/brassclaw/.ssh/authorized_keys
```

Open a new terminal window and confirm you can log in before continuing:

```bash
ssh brassclaw@<IP_ADDRESS>
```
...
With the server hardened, install BrassClaw and start it up:

```bash
curl --proto '=https' --tlsv1.2 -LsSf https://github.com/nearai/brassclaw/releases/latest/download/brassclaw-installer.sh | sh
```

Then start BrassClaw and follow the prompts to complete the setup:
...
<Tip>
We recommend using a session manager like `tmux` or `screen` so you can easily detach and reattach to your running BrassClaw instance between SSH sessions.
</Tip>
...
Follow our [Quickstart Guide](/quickstart) to create your first agent, connect it to Telegram, and start exploring BrassClaw's capabilities.
