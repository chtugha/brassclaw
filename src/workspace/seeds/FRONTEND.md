...
- `index.js` — widget code (calls `BrassClaw.registerWidget()`)
...
**Slot:** only `tab` is currently mounted by the browser runtime — `BrassClaw.registerWidget({ slot: "tab", ... })` adds a new tab to the tab bar. For inline rendering of structured data in chat messages, use `BrassClaw.registerChatRenderer({ id, match, render })` instead. Additional slot names may be accepted by the server but will not be mounted anywhere in the UI yet.
...
- Widgets can call any gateway API the logged-in user can call. The runtime exposes `BrassClaw.api.fetch(...)`, which forwards the user's bearer token automatically — there is no per-widget capability sandbox.
...
- A widget that calls `BrassClaw.api.fetch('/api/memory/write', ...)` can mutate any workspace file, including its own source — review widget code before installing it.
- The widget identifier validator (`is_safe_widget_id` in `crates/brassclaw_gateway/src/layout.rs`, applied to discovery, serving, and `manifest.id`) and the `manifest.id == directory_name` check protect against widget-to-widget confusion, not against malicious-but-well-formed widgets.
