...
use brassclaw_common::{ExtensionName, truncate_preview};
use brassclaw_llm::{ChatMessage, ToolCall, generate_tool_call_id};
...
impl Thread {
    ...
    pub fn restore_from_messages(&mut self, messages: Vec<ChatMessage>) {
        ...
        while let Some(msg) = iter.next() {
            if msg.role == brassclaw_llm::Role::User {
                ...
                while let Some(next) = iter.peek() {
                    if next.role == brassclaw_llm::Role::Assistant && next.tool_calls.is_some() {
                        ...
                        while let Some(tr) = iter.peek() {
                            if tr.role != brassclaw_llm::Role::Tool {
                                break;
                            }
                ...
                let is_final_assistant = iter.peek().is_some_and(|n| {
                    n.role == brassclaw_llm::Role::Assistant && n.tool_calls.is_none()
                });
...
pub struct Turn {
    ...
    #[serde(skip)]
    pub image_content_parts: Vec<brassclaw_llm::ContentPart>,
}
...
mod tests {
    ...
    fn test_messages_includes_tool_calls() {
        ...
        assert_eq!(messages[0].role, brassclaw_llm::Role::User);
        assert_eq!(messages[0].content, "Search for X");

        assert_eq!(messages[1].role, brassclaw_llm::Role::Assistant);
        assert!(messages[1].tool_calls.is_some());
        let tcs = messages[1].tool_calls.as_ref().unwrap();
        assert_eq!(tcs.len(), 1);
        assert_eq!(tcs[0].name, "memory_search");

        assert_eq!(messages[2].role, brassclaw_llm::Role::Tool);
        assert!(messages[2].content.contains("Found X"));

        assert_eq!(messages[3].role, brassclaw_llm::Role::Assistant);
