...
mod tests {
    ...
    fn attachment_with_project_file_path_is_rendered() {
        ...
        att.local_path = Some(".brassclaw/attachments/alice/project/2026-04-12/brief.txt".into());
        att.extracted_text = Some("Hello from disk".to_string());

        let result = augment_with_attachments("review", &[att]).unwrap();
        assert!(
            result.text.contains(
                "project_path=\".brassclaw/attachments/alice/project/2026-04-12/brief.txt\""
            )
        );
        assert!(result.text.contains(
            "Saved to project file: .brassclaw/attachments/alice/project/2026-04-12/brief.txt"
        ));
