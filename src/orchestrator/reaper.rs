...
//!
//! The SandboxReaper periodically scans Docker for BrassClaw-labeled containers
...
//! 1. Scans Docker for containers with the `brassclaw.job_id` label
...
impl Default for ReaperConfig {
    fn default() -> Self {
        Self {
            ...
            container_label: "brassclaw.job_id".to_string(),
        }
    }
}
...
impl SandboxReaper {
    ...
    async fn scan_and_reap(&self) {
        let containers = match self.list_brassclaw_containers().await {
            Ok(c) => c,
            Err(e) => {
    ...
    /// List all BrassClaw-managed containers from Docker.
    ///
    /// Returns tuples of (container_id, job_id, created_at).
    async fn list_brassclaw_containers(
        &self,
                ...
                .and_then(|s| s.parse::<Uuid>().ok())
            {
                ...
                None => {
                    tracing::warn!(
                        ...
                        "Reaper: brassclaw container missing valid job_id label"
                    );
                    continue;
                }
            };
...
mod tests {
    ...
    fn reaper_config_defaults_are_reasonable() {
        ...
        assert_eq!(cfg.container_label, "brassclaw.job_id");
    }
    ...
    // They require Docker to be running and the BRASSCLAW_E2E_DOCKER_TESTS
    // environment variable to be set (to avoid running them in CI by default).
    //
    // Run with: BRASSCLAW_E2E_DOCKER_TESTS=1 cargo test orchestrator::reaper::e2e_tests --lib -- --nocapture
    ...
    mod e2e_tests {
        ...
        fn should_run_e2e() -> bool {
            std::env::var("BRASSCLAW_E2E_DOCKER_TESTS").is_ok()
        }

        /// Test that reaper can list containers with BrassClaw labels
        #[tokio::test]
        async fn e2e_reaper_lists_brassclaw_containers() {
            if !should_run_e2e() {
                eprintln!("Skipping e2e test (set BRASSCLAW_E2E_DOCKER_TESTS=1 to run)");
                return;
            }
            ...
            // Create a test container with BrassClaw labels
            let job_id = Uuid::new_v4();
            let test_name = format!("brassclaw-reaper-test-{}", &job_id.to_string()[..8]);
            ...
            labels_str.insert("brassclaw.job_id", &job_id_str);
            labels_str.insert("brassclaw.created_at", &created_at_str);
            ...
            assert!(
                labels.contains_key("brassclaw.job_id"),
                "Container should have brassclaw.job_id label"
            );
            assert_eq!(
                labels.get("brassclaw.job_id").map(|s| s.as_str()),
        ...
        async fn e2e_reaper_removes_orphaned_containers() {
            ...
            let test_name = format!("brassclaw-orphan-test-{}", &orphaned_job_id.to_string()[..8]);
            ...
            let mut labels: std::collections::HashMap<&str, &str> =
                std::collections::HashMap::new();
            labels.insert("brassclaw.job_id", &job_id_str);
            labels.insert("brassclaw.created_at", &created_at_str);
        ...
        async fn e2e_reaper_respects_age_threshold() {
            ...
            let mut old_labels: std::collections::HashMap<&str, &str> =
                std::collections::HashMap::new();
            old_labels.insert("brassclaw.job_id", &old_id_str);
            old_labels.insert("brassclaw.created_at", &old_time_str);
            ...
            let mut new_labels: std::collections::HashMap<&str, &str> =
                std::collections::HashMap::new();
            new_labels.insert("brassclaw.job_id", &new_id_str);
            new_labels.insert("brassclaw.created_at", &new_time_str);

            let mut containers_to_cleanup = Vec::new();

            // Create old container
            let old_name = format!("brassclaw-age-old-{}", &old_job_id.to_string()[..8]);
            if let Ok(r) = docker
                .create_container(
            ...
            let new_name = format!("brassclaw-age-new-{}", &new_job_id.to_string()[..8]);
            if let Ok(r) = docker
                .create_container(
                    Some(bollard::container::CreateContainerOptions {
