...
/// Capabilities declaration file for a WASM channel.
///
/// Parsing is not yet implemented in V2 — WASM channels are not used.
/// The type is retained for the setup wizard's channel discovery flow.
...
impl ChannelCapabilitiesFile {
    pub(super) fn from_bytes(_bytes: &[u8]) -> Result<Self, String> {
        Err("WASM channel capabilities are not used in V2".to_string())
    }
}
