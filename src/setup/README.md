...
This document is the authoritative specification for BrassClaw's onboarding
...
3. If Command::Run or no command:
   a. Load .env files (dotenvy::dotenv() then load_brassclaw_env())
...
`DATABASE_BACKEND` live in `~/.brassclaw/.env`.
...
auto_setup_database()    → libsql at ~/.brassclaw/brassclaw.db (zero prompts)
...
Step 2/2: Model Selection     ← interactive unless defaulted by detected provider
       ↓
   save_and_summarize()      → includes tip to run `brassclaw onboard`
```

**Local usage profile:** If `BRASSCLAW_PROFILE` is already set, quick mode
...
The selected profile is written to `~/.brassclaw/.env` as `BRASSCLAW_PROFILE`
...
defaults to libsql at `~/.brassclaw/brassclaw.db`, creates the database,
...
The full 9-step wizard remains available via `brassclaw onboard`.
...
**libSQL path:**
1. Offer local path (default: `~/.brassclaw/brassclaw.db`)
...
2. "Allow brassclaw to access this keychain item" (per-app authorization)
...
- **Never probe the keychain in read-only commands** (e.g., `brassclaw status`).
...
- **NEAR AI Chat** path: session token saved to `~/.brassclaw/session.json`.
  Hosting providers can set `NEARAI_SESSION_TOKEN` env var directly (takes
  precedence over file-based tokens).
- **NEAR AI Cloud** path: `NEARAI_API_KEY` saved to `~/.brassclaw/.env`
...
6b. Discover WASM channels from ~/.brassclaw/channels/
...
1. Already installed in `~/.brassclaw/channels/`
...
4. Discover already-installed tools in `~/.brassclaw/tools/`
...
**Layer 1: `~/.brassclaw/.env`** (bootstrap vars)
...
```env
BRASSCLAW_PROFILE="local"
DATABASE_BACKEND="libsql"
LIBSQL_PATH="/Users/name/.brassclaw/brassclaw.db"
...
DATABASE_URL="postgres://user:pass@localhost/brassclaw"
SECRETS_MASTER_KEY="..."
ONBOARD_COMPLETED="true"
```

**Why separate?** Chicken-and-egg: you need `DATABASE_BACKEND` to know
which database to connect to, `BRASSCLAW_PROFILE` to apply built-in defaults
...
1. Writes bootstrap vars to `~/.brassclaw/.env` via `write_bootstrap_env()`
...
Bootstrap vars written to `~/.brassclaw/.env` (only true chicken-and-egg vars
that are needed before the DB is connected):
- `BRASSCLAW_PROFILE` (when quick first-run setup selected a profile)
...
pub struct Settings {
    ...
    // Advanced (not in wizard, set via `brassclaw config set`)
...
- Service name: `"brassclaw"`, account: `"master_key"`
...
1. **NEAR AI Cloud API key (option 4 in auth menu):** Get an API key
   ...
   local listener is needed. The key is saved to `~/.brassclaw/.env`
   and the encrypted secrets store. Uses the OpenAI-compatible
   ChatCompletions API mode.

2. **Custom callback URL:** Set `BRASSCLAW_OAUTH_CALLBACK_URL` to a
   publicly accessible URL (e.g., via SSH tunnel or reverse proxy) that
   forwards to port 9876 on the server:
   ```bash
   export BRASSCLAW_OAUTH_CALLBACK_URL=https://myserver.example.com:9876
   ```
...
6. Run the full shipping checklist:
   ...
   ```
7. Test a fresh onboarding: `rm -rf ~/.brassclaw && cargo run`
