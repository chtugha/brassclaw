//! OS service management for running BrassClaw as a daemon.
//!
//! Generates and manages platform-native service definitions:
//! - **macOS**: launchd plist at `~/Library/LaunchAgents/com.brassclaw.daemon.plist`
//! - **Linux**: systemd user unit at `~/.config/systemd/user/brassclaw.service`
//!
//! The installed service runs `brassclaw run` (the default agent mode) and is
...
const SERVICE_LABEL: &str = "com.brassclaw.daemon";
const SYSTEMD_UNIT: &str = "brassclaw.service";
...
fn install_macos() -> Result<()> {
    ...
    let logs_dir = brassclaw_logs_dir();
    ...
    println!("  Start with: brassclaw service start");
    Ok(())
}
...
fn start() -> Result<()> {
    if cfg!(target_os = "macos") {
        ...
        if !plist.exists() {
            bail!("Service not installed. Run `brassclaw service install` first.");
        }
...
fn brassclaw_logs_dir() -> PathBuf {
    brassclaw_base_dir().join("logs")
}
...
mod tests {
    ...
    fn macos_plist_path_has_expected_suffix() {
        ...
        assert!(
            s.ends_with("Library/LaunchAgents/com.brassclaw.daemon.plist"),
            "unexpected path: {s}"
        );
    }
    ...
    fn linux_unit_path_has_expected_suffix() {
        ...
        let s = path.to_string_lossy();
        assert!(
            s.ends_with(".config/systemd/user/brassclaw.service"),
            "unexpected path: {s}"
        );
    }

    #[test]
    fn logs_dir_under_brassclaw() {
        let path = brassclaw_logs_dir();
        let s = path.to_string_lossy();
        assert!(s.ends_with(".brassclaw/logs"), "unexpected path: {s}");
    }

    #[test]
    fn macos_plist_sets_cli_enabled_false() {
        let plist = macos_plist_content("/tmp/brassclaw", "/tmp/stdout.log", "/tmp/stderr.log");
