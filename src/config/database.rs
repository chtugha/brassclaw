...
pub struct DatabaseConfig {
    ...
    // -- libSQL fields --
    /// Path to local libSQL database file (default: ~/.brassclaw/brassclaw.db).
...
impl DatabaseConfig {
    pub(crate) fn resolve() -> Result<Self, ConfigError> {
        ...
        // DATABASE_URL is loaded from ~/.brassclaw/.env via dotenvy early in startup.
        let url = optional_env("DATABASE_URL")?
            ...
            .ok_or_else(|| ConfigError::MissingRequired {
                ...
                hint: "Run 'brassclaw onboard' or set DATABASE_URL environment variable".to_string(),
            })?;
...
/// Default libSQL database path (~/.brassclaw/brassclaw.db).
pub fn default_libsql_path() -> PathBuf {
    brassclaw_base_dir().join("brassclaw.db")
}
