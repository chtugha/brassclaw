...
impl SecretsConfig {
    ...
    ///    `~/.brassclaw/.env` as `SECRETS_MASTER_KEY=…`.
    ///
    ...
    /// zeroized, but the key ultimately lives in `~/.brassclaw/.env`
    ...
    pub(crate) async fn resolve() -> Result<Self, ConfigError> {
        Self::resolve_with_env_path(&crate::bootstrap::brassclaw_env_path()).await
    }

    /// Testable variant of [`Self::resolve`] that writes its `.env`
    /// fallback to an explicit path. Production code calls
    /// [`Self::resolve`], which targets `~/.brassclaw/.env`.
...
/// brassclaw `.env` file. Returns `None` if the file is missing, the
