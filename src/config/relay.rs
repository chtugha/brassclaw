...
impl RelayConfig {
    ...
    fn from_env_reader(env: impl Fn(&str) -> Option<String>) -> Option<Self> {
        ...
        Some(Self {
            url,
            api_key,
            callback_url: env("BRASSCLAW_OAUTH_CALLBACK_URL"),
            instance_id: env("BRASSCLAW_INSTANCE_ID"),
...
mod tests {
    ...
    fn from_env_reader_loads_overrides() {
        let config = RelayConfig::from_env_reader(|key| match key {
            ...
            "BRASSCLAW_OAUTH_CALLBACK_URL" => Some("https://tunnel.example.com".into()),
            "BRASSCLAW_INSTANCE_ID" => Some("my-instance".into()),
