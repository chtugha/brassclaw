...
async fn list_servers(verbose: bool) -> anyhow::Result<()> {
    ...
    if servers.servers.is_empty() {
        ...
        println!("    brassclaw mcp add <name> <url> [--client-id <id>]");
        println!();
        return Ok(());
    }
...
async fn auth_server(name: String, user_id: String) -> anyhow::Result<()> {
    ...
    if server.has_custom_auth_header() {
        println!();
        println!(
            "  Server '{}' is configured with an Authorization header, so 'brassclaw mcp auth' is not used for this configuration.",
            name
        );
    ...
    match authorize_mcp_server(&server, &secrets, &user_id).await {
        ...
        Err(AuthError::NotSupported) => {
            ...
            println!();
            println!("    brassclaw mcp remove {}", name);
            println!(
                "    brassclaw mcp add {} {} --client-id YOUR_CLIENT_ID",
                name, server.url
            );
            println!();
        }
...
async fn test_server(name: String, user_id: String) -> anyhow::Result<()> {
    ...
    let _client = if has_tokens {
        ...
        println!();
        println!(
            "  ✗ Not authenticated. Run 'brassclaw mcp auth {}' first.",
            name
        );
        println!();
        return Ok(());
