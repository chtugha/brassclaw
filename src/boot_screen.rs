...
//! - **Tier 3 (removed):** Database, tool count, features → use `brassclaw status`.
...
pub fn print_boot_screen(info: &BootInfo) {
    ...
    // Hint to run `brassclaw status` for full details
    println!(
        "  {}Run `brassclaw status` for full system details.{}",
        fmt::hint(),
        fmt::reset()
    );
...
mod tests {
    ...
    fn test_print_boot_screen_full() {
        let info = BootInfo {
            ...
            agent_name: "brassclaw".to_string(),
