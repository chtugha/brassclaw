...
//!
//! ```ignore
//! use brassclaw::secrets::{SecretsStore, PostgresSecretsStore, SecretsCrypto, CreateSecretParams};
...
pub enum GeneratedKeySafetyError {
    ...
    #[error(
        "Secrets store already contains encrypted data, but BrassClaw auto-generated \
...
/// the key to either the OS keychain or `~/.brassclaw/.env` by the
