...
use crate::trace_contribution::{
    ...
    mark_trace_credit_notice_due_for_scope, normalize_trace_selected_tools,
    preflight_trace_contribution_policy, queued_trace_envelope_paths_for_scope,
    read_trace_policy_for_scope, read_trace_queue_holds_for_scope,
    revoke_trace_submission_for_scope_with_policy, snooze_trace_credit_notice_for_scope,
...
pub async fn traces_policy_put_handler(
    ...
    reject_web_managed_trace_ingest_config(&body)?;
    ...
    if let Some(selected_tools) = body.selected_tools {
        policy.selected_tools = normalize_trace_selected_tools(selected_tools);
    }
...
pub async fn traces_revoke_handler(
    ...
    if body.endpoint.is_some() {
        return Err((
            ...
            "Trace revoke endpoints must be configured outside the authenticated web API"
                .to_string(),
        ));
    }
        ...
        .any(|record| record.submission_id == submission_id)
    {
        return Err((
            ...
            "Trace submission not found".to_string(),
        ));
    }
    ...
    revoke_trace_submission_for_scope_with_policy(scope, submission_id, endpoint, &policy)
        .await
        .map_err(internal_error)?;
...
fn reject_web_managed_trace_ingest_config(
    body: &TracePolicyRequest,
) -> Result<(), (StatusCode, String)> {
    let has_ingest_config = body.endpoint.is_some()
        || body.bearer_token_env.is_some()
        || body.upload_token_issuer_url.is_some()
        || body.upload_token_issuer_allowed_hosts.is_some()
        || body.upload_token_audience.is_some()
        || body.upload_token_tenant_id.is_some()
        || body.upload_token_workload_token_env.is_some()
        || body.upload_token_issuer_timeout_ms.is_some();
    if has_ingest_config {
        return Err((
            StatusCode::FORBIDDEN,
            "Trace ingestion endpoint and credential settings must be configured outside the authenticated web policy API".to_string(),
        ));
    }
    Ok(())
}
...
mod tests {
    ...
    use chrono::Utc;
    use brassclaw_llm::recording::{TraceFile, TraceResponse, TraceStep, TraceToolCall};
    ...
    fn empty_policy_request() -> TracePolicyRequest {
        TracePolicyRequest {
            enabled: None,
            endpoint: None,
            bearer_token_env: None,
            upload_token_issuer_url: None,
            upload_token_issuer_allowed_hosts: None,
            upload_token_audience: None,
            upload_token_tenant_id: None,
            upload_token_workload_token_env: None,
            upload_token_issuer_timeout_ms: None,
            include_message_text: None,
            include_tool_payloads: None,
            auto_submit_failed_traces: None,
            auto_submit_high_value_traces: None,
            selected_tools: None,
            require_manual_approval_when_pii_detected: None,
            min_submission_score: None,
            credit_notice_interval_hours: None,
            default_scope: None,
        }
    }

    #[tokio::test]
    async fn traces_policy_put_rejects_web_managed_ingest_credentials() {
        let user_id = format!("trace-web-policy-credential-user-{}", Uuid::new_v4());
        let original = StandingTraceContributionPolicy {
            bearer_token_env: "ORIGINAL_TRACE_TOKEN_ENV".to_string(),
            ..Default::default()
        };
        write_trace_policy_for_scope(Some(&user_id), &original).expect("policy writes");

        let mut body = empty_policy_request();
        body.endpoint = Some("https://attacker.example/v1/traces".to_string());
        body.bearer_token_env = Some("AWS_SECRET_ACCESS_KEY".to_string());
        body.upload_token_issuer_url = Some("https://issuer.example/claims".to_string());
        body.upload_token_issuer_allowed_hosts = Some(vec!["issuer.example".to_string()]);
        body.upload_token_audience = Some("trace-commons".to_string());
        body.upload_token_tenant_id = Some("tenant-a".to_string());
        body.upload_token_workload_token_env = Some("TRACE_WORKLOAD_TOKEN".to_string());
        body.upload_token_issuer_timeout_ms = Some(500);
        body.include_message_text = Some(true);

        let error = traces_policy_put_handler(
        ...
        .await
        .expect_err("web-managed ingest settings should be rejected");

        assert_eq!(error.0, StatusCode::FORBIDDEN);
        let persisted = read_trace_policy_for_scope(Some(&user_id)).expect("policy reads");
        assert_eq!(persisted, original);
    }

    #[tokio::test]
    async fn traces_policy_put_trims_selected_tools_before_persisting() {
        let user_id = format!("trace-web-policy-selected-tools-user-{}", Uuid::new_v4());
        write_trace_policy_for_scope(
            Some(&user_id),
        ...
        let mut body = empty_policy_request();
        body.selected_tools = Some(vec![
            " shell ".to_string(),
            " ".to_string(),
            "http".to_string(),
        ]);

        let Json(response) = traces_policy_put_handler(
            AuthenticatedUser(UserIdentity {
                user_id: user_id.clone(),
                role: "member".to_string(),
                workspace_read_scopes: Vec::new(),
        ...
        assert_eq!(
            response.policy.selected_tools,
            BTreeSet::from(["http".to_string(), "shell".to_string()])
        );
        let persisted = read_trace_policy_for_scope(Some(&user_id)).expect("policy reads");
        assert_eq!(
            persisted.selected_tools,
            BTreeSet::from(["http".to_string(), "shell".to_string()])
        );
    ...
    #[tokio::test]
    async fn traces_revoke_rejects_web_supplied_endpoint() {
        let user_id = format!("trace-web-revoke-endpoint-user-{}", Uuid::new_v4());
        let submission_id = Uuid::new_v4();
        write_trace_records(&user_id, &[submitted_record_with_id(submission_id, 1.0)]);

        let error = traces_revoke_handler(
            AuthenticatedUser(UserIdentity {
                user_id: user_id.clone(),
                role: "member".to_string(),
                workspace_read_scopes: Vec::new(),
            ...
            Path(submission_id),
            Json(TraceRevokeRequest {
                call_remote: true,
                endpoint: Some("https://attacker.example/v1/traces".to_string()),
            }),
        )
        .await
        .expect_err("web-supplied revoke endpoints should be rejected");
    ...
    async fn traces_queue_status_handler_surfaces_safe_hardened_diagnostics() {
        ...
        envelope.schema_version = "brassclaw.trace_contribution.v0".to_string();
