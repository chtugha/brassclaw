#!/usr/bin/env bash
# One-shot bootstrap helper: serves the brassclaw libsql DB over a
# short-lived Cloudflare Quick Tunnel so the Railway runner can fetch
# it via `BRASSCLAW_DB_URL`. See infra/runner/README.md.
#
# Usage:
#   ./infra/runner/seed-runner-db.sh                 # uses $HOME/.brassclaw/brassclaw.db
...
#   4. Prints the public URL to paste into Railway as BRASSCLAW_DB_URL.
...
DB_PATH="${1:-$HOME/.brassclaw/brassclaw.db}"
...
SERVE_DIR="$(mktemp -d -t brassclaw-seed-XXXXXX)"
TUNNEL_LOG="$(mktemp -t brassclaw-seed-tunnel-XXXXXX)"
...
# `brassclaw.db-wal` rather than in the main `brassclaw.db` file. A
...
cp "${DB_PATH}" "${SERVE_DIR}/brassclaw.db"
chmod 600 "${SERVE_DIR}/brassclaw.db"

# Random high port to avoid collision with a local gateway that might
# already be bound to 8000 / 3000.
PORT=$((20000 + RANDOM % 10000))

echo "[seed] Serving $(du -h "${SERVE_DIR}/brassclaw.db" | cut -f1) from 127.0.0.1:${PORT}"
...
if ! curl --silent --fail --max-time 3 \
        --head "http://127.0.0.1:${PORT}/brassclaw.db" >/dev/null; then
...
FETCH_URL="${URL}/brassclaw.db"
printf '\n'
printf '============================================================\n'
printf 'Paste this into Railway as BRASSCLAW_DB_URL:\n\n'
printf '    %s\n\n' "${FETCH_URL}"
printf 'Then redeploy the service. Watch the Railway log for:\n'
printf '    [entrypoint] Fetched N bytes to /runner-data/home/.brassclaw/brassclaw.db\n\n'
...
printf 'remove BRASSCLAW_DB_URL from Railway env.\n'
printf '============================================================\n\n'
